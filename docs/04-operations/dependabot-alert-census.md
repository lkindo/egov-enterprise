...
> 사유 없이 목록만 정리하면 그것은 신호 은폐다([AGENTS.md Evidence guardrails H2](../../AGENTS.md#evidence-guardrails)).
...
**[AGENTS.md Evidence guardrails H5](../../AGENTS.md#evidence-guardrails)가 못박은 대로, prose 로만 존재하는 규칙은 그 규칙을 어길 주체를 막지 못한다.**
...
**pre-push 에서 기계 강제**된다(AGENTS H5 — 실행 경로 없는 게이트는 게이트가 아니다).

### 8.3 게이트 검증 — 그린만으로는 증명이 아니다

위반을 의도적으로 주입해 red 가 되는 것까지 확인했다(AGENTS H5).
...
"빨간 신호를 없애려고" 손대는 것과 구분되지 않으면 그 자체가 AGENTS H2 위반이다.
...
그것은 **판정 근거를 문서에 남긴 뒤에만** 정당하다(AGENTS H2 — 목록 편집은 수정이 아니다).

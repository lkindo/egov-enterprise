...
**미해결 취약점 — 사내망 XFF 위조**: `ClientIpResolver.resolve()` 는 홉이 **전부 신뢰 대역**이면 최좌측 값을 채택한다.
사설 대역 클라이언트가 스스로 `X-Forwarded-For: 10.1.2.3` 을 실어 보내면 그 값이 채택되어,
레이트리밋 키·로그인 IP 제한·감사 IP 를 요청자가 임의로 바꿀 수 있다.

> 2026-08-03 에 이 분기를 `remoteAddr` 폴백으로 바꿔 봤으나 **되돌렸다.** 그 변경은 위조를 닫는 대신
> **사내망 사용자 전원의 IP 를 프록시 1개로 수렴**시켜, 레이트리밋이 전 사내 트래픽에 대해 단일 버킷이 된다 —
> 원장이 경고한 **순서 지뢰 1(전사 로그인 장애)** 과 같은 계열의 가용성 위험이다. 인트라넷 비중이 높은 배포에서는
> 실제 장애가 된다. 어느 쪽이 옳은지는 **위 질문 1·2 의 답에 달려 있다.**
>
> 선택지: **(a)** `remoteAddr` 폴백 — 위조 차단, 사내 IP 해상도 상실. **(b)** 최우측 신뢰 홉 채택 —
> 우리 인프라가 XFF 에 **append** 한다면 실제 클라이언트 IP 를 보존하면서 주입값(항상 좌측)을 무시한다.
> (b) 의 성립 여부는 Next 가 XFF 를 append 하는지 forward 하는지에 달렸고, 이는 실측이 필요하다.
> **(c)** 신뢰 대역을 실제 프록시 IP 로 좁혀 이 분기 자체가 발생하지 않게 한다 — 질문 1·2 가 답해지면 가장 깔끔하다.

> **순서 지뢰 1**: 신뢰 대역을 비우는 변경과 **레이트리밋 용량 하향을 같은 배포에 넣지 말 것.**
> 순서는 ① 신뢰 대역 주입 → ② 관측 → ③ 용량 조정이다.

### G-2 · 메뉴 권한 회수 (승인 필요 — 운영 데이터 DML)
ROLE_USER 에게 노출되지만 프론트 미들웨어가 되돌리는 메뉴 **13건**의 권한 회수가 **미이행**이다.
V2_34 가 손댄 2건(`test`·`test1`)은 잡메뉴 정리(FREE-6)이지 G-2 의 부분 이행이 아니다 — 대상 선정 기준·조치 방식이 다르다.

- 원장 추천: **(A) 메뉴 권한 회수** — 마이그레이션으로 사이드바에서 제거. 최소 9건은 백엔드가 어차피 403 이라
  미들웨어를 열어도 실효가 없다. "보이지만 안 되는 것보다 안 보이는 것이 정직하다."
- 미이행 사유: `tb_menu_authrt_map` DML 은 운영 데이터 변경이라 **개별 명시 승인**이 필요하다
  (GEMINI.md §7 · 오케스트레이션 §3 Stage 1 포괄승인 제외 항목).
- 착수 시: `db-bridge` 로 `tb_menu_info.modern_route` 가 `middleware.ts` 의 `USER_ACCESSIBLE_ADMIN_PATHS` 에
  매칭되지 않는 `/admin/**` 메뉴를 **실측 열거**한 뒤, 대상 `menu_sn` 을 근거(라우트·백엔드 403 여부)와 함께
  마이그레이션 주석에 명시한다. 시드 원본(V2_2)은 이력 보존상 수정하지 않고 델타로만 처리한다(V2_34 선례).

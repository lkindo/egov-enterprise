...
> 2026-08-03 Wave 1 이행 기록은 30개 항목 전부를 `[x] 완료` 로 표시했지만,
...
> **V2_72 현행화 (2026-08-14)**: 위 문단은 2026-08-04 당시 `atch_file_id` 기준의 역사적 census다.
> 이후 FAQ·상담 토론 테이블이 제거되어 최신 스키마의 물리 폐포는 상세 1종과 업무 참조 11종이다.
> V2_72는 이 12개 FK를 모두 `atch_file_sn BIGINT`로 재결속했고, 팝업의 파일 URL은 논리 참조로
> `/api/v1/files/{atchFileSn}` 형식에 동기화했다. 접근 권한 판정 의미는 그대로 유지한다.
...
### 인가 린터 커버리지 (A-2 잔여) — **해소 (2026-08-15)**
...
그 간극을 **없애는 대신 세고 고정**한다. 최초 38건을 `클래스#메서드` 단위로 양방향 동결했다.
새 읽기 엔드포인트가 같은 면제를 타면 red 이고(실제로 하나 추가해 확인), 항목이 사라져도 red 다.

2026-08-12 도메인별 판정으로 알림 3건(수신자 스코프), 비정형결재 2건(참여자 스코프),
쪽지 3건(발신/수신 본인 스코프)에 클래스 인증 가드를 결합해 면제를 제거했다. 현재 잔여는 **30건**이며,
`WRITE_AUTHZ_GUARDED_ELSEWHERE` 클래스 면제도 쪽지를 제거해 **12건**이다.

> ⚠ **이 목록은 '안전한 것' 이 아니라 부채다.** 전부가 취약점인 것도 아니다 — 일부는 이미 서비스
> 계층에 읽기 소유권 가드가 있다. `NoteService.getNoteDetail` 은 그 근거를 클래스 인증 가드와
> 네거티브 테스트로 연결해 이번에 목록에서 졸업했다.
...
- 개인·참여자·부서 스코프 **18건**: 주소록 2, 결재함 2, 발신메일 2, 메모보고 3, 일정 5,
  스크랩 2, 업무보고 2. 현재 사용자 스코프 쿼리 또는 소유자/참여자 서비스 가드를 확인했다.
- 인증 사용자 공유 조회 **12건**: 주소록 사용자 검색 1, 게시판 5, 댓글 1, 커뮤니티 2, 설문 3.
  조직 내부 공용 콘텐츠이므로 객체 소유권 가드 대신 인증 경계를 명시했다.
- 해당 12개 컨트롤러에 `@Authenticated`를 부착하고 `READ_COVERED_BY_WRITE_RATIONALE` 30건과
  `WRITE_AUTHZ_GUARDED_ELSEWHERE` 12개 클래스 면제를 삭제했다. 따라서 쓰기 사유가 읽기를 덮는 경로와
  신규 메서드가 클래스 면제를 자동 상속하는 경로가 모두 사라졌다.
- RED/GREEN/뮤테이션: 면제 제거 직후 두 하네스가 대상 전체를 차단했고, 애노테이션 적용 후 green.
  주소록 클래스 인증을 의도적으로 제거하자 조회·쓰기 하네스가 다시 red가 됐으며 즉시 원복했다.

잔여 자동화 한계는 애노테이션의 존재가 객체 소유권의 올바른 의미까지 증명하지 않는다는 점이다.
개인 데이터의 서비스 음성 테스트와 `OwnershipGuardBaselineLinterTest`는 계속 별도 방어선으로 유지한다.
...
> 당시 Wave 2 이행 기록은 **파일 라벨이 어긋나 있었다** —

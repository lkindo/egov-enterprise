...
> 2026-09-14 이후 일정 경과의 처리 방식은 [ADR-0018](../02-architecture/decisions/ADR-0018-governance-review-lifecycle-and-adoption.md)을 따른다. 이 문서의 2026-09-05 관측·승인 사실은 소급 변경하지 않는다.

## 0. 승인이 무엇을 여는가

`reviewState`가 `approved`이고 `approvals` 두 축이 완결되면, **그 부류의 stateItem만 가진
record**를 전체 승인으로 집계한다. 미승인 부류가 섞인 record를 전체 승인으로 승격하지 않는다.
`reviewBy` 경과는 별도 운영 보고의 재검토 신호이며 정상 승인 범위의 자동 만료가 아니다.
승인 근거에 요구하는 것은 셋이다.
...
| 당시 구현에 2027-01-01 시계를 주입했을 때의 만료 red | 258 |

`119`는 승인 selector의 완전 포괄 record 수이고 `258`은 당시 구현의 미래 시계 오류 수이므로
단순 합계로 census 총수를 계산하는 지표가 아니다. ADR-0018 이후 일반 검토 일정은 기술 CI를
만료시키지 않는다. 이 표는 과거 검토 근거이며 현행 실패 예측으로 사용하지 않는다.
...
기존 2026-12-31 일정은 실제 시계 기반 운영 검토로 추적한다. 남은 부류의 미검토 상태를 날짜
갱신으로 지우지 않는다. 코드·정책의 변경 검토와 기관의 운영 토폴로지 검토는
[검토 수명 가이드](../03-guides/governance-review-lifecycle.md)에 따라 각각 수행한다.

...
UI URL-state census 변경은 새 보조 소스 파일 한 개에 따른 파일 수·manifest 해시 갱신이다. 기존 381개 분류와 승인 범위는 동일하다. SAST의 7.0 미만 탐지는 리포트 전용이며 모든 항목의 수동 검토를 완료했다는 의미가 아니다. 원격 required CI·E2E 및 ruleset 반영은 이 로컬 감사의 완료 범위에 포함하지 않는다.

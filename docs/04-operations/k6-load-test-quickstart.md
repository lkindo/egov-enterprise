...
> **상위 원칙**: 본 문서는 [부하 테스트 종합 가이드 (load-test-guide.md)](./load-test-guide.md)의 임계치 설계 및 병목 분석 방법론을 상위 규범으로 따르는 **k6 실무 퀵스타트**입니다. 부하 시나리오 설계 철학 및 성능 기준은 상위 가이드를 참조하십시오.

...
- **1차 조치(적용됨)**: 키를 `${ALGORITHM_KEY:egovframe}`로 **env 외부화**, 약한 기본키 사용 시 `CryptoUtil`이 경고, 키 원문 로깅 제거. `RrnoEncryptionConverter`는 암호화 실패 시 **평문 저장을 거부(fail-closed)**.
- **현재 PII 규모(실측)**: `tb_user_info` 20행 중 `rrno` 비어있지 않은 행 **0건**. → **재암호화 대상 데이터가 없어 현 시점 키 로테이션은 데이터 무손실**(재암호화 스텝은 no-op). PII가 축적된 뒤에는 아래 4장 절차가 필수다.
...
```
`application.yml`의 `algorithmKey: ${ALGORITHM_KEY:egovframe}` 가 이 값을 읽는다.

## 3. PII가 없을 때(현재) — 단순 로테이션
`rrno` 데이터가 0건이면 재암호화가 불필요하므로:
1. DB 백업(관례상).
2. `ALGORITHM_KEY`에 새 키 설정 후 애플리케이션 재기동.
...
키를 그냥 바꾸면 **기존 암호문이 새 키로 복호화되지 않아 PII가 유실**된다. 다음 절차를 따른다.
...
ARIA 암복호화는 순수 SQL로 불가하므로 **Java 재암호화 러너**로 수행한다(아래 설계). 러너는
`OLD_ALGORITHM_KEY`(기존)와 `ALGORITHM_KEY`(신규) 두 키를 받아 각 `rrno`를 `decrypt(old)`→`encrypt(new)`로 치환한다.
...
class RrnoReencryptRunner implements ApplicationRunner {
  // 프로퍼티: crypto.reencrypt.oldKey, crypto.reencrypt.dryRun(default true), batchSize
  // 1) old 키용 EgovARIACryptoServiceImpl 인스턴스 별도 생성(현재 CryptoUtil.new 키와 병존)
...
```
> 러너는 현재 PII 0건이라 검증 불가하므로 **본 저장소에 미탑재**(untested crypto 코드 미배포 원칙). PII 축적 시 위 설계대로 구현·테스트 후 1회성 실행한다.

### 4.3 키 전환 & 정리
1. 재암호화 완료·검증 후 애플리케이션의 `ALGORITHM_KEY`를 신규 키로 확정.
2. `OLD_ALGORITHM_KEY`(러너용) 폐기.
3. 로그·모니터링에서 복호화 실패(`Rrno decryption failed`)가 없는지 확인.
...
---
*Last Updated: 2026-07-10 (foundation 보안 감사 C1/D 후속 — 키 외부화·fail-closed converter 반영, 재암호화 런북 신설)*

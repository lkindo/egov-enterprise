...
| PD-UX-002 | open (부분 결정) | 로그 검색 조건을 URL에 얼마나 보존할지 — **2026-09-04 Q1 종결**: 현재 URL 에 실리는 사용자 입력 검색어 14건을 **전부 유지**하고, 로그 화면이 주소창에 싣지 않는 현행도 유지한다(경계 = 주소창). 잔여는 Q2~Q5 | 분류 초안이 작성돼 선행 조건이 해소됐다([초안](../01-product/url-state-classification-draft.md)). 판정 단위 396 중 keep 282 · remove 5 · owner 판정 필요 109. Q1 결정으로 그중 14건이 종결됐고 화면별 비대칭이 **승인된 상태**로 확정됐다. ⚠ 결정 근거가 된 실측 정정: 화면 URL 은 `tb_web_log` 에 적재되지 않는다(`getRequestURI()` 는 쿼리 제외, `/api/` 외 요청 폐기) — 잔존 위험은 브라우저 히스토리·다운로드 관리자·저장소 밖 프록시 로그다 | Q2(copy-all 캐리어 34)·Q3(DB 소유 쿼리 키 공간 52)·Q4(죽은 표면 14)·Q5(승인 범위와 2026-10-31 기한)를 이어서 결정한다. Q1 결정은 `scripts/log-search-url-boundary-contract.test.mjs` 가 양방향으로 고정한다 | [분류 초안](../01-product/url-state-classification-draft.md), [IA §14.3](../01-product/information-architecture.md#143-2026-08-23-g1-decision-workshop-기록), [frontend log routes](../../frontend/src/app/admin/system/logs) |

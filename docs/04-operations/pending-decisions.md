...
>
> **잔여**: 프론트 화면이 없다. 만족도는 게시글 상세 위젯이라 별건으로 분리했다.
> 현재는 *"API 는 있고 화면이 없다"* — 이 작업이 고친 *"서비스는 있고 API 가 없다"* 와 같은 부류다.
...
>
> **제안(미착수)**: 이를 기계적으로 잡는 린터가 필요하다 — *"`@Service` 빈의 public 메서드가
> 생산 코드 어디에서도 호출되지 않으면 red"*. 세 번 반복됐다면 네 번째가 있다고 봐야 한다.
> 다만 스프링 배선(이벤트 리스너·스케줄러·프록시 경유 호출)에서 오탐이 나기 쉬워, 도입 시
> 화이트리스트가 아니라 **호출 그래프 기반 판정**으로 설계할 것.

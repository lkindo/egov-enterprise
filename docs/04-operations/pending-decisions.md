...
**`1-A/1-B`(core/app 경계)** · **`5-D`(프런트 언어 정책)**
...
- **✅ 결판·구현(2026-08-15)**: banner·popup·community·survey main **55파일**과 해당 테스트 **8파일**을 패키지/API/DB 변경 없이 `business-app`으로 이관했다. `ServiceLayerIsolationTest`가 core 재유입을 차단한다.
...
>
> **어떻게 안전하게 했나** — AGENTS H4(일괄 치환 금지)를 지켰다.
...
- **⚠ 착수 시도 결과 — 실제 범위 실측 (2026-08-05)**
  ...
  **왜 단순 치환으로 처리하면 안 되는가**
  ...
  - 37개소를 같은 방식으로 바꾸는 것은 **AGENTS H4 가 금지하는 sweep** 이다.
...
- **⚠ 중요 정정 — `AntPathRequestMatcher` 는 아직 '제거' 되지 않았다**
  ...
  | **B** | 업그레이드 + 해당 파일 한정 **시한부 경고 억제** → 매처는 분리 과제 | **CVE 4건을 지금 닫는다.** 억제는 AGENTS H2 가 경계하는 '신호 끄기' 이므로 **기한·사유를 주석과 이 문서에 명기**해야 정당화된다. 범위가 한 파일·한 애노테이션이라 동결 목록 편집과는 성격이 다르다 |
...
- 상세 실측·재개 조건: 위 현행 실측과 2개 결정 대기 항목을 정본으로 삼는다.
...
### 3-D. 뮤테이션 게이트 STRICT flip — **✅ 종결(2026-08-12 헌법 동기화 승인)**
...
- **당시 함께 정정한 곳**: `build.gradle` 주석, 당시 `GEMINI.md`의 뮤테이션 스킬 행, 관련 아키텍처·CI 문서와 운영 스킬.
- **헌법 동기화 완료(2026-08-12 사용자 명시 승인)**: `.agent/knowledge/backend-api-constitution/artifacts/constitution.md` 제16조를 실제 `mutation-scope`의 `STRICT_MUTATION=true` 75% 하드 게이트와 required `mutation-test` 집계 구조에 맞췄다. 임계값·스코프 정책은 변경하지 않았다.
...
- **5-D. i18n 실채택 — ✅ 결판(2026-08-15)**: 프런트는 한국어 단일언어로 확정하고 반쪽 `next-intl`·카탈로그·로케일 쿠키를 제거했다. 백엔드 API 오류의 ko/en `Accept-Language` 계약은 유지한다. 재도입 조건은 [ADR-0002](../02-architecture/decisions/ADR-0002-korean-first-frontend.md).
...
## 7. 관리자 전수감사 잔여 (2026-07-22 최초 감사 · 2026-08-05 현행 실측)
...
>    ⚠ **운영 데이터 DML 이므로 [AGENTS.md의 안전 경계](../../AGENTS.md#공통-작업-원칙)에 따라 개별 명시 승인 필요.**
...
> 출발하면 그 목록이 곧 서랍이 된다(AGENTS H2). 고아 폐포(엔티티 2·리포지토리 2·DTO 3·테스트 6)도
...
> 구현체를 주입했더니 green 이었다. **그린만 확인했다면 못 잡았을 결함**이며, AGENTS H5 가 위반 주입을
> 요구하는 이유가 이것이다.
>
...
- **i18n(F-1)**: ✅ 위 **5-D. i18n 실채택**에서 한국어 UI로 결판·종결.

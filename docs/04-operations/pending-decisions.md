...
> | 5 | **D-3 결재 양식** | ⛔ **`tb_sanctn_form` 은 존재하지 않는다**(2026-08-06 DB 86개 테이블 전수 확인). 결재 본체는 `tb_ifml_atrz_info` ↔ `InformalSanction` 으로 실재하고 **이미 배선돼 있다**(`ApprovalApiController` + `InformalSanctionApiController`, FE `ApprovalHubClient` 가 실제 호출). 껍데기는 **양식·스튜디오 3화면뿐**이며 이를 구현하려면 **새 테이블 신설** = 스키마 변경 | ⚠ **개별 명시 승인 필요**(글로벌 §5 파괴적 경계) |
...
- **미결 D 항목** — 2026-08-05 기준 **10건 중 4건 해소, 6건 잔여**. 해소분은 취소선으로 남긴다(원장에서 지우면 무엇이 왜 사라졌는지 추적이 끊긴다).
  ...
  - **D-3 결재 양식/워크플로우 스튜디오** — ⚠ 원장 서술 정정됨(2026-08-06 실측): **`tb_sanctn_form` 은 존재하지 않고**, 결재 본체(`tb_ifml_atrz_info`)는 **이미 배선돼 있다**. 껍데기는 양식·스튜디오 3화면뿐이고 구현하려면 **새 테이블 신설**이 필요하다 → **개별 승인 필요**. 대안은 메뉴 하차/재매핑(스키마 변경 없음).
  - ~~**D-4 설문 도메인**~~ → ✅ **완료** (#301). 버그 5종 수정 + 응답자 배선 + 응답 제출·통계 신설. 상세는 위 **§D-4 이행 결과**. ⚠ 중복 응답 유니크 제약(스키마 변경)은 미결로 남음.
  - **D-6 메뉴 SSOT 재편** — 중복 18메뉴·고아 19라우트·부모/자식 동일경로. ⚠ **DB 스키마 변경 수반 — 개별 명시 승인 필요**.
  - **D-7 테마 설정 저장 위치** — 사이트 테마 테이블+API+SSR vs "내 브라우저만" 문구 정직화. ⚠ **DB 스키마 변경 수반 — 개별 명시 승인 필요**.

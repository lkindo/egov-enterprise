...
> 요구하는 이유가 이것이다.
>
> **미결로 남은 것**: 엔티티를 지운 **빈 테이블 2개**(`tb_indv_pg`·`tb_memo_todo_info`)의 처분.
> 테이블 삭제는 스키마 변경이라 **개별 승인이 필요**하다. 그리고 `TemplateApiController` 에는
> 수정·삭제 엔드포인트가 없다 — 지운 `TemplateService` 가 그 로직을 갖고 있었으나 원래 도달
> 불가였다. 기능 상실이 아니지만, 필요해지면 라이브 경로에 추가해야 한다.

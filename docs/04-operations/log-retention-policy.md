...
알림 6개월은 DEC-OPS-128이 정한 `read-months` 기본값이며 자동 파기 스위치는 기본 꺼짐이다. 이 SQL은 운영 파기 설정을 변경하지 않는다. 쪽지는 모든 사본의 삭제 상태를 확인하고,
상태가 null인 사본도 보존한다. 첨부 참조가 있다는 사실만으로 실물 삭제 대상으로 삼지 않는다.
PD-NOTE-003(읽은 알림 6개월)과 PD-STORAGE-001(원본은 삭제 게시글·첨부를 파기하지 않고 기관 `attachment-operations` 통제에서 정한다)은 DEC-OPS-128로 닫혔다. 남은 PD-NOTE-001은 복구 창과 보존 기간을 확정한 후 별도로 활성화한다.

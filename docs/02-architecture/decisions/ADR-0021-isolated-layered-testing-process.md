...
4. PR의 E2E와 mutation은 변경 범위 분류 직후 독립적으로 시작한다. main/master push는 전체 회귀를 실행한다. E2E는 현재 전체 모집단을 2개 shard에 분배하고 discovery inventory와 실제 결과를 대조하여 파일 안의 누락까지 실패시킨다.
...
작은 책임 파일은 소유권·실패 위치와 shard 균형을 개선하지만 파일 수 자체가 속도 개선 증거는 아니다. 기존 시간 profile을 선언 수로 배분한 초기 가중치는 추정치이며 새 CI 실행으로 다시 측정한다. 병렬 job은 대기시간을 줄일 수 있으나 runner 사용량은 늘 수 있다.

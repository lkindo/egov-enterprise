...
> **상위 규범:** [프런트엔드 UX 헌법](../../.agent/knowledge/frontend-ux-constitution/artifacts/constitution.md) · [ADR-0003](decisions/ADR-0003-frontend-ux-modernization-principles.md)(과업 우선) · [ADR-0006](decisions/ADR-0006-css-only-responsive-table.md)(CSS-only 반응형) · [ADR-0007](decisions/ADR-0007-reference-default-ia-approval.md)(참조-기본 IA) · [ADR-0009](decisions/ADR-0009-controlled-url-search-state.md)(통제된 URL 검색 상태)
>
...
| G5 | **시스템 기본 정렬을 정의**하고, 열 단위 정렬을 허용한다. 정렬 범위가 전체가 아니면 화면이 그 사실을 말한다. | MS(시스템 뷰에 기본 정렬을 구현하고 열 기준 정렬을 허용) + KRDS 표(표는 대화형 요소가 아니며 **데이터 정렬이 예외**다) |
...
#### A3-1. 폼을 어디에 띄우는가 — 모달 기본 규칙

> **판정 규칙:** 등록·수정 입력의 그릇은 **인라인 ≥ 모달 > 전용 페이지** 순으로 고른다. 앞의 것이 가능하면 뒤의 것을 쓰지 않는다.
>
> - **인라인**(같은 화면에서 폼이 펼쳐지거나 master-detail 우측 패널) — 라우트 이동이 0이라 맥락 손실도 0이다. 가능하면 이것이 최선이다.
> - **모달** — 목록 위에서 끝나는 기본값.
> - **전용 페이지** — 아래 조건 중 **하나 이상**을 충족할 때만.
>
> | # | 조건 | 판정 기준 |
> |---|---|---|
> | 1a | **리치 텍스트 본문 편집** | 에디터 툴바+편집영역이 모달 높이를 구조적으로 넘는다. 페이지 **필수** |
> | 1b | **첨부 파일** | 개수·staged 여부로 본다. 제출과 함께 보내는 1건은 모달에서도 된다(`/admin/system/banner`). 건별 업로드·기존 첨부 관리가 있으면 페이지 |
> | 2 | **다단계 마법사** | 단계 간 이동과 되돌아가기가 필요하다 |
> | 3 | **폼 자체가 공유 대상 URL** | 그 주소를 남에게 보내는 것이 과업의 일부다 |
> | 4 | **대형 구조물 편집** | 계층 트리·권한 격자처럼 좌측 선택 맥락과 함께 봐야 하는 것. 보통 인라인(A2)으로 푼다 |
>
> 열람·공유용 **상세는 라우트로 둔다.** 상세를 라우트로 두는 것과 등록 폼을 모달로 두는 것은 서로 충돌하지 않는다 — 가르는 축은 자원 종류가 아니라 **행위**다(입력이냐 열람이냐).
> **한 라우트가 둘을 겸하면** 열람을 라우트에 남기고 입력 컨트롤만 모달로 올린다. 라우트를 없애지 않는다.

**판정 절차** — 순서를 지킨다. 2단계를 건너뛰면 승인된 메뉴 계층을 건드리는 제안이 나온다.

1. **입력인가** — 등록·수정 입력만 대상이다. 삭제·승인·토글처럼 **확인 대화만 있는 액션은 판정 대상이 아니다**(반려 사유처럼 텍스트를 받는 확인 대화도 액션으로 본다).
2. **메뉴 SSOT 리프인가** — `tb_menu_info`에 그 라우트를 가리키는 활성 메뉴 행이 있으면 **목록의 자식 폼이 아니라 1급 목적지**다. 잃을 목록 맥락이 애초에 없고, 모달 전환은 프런트 리팩터가 아니라 Flyway·IA 변경이 된다([ADR-0017](decisions/ADR-0017-task-oriented-menu-navigation.md)). 이 경우 **판정에서 제외**한다.
3. **맥락 손실을 실측한다** — 목록 화면의 조회 상태가 `useState`인가 URL인가. URL에 실려 복원된다면 페이지 이동 비용이 낮아 위반이 아니다.
...
| 근거 | 실측 |
|---|---|
| 목록 맥락 보존 | 목록 화면의 조회 상태(검색어·페이지·범위 토글·페이지당 건수)는 대부분 컴포넌트 `useState`이고 URL에 싣지 않는다([PD-UX-002 경계](../04-operations/pending-decisions.md)). 라우트를 떠나면 그 상태가 전손되고, 돌아온 사용자는 조건을 다시 만들어야 한다. |
| 저장소 관례 | 등록·수정 진입의 지배적 관례가 모달이다. 위 A3 '현재 화면' 3개가 3/3 모달이고, 가장 무거운 `사용자` 엔티티조차 신규 등록을 `2xl` 모달로 받는다. |
| 폼 크기는 근거가 아니다 | 필드 수로는 갈리지 않는다 — 모달로 받는 폼(6~7필드)이 페이지로 보내는 폼(5필드)보다 크다. 페이지를 정당화하는 것은 필드 수가 아니라 위 3개 조건이다. |
| 좁은 화면 | 모달이 `fixed` 배치라 셸 패딩 밖을 쓴다. 400px에서 모달 작업 폭이 전용 페이지보다 **넓다**(페이지는 셸 패딩과 카드 패딩을 이중으로 먹는다). |

**미저장 보호는 이 판정의 근거가 아니다.** 한동안 '페이지라야 미저장 경고를 걸 수 있다'는 이해가 있었으나 사실이 아니다 — [`UnsavedChangesContext`](../../frontend/src/contexts/UnsavedChangesContext.tsx)의 `navigate(action)`은 라우터를 쓰지 않아도 되는 임의 콜백이고, 모달 닫기용 가드는 [`useDirtyCloseGuard`](../../frontend/src/hooks/useDirtyCloseGuard.ts)가 같은 문구로 제공한다. **두 형태 모두 미저장 보호를 걸어야 하며**(A3 '필수'의 '이탈 시 미저장 경고'), 모달은 닫기 경로 전부(Esc·배경·X·취소)에 걸어야 한다. 저장 중 닫기 차단은 `StandardModal`의 `closeDisabled`가 별도로 소유한다 — 두 가드는 보완 관계다.

**현행 예외 — 부서 업무 등록(미결)**

`/smart-toolkit/dept-job/create`는 위 3개 조건 중 어느 것도 충족하지 않으므로 이 규칙상 모달이어야 한다. 그럼에도 전용 페이지가 **남아 있어야 하는** 이유는 UX가 아니라 **재사용 pack 소유권**이다 — `src/app/admin/work-hub`는 demo pack 소유인데 부서 업무 테이블(`tb_dept_task_info`·`tb_dept_job_bx`)은 core 소유다. core·collaboration 프로필에서는 `work-hub`가 제거되고 그것을 import하는 목록 라우트까지 [cascade 제거](../../scripts/generate-reusable-base-source.mjs)되므로, `create`·`[id]` 두 페이지가 **그 프로필의 유일한 부서 업무 UI**가 된다.

이 예외는 **면제가 아니라 미결**이다. 정리하려면 pack 소유권 판정(work-hub를 core로 옮길 것인가, core에 자체 목록을 줄 것인가)이 선행하고, 라우트 처분은 [ADR-0007](decisions/ADR-0007-reference-default-ia-approval.md) §Decision 4의 owner 승인 경계다. 그 판정 전까지 두 페이지를 지우거나 리다이렉트로 바꾸지 않는다.
...
- **URL 검색 상태:** 현재 로그 화면의 주소창 검색어는 로컬 상태로 유지한다. [ADR-0009](decisions/ADR-0009-controlled-url-search-state.md)의 허용은 모든 화면의 URL 동기화를 의무화하지 않는다. 이후 주소창 검색을 채택하려면 화면별 route/query key allowlist, unknown query 차단, same-view `replace`, client log·analytics 비복제를 함께 계약으로 고정한다. 조회 조건과 파일 조건을 맞추는 same-origin binary GET의 `searchKeyword`는 별도 exact 계약으로 허용한다.
- **금지:** 현재 페이지만 내보내면서 `전체 내보내기`로 라벨링하기, 자격증명·token·고유식별정보·고위험 개인정보·응답 본문용 전용 URL field를 만들거나 일반 검색창에서 해당 입력을 요구·유도하기, 선언하지 않은 query를 복사하기. 자유 검색어에 예상 밖 값이 들어올 가능성은 별도 잔여 위험이며 고위험 용도 승인이 아니다.
- **키보드:** A1과 동일 + 기간 프리셋 단축 이동.
- **합격:** 대용량 조회에서 첫 페이지 응답이 열화되지 않음 · export 건수와 총 건수 일치.
- **현재 화면:** `/admin/system/logs`(+ `login`·`privacy`·`system`·`user`·`web`), `/admin/system/audit`, `/admin/security/audit`. **정렬·페이지당 건수 이행 완료**(§6). **기간 프리셋은 2026-08-26 이행** — [PeriodFilter](../../frontend/src/app/components/patterns/period-filter.tsx)(최근 1일·1주·1개월·전체 + 직접 입력). 서버 저장소 5종은 처음부터 `searchBgnDe`/`searchEndDe` 를 구현하고 있었고 화면이 그 값을 보내지 않았을 뿐이다. 배선하면서 **두 가지 침묵**을 함께 고쳤다 — ① 엔드포인트마다 날짜 파싱 형식이 달라 틀려도 오류가 아니라 조용한 무시·빈 결과였고, ② 사용자·웹·개인정보 서비스는 화면이 보낸 기간을 저장소에 `null` 로 버리고 있었다(저장소·컨트롤러는 지원하는데 가운데 한 층이 삼킴). 백엔드는 [LogSearchPeriod](../../business-core/src/main/java/nuri/business/domain/log/LogSearchPeriod.java)로 통일해 두 형식을 모두 받고 해석 불가 값은 400 으로 실패시키며(존재하지 않는 날짜를 말일로 보정하던 SMART 리졸버도 STRICT 로 교정), 프런트는 컬럼 표준과 같은 8자리 하나만 보낸다. 전달 여부는 [프런트 계약](../../frontend/src/__tests__/cross-stack/log-period-filter-contract.test.ts)과 [서비스 계층 계약](../../business-core/src/test/java/nuri/business/service/log/LogPeriodForwardingTest.java)이 양쪽에서 고정한다. **서버측 전체 export 는 2026-08-26 로그 5종 전부 이행 완료**(로그인 로그는 DEC-OPS-016, 나머지 4종은 같은 규칙을 [LogExcelExport](../../api-server/src/main/java/nuri/api/controller/foundation/controller/system/log/LogExcelExport.java)로 공유). 행 상한 초과는 **파일을 만들기 전에** 400 으로 실패하며(전량 조회를 시도조차 하지 않는다), 프런트는 [requestFullExport](../../frontend/src/app/components/patterns/full-result-export.ts)로 URL 조립을 단일화했다 — 화면마다 조립하면 어떤 화면이 기간을 빼먹어 파일과 화면이 어긋난다(실제로 로그인 로그가 그 상태였다). 현재 페이지 반출도 함께 남아 있으며 `DataExportExcel` 의 `scope` 가 **필수 prop** 이라 버튼 문구가 범위를 밝힌다(`page` → `현재 페이지 엑셀 내보내기`). 서버 페이징 화면이 `loaded` 를 주장하면 [census](../../frontend/src/__tests__/export-scope-census.test.ts)가 red 다.
...
| 그중 `WorkListPage` 셸 경유 | **40** | W3 wave 1~13 — 되돌리기는 게이트가 red |
| 그중 셸 없이 직접 조립 | **4** | 신규 유입은 게이트가 red. 남은 4건은 전부 이행 대상이 아니다(아래 참조) |
...
| W3 | **완료(wave 1~13, 2026-08-25)** | A1 이행 웨이브 | wave 1(협업 2): 스크랩·주소록. wave 2(9): 로그 4종 + 운영 5화면. wave 3(5): 설문 응답자·네트워크·HPCM·온라인 매뉴얼·마이페이지 설정. wave 4(4): 행정구역 코드·기관 코드(탭형)·공개 설문 목록·커뮤니티 목록. wave 5~10: 설문 관리·통계·커뮤니티 허브·보안 그룹/역할·프로그램·템플릿·SMS·도움말·ISM·로그 대시보드·쪽지·게시판 마스터·온라인 설문 등. wave 11(3): 협업 허브·조직 권한(A2)·커뮤니티 상세. wave 12(2): 배너/팝업·워크허브(한 클라이언트가 3개 라우트를 담당). wave 13(1): 모니터링 허브 7탭 — 좌측 3열 세로 내비를 헤더 탭리스트로 압축하고 우측 상세 패널을 표 아래 상세 섹션으로 옮겼으며, 비활성 탭 6개가 존재하지 않는 id 를 참조하던 `aria-controls` 결함을 함께 고쳤다. 웨이브마다 장식 사이드바·조작 지표(`1.2k+` 등 계측 원천 없는 고정 문구)·죽은 검색(`console.log` onSearch)·근거 없는 지표 카드를 함께 제거했고, 조회 시점을 타이핑 디바운스에서 `조회`/Enter 로 통일했다(G2). 공통 조립기 [KeywordFilter](../../frontend/src/app/components/patterns/keyword-filter.tsx)·[emptyResultMessage](../../frontend/src/app/components/patterns/empty-result-message.ts) 신설. [채택 census 게이트](../../frontend/src/__tests__/work-list-adoption-census.test.ts) **40/4**(유입·이중 표기 red 실측, 오탐 2건 수정), status color BASELINE 656→592, hardcoded color BASELINE 64→61, h1 소유 계약을 셸 위임형으로 개정. **남은 직접 표 4건은 전부 이행 대상이 아니다** — A5 스펙 계약(권한 매트릭스)·도달 불가 alias 1건(`/cop/sms/selectSmsList` — [next.config](../../frontend/next.config.ts) 리다이렉트)·공유 컴포넌트 1건이라 대상이 아니다.  는 2026-08-27 리다이렉트 제거로 도달 가능해져 이행 대상으로 복귀했다(미이행, §6 표 참조) |
...
| W5 | **진행 중(A2 3차 완료 2026-08-24)** | A2·A4·A5 셸 신설 + 이행 | [MasterDetailPage](../../frontend/src/app/components/patterns/master-detail-page.tsx) 신설 후 1차 부서·메뉴, 2차 메일 이력, 3차 공통코드 STANDARD까지 4화면을 이행했다. 단일 responsive DOM, 고정폭 master, 미선택 안내, 검색, `aria-current`, `↑`/`↓`, 상세 `Tab`, 저장 가능한 화면의 선택+변경 시 `Ctrl+S`를 계약으로 고정했다. 메일은 저장 동작이 없는 조회·삭제 화면이라 단축키를 만들지 않고, 6열 표 대신 compact master + 상세 구조로 바꿨다. 공통코드 STANDARD는 포털 hero·metrics·진입 motion을 제거해 1280×720에서도 탐색·상세 업무 영역을 노출하고, DnD handle/선택 분리·검색 중 이동 비활성·그룹의 교차 분류 이동만 저장·검색 stale 선택 해제를 결속했다. 허브 공통 h1·breadcrumb 뒤의 활성 panel에서 STANDARD A2와 ADMINIST·INSTITUTION A1을 h2로 임베드하고, 중복 포털 래퍼를 제거했으며 `/admin/system/common-code/codes` alias도 유지한다. [A2 exact census](../../frontend/src/__tests__/master-detail-adoption-census.test.ts)가 네 route의 실제 소비를 검증하며, `StandardDataTable` 50·직접 조립 29·`WorkListPage` 21·`PagePagination` 4는 불변이다. status color BASELINE은 메일 이행 642→629에 이어 공통코드의 필수·오류·사용 상태 리터럴을 semantic token으로 이행해 629→621이 됐다. **선택 새로고침 복원은 보류:** 당시에는 메뉴·부서·메일 식별자의 IA와 privacy review가 미확정이어서 새 query·브라우저 저장소를 만들지 않았고, 공통코드의 기존 `groupId` query consumer 1건도 `deny`·`unverified` legacy로 보존했다. 2026-09-05 ADR-0009 승인 뒤에도 허용은 의무가 아니며 이 식별자들은 승인된 검색 key가 아니므로 새 producer·저장소를 만들지 않는 현행을 유지한다. 4차로 **결재 허브(`/approvals`)** 를 A2 로 이행해 A2 소비자는 5개다 — 3열 유리질 카드를 걷고 마스터를 compact 목록으로 바꿨으며, 조회 함수가 처리 이력과 같아 **같은 데이터를 다른 이름으로 보여주던 `결재 문서 보관함` 탭**은 사유를 밝힌 비활성 컨트롤로 정정했다. **A4(작업 큐)는 실소비자가 없어 셸을 만들지 않는다**(§5 A4 판정). **A5(매트릭스)는 실소비자가 권한 매트릭스 1개뿐이라 셸 대신 스펙 계약으로 고정했다**(§5 A5 판정) — 변경 셀 표시·저장 전 요약·변경 없음 시 저장 불가·격자 방향키·`Ctrl+S`를 계약이 강제하고, 저장이 손대지 않은 역할까지 다시 쓰던 결함을 같은 변경에서 고쳤다. **W5 범위는 이로써 종료다** |

**VRT 기준선 재생성(완료, 2026-08-24):** `/admin/system/logs/login` 을 A1 으로 이행하면서 시각 회귀 기준선 4장 중 하나(DEC-OPS-017: 무매칭 검색 empty-state)가 어긋났고, `update-visual-baseline` workflow_dispatch(commit=true)로 리눅스 기준선을 다시 만들어 커밋했다(`chore(e2e): 비주얼 회귀 기준선 갱신 (linux)`). win32 로컬 스냅샷 커밋은 폰트 렌더 차이로 금지다. e2e 캡처 스텝의 placeholder 셀렉터도 조회 조건 이행에 맞춰 함께 갱신했다. 이후 wave 11~13 은 기준선 4장이 캡처하는 화면(`/admin`·로그인 로그·공통코드 허브·로그인)을 건드리지 않았고, PR CI 의 `e2e-test` 가 green 이다.

...
> **판정 규칙:** 등록·수정 입력은 **모달을 기본**으로 한다. 전용 페이지는 아래 셋 중 **하나 이상**을 충족할 때만 쓴다.
>
> 1. **첨부 파일 또는 리치 텍스트 에디터**가 있다 — 업로드 진행·본문 편집이 모달 높이를 넘어선다.
> 2. **다단계 마법사**다 — 단계 간 이동과 되돌아가기가 필요하다.
> 3. **폼 자체가 공유 대상 URL**이다 — 그 주소를 남에게 보내는 것이 과업의 일부다.
>

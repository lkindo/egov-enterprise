...
상태: **구현 반영 / 격리 Windows 일반·계측 전수 E2E 통과 / 문서·메모리 현행화**. 검증일은 2026-09-21이다. 현재 모집단은 계약별 **50개 spec·135개 본 테스트와 인증 setup 2개**다. 이 수는 이행 대조 결과이며 유지해야 할 개수 목표가 아니다. Linux CI 전수 실행과 성능 재측정이 끝났다는 의미도 아니다.
...
변경 전 [CI run 35555990020](https://github.com/lkindo/egov-enterprise/actions/runs/35555990020)의 두 JSON 리포트에는 `full-suite` **27개 파일·140개 테스트**가 있었다. 테스트별 duration 합은 약 **516.8초**였고, 각 샤드의 실행 스텝은 약 **132초**, 전체 샤드 잡은 약 **7.1분**이었다. 이는 이전 구조의 단일 실행 표본이며 새 구조의 실측, PR 전체 경과시간 또는 p50/p95가 아니다.
...
| 실제 E2E·Linux VRT | Windows의 새 일회용 DB에서 **136 passed(본 테스트 134 + setup 2), 플랫폼 skip 1, unexpected/flaky/global error 0**. 목록·결과 완전성 및 cleanup 통과. Playwright 구간 540.8초 | 로컬 worker 1과 정적 검사 병행 결과로, 이전 Linux CI 성능과 직접 비교하지 않는다. Linux VRT 1개 및 새 원격 CI는 미검증 |
| 실제 계측 E2E | 별도의 새 일회용 DB와 계측 production build에서 **136 passed, 플랫폼 skip 1, unexpected/flaky/global error 0**. Playwright 구간 595.0초. 수집 JSON 104개의 형식 검증과 HTML 보고서 생성 성공 | 일반 실행과 분리된 선택적 계측 결과다. 두 실행 모두 소유한 프로세스·DB를 회수했으며 공유 DB를 사용하지 않음 |
...
[duration profile](../../frontend/e2e/shard-duration-profile.json)은 이전 CI run `33525822467`의 파일별 duration을 **보존한 test 선언 비율로 새 파일에 재배분한 추정치**다. `historical-declaration-allocation`이라는 계산 방법을 기록했으며 **새 50개 파일을 측정한 값이 아니다**. 날짜를 새 실행으로 위장하지 않고, 이 값으로 배분한 두 shard의 실제 균형은 새 Linux CI에서 재측정한다. 오래된 측정값은 재측정 신호로 보고하되 잘못된 provenance·누락 spec·잘못된 duration은 계속 차단한다.

| 다음 확인 | 완료 판단 |
|---|---|
| 재편된 전수 런타임 | 실행 inventory와 결과가 일치하고 예상 밖 skip·flaky·오류가 없음. Linux에서는 VRT도 실행 |
| CI 병렬화·캐시 | 기존 필수 실패 차단을 유지하고 cold/warm·대표 PR의 준비/완료 시간을 비교 |
| shard 가중치 | 실제 API/browser duration으로 현재 추정치를 갱신하고 worker·shard 수와 함께 기록 |
...
최초 분석에서 독립 작업을 병렬화할 경우 backend PR 약 32분→20분, FE PR 약 20분→11~12분을 계산상 후보로 보았다. 이는 이전 표본에 대한 조건부 추정이며 이번 변경의 달성 실적이 아니다. 최적 프로세스의 성과는 **결함 탐지와 실행 완전성을 지키면서 실제 PR 대기와 유지 비용이 줄었다는 후속 증거**로 판단한다.

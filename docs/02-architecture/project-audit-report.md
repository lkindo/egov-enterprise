...
### 4.1-b. Zod 코드젠 동기화 파이프라인 — ✅ 해결(재조정 완료 2026-07-07)
...
- **✅ 재조정 실행(2026-07-07)**: DB 복구 후 백엔드(`:8080`)를 기동해 live `/v3/api-docs` 를 취득 → **fresh 스펙에 `NetworkDto` 정상 방출 확인**(schemas 256→**257**, props `ntwrkId/manageIem/ntwrkIp/gtwy/subnet/domnServer/userNm/useYn`). `api-docs.json` 갱신 후 `codegen:zod`+`codegen:file` 재생성 → `NetworkDtoSchema` 반영, `NetworkForm` 무결. **`tsc --noEmit`·`next build` 둘 다 exit 0**, 두 생성물 **재생성 결정성(deterministic) 확인**(md5 동일)이라 커밋 후 `codegen:verify:zod` 가드 그린. *(주: 백엔드 기동은 Flyway V1.10 체크섬 불일치 + V1.11/V1.12 소스 유실로 선(先)차단되어 있었음 — 로드맵 §7 참조.)*
...
- **📋 리프 다운 감사(2026-07-06)**: 79개 client `page.tsx` 를 훅·이벤트 핸들러 직접 사용 여부로 분류했다:
  ...
  - **✅ 아일랜드 추출 후 승격(refactor) 39개 — 실행 완료(2026-07-07)**: 워크플로우 병렬 오케스트레이션(40 agents)으로 **39개 전부** page 내부 훅/핸들러/JSX 를 신규 `*Client.tsx` 아일랜드로 verbatim 추출 → page 는 `'use client'` 없는 서버 셸로 전환. 동적 `[id]` 페이지는 `useParams()` 훅을 아일랜드 내부 유지. 39 신규 Client 파일 + 39 page 셸. `tsc --noEmit`·`next build` 둘 다 exit 0(RSC 서버/클라이언트 경계) + 원본 대비 behavior-preservation diff 확인. 커밋 `dcd8f369f`.
  - **✅ 실행(2026-07-06)**: 아일랜드 위임 `page.tsx` **31개**에서 `'use client'` 제거→서버 셸화 완료. 그 과정에서 정작 `'use client'` 가 없던 클라이언트 아일랜드 **3개**(`KnowledgeHubClient`/`WorkflowHubClient`/`LayoutManagerClient`, 훅 사용)에 `'use client'` 를 추가 — 부모 지시자에 얹혀 있던 **잠재 결함을 교정**(리프-다운의 실체). `next build` **Compiled successfully(서버/클라이언트 경계 검증 통과)** + `tsc --noEmit` 클린으로 검증. `next/dynamic({ ssr: false })` 를 쓰는 `admin/community/boards/master/page.tsx` **1개**는 'use client' 필수라 스킵. **✅ 추가 실행(2026-07-07)**: 정적 placeholder `page.tsx` **5개**(`admin/survey/templates`·`questions`·`respondents`·`polls/manage`·`items` — 훅·핸들러 0건, `PageHeader`+lucide 아이콘+텍스트만)에서 `'use client'` 제거→서버 컴포넌트 승격, `tsc --noEmit`+`next build` **경계 검증 통과(둘 다 exit 0)**. `dynamic({ssr:false})` 를 쓰는 `admin/observability`·`admin/community/boards/maker` **2개**는 'use client' 필수라 스킵(master 포함 총 3개 ssr:false 잔류). 🟡 잔여: refactor **39개**(page 내부에서 훅 직접 사용 → 아일랜드 추출 선행 필요).

---

## 5. 유지보수 및 안정성 최적화 방안 (이행 현황 반영)

### 방안 1: 자동 생성 Zod 스키마 활용 강제화 — ✅ 정착 + 배선 + 재조정 완료
...
5. **✅ 감사+실행 (2026-07-06)**: 리프 다운 감사 + **아일랜드 위임 31개 서버 셸화 실행 완료**(클라이언트 아일랜드 3개 `'use client'` 보정 포함, `next build` 경계 검증 통과). 1개 스킵(`ssr:false` dynamic). **✅ 정적 placeholder 5개 승격(2026-07-07, `tsc`+`next build` exit 0)** + `ssr:false` 2개(observability·maker) 추가 스킵. **✅ refactor 39개 아일랜드 추출 완료(2026-07-07, 워크플로우 40 agents, 커밋 `dcd8f369f`, tsc+next build exit 0)** — §4.2 리프 다운 전량 종결. **✅ react-hooks 백로그 전수 트리아지(2026-07-07, 6 fixed/9 false-positive/51 deferred, 커밋 `469b7eb1b`)**.
7. **✅ Flyway 마이그레이션 이력 복구 (2026-07-07)**: api-docs 재조정을 위해 백엔드를 기동하려다 **Flyway validate 실패**로 선(先)차단됨 — 근본 원인 2건. **(a) V1.10 체크섬 불일치**: 구조 재편 커밋 `974d7bb7a`(2026-06-15)에서 파일 바이트 변경, DB 적용(2026-07-05)본과 불일치. DB 실스키마는 의도와 100% 일치(개명·DROP 반영 확인)라 DDL 무변경 → **history 체크섬만 정합**(flyway repair 등가). **(b) V1.11·V1.12 소스 유실**: `create pst id seq`·`add menu soft delete index`가 DB엔 적용(2026-07-05)됐으나 어떤 git 브랜치에도 파일 부재 → 공유 OCI DB가 소스보다 앞선 SSOT 드리프트. **DB 실객체(`pst_id_seq`·`idx_tb_menu_info_del_yn`)에서 마이그레이션 파일 복원**. flyway CLI/플러그인 부재로 repair는 **Flyway 체크섬 알고리즘을 Node로 재현(V1.10 known값 2145427713으로 교차검증 일치) → `flyway_schema_history` 3건 체크섬 UPDATE**로 수행. 재기동 성공(18.5s, validate 통과). ⚠️ **운영 주의**: 공유 OCI DB에 마이그레이션을 직접 적용하고 `.sql`을 커밋하지 않으면 재발 — 적용=커밋 원칙 필요.

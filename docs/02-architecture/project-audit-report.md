...
5. **✅ 감사+실행 (2026-07-06)**: 리프 다운 감사 + **아일랜드 위임 31개 서버 셸화 실행 완료**(클라이언트 아일랜드 3개 `'use client'` 보정 포함, `next build` 경계 검증 통과). 1개 스킵(`ssr:false` dynamic). **✅ 정적 placeholder 5개 승격(2026-07-07, `tsc`+`next build` exit 0)** + `ssr:false` 2개(observability·maker) 추가 스킵. 🟡 잔여: refactor 39개(page 내부 훅 직접 사용 → 아일랜드 추출 선행 필요).

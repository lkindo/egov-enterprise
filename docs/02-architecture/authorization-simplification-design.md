...
메뉴를 보여줄 권한과 데이터를 읽을 권한은 서로 다르다. NAVIGATION은 그룹 권한 테이블에 저장하며, NAVIGATION 보유만으로 OPERATION을 얻지 못한다. 아래 전환 수치는 §3의 조사 시점 기준이며 현재 배정 건수를 뜻하지 않는다.
...
실제 사용자에게 보이는 메뉴는 **소속 그룹의 NAVIGATION 합집합**에 사용 상태와 조상 연결 조건을 적용한 결과다. 한 그룹에서 부모와 후손을 회수해도 다른 그룹의 배정은 유지된다. 서로 다른 그룹이 제공한 부모와 자식이 합집합에서 연결될 수도 있다. 상단 메뉴와 하위 메뉴 조회는 같은 [메뉴 서비스의 필터링된 트리](../../business-core/src/main/java/nuri/business/service/menu/MenuService.java)를 사용하므로 하위 메뉴 번호를 직접 조회해 숨겨진 가지를 복구하지 않는다.

**메뉴 숨김만으로 화면의 직접 URL 진입을 차단하지 않는다.** `/admin` 화면 진입은 [등록 페이지별 정책](../../config/governance/permission-catalog.json)을 [페이지 인가 함수](../../frontend/src/lib/auth/page-authorization.ts)가 별도로 판정한다. 필요한 OPERATION을 가진 사용자는 메뉴가 없어도 해당 화면에 진입할 수 있고, 등록 정책에 따라 인증만 필요한 화면도 있다. 미등록 하위 경로는 부모 화면의 허용을 상속하지 않는다. 메뉴가 보이거나 화면에 진입했더라도 실제 API 조회·변경에는 기능 권한과 소유자·참여자·개인정보 등 자료별 조건이 계속 적용된다.

```mermaid
flowchart LR
    G[현재 사용자 소속 그룹] --> N[NAVIGATION 합집합]
    N --> T[사용 상태와 조상 연결 확인]
    T --> M[메뉴 표시]
    G --> O[OPERATION 합집합]
    G --> P[등록 페이지 정책: 인증 또는 필요한 기능]
    O --> P
    P --> E[화면 진입]
    O --> A[API 기능 권한과 자료별 조건]
    A --> D[조회·변경]
```
...
관리 화면의 주 흐름은 **그룹별 기능 선택**과 **사용자별 복수 그룹 배정** 두 가지다. 기능 정의·URL 패턴·그룹 상속 편집은 노출하지 않는다. 현재 [그룹 편집 화면](../../frontend/src/app/admin/security/authority/components/AuthorizationGroupEditor.tsx)은 도메인별 검색이 가능한 기능권한 표와 별도의 메뉴 계층 트리를 제공한다. 검색 결과 밖의 기능 선택도 유지하며, 메뉴 계층 조작은 §4.5의 선택 규칙을 따른다. 전체 할당과 최신 버전을 확보하지 못했거나 상위 메뉴 선택이 빠진 상태에서는 저장할 수 없다. 저장 전 추가·회수 건수와 현재 그룹에 미치는 영향을 표시하고, 다른 그룹이 제공하는 같은 권한은 유지됨을 안내한다. 메뉴만 선택하고 기능권한이 하나도 없는 경우에는 확인 안내를 표시하되 기능권한을 자동 부여하지 않는다. 사용자 분류 그룹은 권한 그룹과 구분한다.

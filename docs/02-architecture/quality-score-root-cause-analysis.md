...
- **남은 낮은 점수는 더 이상 CI 부재 때문이 아니다** — FE unit line 23.51%, 읽기 인가 30건, 수동 PK 51엔티티(주소록 2종 V2_49부터 메모보고 V2_60까지 12개 엔티티 BIGINT IDENTITY 전환 완료, `Template`은 자연키 유지), i18n 0.7%, DTO 길이 자동전파 미완이 측정 가능한 상한이다.

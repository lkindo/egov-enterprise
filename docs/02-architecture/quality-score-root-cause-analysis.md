...
> **갱신: 2026-08-15** — 4축 전면 재측정(§1.6)의 DB **91/91/85**·BE **90/82/74**·FE **85/83/70**·SEAM **77/84/85**, 12축 단순평균 **83.1**은 유지한다. 이후 E2E 3샤드 required check 131건, 하네스 30클래스/37테스트, 프런트 88파일/444테스트와 30/25/25/30 coverage 하한을 재검증했고 입력 계약 2차 표적을 총 11 DTO·길이 49필드·enum 7필드로 확장했다. 별도 위험가중 완성도 점수와 이 구조 성숙도 평균은 혼용하지 않는다.
...
**핵심 관찰(현재)**: 최대 누적 이동은 SEAM 추적성 **+37**, 적합성 **+26**, 계약체인 **+13**이다. CI·E2E·하네스가 실제 병합 신호로 바뀐 결과이며, 07-28에 이미 점수화한 OpenAPI/DB 체인은 중복 가산하지 않았다. 반대로 가장 낮은 축은 FE 단순성 70·BE 단순성 74다. 읽기 인가 census·FE coverage 30/25/25/30·입력 DTO 의미 계약 2차 표적은 08-15에 폐포됐고, 이제 상한은 **표적 밖 요청 계약, 암호 호환, i18n/core 경계 제품결정과 운영 실증**이다.
...
- **프런트 검증(2026-08-15 재측정)**: Vitest **88파일/444테스트** 전부 통과. 전체 소스 coverage는 statements **30.11%**, branches **26.05%**, functions **25.04%**, lines **30.64%**이고 하한도 30/25/25/30으로 고정했다. include 축소 없이 사용자 권한·보안 그룹·배너·공통코드·모니터링·오류 경계를 보강한 결과다.
- **계약 체인**: 하네스는 **30클래스/37테스트·실패 0**이다. 읽기 부채를 세기만 하던 census를 제거해 조회·쓰기 하네스가 면제 없이 직접 차단하고, `InputContractMirrorLinterTest` 2건이 관리자 입력 11 DTO의 길이 49필드·Y/N enum 7필드를 Entity와 OpenAPI에 결속한다.
...
|---:|---|---|---|
| **1** | 입력 계약 자동 전파 범위 | 사용자·콘텐츠·공통코드·권한·조직 11 DTO의 길이 49필드·enum 7필드는 자동 대조, 그 밖의 입력 DTO는 미측정 | 게시판 마스터 등 다음 고위험 `@RequestBody` 묶음의 RED census와 표적 추가 |
| **2** | 구조 레거시 | 자동 PK 후보 43종은 V2_83까지 전환 완료했고 잔여 19종은 자연·복합·외부키 유지 판정. eGov 결합은 비밀번호 호환·ARIA 암호에 좁게 잔존 | 기존 암호 데이터 호환과 키 회전 설계 후 레거시 암호 경로 축소 |
| **3** | 제품화 결정 | i18n API 소비 파일 약 0.7%, business-core 안 survey/community 45 Java | 다국어 파일럿/단일언어 선언 및 core/app 경계 결정 |
| **4** | FE 후속 회귀 탐지력 | statements 30.11%, branches 26.05%, functions 25.04%, lines 30.64% | 현 하한 유지 + 신규 고위험 기능 파일 단위 80% 지향 |
...
현재 추가 상승을 막는 핵심은 **좁게 남은 암호 호환(A), 코어/샘플 및 i18n 제품결정(B), 입력 DTO까지 완전 자동 전파되지 않은 계약(D), 외부 운영 실증(H)**이다. 수동 PK 현대화·읽기 인가 census·FE 30/25/25/30 coverage는 완료됐으며, 이후에는 도메인별 설계 결정과 단계적 래칫이 필요하다.
...
- **잔여**: 2차 표적 밖의 요청 DTO는 아직 필드 단위 의미 대응을 측정하지 않았다. 따라서 “전수 자동전파 완료”가 아니라 **고위험 입력 2차 묶음까지 자동화 완료**이며, 다음 묶음도 서비스 저장 경로를 확인한 뒤 표적을 늘려야 한다.
...
- **누르는 점수(정정)**: FE auth는 **HttpOnly+프록시+서명검증으로 현대화 완료**했고 DB RBAC도 집행 중이다. unit coverage는 30/25/25/30 하한까지 상향됐으며, FE 상한은 i18n 미결정·잔여 색상 104건·nonce 기반 CSP 제품결정이다.
...
| D. 입력 계약 자동전파 | 계약체인 | 2차 표적 자동화(11 DTO·길이 49·enum 7) | 게시판 마스터 등 다음 고위험 요청 DTO 묶음 확장 |
...
- **남은 낮은 점수는 더 이상 CI 부재 때문이 아니다** — FE coverage·읽기 인가·자동 PK 후보·입력 계약 2차 표적은 폐포됐다. 측정 가능한 상한은 FE/BE 단순성, 표적 밖 요청 계약, i18n 0.7%, core/app 제품 경계와 레거시 암호 호환이다.
- **다음 상승은 두 갈래다** — 저장소 내부에서는 입력 계약 3차 묶음과 FE coverage 후속 래칫을 계속하고, 구조/제품 영역에서는 i18n·core/app 경계·암호 호환을 명시 결정 후 단계적으로 바꾼다.

> 한 줄: **“검증 뼈대와 30/25/25/30 커버리지 하한은 완성됐고, 이제 제품·데이터 계약 결정과 운영 실증을 갚는 단계”**다.

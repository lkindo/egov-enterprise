...
**핵심 관찰(현재)**: 최대 누적 이동은 SEAM 추적성 **+37**, 적합성 **+26**, 계약체인 **+13**이다. CI·E2E·하네스가 실제 병합 신호로 바뀐 결과이며, 07-28에 이미 점수화한 OpenAPI/DB 체인은 중복 가산하지 않았다. 반대로 가장 낮은 축은 FE 단순성 70·BE 단순성 74다. 읽기 인가 census·FE coverage 30/26/25/31·입력 DTO 의미 계약 3차 표적은 08-15에 폐포됐고, 이제 상한은 **표적 밖 요청 계약, 암호 호환, i18n/core 경계 제품결정과 운영 실증**이다.
...
현재 추가 상승을 막는 핵심은 **좁게 남은 암호 호환(A), 코어/샘플 및 i18n 제품결정(B), 입력 DTO까지 완전 자동 전파되지 않은 계약(D), 외부 운영 실증(H)**이다. 수동 PK 현대화·읽기 인가 census·FE 30/26/25/31 coverage는 완료됐으며, 이후에는 도메인별 설계 결정과 단계적 래칫이 필요하다.
...
- **남은 낮은 점수는 더 이상 CI 부재 때문이 아니다** — FE coverage·읽기 인가·자동 PK 후보·입력 계약 3차 표적은 폐포됐다. 측정 가능한 상한은 FE/BE 단순성, 표적 밖 요청 계약, i18n 0.7%, core/app 제품 경계와 레거시 암호 호환이다.
- **다음 상승은 두 갈래다** — 저장소 내부에서는 위험 기반 입력 계약 표적과 FE coverage 래칫을 계속하고, 구조/제품 영역에서는 i18n·core/app 경계·암호 호환을 명시 결정 후 단계적으로 바꾼다.

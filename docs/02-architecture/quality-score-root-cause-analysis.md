...
- **남은 낮은 점수는 더 이상 CI 부재 때문이 아니다** — FE unit line 23.51%, 읽기 인가 30건, 단일 수동 PK 32엔티티(주소록 2종 V2_49부터 블로그 V2_70까지 31개 엔티티 BIGINT IDENTITY 전환 완료, `SurveyRespondent`는 물리 복합키 매핑 정합화, `BoardMaster`·`Template`은 업무 자연키 유지), i18n 0.7%, DTO 길이 자동전파 미완이 측정 가능한 상한이다.

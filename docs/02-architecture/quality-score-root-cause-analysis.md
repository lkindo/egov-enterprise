...
> **갱신: 2026-08-15** — 4축 전면 재측정(§1.6)의 DB **91/91/85**·BE **90/82/74**·FE **85/83/70**·SEAM **77/84/85**, 12축 단순평균 **83.1**은 유지한다. 이후 E2E 3샤드 required check 131건, 하네스 30클래스/37테스트, 프런트 88파일/444테스트와 30/25/25/30 coverage 하한을 재검증했다. 별도 위험가중 완성도 점수와 이 구조 성숙도 평균은 혼용하지 않는다.
...
**핵심 관찰(현재)**: 최대 누적 이동은 SEAM 추적성 **+37**, 적합성 **+26**, 계약체인 **+13**이다. CI·E2E·하네스가 실제 병합 신호로 바뀐 결과이며, 07-28에 이미 점수화한 OpenAPI/DB 체인은 중복 가산하지 않았다. 반대로 가장 낮은 축은 FE 단순성 70·BE 단순성 74다. 읽기 인가 census·FE coverage 30/25/25/30·입력 DTO 의미 계약 1차 표적은 08-15에 폐포됐고, 이제 상한은 **입력 계약의 다음 고위험 범위, 암호 호환, i18n/core 경계 제품결정과 운영 실증**이다.
...
- **계약 체인**: 하네스는 **30클래스/37테스트·실패 0**이다. 읽기 부채를 세기만 하던 census를 제거해 조회·쓰기 하네스가 면제 없이 직접 차단하고, `InputContractMirrorLinterTest` 2건이 관리자 입력 4 DTO의 길이 20필드·Y/N enum 4필드를 Entity와 OpenAPI에 결속한다.
...
|---:|---|---|---|
| **1** | 입력 계약 자동 전파 범위 | 사용자·배너·팝업·커뮤니티 4 DTO의 길이 20필드·enum 4필드는 자동 대조, 그 밖의 입력 DTO는 미측정 | 다음 고위험 `@RequestBody` 묶음의 RED census와 표적 추가 |
...
- **잔여**: 1차 표적 밖의 요청 DTO는 아직 필드 단위 의미 대응을 측정하지 않았다. 따라서 “전수 자동전파 완료”가 아니라 **고위험 입력 1차 묶음의 자동화 완료**이며, 다음 묶음도 서비스 저장 경로를 확인한 뒤 표적을 늘려야 한다.
...
| D. 입력 계약 자동전파 | 계약체인 | 1차 표적 자동화(4 DTO·길이 20·enum 4) | 다음 고위험 요청 DTO 묶음 확장 |
...
- **남은 낮은 점수는 더 이상 CI 부재 때문이 아니다** — FE coverage·읽기 인가·자동 PK 후보·입력 계약 1차 표적은 폐포됐다. 측정 가능한 상한은 FE/BE 단순성, 입력 계약의 다음 범위, i18n 0.7%, core/app 제품 경계와 레거시 암호 호환이다.
- **다음 상승은 두 갈래다** — 저장소 내부에서는 입력 계약 2차 묶음과 FE coverage 후속 래칫을 계속하고, 구조/제품 영역에서는 i18n·core/app 경계·암호 호환을 명시 결정 후 단계적으로 바꾼다.

...
- **완성도(+2)**: #4(상태코드 시맨틱 정합 404/400)·#5(tx readOnly 경계 일관)가 직접 기여하는 correctness 개선. 기반 견고(GlobalExceptionHandler 13핸들러·@Valid 44파일/95회·서비스 hasRole 이중검증+SecurityAuthAnnotationLinterTest 오딧[⚠ 2026-08-02 실측 정정: '전수' 아님 — 쓰기 전수 + 읽기 7.0%, `.business`/`.foundation` 읽기 49건은 사각지대]·컨트롤러 Entity반환 위반 0). #1~#3 은 단순성 성격이라 완성도 기여 한계적.

# eGov Enterprise 프레임워크 재사용성·확장성 진단 (역사 스냅샷)

> **상태: Superseded / 삭제 후보** — 리팩터 전 진단과 실행 로그가 한 파일에 누적되어 본문의 점수·경로·결론 대부분이 현재 코드와 맞지 않는다. 현행 제품 경계는 [ADR-0001](decisions/ADR-0001-core-app-product-boundary.md), 생성 범위는 [reusable-base-guide.md](../03-guides/reusable-base-guide.md)와 `config/reusable-base-profiles.json`, 활성 위험은 [.agent/memory/known-gaps.md](../../.agent/memory/known-gaps.md)를 사용한다.
...
- **CI/CD·품질 게이트의 이식성 불명** — 파생본이 현재 required checks와 JaCoCo/PITest/보안 게이트를 어떻게 승계할지는 생성 산출물에서 직접 검증해야 한다.
...
**역사적 결론**: 작성 당시에는 재사용 준비도를 약 50/100으로 평가했으나, 이후 코어 승격·모듈 분리·베이스라인·생성 프로필이 구현됐다. 이 수치와 결론을 현재 평가로 재사용하지 않는다.

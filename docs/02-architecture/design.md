...
본 시스템은 기존의 실용주의적 관리자 UI와 현대적인 프리미엄 대시보드 UI를 하나의 디자인 언어로 통합한 **"Unified Premium"** 전략을 채택합니다. 단순히 시각적 효과를 나열하는 것이 아니라, 작업의 성격(데이터 중심 vs 인사이트 중심)에 따라 시각적 밀도를 유연하게 조정합니다.

*   **Core Philosophy**: 
    *   **Cohesive Identity**: 어떤 페이지에서도 동일한 브랜드 컬러(Hub Blue)와 서체(Pretendard)를 사용하여 일관성을 유지합니다.
    *   **Adaptive Density**: 데이터 밀집도가 높은 페이지는 샤프하고 촘촘하게, 시각적 요약이 중요한 페이지는 부드럽고 여유롭게 구성합니다.
    *   **Selective Premium**: `backdrop-blur`와 같은 고비용 시각 효과는 핵심 대시보드 위젯에 집중하여 성능과 가독성을 동시에 확보합니다.
...
*   **Scale**:
    ...
    *   **Label**: 900 Black / 0.4em Tracking / Uppercase. 메타데이터 및 태그.
...
*   **Premium Hub Radius (Dynamic Scale)**: 어드민 페이지(`LayoutManagerClient`)의 **System Design Engine**을 통해 전역으로 제어되는 동적 곡률 시스템입니다. (기본 Base Factor `1.2rem` 기준)
    ...
    *   `--radius-hub-widget` (Base × 2.0): 벤토 카드 등 중간 크기 위젯.
    *   `--radius-hub-item` (Base × 1.5): 핵심 강조 버튼 및 목록 아이템.
    *   *Note: Hub 관련 컴포넌트 개발 시 하드코딩된 Tailwind 클래스(`rounded-xl` 등) 대신, 가급적 이 동적 CSS 변수를 활용하여 시스템 제어와 동기화되도록 구성해야 합니다.*
...
*   **Hierarchy of Depth**:
    1.  **Surface (Sunken)**: 배경 (`slate-50` / `dark:slate-950`)
    2.  **Base (Raised)**: 일반 카드 및 콘텐츠 영역.
    3.  **Floating (Premium)**: 주요 요약 정보. `premium-glow` 효과로 강조.
*   **Grid Strategy**: 
    *   데이터 중심 페이지: 표준 12컬럼 그리드.
    *   인사이트 중심 페이지: 유연한 벤토(Bento) 스타일 그리드.

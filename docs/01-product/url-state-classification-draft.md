...
| 2 | 회의에서 **Q4(죽은 표면 14건)를 먼저 처리** — 잃는 동작이 없어 다른 답을 기다릴 필요가 없다 | 없음 |
| 3 | **Q1·Q2 의 외부 입력 확보** — 운영 리버스프록시·WAF·CDN 의 쿼리스트링 로깅 여부와 보존 기간 | 운영자만 답할 수 있다. **저장소로는 확인 불가**(census `limitations` 3번) |
| 4 | **Q3·Q4 의 외부 입력 확보** — 운영 DB `tb_menu_info.modern_route`·`chk_url` 의 쿼리 키 전수(읽기 전용 census) | `db-bridge` 허용 읽기 SQL 범위. 이 환경에는 접속 변수가 없어 실행하지 못했다 |

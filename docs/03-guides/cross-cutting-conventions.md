...
| **집행 게이트(부분)** | `PkGenerationStandardLinterTest` — 신규 @Entity 단일 @Id에 `@GeneratedValue` 부재 시 위반. GRANDFATHERED 62종 동결. 주소록 2종은 `V2_49`에서 BIGINT IDENTITY로 전환해 동결 목록에서 제거했다. |

...
| **집행 게이트(부분)** | `PkGenerationStandardLinterTest` — 신규 @Entity 단일 @Id에 `@GeneratedValue` 부재 시 위반. GRANDFATHERED 56종 동결. 주소록 2종은 `V2_49`, 도움말은 `V2_50`, 인터넷 서비스 안내는 `V2_51`, 온라인 매뉴얼은 `V2_52`, 팝업은 `V2_53`, 마이페이지 콘텐츠는 `V2_54`, 부서업무함은 `V2_55`에서 BIGINT IDENTITY로 전환해 동결 목록에서 제거했다. `Template`은 값 자체가 레이아웃 계약인 자연키로 재판정해 유지한다. |

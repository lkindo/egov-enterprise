...
#### 3.1 사전 검증 (Pre-Audit)
모든 개발 및 수정 작업 착수 전:
1. **실증적(Evidence-Based) 표준 조회**: 새로운 테이블/필드를 설계하거나 쿼리를 작성할 때, 에이전트의 지레짐작(Assumption)을 엄격히 금지한다. 반드시 `database/Public Data Standard.xlsx` 또는 실제 로컬 DB 브리지 쿼리(예: `information_schema` 조회)를 가동하여 **"실제 존재하는"** 물리 스키마와 표준 명칭을 눈으로 확인한 뒤 실행한다.
...
- **Frontend**: 서버 컴포넌트 우선 원칙, 디자인 토큰 사용, 반응형 브레이크포인트 준수 여부. [프론트엔드 헌법 제2조, 제6조]

    ...
    public List<SystemConnectionStatsDto> findSystemConnectionStats(String searchKeyword) {
        ...
        return queryFactory
                .select(Projections.constructor(SystemConnectionStatsDto.class,
                        ...
                        new CaseBuilder()
                                .when(transmitReceiveLog.transmitReceiveSeCode.in("S01", "S04")).then(1L)
                                .otherwise(0L).sum().as("cntAll"),
                        new CaseBuilder()
                                .when(transmitReceiveLog.transmitReceiveSeCode.in("S02", "S05")).then(1L)
                                .otherwise(0L).sum().as("cntSuccess"),
                        new CaseBuilder()
                                .when(transmitReceiveLog.transmitReceiveSeCode.in("S03", "S06")).then(1L)

...
public interface ScheduleRepository extends JpaRepository<Schedule, String> {
    ...
    @Query("SELECT s FROM Schedule s WHERE s.schdulSe = :schdulSe AND " +
            "((:schdulSe = '1' AND s.frstRegisterId = :ownerId) OR (:schdulSe = '2' AND s.schdulDeptId = :ownerId))")
    Page<Schedule> findSchedules(@Param("schdulSe") String schdulSe, @Param("ownerId") String ownerId,
            Pageable pageable);

    @Query("SELECT s FROM Schedule s WHERE s.schdulSe = :schdulSe AND " +
            "((:schdulSe = '1' AND s.frstRegisterId = :ownerId) OR (:schdulSe = '2' AND s.schdulDeptId = :ownerId)) AND "
            +
    ...
    @Query("SELECT s FROM Schedule s WHERE s.frstRegisterId = :userId")

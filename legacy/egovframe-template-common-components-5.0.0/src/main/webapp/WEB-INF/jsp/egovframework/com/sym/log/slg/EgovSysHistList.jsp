		...
		<%@ taglib prefix="ui" uri="http://egovframework.gov/ctl/ui" %>
			<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
				<%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
					<% /** * @Class Name : EgovSysHistList.jsp * @Description : 시스템 이력 정보목록 화면 * @Modification
						Information * @ * @ 수정일 수정자 수정내용 * @ ---------- -------- --------------------------- * @
						2009.03.06 이삼섭 최초 생성 * 2011.07.08 이기하 패키지 분리로 경로 수정(sym.log -> sym.log.slg)
						...
						*/
						...
						<body>
							...
							<div class="board">
								<h1>
									<spring:message code="comSymLogSlg.sysHistList.pageTop.title" />
								</h1><!-- 시스템이력 조회 -->

								<form name="frm" action="<c:url value='/sym/log/slg/SelectSysHistoryList.do'/>"
									method="post">
									...
									<ul>
										<li>
											...
											<option value="0" <c:if test="${searchVO.searchCnd == '0'}">
												selected="selected"</c:if> >
												<spring:message code="comSymLogSlg.sysHistList.sysNm" />
											</option><!-- 시스템명 -->
											<option value="1" <c:if test="${searchVO.searchCnd == '1'}">
												selected="selected"</c:if> >
												<spring:message code="comSymLogSlg.sysHistList.histSeCodeNm" />
											</option><!-- 이력구분 -->
											</select>
							...
							<table class="board_list">
								...
								<thead>
									<tr>
										<th scope="col">
											<spring:message code="comSymLogSlg.sysHistList.index" />
										</th><!-- 번호 -->
										<th scope="col">
											<spring:message code="comSymLogSlg.sysHistList.histId" />
										</th><!-- 이력 ID -->
										<th scope="col">
											<spring:message code="comSymLogSlg.sysHistList.sysNm" />
										</th><!-- 시스템명 -->
										...
										<th scope="col">
											<spring:message code="comSymLogSlg.sysHistList.frstRegisterNm" />
										</th><!-- 등록자 -->
										<th scope="col">
											<spring:message code="comSymLogSlg.sysHistList.frstRegisterPnttm" />
										</th><!-- 등록일자 -->
										<th scope="col">
											<spring:message code="comSymLogSlg.sysHistList.fnEgovInqireSysHist" />
										</th><!-- 상세보기 -->
									</tr>
								</thead>
								<tbody>

...
백엔드 헌법은 JPA Entity의 class-level `@Builder`·`@SuperBuilder`를 금지한다. 따라서 새 Entity에서 이 패턴을 도입하지 말고 의도가 드러나는 정적 factory에 method-level `@Builder`를 적용한다. 이 문서의 shadowing 지침은 허용된 비-Entity 상속 DTO 또는 기존 호환 코드에만 적용한다.
...
- Entity 변경이면 `EntityConventionRules` 등 관련 아키텍처 테스트도 실행한다.

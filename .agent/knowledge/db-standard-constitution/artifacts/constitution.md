...
2. 엔터프라이즈 데이터 무결성과 복원력을 위해, 물리적인 `DELETE` 실행은 철저히 금지된다. 삭제 작업은 반드시 `use_yn = 'N'` 또는 `del_yn = 'Y'` 컬럼 상태값 변경을 통한 **논리 삭제(Soft Delete)** 방식을 원칙으로 한다. 단, 테스트 환경(`@ActiveProfiles("test")`)에서의 데이터 클린업 목적 물리 DELETE는 예외로 허용한다.

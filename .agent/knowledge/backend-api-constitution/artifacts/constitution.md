...
2. **합성 인가 애노테이션의 실행 의미** *(2026-08-20 승인 현행화)*: 컨트롤러 1차 인가의 표준 수단은 합성 메타 애노테이션(`@AdminOnly`, `@AdminOrSystem`, `@Authenticated`)이다. 단, **`@AdminOnly`(=`hasRole('ADMIN')`)는 역할 계층(`ROLE_SYSTEM > ROLE_ADMIN`, V2_3 시드)에 의해 SYSTEM 역할도 통과시킨다** — 이름이 암시하는 "ADMIN 단독"과 실행 의미가 다르므로, SYSTEM 까지 배제해야 하는 자원에는 이 애노테이션을 쓰지 말고 별도 명시 인가를 설계한다. URL 게이트(secure-paths/RBAC)가 걸리지 않는 별칭·공개 경로에서는 **메서드 애노테이션이 단독 방어선**이므로 이를 지우는 것은 인가 제거이며, endpoint↔SpEL↔정책 의미 정합은 `SecurityAuthAnnotationLinterTest`(정책 registry `authorization-policies.json` exact-match)로 기계 강제된다. *(적용 사례: 2026-08-20 제품 결정으로 설문 열람·문항 조회·응답 제출을 `@Authenticated` 로 일반 개방하고, 설문 별칭 URL 게이트는 V2_84 로 제거하되 관리 기능은 `@AdminOrSystem` 메서드 인가로 이관 — GAP-AUTH-001 해소.)*
3. **인증 주체의 정체성 축 구분**: 인증 주체의 식별자는 두 축으로 구분한다 — `esntlId`(User 엔티티 PK, 시스템 내부 식별자)와 `loginId`(감사 컬럼 `frst_rgtr_id`/`last_mdfr_id` 및 소유권 비교의 표준 축). 소유권 비교 시에는 반드시 대상 도메인의 감사/소유 컬럼이 **실제로 저장하는 축**과 일치시킨다. 대부분의 도메인은 감사 컬럼(loginId)을 소유자 축으로 쓰므로 `SecurityUtil.getCurrentLoginId()`/`assertOwnerOrAdmin()` 을 사용하고, 소유자를 `esntlId` 로 저장하는 도메인(예: `InformalSanction.aplcntId`, `Board.userId`)은 `SecurityUtil.getCurrentEsntlId()` 로 비교하여 축을 일치시킨다. 이름과 달리 `esntlId` 를 반환하여 축을 뒤섞던 `getCurrentUserId()` 는 `@Deprecated` 처리되었으며, 이 회귀(프로덕션 `getCurrentUserId()` 호출 및 도메인 코드의 `SecurityContextHolder` 직접 접근)는 `IdentityAxisLinterTest`(api-server 하네스, 프로덕션 호출 0 + 직접접근 allow-list 동결)로 기계 차단된다. 상세 규약은 `docs/03-guides/identity-model-guide.md` 를 따른다.

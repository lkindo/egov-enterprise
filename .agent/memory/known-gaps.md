...
| GAP-FE-001 | P2 | open | frontend security | production CSP의 `script-src`·`style-src`에 `unsafe-inline`이 남아 있다. 2026-08-20 Phase 2로 `script-src-attr 'none'`을 추가해 inline 이벤트 핸들러(반사형 XSS 대표 벡터)는 prod/dev 모두 차단했고, 유일한 실사용처였던 Atlas HTML의 핸들러 10건을 addEventListener로 전환했다(재유입은 csp-policy 계약이 차단). 남은 `unsafe-inline` 완전 제거는 nonce+strict-dynamic(Phase 4)인데, nonce는 요청마다 달라야 하므로 **전 페이지가 동적 렌더로 강등된다(PPR 포기)** — next.config.ts 주석이 명시한 제품 결정 게이트다. style 쪽(Phase 3)은 sonner·framer-motion 런타임 `<style>` 주입 검증이 선행돼야 한다. | [next.config.ts](../../frontend/next.config.ts), [csp-policy contract](../../frontend/src/__tests__/csp-policy.test.ts), [pending decisions](../../docs/04-operations/pending-decisions.md) | PPR 포기 여부를 제품 결정으로 확정한다. 포기를 택하면 미들웨어 nonce 배선 + strict-dynamic 전환을 별도 사이클로 진행하고 CI E2E로 검증한다. 유지를 택하면 Phase 3(style-src-elem 세분화 검증)만 남기고 accepted-risk로 전환을 검토한다. | 보안/제품 소유자 | 2026-08-20 |

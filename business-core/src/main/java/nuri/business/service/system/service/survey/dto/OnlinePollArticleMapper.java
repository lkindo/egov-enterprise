 ...
 * {@code pollSn} 은 연관 엔티티 {@code pollManage.pollSn} 에서 null-safe 로 파생하며(원본 from() 과 동일),
 * {@code pollIemCo}(투표 수)는 from() 이 설정하지 않고 서비스 레이어에서 별도 집계·주입하므로 ignore 한다.
 *
 * <p>역방향(toEntity)은 flat {@code pollSn} → 연관 {@code pollManage} 로의 복원이 손실적(lossy)이라
 * 의도적으로 정의하지 않는다(안전성 우선). from() 의 유일한 책무인 entity→DTO 만 제공한다.
 */
@Mapper(componentModel = "spring", unmappedTargetPolicy = ReportingPolicy.IGNORE)
public interface OnlinePollArticleMapper {

    @Mapping(target = "pollSn", source = "pollManage.pollSn")

...
public record UserDto(
    ...
    //
    //   ⚠ 당시 이메일이 비는 것은 예외가 아니라 **정상 경로**였다: registerUser 가 emlAddr 을 받지도 않아
    //     등록 시 저장되지 않았다(mblTelno·ognzId 도 동일). 즉 방금 만든 사용자는 항상 이메일이 없었다.
    //     [2026-09-23 갱신] 그 누락은 같은 날 UserService 가 고쳤다 — 등록 빌더가 선택 필드를 함께 싣는다.
    //     아래 형식 제약 규칙은 그와 무관하게 유효하다(값이 있을 때만 형식을 본다).
    //

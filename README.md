...
> 🧩 **이 브랜치는 재사용 Base(`template/reusable-base`)입니다.** 데모 샘플 도메인 20+개가
> 코드·라우트·DB 테이블까지 제거된 "깨끗한 골격"(커널 + 게시판 + 통신 4종)이며, 신규 SI/재개발의 시작점입니다.
> ⚠ **신규(빈) 데이터베이스 전용** — 공유 데모 DB 에 부팅하지 마십시오(V2_25 가 샘플 테이블을 드롭).
> 상세: **[재사용 Base 가이드](docs/03-guides/reusable-base-guide.md)** · 데모 전량은 `main` 브랜치에 보존.

...
/** Tests a bounded MariaDB value surface with its own driver and live physical schema. */
class MariaDbValueTypesIntegrationTest extends MariaDbPostgresTestSupport {
    ...
    @Test
    void decimalAndUnsignedBigintMaximumPreserveCompleteNumericValuesAndChecksums() throws Exception {
        String name = uniqueName("numeric");
        String source = MARIADB.getDatabaseName() + "." + name;
        String target = "public." + name;
        sourceJdbc().execute("CREATE TABLE " + source + " (id bigint PRIMARY KEY, amount decimal(38,9), payload bigint unsigned) ENGINE=InnoDB");
        targetJdbc().execute("CREATE TABLE " + target + " (id bigint PRIMARY KEY, amount numeric(38,9), payload numeric(20,0))");
        ...
        assertThat(sourceJdbc().queryForObject("SELECT numeric_precision FROM information_schema.columns"
                + " WHERE table_schema=? AND table_name=? AND column_name='amount'", Integer.class,
                MARIADB.getDatabaseName(), name)).isEqualTo(38);
        ...
        var spec = spec(source, target, "mariadb-numeric-types", List.of(
        ...
        assertThat(targetJdbc().queryForObject("SELECT payload FROM " + target + " WHERE id=2", BigDecimal.class)).isEqualByComparingTo(BigDecimal.ZERO);
    ...
    @Test
    void unsignedBigintBeyondSignedLongRangeIsRejectedBeforeWriting() {
        ...
        var spec = mapping(source, target, "mariadb-unsigned-long-rejected", "long");
    ...
    @Test
    void dateAndDatetimeMicrosecondsPreserveLoadedValuesAndWholeRowChecksums() throws Exception {
        String name = uniqueName("temporal");
        String source = MARIADB.getDatabaseName() + "." + name;
        String target = "public." + name;
        sourceJdbc().execute("CREATE TABLE " + source + " (id bigint PRIMARY KEY, payload date, occurred_at datetime(6)) ENGINE=InnoDB");
        ...
        LocalDateTime time = LocalDateTime.of(2026, 9, 15, 12, 34, 56, 123456000);
        sourceJdbc().update("INSERT INTO " + source + " VALUES (1, ?, ?)", java.sql.Date.valueOf(date), java.sql.Timestamp.valueOf(time));
        sourceJdbc().update("INSERT INTO " + source + " VALUES (2, NULL, NULL)");
        try (var connection = MARIADB.createConnection("");
             var columns = connection.getMetaData().getColumns(connection.getCatalog(), connection.getSchema(), name, "occurred_at")) {
            assertThat(columns.next()).isTrue();
            assertThat(columns.getInt("DATA_TYPE")).isEqualTo(Types.TIMESTAMP);
            // The measured Connector/J metadata path reports zero; the physical precision remains six.
        ...
        assertThat(sourceJdbc().queryForObject("SELECT datetime_precision FROM information_schema.columns"
                + " WHERE table_schema=? AND table_name=? AND column_name='occurred_at'", Integer.class,
                MARIADB.getDatabaseName(), name)).isEqualTo(6);
        assertThat(sourceJdbc().queryForObject("SELECT payload FROM " + source + " WHERE id=1", java.sql.Date.class).toLocalDate()).isEqualTo(date);
        assertThat(sourceJdbc().queryForObject("SELECT occurred_at FROM " + source + " WHERE id=1", java.sql.Timestamp.class).toLocalDateTime()).isEqualTo(time);
        var spec = spec(source, target, "mariadb-temporal-types", List.of(
        ...
        assertThat(targetJdbc().queryForObject("SELECT payload FROM " + target + " WHERE id=1", java.sql.Date.class).toLocalDate()).isEqualTo(date);
        assertThat(targetJdbc().queryForObject("SELECT occurred_at FROM " + target + " WHERE id=1", java.sql.Timestamp.class).toLocalDateTime()).isEqualTo(time);
    ...
    @Test
    void fixedCharPreservesMariaDbUnicodeAndPaddingReadSemantics() throws Exception {
        String name = uniqueName("char");
        String source = MARIADB.getDatabaseName() + "." + name;
        String target = "public." + name;
        sourceJdbc().execute("CREATE TABLE " + source + " (id bigint PRIMARY KEY, payload char(12)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
        ...
        String sourceValue = sourceJdbc().queryForObject("SELECT payload FROM " + source + " WHERE id=1", String.class);
        assertThat(sourceValue).isEqualTo("  한글🙂");
        var spec = mapping(source, target, "mariadb-char-types", null);
        assertSuccessfulLoad(spec, execute(spec, MigrationMode.COMMIT), 3);
        assertThat(targetJdbc().queryForObject("SELECT payload FROM " + target + " WHERE id=1", String.class)).isEqualTo(sourceValue);
        ...
        assertCheckpoint(spec, 1, Map.of("id", 1L, "payload", sourceValue));
        assertSuccessfulLoad(spec, execute(spec, MigrationMode.COMMIT), 3);
    ...
    private void assertColumnType(String table, String column, int type, int precision, int scale) throws Exception {
        try (var connection = MARIADB.createConnection("");
             var columns = connection.getMetaData().getColumns(connection.getCatalog(), connection.getSchema(), table, column)) {
            assertThat(columns.next()).isTrue();
    ...
    private void assertCheckpoint(MappingSpec spec, long id, Map<String, ?> expected) {
        ...
        assertThat(RowChecksum.calculate(columns, targetJdbc().queryForMap("SELECT " + String.join(",", columns)
                + " FROM " + spec.tables().getFirst().target() + " WHERE id=?", id))).isEqualTo(recorded);
    }
    ...
    private static MappingSpec spec(String source, String target, String run, List<MappingSpec.ColumnMapping> columns) {
        ...
        return new MappingSpec(sourceConfig(), targetConfig(), List.of(table), Map.of(),
                new MappingSpec.RunContext(run, "synthetic-mariadb-value-types"));
    }
    private static String uniqueName(String scenario) {
        return "tb_mariadb_type_" + scenario + "_" + UUID.randomUUID().toString().replace("-", "").substring(0, 8);
    }
}

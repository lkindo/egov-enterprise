...
> **상태**: ✅ 정적 게이트 4종 전량 통과 / ⏸ 런타임 검증 보류 (인프라 다운 — SOP §4.1)
...
| 지표 | 변경 전 | 변경 후 | 비고 |
|---|---:|---:|---|
| **로컬 기본 실행**(`test:e2e`) | **226건** | **120건** | -47%. 전 테스트가 2회 돌고 있었다 |
| CI 실행(`--project=full-suite`) | 114건 | 120건 | 실행 수는 +6, **구성은 대폭 교체** |
| 스펙 테스트 | 112 | 118 | 삭제 **-10** / 신설 **+16** |
...
## 4. 신설 16건 — 전부 "구현돼 있으나 아무도 지키지 않던 계약"
...
④ 구성 무결성        --list 미지정 == --list --project=full-suite → 120 == 120 ✅
```

### ⏸ 보류 (SOP §4.1 — 정직한 보류)

**런타임 검증은 수행하지 못했다.** 착수 시점 실측:

```
:8080 backend  → 000 (DOWN)
:3001 frontend → 000 (DOWN)
docker         → daemon 미기동
```

- **확증된 것**: 타입 정합성, Playwright 구성 무결성(열거 실측), 셀렉터·엔드포인트의 **소스 대조**
  (모든 신규 단언은 `middleware.ts`·`logout/route.ts`·`UserOrgHubClient.tsx`·`UserManageForm.tsx` 원본에서 확인)
- **미확증**: 실제 실행 통과 여부
- **재개 조건**: 백엔드(:8080) + 프론트(:3001) 기동 후 전량 실행, 또는 **CI 초록 확인**

> ⚠ 이 저장소는 pre-push 에 E2E 가 없고, **UI 변경은 CI 초록이 유일한 증거**다
> (SOP §4.1 — 2026-08-06 정정). 병합 전 `mergeStateStatus` 확인 필수.
>
> 신규 16건은 셀렉터 의존이 거의 없는 **HTTP 층 단언**(URL·status·Location·JSON code)에 집중돼
> 있어 UI 셀렉터 기반 테스트보다 실패 위험이 구조적으로 낮다. 예외는 02 의 Update 단계로,
> 이것만 UI 셀렉터에 의존한다.

---

## 6. 미적용 — 근거와 함께 남기는 후속 과제

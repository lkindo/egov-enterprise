...
> 결과 판정: 위 통합 완료 조건을 충족하면 저장소 증거 기반 위험 가중 점수 **70.3/100 → 92.8/100**으로 확정하고, 그 전에는 같은 수치를 후보로 본다.
...
|---|---:|---:|---:|---:|---|
| 백엔드/API | 12% | 74 | 95 | +21 | 알림·결재·쪽지 소유권 결속, 사용자 식별자 축 정정, 읽기·쓰기 명시 인증 경계 |
...
| 테스트·품질 게이트 | 10% | 79 | 98 | +19 | JaCoCo, FE 444 tests·coverage 30/25/25/30 exact 래칫, strict mutation 10범위, 하네스 29종·인가 제거 뮤테이션 차단 |
...
| **위험 가중 종합** | **100%** | **70.3** | **92.8** | **+22.4** | 원시값 `92.76 - 70.33 = 22.43`, 각 수치는 소수 첫째 자리 반올림 |
...
- Wave 6 루트 컴파일 + 전체 하네스: **29 suites / 35 tests / 실패·오류·skip 0**
...
|---:|---|---|---|
| 1 | 입력 DTO 길이·enum 표적 미러 게이트 | DB→API 의미 계약 강화 | `@Column(length=)` 536 vs `@Size` 393 직접 대응 census |

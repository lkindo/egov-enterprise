...
> 결과 판정: 위 통합 완료 조건을 충족하면 저장소 증거 기반 위험 가중 점수 **70.3/100 → 93.0/100**으로 확정하고, 그 전에는 같은 수치를 후보로 본다.
...
|---|---:|---:|---:|---:|---|
| 백엔드/API | 12% | 74 | 96 | +22 | 알림·결재·쪽지 소유권 결속, 사용자 식별자 축 정정, 읽기·쓰기 인증 경계, 입력 길이·enum 표적 계약 |
| 보안·개인정보 | 14% | 58 | 96 | +38 | WS 인증·구독 권한·Origin 방어, 비밀정보 제거/스캔, 읽기 인가 census·클래스 면제 폐포 |
| 백엔드 신뢰성·성능 | 9% | 68 | 91 | +23 | 비동기 포화/실패 상태, 인증 인프라 장애 분류, Hikari 정규화, 저장 실패 보상 |
| 프론트 아키텍처 | 9% | 73 | 93 | +20 | 중복 WS 제거, Next proxy, 검색·로그 생성 API 타입과 페이지 계약 및 커뮤니티 1-based 페이지 정렬 |
| UX·접근성 | 8% | 67 | 87 | +20 | 테이블 키보드/ARIA, 업로더 포커스·파일 선택 경계, 시맨틱 색상 보강 |
| 프론트 보안·계약 | 8% | 83 | 97 | +14 | 사용자 전용 토픽, 검색·로그 PageResponse, Entity→DTO→OpenAPI→TypeScript/Zod 의미 제약 결속 |
...
| 테스트·품질 게이트 | 10% | 79 | 98 | +19 | JaCoCo, FE 444 tests·coverage 30/25/25/30 exact 래칫, strict mutation 10범위, 하네스 30종·계약/인가 뮤테이션 차단 |
...
| **위험 가중 종합** | **100%** | **70.3** | **93.0** | **+22.6** | 원시값 `92.96 - 70.33 = 22.63`, 각 수치는 소수 첫째 자리 반올림 |
...
### Wave 6 — 읽기 인가 census 폐포

- [x] 쓰기 사유로 읽기까지 통과하던 30건을 개인·참여자·부서 스코프 18건과 인증 사용자 공유 조회 12건으로 도메인별 판정
- [x] 주소록·스크랩·일정·메모보고·댓글·업무보고·메일·게시판 2종·설문·커뮤니티·결재 컨트롤러에 `@Authenticated` 명시
- [x] `READ_COVERED_BY_WRITE_RATIONALE` 30건과 `WRITE_AUTHZ_GUARDED_ELSEWHERE` 12개 클래스 담요 면제 제거
- [x] 면제 제거 RED → 12개 경계 적용 GREEN → 주소록 인증 제거 뮤테이션 RED → 원복 순서로 게이트 실효성 증명

### Wave 7 — 프론트 커버리지 목표 폐포

- [x] 전체 소스 include를 유지한 채 사용자 권한·보안 그룹·배너·공통코드·모니터링·오류 경계의 성공/실패/검색/페이지/파괴적 변경 흐름 25건 보강
- [x] 전체 Vitest를 88파일/444테스트로 확대하고 coverage를 statements 30.11%, branches 26.05%, functions 25.04%, lines 30.64%로 상향
- [x] coverage 하한을 22/18/17/23에서 목표치 30/25/25/30으로 강화하여 하락을 CI에서 차단
- [x] 커뮤니티 첫 페이지를 공통 0-based `page`로 넘겨 `pageIndex=2`가 되던 오프바이원 결함을 명시적 1-based `pageIndex` 계약으로 수정
- [x] ESLint 현재 실측 255건에 맞춰 경고 상한을 295에서 260으로 강화해 35건의 추가 false-green 여유 제거

### Wave 8 — 입력 DTO 의미 계약 1차 표적 자동화

- [x] 실제 요청 본문으로 저장되는 사용자·배너·팝업·커뮤니티 DTO 4종의 길이 20필드와 Y/N enum 4필드를 Entity 저장 계약에 명시적으로 결속
- [x] 배너 URL 1000→512, 인증서 DN 255→100의 저장 상한 초과 2건과 커뮤니티 길이 누락 3건을 수정
- [x] Y/N 4필드를 런타임 `@Pattern`·OpenAPI enum·생성 TypeScript union·Zod enum으로 전파
- [x] `InputContractMirrorLinterTest` 2건을 pre-push 하네스와 동결 매니페스트에 결속하고 전체 30클래스/37테스트 통과
- [x] DTO 길이 512→513 및 OpenAPI enum `[Y,N]`→`[Y]` 뮤테이션이 각각 정확히 RED가 됨을 확인 후 원복
...
- Wave 8 전체 하네스: **30 suites / 37 tests / 실패·오류·skip 0**

### 프론트엔드

- Vitest: **88 files / 444 tests**, 전부 성공
- coverage: statements **30.11%**, branches **26.05%**, functions **25.04%**, lines **30.64%** — 강화한 고정 하한 30/25/25/30% 통과
- ESLint: 오류 **0**, 경고 **255** — 강화된 고정 상한 260 통과
...
|---:|---|---|---|
| 1 | 입력 계약 표적 2차 확장 | DB→API 의미 계약 범위 확대 | 1차 4 DTO·길이 20필드·enum 4필드 완료; 다음 고위험 `@RequestBody` 묶음은 별도 RED census 후 추가 |
| 2 | i18n과 survey/community core 귀속 결정 | 템플릿 재사용성·제품 경계 확정 | 다국어 파일럿/단일언어, core/app 제품 결정 필요 |
| 3 | eGov 암호 호환 단계 축소 | BE 단순성·키관리 개선 | 자동 PK 43종은 V2_83까지 완료. 잔여 19종은 자연·복합·외부키 유지 판정이며 실제 현대화 잔여는 레거시 암호 호환이다. |
| 4 | 프론트 coverage 후속 래칫 | 회귀 탐지·타입 안정성 | 30/25/25/30 하한 유지, 신규 고위험 기능은 파일 단위 80% 지향 |
| 5 | 외부 AV/CDR·운영 배포·백업복구·DR 실증 | 저장소 밖 운영 위험 검증 | 외부 인프라와 운영 상태 변경 승인 필요 |

...
| #15 마감 후 | **1 failed** · 2 skipped · 111 passed (10.4m) |
| 03 flaky 규명·수정 후 | **1 failed**(05) · 2 skipped · 111 passed (8.5m) |
| 05 결함 2건 + 삭제 결함 마감 후 (**최종**) | **0 failed** · 2 skipped · **112 passed** (11.2m) |

**전량 그린 도달.** passed 가 111 → 112 로 는 것은 삭제 단계가 실제 검증으로 바뀌며 종전에
'통과한 척' 하던 경로가 정직한 단언으로 대체됐기 때문이 아니라, 03 스펙의 테스트 수 자체는 그대로이고
05 가 red 에서 green 으로 전환됐기 때문이다(1 failed → 0 failed, 111 → 112 passed).
...
### ✅ 규명 완료 — 앱 결함이 아니라 **테스트가 자기 요청을 끊고 있었다**

위 '판정 보류'를 재조사해 원인을 확정했다. 범인은 등록 완료 대기의 `Promise.race` 두 번째 갈래다.

```ts
page.waitForSelector('text=/성공|저장|완료/', { timeout: 30000 })
```

이 정규식은 토스트가 아니라 **제출 버튼 자신**에 매칭된다 — 버튼 라벨이 `<span>저장</span>`이고
제출 중에는 `저장 중...` 이다(BoardRegistClient).

**결정적 실측**: 제출을 전혀 하지 않은 등록 페이지에서 이 셀렉터는 **1152ms 만에 매칭**되며
matched tag=`<span>` text=`"저장"`. 반면 sonner 컨테이너로 좁힌 셀렉터는 매칭되지 않는다.

인과 사슬:

```
race 즉시 종료(버튼 매칭) → URL 은 아직 insert-board-article
  → "아직 등록 페이지면 목록으로" 분기가 page.goto 실행
    → 진행 중이던 Server Action POST 가 net::ERR_ABORTED
      → 앱의 await 가 TypeError: Failed to fetch 로 거절 → 콘솔 오류 → ConsoleGuard red
```

부하가 높을수록 액션이 늦어져 이 창이 넓어진다 — **"full-suite 에서만 실패하고 격리 실행은 전부
통과"의 정체가 이것이다.** 앱은 처음부터 정상이었고, 우려했던 '사용자 중복 등록 위험'도 이 경로에서는
성립하지 않는다(사용자는 저장 직후 0ms 에 다른 페이지로 이동하지 않는다).

> **🚨 종전 조사의 핵심 증거가 반증됐다.** 문서 상단은 *"같은 요청에 200 과 ERR_ABORTED 가 동시에
> 기록된다 = 응답이 시작된 뒤 연결이 끊긴다"* 를 이상 징후로 보고 조사 방향을 잡았다. 그러나 부하를
> 걸고 등록을 8회 반복한 진단 결과, **정상 저장·정상 이동한 회차에서도 예외 없이 같은 패턴이
> 기록된다**(200 + ERR_ABORTED). `router.push` 가 액션 응답의 RSC 스트림을 취소하는 **정상 흔적**이며
> 결함의 증거가 아니다. 이 오독이 조사를 앱 쪽으로 오래 끌고 갔다.

**처방**: 셀렉터를 sonner 컨테이너(`[data-sonner-toast]`)로 좁혀 버튼과 분리했다. 토스트는
`await saveBoardArticle()` 이 resolve 된 **뒤에만** 뜨므로, 토스트를 진짜로 기다리면 액션 완료가
보장된다. 수정(update) 단계에도 동일한 결함이 있어 함께 고쳤다.

*(race 를 걷어내고 URL 전환만 기다리는 안도 실측했으나 채택하지 않았다 — 이후 단계의 진입 페이지가
목록→상세로 바뀌며 Step 4 삭제가 깨졌다. 아래 별건 참조.)*

### 🚨 신규 발견 — Step 4 삭제는 삭제를 증명한 적이 없다 (false-green, 미해결)

추적 중 드러났다. 삭제는 확인 모달(`useConfirm`)을 통과해야 실행된다
(`const isConfirmed = await confirm({...}); if (!isConfirmed) return;`). 그런데 테스트는 모달을
확인하지 않고 곧바로 목록으로 이동해 '사라졌는지'만 본다 — **삭제 액션은 실행되지 않는다.**

그럼에도 통과하던 이유: 대상이 목록 1페이지 밖이면 locator 가 아무것도 잡지 못해 `toBeHidden` 이
성립한다. **그린은 삭제의 증거가 아니라 페이지네이션의 부산물이었다.**

수정 시도는 보류했다. 모달 확인 클릭을 넣었더니 델리트 버튼은 포커스를 받는데(`[active]`) 모달이
열리지 않아 **15분 타임아웃까지 매달렸다**. 하이드레이션 완료 전 `<form action={fn}>` 의 네이티브
submit 이 조용히 삼켜지는 것으로 의심되며, 사실이라면 **빠른 클릭에 삭제가 무반응인 앱 측 문제**다.
테스트 수정과 앱 조사를 함께 해야 하므로 별도 과제로 분리한다. 코드에는 false-green 사실을 주석으로
명시해 두었다 — 고치기 전까지 이 그린을 삭제의 증거로 인용하지 말 것.

### ✅ 05 Portal Promotion Flow — flaky 가 아니라 **결함 2건**이었다

"4회 중 2회 실패"라 flaky 로 분류했으나, 격리 실행에서 **100% 실패**함을 확인하고 조사했다.
결과는 시간에 따라 악화되는 구조적 결함 두 개였다(그래서 초반 회차만 통과했다).

**① 청소 스크립트가 배너를 한 건도 지우지 못했다 (false-green)**

`cleanup-db.ts` 는 `b.bannerNm` / `banner.bannerId` 로 필터·삭제하는데 `BannerDto` 의 실제 필드는
**`bnrNm` / `bnrId`** 다. `testBanners` 가 항상 빈 배열이라 로그는 늘 `=> 0 banner(s) cleaned` 를
찍었고 — **청소된 것처럼 보였다.** 필드명을 고치자 즉시 **7건이 삭제**됐다(전부 E2E 잔재).

**② 메인 배너는 한 번에 한 장만 보여주는 캐러셀이다**

`BannerSlider` 는 `banners[currentIndex]` 하나만 렌더하고 5초마다 회전한다. 그런데 테스트는
"안 보이면 reload" 를 3회 반복했다 — **reload 는 currentIndex 를 0 으로 되돌리므로, ①로 누적된
배너 뒤에 붙은 새 배너는 몇 번을 새로고침해도 영원히 보이지 않는다.**
→ 순서 의존을 없애고 '다음 슬라이드' 버튼으로 한 바퀴 순회해 찾도록 바꿨다.

**검증(위반 주입)**: 필러 배너 3건을 주입해 새 배너가 첫 장이 아닌 조건 — 즉 종전 코드가 100%
실패하던 조건 — 을 인위적으로 만든 뒤 실행해 **통과**를 확인했다.

### 🚨 앱 결함 — 게시글 삭제가 아예 동작하지 않았다 (E2E false-green 이 덮고 있었다)

위 '삭제 단계 false-green' 을 제대로 고치려다 드러났다. **실사용자도 삭제할 수 없는 상태였다.**

측정 사실(프로덕션 빌드):

| 시도 | 결과 |
|---|---|
| 삭제 버튼 클릭 | 액션 진입 로그는 찍힌다 → 액션은 실행됨 |
| 확인 모달 | `dialog count = 0` — 끝내 열리지 않는다 |
| 3초 대기 후 재클릭 | 동일하게 0 |
| `form.requestSubmit()` 직접 호출 | 동일하게 0 |
| 콘솔·페이지 오류 | **없음** (그래서 "눌러도 아무 일이 없는" 증상으로만 드러남) |

**원인**: React 19 의 form action 은 transition 으로 실행된다. `<form action={async (fd) => {
await confirm(...) }}>` 구조에서는 `confirm()` 이 일으키는 모달 open state 갱신이 그 transition 에
묶인다. 액션은 모달의 응답을 기다리고, 모달의 렌더는 액션이 끝나야 커밋되므로 **서로를 막는 교착**이
된다.

**처방**: 확인을 transition 밖(`onClick`)에서 먼저 받고, 확정된 뒤에 서버 액션을 호출한다. 폼이
없어져 `useFormStatus` 를 쓸 수 없으므로 중복 클릭 방지용 `deleting` 상태를 명시적으로 뒀다.

**위반 주입 증거**: 앱 수정 **전** 상태에서 이 E2E 변경(모달 확인 + 완료 대기)은 15분 테스트
타임아웃까지 매달려 red 였고, 앱 수정 **후** green 이다. 즉 새 단언은 이 결함을 실제로 잡는다.

> **교훈**: false-green 테스트는 단순히 "검증을 안 하는" 것에 그치지 않는다. **실제 결함을 덮는다.**
> 삭제 단계가 모달을 확인하지 않은 덕분에 "삭제가 전혀 동작하지 않는다"는 P0 급 결함이 E2E 22티어를
> 통과해 왔다. 그린의 출처를 묻지 않으면 그린은 안전의 증거가 되지 못한다.

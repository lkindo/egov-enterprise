...
### ⏸ 잔여 red — 05 Portal Promotion Flow (별개 flaky)

03 을 고친 뒤 남은 유일한 red 다. 4회 전량 실행 중 2회 실패했고 증상은 매번 동일하다 —
`bannerTitleLoc` 미표시(10s 타임아웃, 요소 자체가 없음). 등록한 배너가 사용자 화면에 노출되기까지의
반영 지연 또는 노출 조건(기간·사용여부) 문제로 보이나 **조사하지 않았다.** 추정으로 닫지 않는다.

...
| 그 11건의 총 LOC | 316 |
| **11건의 getter 호출부** | **0개소** ← 전량 `.builder()` 로 생성돼 Jackson 이 직렬화할 뿐, Java 코드가 getter 로 읽지 않는다 |
| 하네스 린터 결속 | `UserSignupRequest` 1건만 (`SignupContractLinterTest`) → **배치에서 제외** |

**중요 발견**: getter 호출부가 0이므로 record 전환의 최대 비용(`getX()` → `x()` 전면 개명)이 **발생하지 않는다.**
Lombok `@Builder` 는 record 에서도 동작하므로 `.builder()` 호출부도 무변경이다.
즉 **이 11건은 비용 0의 깨끗한 전환 대상**이다.

**그럼에도 이번에 적용하지 않은 이유**
- 이 11건은 **이미 불변**이다(`@Setter` 없음, 다수가 `private final`). 따라서 얻는 것은
  **LOC 316 뿐이고 §2.4 가 이 Phase 의 정당화 근거로 삼은 '불변화' 이득은 0** 이다.
- 자동 변환 스크립트가 CRLF·애노테이션 캡처에서 반복 실패했다. 억지로 밀어붙이는 것은
  §0.7-H4 가 금지하는 기계적 sweep 이며, 세션 말미에 계약 레이어를 반쯤 변환된 상태로 두는 것은
  더 나쁘다. **`--apply` 를 실행하지 않아 워킹트리는 깨끗하다.**

**재개 시 권장 순서**
1. 11건을 **수동으로** 하나씩 전환(파일당 20~50줄, 구조 균일). 각 건마다 `codegen:verify` 로 계약 무변경 확인.
2. 그 다음이 진짜 목표다 — **`@Setter` 보유 64건의 불변화.** 여기가 §2.4 가 말한 실익이며,
   저장소 전체 `setXxx(` 호출 565개소 중 대상 DTO 를 가리키는 것을 **건별 실측**해야 한다.
   일괄 전환 금지(§0.7-H4).

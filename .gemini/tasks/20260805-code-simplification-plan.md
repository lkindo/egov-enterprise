...
> ### ⚠ 파일럿 실행 결과 — **"순수 분할로 지표를 개선한다" 는 전제가 틀렸다** (2026-08-05)
>
> `MonitoringHubClient` 를 파일럿으로 실행했다. 표시 전용 컴포넌트 6개(391줄)를
> 로직 변경 없이 분리해 **1,387 → 997줄**(−28%)을 달성했고 `tsc`·`next build`·FE 단위
> 213건 전량 그린이다. 그런데 **§1.3 지표는 하나도 움직이지 않았다.**
>
> | 지표 | 파일럿 전 | 파일럿 후 |
> |---|---:|---:|
> | 600줄 초과 파일 수 | FE 10 | **FE 10** (997 > 600 이라 여전히 계상) |
> | FE 클라이언트 LOC 비중 | 64.4% | **64.4%** (client→client 이동이라 불변) |
>
> **왜 안 움직이는가**: 이 파일에서 순수 이동이 가능한 것은 **꼬리의 표시 컴포넌트뿐**이고,
> 그것을 전부 떼어내도 본체가 997줄이다. 본체는 `useQuery` 9개와 탭 분기가 얽힌 덩어리라
> 더 쪼개려면 **상태 경계를 판정**해야 한다 — 그것은 '순수 이동' 이 아니라 별개의 리팩터다.
>
> **따라서 4-2 를 지표 개선 수단으로 계획한 것은 오류다.** 4-2 가 실제로 기여하는 것은
> **리뷰 정확성**이다(1,387줄에 인가·쿼리 로직과 정적 안내 패널이 뒤섞여 있던 것이 분리됐다).
> 지표 개선은 **4-1(데이터 페칭 서버 이관)** 이 담당해야 한다 — 클라이언트 LOC 를 실제로
> 줄이는 것은 그쪽뿐이다.
>
> **재계획 시**: 600줄 초과 파일 10개를 600 아래로 내리려면 파일당 **본체 분할**이 필요하고,
> 그것은 파일마다 상태 경계 판정이 선행되는 개별 작업이다. "N개 파일을 분할한다" 로
> 일괄 계획할 수 있는 성격이 아니다.

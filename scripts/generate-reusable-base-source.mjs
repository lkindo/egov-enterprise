...
export function adaptGeneratedHarness(output) {
  const replacements = [
    ...
    /*
      투영본은 공용 support를 사용하는 역사적 migration 검증 44개를 제거한다(pruneHistoricalMigrationTests —
      ...
      DisplayName 도 함께 고친다 — 0건을 세면서 "44개" 라고 말하면 화면이 사실과 다른 말을 한다.
    */
    {
      path: join(output, 'api-server', 'src', 'test', 'java', 'nuri', 'api', 'harness', 'SharedPostgresMigrationHarnessContractTest.java'),
      from: 'private static final int EXPECTED_MIGRATION_TEST_COUNT = 44;',
      to: 'private static final int EXPECTED_MIGRATION_TEST_COUNT = 0;',
    },
    {
      path: join(output, 'api-server', 'src', 'test', 'java', 'nuri', 'api', 'harness', 'SharedPostgresMigrationHarnessContractTest.java'),
      from: '@DisplayName("44개 migration 검증은 개별 container lifecycle 없이 공용 PostgreSQL support를 사용한다")',

...
function validReviewBy(value, checkedAt) {
  return validDate(value) && validDate(checkedAt) && value >= checkedAt;
}
...
export function validateKrdsMapping(contract, { root = ROOT } = {}) {
  const errors = [];
  ...
  if (!validDate(contract.checkedAt) || !validReviewBy(contract.checkBy, contract.checkedAt)) {
    errors.push('upstream review requires real dates and checkBy on or after checkedAt');
  }
  if (!contract.owner?.trim()) errors.push('mapping owner is missing');
  ...
  for (const profile of profiles) {
    if (!profile.purpose || !profile.currentClaim || !profile.maximumClaimWithoutReleaseEvidence
        || !profile.identityElements || !profile.identityOwner?.trim() || !validReviewBy(profile.reviewBy, contract.checkedAt)
  ...
  for (const entry of mappings) {
    ...
    if (entry.disposition === 'deferred'
        && (!entry.reason || !entry.owner?.trim() || !validReviewBy(entry.reviewBy, contract.checkedAt))) {

...
for root, dirs, files in os.walk(app_dir):
    if "page.tsx" in files or "page.js" in files:
        ...
        existing_routes[route] = os.path.join(root, "page.tsx" if "page.tsx" in files else "page.js")
...
for item in db_data:
    ...
    # Direct match check
    if norm_url in existing_routes:
        continue
        
    # Dynamic route match check
    found = False
...
print("### 링크(DB URL)와 실제 프론트엔드 파일(page.tsx)이 일치하지 않는 목록 ###")
print("| 순번 | DB에 등록된 URL | 상태 | 비고 |")
print("| :--- | :--- | :--- | :--- |")
for i, url in enumerate(sorted(list(set(mismatches))), 1):
    print(f"| {i} | `{url}` | ❌ 파일 없음 | 경로 수동 확인 필요 |")

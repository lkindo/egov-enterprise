...
- [x] 프로젝트 공유 및 이식성 설정 (체크리스트 루취 이관, 초기화 경로 상대화)
- [/] 프로그램 관리 (`NPROGRMLIST`) JPA 전환 가이드 작성 및 적용
- [/] 메뉴 관리 (`NMENUINFO`) JPA 전환 가이드 작성 및 적용
- [ ] 메뉴 생성 관리 (`NMENUCREATDTLS`) 권한 매핑 검증
- [ ] **사용자 확인 요청**: 1순위 시스템관리 기본 기능 작동 여부

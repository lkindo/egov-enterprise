...
import { expect, test as baseTest } from '../fixtures/browser-test';
...
test.describe('Enterprise Workflow & Productivity', () => {
     ...
     */
    // 역순 teardown: actor page의 관찰·종료를 마친 뒤 두 임시 결재자를 삭제한다.
    test('Workflow: 결재를 올리고 승인해 세 탭을 완주한다', async ({ page, request, approver, finalApprover, actorPage }) => {
        ...
        expect(((await typesRes.json()).data as Array<{
            dtlCd: string;
        }>).some(code => code.dtlCd === taskCode)).toBe(true);
        const meRes = await request.get(`${API_BASE}/users/me`, { headers });
        expect(meRes.ok()).toBeTruthy();
        const me: {
            userNm?: string;
            esntlId?: string;
        } = (await meRes.json()).data;
        expect(me.userNm).toBeTruthy();
        expect(me.esntlId).toBeTruthy();
        expect(approver.esntlId).not.toBe(me.esntlId);
        expect(finalApprover.esntlId).not.toBe(me.esntlId);
        ...
        await dialog.getByRole('button', { name: '다음 단계 추가' }).click();
        await pickActor(2, finalApprover);
        await dialog.getByLabel('단계 유형').nth(1).selectOption('AGREEMENT');
        await dialog.getByRole('button', { name: '다음', exact: true }).click();
        ...
        await expect(item(page, documentTitle)).toHaveCount(0);
        const { page: firstPage } = await actorPage({ storageState: approver.storageState });
        const { page: finalPage } = await actorPage({ storageState: finalApprover.storageState });
        await firstPage.goto('/approvals');
        await finalPage.goto('/approvals');
        // 다음 단계 사용자는 문서를 조회해도 현재 차례를 처리할 수 없다.
        const premature = await request.get(`${API_BASE}/approvals/${approvalId}`, { headers: finalApprover.authorization });
        ...
        await item(page, documentTitle).getByRole('button').click();
        ...
        expect((await resubmitted.json()).data).toBe(approvalId);

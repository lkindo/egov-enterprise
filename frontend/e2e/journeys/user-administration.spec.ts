    ...
    test.describe('Admin System (Core Management)', () => {
        ...
        test.describe('User Management Lifecycle', () => {
            ...
            test('Create-Search-Update-Delete Flow', async ({ page }) => {
                ...
                await expect(page.locator('input[name="emlAddr"]'), '등록 때 입력한 이메일이 수정 폼에 실려 오지 않았다 — 등록이 입력값을 버렸을 수 있다').toHaveValue(`${testId}@egov.kr`, { timeout: 15000 });
                ...
                if (!updateResponse.ok()) {
                    ...
                    expect(updateResponse.ok(), `사용자 수정 PUT 이 ${updateResponse.status()} 로 실패했다.\n`
                        + `  응답 본문: ${body}\n`
                        + `  보낸 페이로드: ${sent}`).toBeTruthy();
                }
...
test.describe('인증 경계', () => {
    ...
    test.describe('Deterministic empty state', () => {
        test.use({ storageState: 'playwright/.auth/admin.json' });
        test('user search with guaranteed-nonexistent term shows a real empty state', async ({ page }) => {
            ...
            await expect(page.getByText(/검색 결과가 없습니다|데이터가 존재하지 않습니다/).first()).toBeVisible({ timeout: 15000 });
        });
    });
});

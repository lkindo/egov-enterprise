...
test.describe('Enterprise Mail System E2E', () => {
    test.use({ storageState: 'playwright/.auth/admin.json' });
    let mailPage: MailPage;
    test.beforeEach(async ({ page }) => {
        mailPage = new MailPage(page);
    });
    test('should preserve a 256-character mail subject in history', async () => {
    ...
    test('Mail: Multi-recipient Dispatch', async () => {
    ...
    test('Mail: Empty Recipient Validation', async ({ page }) => {
        ...
        await expect(page.locator('[data-form-error-summary="true"]')
            .getByRole('button', { name: '수신자: 수신자를 선택해 주세요.' })).toBeVisible();
    ...
    test('Mail: pick myself by name via recipient finder', async ({ page, request }) => {
        ...
        const adminToken: string | undefined = authData.cookies.find((c: {
            name: string;
            value: string;

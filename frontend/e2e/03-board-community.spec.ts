    ...
    test.describe('Article Lifecycle Management', () => {
        ...
        for (const template of templates) {
            test(`CRUD Flow for ${template.name}`, async ({ page }) => {
                ...
                await expect(page.locator('input[name="pstTtl"]').first()).toBeVisible({ timeout: 30000 });
                
                console.log('>>> Step 2: Creating Article');
                await page.locator('input[name="pstTtl"]').fill(articleTitle);
                await page.locator('.ProseMirror').fill('Initial E2E test content.');

                const commitBtn = page.locator('button:has-text("Commit Knowledge"), button:has-text("게시글 등록"), button[aria-label="게시글 등록"]').first();
                await commitBtn.click();

                // Wait for navigation with relaxed criteria
                await page.waitForURL((url) => !url.href.includes('insertBoardArticle'), { timeout: 45000, waitUntil: 'domcontentloaded' });
                ...
                await test.step('User: Navigate to Created Article', async () => {
                    if (createdpstId) {
                        ...
                        const firstArticleLink = page.locator('a[href*="pstId="]').first();
                ...
                await page.waitForURL((url) => url.href.includes('detail') || url.href.includes('selectBoardList'), { timeout: 30000 });
                ...
                if (createdpstId) {
                    ...
                    await page.locator('a:has-text("[Updated]")').first().click();
                }

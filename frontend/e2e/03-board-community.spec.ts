...
test.describe('Tier 3: Board & Community (Business Flow)', () => {
    ...
    test.describe('Board Master Wizard Flow', () => {
        ...
        test('End-to-End Board Creation and Deployment', async ({ boardMasterPage }) => {
            ...
            await boardMasterPage.fillStep2('ì§€???ˆë¸Œ');
    ...
    test.describe('Article Lifecycle Management', () => {
        ...
        for (const template of templates) {
            test(`CRUD Flow for ${template.name}`, async ({ page }) => {
                // Triple the timeout for this multi-step CRUD flow (120s Ã— 3 = 360s)
                test.slow();
                ...
                const commitBtn = page.locator('button:has-text("Commit Knowledge"), button[aria-label="ê²Œì‹œê¸€ ?€??]').first();
                ...
                const editBtn = page.getByLabel('ê²Œì‹œê¸€ ?˜ì •').first();
                ...
                await expect(page.getByText(/?±ê³µ?ìœ¼ë¡?(?˜ì •|?±ë¡)?˜ì—ˆ?µë‹ˆ???€?¥ë˜?ˆìŠµ?ˆë‹¤/)).toBeVisible({ timeout: 15000 });
                ...
                const deleteBtn = page.getByLabel('ê²Œì‹œê¸€ ?? œ').first();

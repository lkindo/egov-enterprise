 ...
 * 
 * ?œìŠ¤?œì˜ ë³´ì•ˆ ê²½ê³„ë¥??¬ì¸µ?ìœ¼ë¡?ê²€ì¦í•©?ˆë‹¤.
 * ?¹ížˆ ê¶Œí•œ???†ëŠ” ?¬ìš©?ê? IDë¥?ì¡°ìž‘?˜ì—¬ ?¤ë¥¸ ?¬ìš©?ì˜ ?°ì´?°ì— ?‘ê·¼?˜ëŠ” ?œë‚˜ë¦¬ì˜¤(IDOR)?€ 
 * ë³µí•©?ì¸ ?¸ì ??ê³µê²©???€??UI/API ë°©ì–´?¥ì„ ì²´í¬?©ë‹ˆ??
 */
    ...
    test.describe('IDOR (Insecure Direct Object Reference) Protection', () => {
        ...
        test('Access Denied for Direct User ID Manipulation', async ({ page, consoleGuard }) => {
            ...
            if (url.includes('auth_error=unauthorized') || url === 'http://localhost:3001/') {
                ...
                expect(bodyText).toMatch(/ê¶Œí•œ|?‘ê·¼|Deny|Unauthorized|Forbidden/i);
        ...
        test('XSS Sanitization: Complex Payloads in Board Comments', async ({ page }) => {
            ...
            if (await commentInput.isVisible()) {
                for (const payload of payloads) {
                    ...
                    await page.locator('button:has-text("?±ë¡"), button:has-text("Comment"), button:has-text("Commit Response")').click();
    ...
    test.describe('URL Integrity & Navigation Guards', () => {
        ...
        test('Handling Malformed UUID/IDs in URLs', async ({ page, consoleGuard }) => {
            ...
            for (const path of malformedPaths) {
                ...
                const errorState = page.locator('text=?†ìŠµ?ˆë‹¤, text=not found, text=?¤ë¥˜, text=invalid, .error-container').first();

 ...
 * 
 * ?œìŠ¤?œì˜ ë¹„ê¸°?¥ì  ?ˆì§ˆê³??Œë³µ?¥ì„ ê²€ì¦í•©?ˆë‹¤.
 * 1. ë³´ì•ˆ (RBAC, CSRF, XSS)
 * 2. UX (?™ê????…ë°?´íŠ¸, ?ë™ ?€??
 * 3. ?‘ê·¼??(WCAG 2.1) ë°??œê°???Œê?
 * 4. ê´€ì¸¡ì„± (ê°ì‚¬ ë¡œê·¸, ì½˜ì†” ê°€??
 */
...
test.describe('Tier 4: Quality & Resilience', () => {
    ...
    test.describe('Advanced UX & Performance', () => {
        ...
        test('Optimistic UI: Post Like/Reaction', async ({ page }) => {
            ...
            const likeBtn = page.locator('button').filter({ hasText: /ì¶”ì²œ|ì¢‹ì•„??Like/i }).first();
    ...
    test.describe('System Observability', () => {
        ...
        test('Audit Log Consistency', async ({ page }) => {
            ...
            const noResults = page.getByText(/ê²€??ê²°ê³¼ê°€ ?†ìŠµ?ˆë‹¤|No results/i);

...
describe('커뮤니티 상세: 지어낸 지표와 죽은 버튼을 두지 않는다', () => {
  ...
  /*
    [2026-08-28] 원 계약: 이 섹션은 어떤 조회도 하지 않으면서 '등록된 게시글이 없습니다' 라고
    단정했다 — 실제로 글이 있는 커뮤니티에서도 비었다고 말했다. 그래서 '아직 제공되지 않습니다'
    (미제공 고지)를 동결했다.

    [2026-09-08 PD-CMTY-001] 이제 실제로 조회한다. 미제공 고지를 그대로 두면 그 문장이 거짓이
    되므로 동결 대상을 옮긴다 — 지키려던 규칙은 "미제공이라고 말하라" 가 아니라
  ...
  */
  it('조회하지 않은 채 게시글이 없다고 단정하지 않는다', () => {
    ...
    // 미제공 고지는 더 이상 사실이 아니다.
    expect(client).not.toContain('아직 제공되지 않습니다');
    // 조회한 뒤의 빈 결과와, 조회하지 않은 사유는 서로 다른 문장이어야 한다.
    expect(client).toContain('이 커뮤니티에 등록된 게시판이 없습니다.');
...
describe('생성 마법사가 만드는 상태를 사실대로 말한다', () => {
  ...
  it('권한 삭제 안내가 서버의 정리 범위를 넘어 약속하지 않는다', () => {
    const hub = stripComments(readSrc('app/admin/security/authority/components/AuthorizationGroupEditor.tsx'));
    ...
    const service = stripComments(
      readRepo('business-core/src/main/java/nuri/business/service/auth/AuthorizationAdministrationService.java'),
    );
    const start = service.indexOf('public void deleteGroup(');
    expect(start, '정규 삭제 실행 경로를 찾지 못했다').toBeGreaterThan(-1);
    const end = service.indexOf('@Transactional', start + 1);
    expect(end).toBeGreaterThan(start);
    const body = service.slice(start, end);
    expect(body).toContain('requireVersion(before.version(),expectedVersion)');
    expect(body).toContain('RESERVED.contains(code)');
    expect(body).toMatch(/SELECT count\(\*\) FROM tb_authrt_user_map[\s\S]*RESOURCE_IN_USE/);
    expect(body.indexOf('RESOURCE_IN_USE')).toBeLessThan(body.indexOf('DELETE FROM tb_authrt_info'));
    expect(body).not.toContain('DELETE FROM tb_authrt_user_map');
  });

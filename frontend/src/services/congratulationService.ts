...
/**
 * 경조사 관리 서비스 (Admin)
 * 백엔드: com.company.project.api.controller.system.CongratulationManageController
 */
...
export const congratulationService = {
    ...
    getCongratulation: async (congratulationId: string, ..._args: any[]) => {
        return client.get<CongratulationManage>(`${BASE_URL}/${congratulationId}`);
    },

    /** 경조사 직접 등록 */
    createCtsnn: async (data: Partial<CongratulationManage>, ..._args: any[]) => {
        return client.post<string>(BASE_URL, data);
    },

    /** 경조사 정보 수정 */
    updateCtsnn: async (congratulationId: string, data: Partial<CongratulationManage>, ..._args: any[]) => {

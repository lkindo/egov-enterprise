...
class DeptAuthorityAdminService extends AdminService {
 ...
 async getDeptAuthorities(deptId: string, config?: any): Promise<DeptAuthorProjection[]> {
 return this.get<DeptAuthorProjection[]>(`/${deptId}`, config);
 }

 /** 부서 권한 일괄 설정 */
 async updateDeptAuthorities(data: DeptAuthorBatchRequest, config?: any): Promise<void> {

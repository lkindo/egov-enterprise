...
import { monitoringAdminService, ProcessMon } from '@/services/admin/system/MonitoringAdminService';
...
export function ProcessMonitor() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const result = await monitoringAdminService.getProcessMonList({ page: 0, size: 50 });
  ...
  const handleCheck = async (id: string) => {
    try {
      setLoading(true);
      await monitoringAdminService.checkProcessStatus(id);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await monitoringAdminService.deleteProcessMon(id);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await monitoringAdminService.createProcessMon(formData);

    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="p-4 space-y-12 text-left">
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[11px] font-black text-slate-800 flex items-center gap-1.5 ml-1 uppercase tracking-tight">
                  보안 역할 식별자(Role Code) <span className="text-rose-500 font-extrabold text-[10px]">*</span>
                </FormLabel>
                ...
                <FormMessage className="text-[10px] font-bold text-rose-500 mt-2 ml-2 tracking-tight" />
              </FormItem>
            )}
          />
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-[11px] font-black text-slate-800 flex items-center gap-1.5 ml-1 uppercase tracking-tight">
                  역할 레이블 명칭 <span className="text-rose-500 font-extrabold text-[10px]">*</span>
                </FormLabel>
        ...
        <div className="flex w-full gap-6 pt-4 border-t border-slate-100">
          <Button type="button" variant="outline" onClick={onCancel} className="flex-1 h-14 rounded-xl font-black text-[10px] tracking-widest border-2">취소</Button>
          <Button
            type="submit"
            disabled={isSubmitting}
            className="flex-[2] h-14 rounded-xl bg-slate-900 border-none text-white font-black text-[10px] tracking-widest shadow-2xl hover:bg-primary transition-all hover:-translate-y-2 group px-6"
          >

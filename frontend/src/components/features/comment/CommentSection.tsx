    ...
    <div className="space-y-10 pt-16 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      <div className="flex items-center justify-between border-b pb-6">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-lg bg-slate-900 flex items-center justify-center shadow-xl shadow-slate-200">
      ...
      <div className="space-y-6">
        {comments.length === 0 ? (
          <div className="py-20 text-center border-2 border-dashed border-slate-100 rounded-lg bg-slate-50/50">
            <p className="text-slate-400 font-bold tracking-tight">아직 등록된 댓글이 없습니다. 첫 번째 댓글을 남겨보세요!</p>
          </div>
        ) : (
          comments.map((comment) => (
            <Card key={comment.id} className="border-none shadow-xl shadow-slate-100/50 rounded-lg overflow-hidden bg-white ring-1 ring-slate-50 hover:ring-slate-100 transition-all group">
              <CardContent className="p-8">
                <div className="flex flex-col gap-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-slate-100 border-2 border-white shadow-sm flex items-center justify-center">
                  ...
                  {editingId === comment.id ? (
                    <Textarea
                      ...
                      className="min-h-[100px] rounded-lg border-slate-200 focus:ring-slate-900 border-2 text-slate-700 font-medium"
                    />
                  ) : (
      ...
      <form onSubmit={handleSubmit} className="relative group pt-10">
        <div className="absolute -inset-1 bg-gradient-to-r from-slate-200 to-slate-100 rounded-lg blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"></div>
        <Card className="relative border-none shadow-2xl rounded-lg bg-white ring-1 ring-slate-100 overflow-hidden">
          <CardContent className="p-8 space-y-4">
            ...
            <div className="flex justify-end border-t pt-6">
              <Button
                ...
                className="h-11 px-8 rounded-lg bg-slate-900 hover:bg-black text-white font-bold shadow-xl shadow-slate-200 flex gap-2 active:scale-95 transition-all"
              >

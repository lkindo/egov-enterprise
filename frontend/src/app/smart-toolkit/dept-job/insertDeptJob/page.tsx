...
const InsertDeptJobPage = () => {
 ...
 alert(error.response?.data?.message || '?±ë¡???¤íŒ¨?˜ì??µë‹ˆ??');
 } finally {
 setLoading(false);
 }
 };
 ...
 <span className="text-[10px] font-black tracking-[0.25em] text-white/80">?Œí¬?Œë¡œ???œìŠ¤??2.0</span>
 </div>
 ...
 ë¶€?œì˜ ?ˆë¡œ???…ë¬´ë¥??•ì˜?˜ê³  ? ë‹¹?©ë‹ˆ?? <br />ëª…í™•??ëª©í‘œ ?¤ì •???µí•´ ?¨ìœ¨?ì¸ ?‘ì—…???œì‘?˜ì„¸??
 </p>
 </div>
 </div>
 </CardHeader>
 ...
 <Textarea
 id="deptJobCn"
 placeholder="?…ë¬´???ì„¸ ëª©í‘œ, ?˜í–‰ ë°©ë²•, ?”ì²­ ?¬í•­ ?±ì„ êµ¬ì²´?ìœ¼ë¡??œìˆ ?˜ì„¸??."
 ...
 <p className="font-black text-2xl text-slate-900 tracking-tight ">? íš¨??ê²€???„ìš”</p>
 <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-[450px]">
 ?±ë¡???…ë¬´??ë¶€???„ì²´ ?€?œë³´?œì— ì¦‰ì‹œ ?¸ì¶œ?©ë‹ˆ?? ê¸°ì…???´ìš©??ë¶€???´ì˜ ê°€?´ë“œ?¼ì¸??ì¤€?˜í•˜?”ì? ?•ì¸??ì£¼ì„¸??
 </p>
 </div>
 </div>
 </CardContent>
 ...
 <ArrowLeft className="w-6 h-6 mr-4" /> ì·¨ì†Œ
 </Button>
 </Link>
 ...
 <div className="w-3 h-3 bg-white rounded-full" /> ?€??ì¤?..
 </span>
 ) : (
 <>

...
export default function DeptJobPage() {
  ...
  const loadJobs = useCallback(async () => {
    try {
      setLoading(true);
      const res = await deptJobUserService.getDeptJobs({ page: 0, size: 20 });
      setData(res.list || []);
    } catch (error) {
  ...
  return (
    <div className="space-y-6 pb-12">
      <PageHeader
        title="부서 업무 작업 센터"
        breadcrumbs={[{ label: '작업지원' }, { label: '부서업무' }]}
        actions={
          <button onClick={() => router.push('/smart-toolkit/dept-job/insertDeptJob')} className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl font-bold shadow-md hover:shadow-lg transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatItem title="완료" count={data.filter(j => j.sttus === '2').length} icon={<CheckCircle2 size={20} />} color="green" />
      </div>

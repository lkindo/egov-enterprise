...
export function DashboardVisitorChart({ data }: ChartProps) {
 ...
 if (!chartData.length) return <div className="h-[300px] flex items-center justify-center text-muted-foreground">?곗씠?곌? ?놁뒿?덈떎.</div>;
...
export function DashboardPostChart({ data }: ChartProps) {
 ...
 if (!chartData.length) return <div className="h-[200px] flex items-center justify-center text-muted-foreground text-sm font-bold tracking-tight opacity-30">?꾩뒪 鍮꾪솢님/div>;

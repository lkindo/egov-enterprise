...
export function BannerSlider() {
 ...
 const [current번호, setCurrent번호] = useState(0);
 ...
 setCurrent번호((prev) => (prev + 1) % banners.length);
 }, 5000);
 return () => clearInterval(timer);
 }, [banners.length]);

 const prevSlide = () => {
 setCurrent번호((prev) => (prev === 0 ? banners.length - 1 : prev - 1));
 };
 ...
 onClick={() => setCurrent번호(idx)}
 className={cn(
 "w-2 h-2 rounded-full transition-all",
 idx === current번호 ? "bg-white w-6" : "bg-white/40"
 )}
 />
 ))}
 </div>

...
export function SmartNotificationHub() {
 ...
 id: 'nt-001',
 type: 'system',
 title: 'ë³´ì•ˆ ?•ì±… ë³€ê²??ˆë‚´',
 content: '?¨ìŠ¤?Œë“œ ë§Œë£Œ ì£¼ê¸° ?•ì±…??30?¼ë¡œ ?¨ì¶•?˜ì—ˆ?µë‹ˆ??',
 recipient: '?„ì²´ ?¬ìš©??,
 ...
 type: 'mail',
 title: '?°ê°„ ?±ê³¼ ë¶„ì„ ë³´ê³ ??,
 content: '2025?„ë„ ?±ê³¼ ë¶„ì„ ê²°ê³¼ ë¦¬í¬?¸ê? ë°œì†¡?˜ì—ˆ?µë‹ˆ??',
 ...
 type: 'sms',
 title: '?œë²„ ê³¼ë???ê²½ê³ ',
 content: '[EGov] DB ?œë²„ CPU ?ìœ ??95% ì´ˆê³¼ ë°œìƒ',
 ...
 type: 'system',
 title: '? ê·œ ?…ë°?´íŠ¸ ?„ë£Œ',
 content: '?Œë«??v2.4.0 ?¨ì¹˜ê°€ ?±ê³µ?ìœ¼ë¡??ìš©?˜ì—ˆ?µë‹ˆ??',
 recipient: 'Admin.Lee',
 ...
 {[
 { label: '?„ì²´ ?Œë¦¼', count: 1284, delta: '+12%', icon: <Bell />, color: 'primary' },
 { label: '?œìŠ¤??ê³µì?', count: 42, delta: '+2', icon: <Bot />, color: 'indigo' },
 { label: 'ë©”ì¼ ë°œì†¡', count: 856, delta: '+45', icon: <Mail />, color: 'blue' },
 { label: 'SMS ?„ì†¡', count: 386, delta: '-5%', icon: <MessageSquare />, color: 'emerald' },
 ].map((stat, i) => (
 ...
 <div>
   <h2 className="text-xl font-black tracking-tighter ">?¤ì‹œê°??Œë¦¼ ?¤íŠ¸ë¦?/h2>
 <div className="flex items-center gap-2 mt-0.5 text-[10px] font-bold text-muted-foreground opacity-50 tracking-tight">
   <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" /> ì±„ë„ ë¬´ê²°?? ìµœì ?”ë¨
 </div>
 </div>
 </div>
 ...
 {getStatusIcon(log.status)}
   ...
   <span className="text-[9px] font-black text-muted-foreground opacity-40">{log.status === 'sent' ? 'ë°œì†¡?„ë£Œ' : log.status === 'pending' ? '?€ê¸°ì¤‘' : '?¤íŒ¨'}</span>
 </div>
 ...
 <div>
   <h3 className="text-2xl font-black tracking-tighter">AI ?¤ë§ˆ??ë°°ë‹¬</h3>
 <p className="text-sm font-bold opacity-80 mt-2 leading-relaxed">?˜ì‹ ?ì˜ ?…ë¬´ ?¨í„´??ë¶„ì„?˜ì—¬<br />ìµœì ???œê°„???Œë¦¼??ë°°ë‹¬?©ë‹ˆ??</p>
 </div>
 <Button className="w-full bg-white text-primary hover:bg-white/90 rounded-2xl h-14 font-black shadow-xl">ë°œì†¡ ìµœì ???”ì§„ ?œì„±</Button>
 </div>
 </div>

 <div className="p-10 bg-card border-2 border-primary/5 rounded-[3.5rem] shadow-xl space-y-8 flex-1">
 <div>
   <h4 className="text-[10px] font-black text-primary tracking-[0.3em] mb-6">ì±„ë„ ?íƒœ ì§€??/h4>
 ...
 <div className="pt-8 border-t border-primary/5 space-y-4">
   <h4 className="text-[10px] font-black text-muted-foreground tracking-tight opacity-50">ì§€?¥í˜• ?¸ì‚¬?´íŠ¸</h4>
 ...
 ?¤ì „ 10?œì—??11???¬ì´??ë°œì†¡???Œë¦¼???•ì¸?¨ì´ ê°€???’ìŠµ?ˆë‹¤. ì¤‘ìš”??ê³µì??????œê°„???œìš©?˜ì„¸??
 </p>
 </div>
 </div>
 </div>

...
export function GlobalCommandCenter() {
  ...
  const quickActions: CommandItem[] = [
    { id: 'act-notif', name: '?ㅻ쭏??硫붿떆吏??쇳꽣', url: '/admin/notifications', icon: <Bell size={16} />, category: '硫붾돱', description: '?꾩궗 ?뚮┝ ?듯빀 紐⑤땲?곕쭅 諛?AI ?붿뒪?⑥튂' },
    { id: 'act-collab', name: '?묒뾽 ?듯빀 ?덈툕', url: '/admin/collaboration', icon: <Users size={16} />, category: '硫붾돱', description: '議곗쭅??諛?吏?ν삎 ?뚯쓽/?먯썝 愿由? },
    { id: 'act-audit', name: '蹂댁븞 媛먯궗 ??꾨㉧??, url: '/admin/system/audit', icon: <HistoryIcon size={16} />, category: '硫붾돱', description: '?곗씠??蹂寃??대젰 異붿쟻 諛??쒓컖??媛먯궗 遺꾩꽍' },
    { id: 'act-workflow', name: '?꾨줈?몄뒪 罹붾쾭??, url: '/admin/workflow', icon: <GitBranch size={16} />, category: '硫붾돱', description: '鍮꾩쫰?덉뒪 ?뚰겕?뚮줈???ㅺ퀎 諛?紐⑤땲?곕쭅' },
    { id: 'act-form', name: '?ㅻ쭏???쒖떇 ?붿쭊', url: '/admin/sanctn/forms', icon: <FileText size={16} />, category: '硫붾돱', description: '?됱젙 ?쒖떇 ?ㅺ퀎 諛?臾몄꽌 ?먮룞??愿由? },
    { id: 'act-create-post', name: '??寃뚯떆湲 ?묒꽦', url: '/admin/community/boards/insertBoardArticle', icon: <FileText size={16} />, category: '?≪뀡', description: '怨듭??ы빆 諛?媛ㅻ윭由?寃뚯떆湲 ?좉퇋 ?깅줉' },
    { id: 'sys-1', name: '留덉씠?섏씠吏', url: '/mypage', icon: <User size={16} />, category: '?쒖뒪?? },
    { id: 'sys-settings', name: '?섍꼍 ?ㅼ젙', url: '/admin/system/settings', icon: <Settings size={16} />, category: '?쒖뒪?? },
    { id: 'sys-logout', name: '濡쒓렇?꾩썐', action: logout, icon: <LogOut size={16} />, category: '?쒖뒪?? },
  ];

  // 4. ?듯빀 寃???꾪꽣留?  const filteredItems = useMemo(() => {
    ...
    // 留뚯빟 ?쇱튂?섎뒗 寃??녿떎硫?愿묒뿭 寃???쒖븞 異붽?
    if (search && results.length === 0) {
      results = [{
        id: 'global-search',
        name: `"${search}" 寃?됱뼱濡??ъ씠???꾩껜 寃??,
      ...
      >
        ...
        <div className="flex items-center px-10 py-8 border-b border-primary/10 gap-6">
          ...
          <input
            ref={inputRef}
            placeholder="寃??.."
            className="flex-1 bg-transparent border-none focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 text-2xl font-black placeholder:text-muted-foreground/30 tracking-tight"
            value={search}
            onChange={(e) => {
          ...
          <div className="hidden sm:flex items-center gap-3">
            <div className="px-3 py-1.5 bg-muted rounded-[0.1rem] border text-[10px] font-black text-muted-foreground tracking-tight">ESC濡??リ린</div>
          </div>
        </div>
        ...
        <div className="max-h-[500px] overflow-y-auto p-6 scrollbar-hide">
          {filteredItems.length > 0 ? (
                      ...
                      {catItems.map((item) => {
                        ...
                        return (
                          <button
                            key={item.id}
                            className={cn(
                              "w-full flex items-center justify-between p-5 rounded-[0.1rem] transition duration-300 group text-left",
                              isFocused
            ...
            <div className="py-24 text-center space-y-6">
              ...
              <div>
                <p className="text-xl font-black text-foreground">寃곌낵瑜?李얠쓣 ???놁뒿?덈떎.</p>
                <p className="text-sm text-muted-foreground font-bold mt-1">?꾩????꾩슂?섏떆硫??쒖뒪??愿由ъ옄?먭쾶 臾몄쓽?섏꽭??</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-muted px-10 py-6 border-t border-primary/10 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-2">
              <kbd className="px-2 py-1 bg-background border rounded-lg text-[10px] font-black">?대룞</kbd>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black text-muted-foreground tracking-tight tracking-tighter">
                ?쒖뒪???곹깭: <span className="text-emerald-500">理쒖쟻?붾맖</span>
              </span>
            </div>
          </div>
        </div>

...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const toast = useCallback((message: unknown, type: ToastType = 'info') => {
    ...
    const displayMessage = typeof message === 'string'
      ? message
      : ((message as { message?: string })?.message || JSON.stringify(message) || '?????†ëŠ” ?¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.');
  ...
  // API ?„ì—­ ?ëŸ¬ ë¦¬ìŠ¤??  React.useEffect(() => {
    const handleApiError = (e: Event) => {
      const detail = (e as CustomEvent).detail;
      if (!detail) return;
      const { message, status } = detail;
      // 401?€ ? í° ?¬ë°œê¸?ë¡œì§??ì²˜ë¦¬?˜ë?ë¡??¬ìš©?ì—ê²ŒëŠ” ?Œë¦¬ì§€ ?ŠìŒ
      if (status !== 401) {
        error(message);
      }
    };

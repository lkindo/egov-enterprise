...
export function VisualAuditTimeline({ logs, className, title = "Security Audit Intelligence" }: VisualAuditTimelineProps) {
 ...
 <div className="flex items-center gap-1.5 text-[10px] font-black text-emerald-500 tracking-tight">
   <ShieldCheck size={12} /> Î∂àÎ? Í±∞Î≤Ñ?åÏä§ ?êÏû• ?úÏÑ±
 </div>
 <div className="h-3 w-px bg-muted" />
   <span className="text-[10px] font-bold text-muted-foreground opacity-50 tracking-tight leading-none">?§ÏãúÍ∞??∞Ïù¥??Î¨¥Í≤∞??Î™®Îãà?∞ÎßÅ</span>
 </div>
 </div>
 </div>
 ...
 <p className="text-sm font-bold text-muted-foreground/60 tracking-tight">
   <span className="text-primary">{log.action === 'CREATE' ? '?ùÏÑ±' : log.action === 'UPDATE' ? '?òÏ†ï' : log.action === 'DELETE' ? '??†ú' : 'Î≥µÍµ¨'}</span> ??{log.entityName}
 </p>
 </div>
 </div>
 ...
 <h4 className="text-[10px] font-black text-primary tracking-[0.3em] mb-4 flex items-center gap-2">
   <Cpu size={12} /> AI Í∏∞Î∞ò Î≥ÄÍ≤?Í∞êÏ? ?îÏßÑ
 </h4>
 ...
 <Monitor size={16} /> ?úÍ∞Å??Í≤ÄÏ¶? </Button>
 ...
 <div className="w-2 h-2 rounded-full bg-emerald-500" />
   <span className="text-[10px] font-black text-muted-foreground tracking-tight">ÎßàÏä§???êÏû• ?ôÍ∏∞?îÎê®</span>
 </div>
 <div className="flex items-center gap-2">
 <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
   <span className="text-[10px] font-black text-muted-foreground tracking-tight">?îÌò∏???? AES-256 (NIST)</span>
 </div>
 </div>
 <p className="text-[10px] font-black text-muted-foreground tracking-tight opacity-40 mt-4 md:mt-0">
   Total Audit Records: {logs.length} ??Î¨¥Í≤∞??Í≤ÄÏ¶??ÑÎ£å
 </p>
 </div>
 </div>
 );

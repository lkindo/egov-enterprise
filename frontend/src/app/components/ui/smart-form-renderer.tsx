...
export function SmartFormRenderer({ schema, onSubmit, className }: SmartFormRendererProps) {
    ...
    schema.fields.forEach(field => {
        ...
        if (field.required) {
            if (field.type === 'checkbox') rule = z.boolean().refine(v => v === true, { message: "?ÑÏàò ?ôÏùò ??™©?ÖÎãà??" });
            else rule = (rule as z.ZodString).min(1, { message: `${field.label}?Ä(?? ?ÑÏàò ?ÖÎ†• ??™©?ÖÎãà??` });
        } else {
            rule = rule.optional();
        }
    ...
    return (
        <div className={cn("bg-card border-2 border-primary/5 rounded-[2.5rem] p-10 shadow-xl", className)}>
            <div className="mb-8">
                        ...
                        <UIFormField
                            ...
                            render={({ field: formField }) => (
                                <FormItem className={cn(field.width === 'full' ? "col-span-2" : "col-span-1")}>
                                    <FormLabel className="text-[10px] font-black text-muted-foreground tracking-tight ml-1">
                                    ...
                                    <FormControl>
                                        {field.type === 'textarea' ? (
                                            <Textarea
                                                ...
                                                className="rounded-2xl border-2 border-primary/5 focus:border-primary/20 transition-all min-h-[120px]"
                                            />
                                        ) : field.type === 'select' ? (
                                            <Select onValueChange={formField.onChange} defaultValue={formField.value as string}>
                                                <SelectTrigger className="h-12 rounded-2xl border-2 border-primary/5 focus:ring-primary/20 transition-all font-bold">
                                                    <SelectValue placeholder={field.placeholder || "?†ÌÉù?òÏÑ∏??} />
                                                </SelectTrigger>
                                            ...
                                            <div className="flex items-center space-x-3 p-4 bg-muted/30 rounded-2xl border-2 border-transparent hover:border-primary/10 transition-all group">
                                                <Checkbox
                                            ...
                                            <Input
                                                ...
                                                {...formField}
                                                value={(formField.value as string) || ''}
                                                className="h-12 rounded-2xl border-2 border-primary/5 focus:border-primary/20 transition-all font-bold"
                                            />
                                        )}
                                    </FormControl>

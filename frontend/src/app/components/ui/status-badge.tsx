...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  'C': { label: '완료', color: 'bg-muted text-foreground dark:bg-gray-800 dark:text-gray-300' },
};

export function StatusBadge({ status, className }: StatusBadgeProps) {
  const config = STATUS_CONFIG[status] || { label: status, color: 'bg-muted text-foreground dark:bg-zinc-800 dark:text-zinc-300' };

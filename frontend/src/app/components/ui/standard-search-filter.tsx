...
export function SmartSearchPanel({ fields, onSearch, onReset, className }: StandardSearchFilterProps) {
 ...
 <div>
 <h3 className="text-sm font-black text-foreground tracking-tight leading-none">상세 검색 필터</h3>
 <p className="text-[10px] font-bold text-muted-foreground opacity-50 tracking-tight mt-1">다차원 데이터 검색</p>
 </div>
 </div>
 <Button
 type="button"
 ...
 <label className="text-[10px] font-black text-muted-foreground tracking-tight ml-1 opacity-70">
 {field.label}
 </label>
 ...
 className="text-sm font-bold rounded-xl m-1"
 >
 {opt.label}
 </SelectItem>
 ))}
 ...
 variant="ghost"
 onClick={handleReset}
 className="rounded-lg h-10 px-6 font-semibold gap-2 text-sm text-muted-foreground hover:bg-muted transition-all"
 >
 <RotateCcw size={16} />
 필터 초기화
 </Button>
 <Button
 type="submit"
 className="rounded-lg h-10 px-8 font-semibold gap-2 shadow-sm hover:scale-[1.02] active:scale-[0.98] transition-all bg-primary text-primary-foreground text-sm"
 >
 <Search size={16} />
 검색 실행
 </Button>

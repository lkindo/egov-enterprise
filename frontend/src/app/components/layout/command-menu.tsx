...
export function CommandMenu() {
  ...
  return (
    <CommandDialog 
      ...
      className="hub-glass-premium border-2 border-border/50 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] sm:max-w-[600px] overflow-hidden"
    >
      <div className="relative">
        <CommandInput 
          placeholder="실행할 명령어나 메뉴를 입력하십시오..." 
          className="h-11 border-none bg-transparent font-bold text-base placeholder:text-muted-foreground focus:ring-0"
        />
        <div className="absolute right-4 top-1/2 -translate-y-1/2 flex gap-1">
          <kbd className="pointer-events-none hidden h-6 select-none items-center gap-1 rounded border bg-muted px-1.5 font-mono text-xs font-medium opacity-100 sm:flex">
      ...
      <CommandList className="max-h-[450px] scrollbar-hide pb-4">
        <CommandEmpty className="py-20 flex flex-col items-center justify-center text-muted-foreground gap-4">
          <div className="w-12 h-12 rounded-lg bg-muted flex items-center justify-center border border-border shadow-inner">
        ...
        <CommandGroup heading={<span className="text-xs font-bold tracking-[0.3em] uppercase opacity-40 ml-2 mb-2 block">System_Control</span>}>
          ...
          <CommandItem onSelect={() => runCommand(() => setTheme('dark'))} className="h-12 rounded-lg mx-2 my-1 cursor-pointer transition-all">
            <Moon size={18} className="mr-3 text-foreground" />
          ...
          <CommandItem onSelect={() => runCommand(() => window.print())} className="h-12 rounded-lg mx-2 my-1 cursor-pointer transition-all">
            <Printer size={18} className="mr-3 text-muted-foreground" />
      ...
      <div className="p-4 bg-muted/50 border-t border-border/50 flex items-center justify-between">
        <div className="flex gap-4">
          <div className="flex items-center gap-2">
            <kbd className="h-5 px-1.5 rounded border bg-white text-xs font-bold shadow-sm">↑↓</kbd>
            <span className="text-xs font-bold text-muted-foreground tracking-tighter">NAVIGATE</span>
          </div>
          <div className="flex items-center gap-2">
            <kbd className="h-5 px-1.5 rounded border bg-white text-xs font-bold shadow-sm">ENTER</kbd>
            <span className="text-xs font-bold text-muted-foreground tracking-tighter">SELECT</span>
          </div>
          <div className="flex items-center gap-2">
            <kbd className="h-5 px-1.5 rounded border bg-white text-xs font-bold shadow-sm">ESC</kbd>
            <span className="text-xs font-bold text-muted-foreground tracking-tighter">CLOSE</span>
          </div>
        </div>

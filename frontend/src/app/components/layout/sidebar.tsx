...
const ICON_MAP: Record<string, any> = {
  ...
  'ê´€ë¦¬ì': Settings,
  '?¬ìš©?ê?ë¦?: Users,
  '?¬ìš©??ê³„ì • ë°?ê¶Œí•œ ê´€ë¦?: Users,
  'ë³´ì•ˆê´€ë¦?: ShieldCheck,
  '?µí•© ë³´ì•ˆ ë°??‘ì† ?•ì±…': ShieldCheck,
  '?œìŠ¤?œê?ë¦?: Settings,
  '?œìŠ¤???¤ì •': Settings,
  'ê²Œì‹œ??: MessageSquare,
  '?‘ì—…': Users,
  '?¼ì •ê´€ë¦?: CalendarDays,
  '?¤ë§ˆ???¼ì •/?¼ì? ê´€ë¦?: CalendarDays,
  '?µê³„': LayoutDashboard,
  'ê°ì‚¬ ë°??µê³„ ëª¨ë‹ˆ?°ë§': BarChart3,
  '?„ì?ë§?: BookOpen,
  '?¬ìš©?ì???: UserCheck,
  '?¤ë¬¸ì¡°ì‚¬': ClipboardList,
  '?¤ë¬¸ì¡°ì‚¬ ë°??¬í‘œ ?¼í„°': ClipboardList,
  'ë§ˆì´?˜ì´ì§€': UserCircle,
  'ë§ˆì´?˜ì´ì§€ê´€ë¦?: Settings,
  'ê³µí†µì½”ë“œê´€ë¦?: Database,
  '?‰ì •ì½”ë“œê´€ë¦?: Database,
  'ê¸°ê?ì½”ë“œ?˜ì‹ ': Database,
  'ë¡œê·¸ê´€ë¦?: FileText,
  '?„ì§??ë°?ë¶€??ê´€ë¦?: Building2,
  'ê¸°ë³¸': CircleDot
};
...
const DOMAIN_ICON_MAP: Record<number, any> = {
  10: LayoutGrid, // ?Œí¬?¤í˜?´ìŠ¤
  11: MessageSquare, // ì»¤ë??ˆí‹°
  12: BookOpen, // ê³ ê°ì§€?ì„¼??  90: Settings, // ?µí•© ê´€ë¦??¼í„°
  1000000: Briefcase, // ?¢ Workspace (New Domain Layout)
  2000000: Library, // ?’¬ Community & Content (New Domain Layout)
  3000000: Sparkles, // ?™‹?â™‚ï¸?Service & Operation (New Domain Layout)
};
...
const NavItem = ({ item, depth = 0 }: { item: MenuItem; depth?: number }) => {
  ...
  const Icon = ICON_MAP[item.menuNm] || ICON_MAP['ê¸°ë³¸'];
  ...
  const navContent = (
    <div className={cn(
      ...
      depth === 2 && "pl-14",
      depth >= 3 && "pl-16",
      depth > 0 && "font-medium" // ?˜ìœ„ ë©”ë‰´???°íŠ¸ ?ê»˜ë¥??½ê°„ ì¡°ì ˆ
    )}>
        ...
        {hasChildren && isOpen && (
          ...
          >
            <div className={cn(
              ...
              depth === 0 && "ml-5 border-l border-border/40" // 1?¨ê³„ ?˜ìœ„?ë§Œ ê°€?´ë“œ?¼ì¸ ì¶”ê?
            )}>
          ...
          <div className="flex items-center justify-between mb-10 px-2 lg:hidden">
            <Link href="/" className="flex items-center gap-3.5" onClick={() => setSidebarOpen(false)}>
              ...
              <div className="flex flex-col">
                <span className="text-base font-bold tracking-tight leading-none text-foreground">?”í„°?„ë¼?´ì¦ˆ</span>
                <span className="text-[10px] text-muted-foreground/60 font-semibold tracking-wider">?¬í„¸ 5.0</span>
              </div>
            </Link>
            <Button
              variant="ghost"
            ...
            <div className="hidden lg:block space-y-1">
              ...
              {loading ? (
                ...
                <div className="p-8 text-center space-y-3 opacity-20">
                  ...
                  <p className="text-sm font-bold tracking-tight">ë©”ë‰´ë¥?ë¶ˆëŸ¬?????†ìŠµ?ˆë‹¤.</p>
                </div>
              ) : (

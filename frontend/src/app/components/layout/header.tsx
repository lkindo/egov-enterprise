...
const DOMAIN_ICON_MAP: Record<number, React.ComponentType<{ size?: number; className?: string }>> = {
  10: LayoutGrid, // ?åÌÅ¨?§Ìéò?¥Ïä§
  11: Users, // Ïª§Î??àÌã∞
  12: HeartHandshake, // Í≥†Í∞ùÏßÄ?êÏÑº??  90: ShieldCheck, // ?µÌï© Í¥ÄÎ¶??ºÌÑ∞
};
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="flex h-16 items-center px-4 md:px-6 gap-4">
        ...
        <Link href="/" className="flex items-center gap-2.5 transition-opacity hover:opacity-80 shrink-0">
          ...
          <div className="hidden sm:flex flex-col">
            <span className="text-sm font-bold leading-tight text-foreground">?ÑÏûê?ïÎ? 5.0</span>
            <span className="text-[10px] text-muted-foreground font-semibold tracking-tight">?ÑÏûê?ïÎ? ?¨ÌÑ∏</span>
          </div>
        </Link>
                ...
                <PopoverTrigger asChild>
                  <Button variant="ghost" className="flex items-center gap-2 pl-2 pr-1 h-9 hover:bg-accent rounded-full">
                    ...
                    <div className="flex flex-col items-start mr-1 hidden sm:flex">
                      ...
                      <span className="text-[10px] text-muted-foreground font-semibold mt-0.5">{user.userSe === 'USR' ? '?¨Ïö©?? : 'Í¥ÄÎ¶¨Ïûê'}</span>
                    </div>
                ...
                <PopoverContent className="w-56 p-2 mt-1" align="end">
                  ...
                  <div className="space-y-0.5">
                    <Link href="/mypage" className={cn(buttonVariants({ variant: "ghost" }), "w-full justify-start text-sm h-9 gap-2 font-medium")}>
                      <span className="flex items-center gap-2"><User size={14} /> Í∞úÏù∏?ïÎ≥¥?òÏ†ï</span>
                    </Link>
                    <Link href="/admin/system/settings" className={cn(buttonVariants({ variant: "ghost" }), "w-full justify-start text-sm h-9 gap-2 font-medium")}>
                      <span className="flex items-center gap-2"><Settings size={14} /> ?òÍ≤Ω?§Ï†ï</span>
                    </Link>
      ...
      <AppNotificationDrawer
        ...
        notifications={(notifications || []).map((n, i) => ({
          ...
          time: n.ntfcPnttm,
          isRead: n.readYn === 'Y',
          type: n.ntfcSj?.includes('Î≥¥Ïïà') ? 'SECURITY' : n.ntfcSj?.includes('?úÏä§??) ? 'SYSTEM' : 'ACTIVITY'
        }))}
      />

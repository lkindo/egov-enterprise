...
export function Header({ initialMenus = [] }: { initialMenus?: any[] }) {
 ...
 <span className="text-[10px] text-muted-foreground font-bold tracking-tight opacity-70">전자정부 포털</span>
 </div>
 </Link>
 ...
 size="sm"
 className={cn(
 "px-4 h-9 font-bold text-sm transition-all rounded-lg gap-2",
 isActive
 ...
 <Link href="/mypage" className={cn(buttonVariants({ variant: "ghost" }), "w-full justify-start text-sm h-9 gap-2")}>
 <User size={14} /> 개인정보수정
 </Link>
 <Link href="/admin/system/settings" className={cn(buttonVariants({ variant: "ghost" }), "w-full justify-start text-sm h-9 gap-2")}>
 <Settings size={14} /> 환경설정
 </Link>
 <div className="h-px bg-muted my-1" />
 <Button
 variant="ghost"
 className="w-full justify-start text-sm h-9 gap-2 text-red-500 hover:text-red-600 hover:bg-red-50"
 onClick={() => logout()}
 >
 <LogOut size={14} /> 로그아웃
 </Button>

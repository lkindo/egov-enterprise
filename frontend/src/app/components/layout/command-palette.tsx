...
export function CommandPalette() {
 ...
 const [selected번호, setSelected번호] = useState(0);
 ...
 onKeyDown={(e) => {
 if (e.key === 'Enter' && results[selected번호]) onSelect(results[selected번호].url);
 if (e.key === 'ArrowDown') setSelected번호((i) => Math.min(results.length - 1, i + 1));
 if (e.key === 'ArrowUp') setSelected번호((i) => Math.max(0, i - 1));
 }}
 />
 ...
 globalIdx === selected번호 ? "bg-primary text-primary-foreground shadow-lg scale-[1.01]" : "hover:bg-muted text-foreground"
 )}
 onClick={() => onSelect(result.url)}
 onMouseEnter={() => setSelected번호(globalIdx)}
 >
 <div className="flex items-center gap-4 text-left">
 <div className={cn(
 "p-2 rounded-xl border flex items-center justify-center",
 globalIdx === selected번호 ? "bg-white/20 border-white/10" : "bg-muted border-none"
 )}>
 {result.icon}
 </div>
 <div>
 ...
 globalIdx === selected번호 ? "text-primary-foreground/70" : "text-muted-foreground"
 )}>
 {result.description}
 </p>
 )}
 ...
 globalIdx === selected번호 ? "opacity-100 translate-x-0" : "group-hover:opacity-100 -translate-x-2"
 )} size={16} />
 </button>
 );
 })}

...
export default function WorkHubClient({ jobs = [], reports = [], defaultTab = 'job' }: WorkHubClientProps) {
  ...
  const renderJobList = () => (
    <div className="space-y-4">
      {jobs.length === 0 ? (
        <div className="p-10 text-center opacity-30 font-black text-xs tracking-widest border-2 border-dashed border-border rounded-3xl">?±Î°ù???ÖÎ¨¥Í∞Ä ?ÜÏäµ?àÎã§.</div>
      ) : jobs.map((item) => (
        <WorkListItem
  ...
  const renderReportList = () => (
    <div className="space-y-4">
      {reports.length === 0 ? (
        <div className="p-10 text-center opacity-30 font-black text-xs tracking-widest border-2 border-dashed border-border rounded-3xl">?±Î°ù??Î≥¥Í≥†?úÍ? ?ÜÏäµ?àÎã§.</div>
      ) : reports.map((item) => (
        <WorkListItem
          key={item.reprtId}
          id={item.reprtId}
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="p-8 rounded-[2rem] bg-white border-2 border-slate-100 shadow-xl space-y-8">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-black tracking-tighter uppercase">?µÌï© ?§Îßà??Ï∫òÎ¶∞??/h3>
          <div className="flex gap-2">
            <Button size="sm" variant="outline" className="rounded-full text-[9px] font-black">Í∞úÏù∏ ?ºÏ†ï</Button>
            <Button size="sm" variant="default" className="rounded-full text-[9px] font-black bg-slate-900 text-white">Î∂Ä???ºÏ†ï</Button>
          </div>
        </div>
        <div className="aspect-[4/3] bg-slate-50 rounded-[2.5rem] flex flex-col items-center justify-center border-2 border-dashed border-slate-100 p-12 space-y-4">
          ...
          <p className="text-sm font-black text-slate-900 tracking-tighter uppercase">?Ä?îÌòï ?§Ï?Ï§ÑÎßÅ ?úÏä§??/p>
          <p className="text-[10px] font-bold text-slate-400 max-w-[200px] text-center leading-relaxed">Ï§ÄÎπÑÎêú Ï∫òÎ¶∞???îÏßÑ??ÎπÑÏ¶à?àÏä§ ?ºÏ†ï???§ÏãúÍ∞ÑÏúºÎ°??ôÍ∏∞?îÌï©?àÎã§.</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      ...
      <HubHeader
        title="?ÖÎ¨¥ Î∞??∏ÌÖîÎ¶¨Ï†Ñ??
        highlight="?àÎ∏å"
        subtitle="?ÑÏÇ¨ Î∂Ä???ÖÎ¨¥ Ï≤òÎ¶¨ Î∞?ÎπÑÏ¶à?àÏä§ ?∞Ïù¥???êÏÇ∞ ?µÌï© Í¥ÄÎ¶??ºÌÑ∞"
        icon={Briefcase}
        actions={
        ...
        <div className="col-span-12 lg:col-span-7 space-y-10">
          <HubSectionCard
            ...
            description={selectedItemId
              ? `?†Ï∂ú???îÌã∞??#${selectedItemId})???Ä???§ÏãúÍ∞??∞Îèô Î∞?ÎπÑÏ¶à?àÏä§ Î°úÏßÅ Î∂ÑÏÑù???úÏÑ±?îÎêò?àÏäµ?àÎã§.`
              : activeTab === 'calendar' ? "?ÑÏÇ¨ Î∞?Í∞úÏù∏ ?ºÏ†ï???µÌï©?òÏó¨ ÎπÑÏ¶à?àÏä§ Í∞Ä?©ÏÑ±???úÎàà???åÏïÖ?©Îãà??"
                : "?ºÏ™Ω Î¶¨Ïä§?∏Ïóê??Î∂ÑÏÑù???ÖÎ¨¥ Í∞úÏ≤¥ ?êÎäî Î≥¥Í≥† ?êÎ£åÎ•??†ÌÉù?òÏó¨ ?∞Ïù¥???îÏïΩ???úÏûë?òÏã≠?úÏò§."}
            icon={selectedItemId ? Sparkles : activeTab === 'calendar' ? Calendar : Activity}
            statusBadges={
              <>
                <HubStatusBadge label="?úÏä§???ïÏÉÅ" icon={CheckCircle2} variant="success" className="text-[9px] font-black tracking-widest" />
                <HubStatusBadge label="?§Ìä∏Î¶¨Î∞ç ?úÏÑ±" icon={Clock} variant="default" className="text-[9px] font-black tracking-widest" />
              </>
            }
          >
            <AnimatePresence mode="wait">
              {selectedItemId ? (
                ...
                >
                  <div className="h-96 rounded-[var(--radius-hub-section)] bg-slate-50 dark:bg-muted/10 border-2 border-dashed border-border/50 shadow-inner flex flex-col items-center justify-center p-12 text-center relative overflow-hidden group">
                    ...
                    <p className="text-xs font-black text-muted-foreground tracking-[0.4em] uppercase relative z-10">?∏ÌÖîÎ¶¨Ï†Ñ???îÏßÑ ?úÍ∞Å??/p>
                    <p className="text-xl font-black text-foreground tracking-tighter mt-4 max-w-sm relative z-10">?∞Ïù¥??Íµ¨Ï°∞ Î∂ÑÏÑù Î∞??åÌÅ¨?åÎ°ú???úÍ∞Å??Ïª¥Ìè¨?åÌä∏ Ï§ÄÎπÑÎê®</p>
                  </div>
                ...
                <div className="p-20 border-4 border-dashed border-border/20 rounded-[var(--radius-hub-section)] flex flex-col items-center justify-center text-center space-y-8 bg-slate-50/50 dark:bg-muted/5 grayscale">
                  ...
                  <div className="space-y-4">
                    <h3 className="text-3xl font-black text-foreground tracking-tighter uppercase opacity-40">{activeTab === 'calendar' ? 'Ready to Sync' : '?úÏä§???ÄÍ∏?}</h3>

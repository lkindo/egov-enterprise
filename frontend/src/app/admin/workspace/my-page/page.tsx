      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
        {loading ? (
          ...
          contents.map((item) => (
            ...
            >
              <div className="flex justify-between items-start mb-8">
                <div className={`w-16 h-11 rounded-lg flex items-center justify-center transition-all duration-500 ${
              ...
              <h3 className="text-xl font-bold text-foreground tracking-tighter uppercase leading-tight">{item.cntntsNm}</h3>
              ...
              <div className="mt-8 pt-8 border-t border-slate-50 flex items-center gap-3 overflow-hidden">
                <div className="w-2 h-2 rounded-lg bg-primary shrink-0" />
          ...
          <div className="col-span-full py-32 text-center bg-slate-50 rounded-lg border-4 border-dashed border-slate-200 flex flex-col items-center gap-6">
            ...
            <p className="text-slate-400 font-bold tracking-widest uppercase text-xs">등록된 마이페이지 콘텐츠가 현재 클러스터에 존재하지 않습니다.</p>
          </div>
        )}
      </div>
    </div>

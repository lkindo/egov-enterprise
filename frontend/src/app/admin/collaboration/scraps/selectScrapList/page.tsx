...
const ScrapListPage = () => {
    ...
    const [pageÎ≤àÌò∏, setPageÎ≤àÌò∏] = useState(1);
    const [loading, setLoading] = useState(false);

    const fetchList = async () => {
        setLoading(true);
        try {
            const params = { pageÎ≤àÌò∏, pageUnit: 10 };
            ...
            <Card className="border-none shadow-md overflow-hidden">
                <CardHeader className="flex flex-row items-center justify-between pb-6 bg-gradient-to-r from-muted/50 to-transparent border-b">
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-muted-foreground">?òÏ§ë???§Ïãú Î≥?Ï§ëÏöî?????òÏù¥ÏßÄ?Ä ?ïÎ≥¥?§ÏùÑ Í¥ÄÎ¶¨Ìïò?∏Ïöî.</p>
                    </div>
                    <CardAction>
                ...
                <CardContent className="pt-8">
                    <div className="mb-6 flex items-center justify-between">
                        <div className="bg-muted px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 border">
                            ?ÑÏ≤¥ <span className="text-primary font-black">{totalCount}</span>Í±¥Ïùò ?§ÌÅ¨??                        </div>
                    </div>
                                ...
                                {loading ? (
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={5} className="h-32 text-center text-muted-foreground font-medium">
                                            ?Ä?•Îêú ?§ÌÅ¨?©Ïù¥ ?ÜÏäµ?àÎã§. ?πÏùò ?†Ïö©???ïÎ≥¥Î•??Ä?•Ìï¥ Î≥¥ÏÑ∏??
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    list.map((item, idx) => (
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-center gap-4 mt-10">
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={() => setPageÎ≤àÌò∏(p => Math.max(1, p - 1))}
                                disabled={pageÎ≤àÌò∏ === 1}
                                className="px-6 font-bold shadow-sm"
                            >
                                ?¥Ï†Ñ
                            </Button>
                            <div className="flex items-center gap-2 px-6 py-2 bg-muted rounded-full">
                                <span className="text-sm font-black text-primary">{pageÎ≤àÌò∏}</span>
                            ...
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={() => setPageÎ≤àÌò∏(p => Math.min(totalPages, p + 1))}
                                disabled={pageÎ≤àÌò∏ === totalPages}
                                className="px-6 font-bold shadow-sm"
                            >
                                ?§Ïùå
                            </Button>

    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {addresses.map((address: any) => (
        <Card
          ...
          className={cn(
            "rounded-xl border-2 transition-all cursor-pointer p-6 flex items-center gap-6",
    ...
    <div className="space-y-10 pb-20 animate-in fade-in duration-1000">
      ...
      <div className="grid grid-cols-12 gap-8 px-2 min-h-[700px]">
        <div className="col-span-12 lg:col-span-3 space-y-6">
          <Card className="rounded-xl border-0 bg-white shadow-2xl p-4 ring-1 ring-slate-100 overflow-hidden">
          ...
          <Card className="rounded-xl border-0 bg-primary text-white p-10 space-y-6 shadow-2xl relative overflow-hidden group">
        ...
        <div className="col-span-12 lg:col-span-5 h-full flex flex-col gap-6">
          <Card className="flex-1 rounded-xl border-0 bg-white shadow-2xl overflow-hidden flex flex-col ring-1 ring-slate-100">
            <CardHeader className="bg-slate-50/50 border-b p-10 space-y-8">
              ...
              <div className="flex gap-4">
                <div className="relative flex-1 group">
                  ...
                  <Input
                    className="pl-12 h-14 bg-white border-slate-100 rounded-xl text-sm font-bold shadow-sm"
                    placeholder="목록 검색..."
                    value={searchKeyword}
                ...
                <Button
                  ...
                  className="h-14 px-10 rounded-xl bg-slate-900 text-white font-black tracking-tighter shadow-xl hover:-translate-y-1 transition-all"
                >
                  검색 실행
                </Button>
              </div>
        ...
        <div className="col-span-12 lg:col-span-4 h-full">
          <AnimatePresence mode="wait">
            {selectedItemId ? (
              ...
              >
                <Card className="flex-1 rounded-xl border-0 bg-white shadow-2xl flex flex-col ring-1 ring-slate-100 overflow-hidden">
                  ...
                  <CardContent className="flex-1 p-10 space-y-12">
                    ...
                    <div className="flex gap-4">
                      <Button className="flex-1 h-14 rounded-xl bg-slate-900 text-white font-black tracking-tight text-[9px]">답장 / 열기</Button>
                      <Button variant="outline" className="h-14 w-14 rounded-xl border-2"><Trash2 size={20} /></Button>
                    </div>
                  </CardContent>
                </Card>
              </motion.div>
            ) : (
              <Card className="h-full rounded-xl border-2 border-dashed border-slate-200 bg-white/50 flex flex-col items-center justify-center p-20 text-center grayscale opacity-30">
...
function NavButton({ icon, label, active, onClick }: { icon: React.ReactNode, label: string, active: boolean, onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={cn(
        "w-full group p-6 rounded-xl border-2 transition-all flex items-center gap-5",
        active
          ? "bg-slate-900 border-slate-900 text-white shadow-xl"

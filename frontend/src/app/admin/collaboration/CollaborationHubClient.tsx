        ...
        >
          <div className="flex items-start gap-6">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[8px] font-black tracking-tight opacity-40">Î≥¥ÎÇ∏ ?¨Îûå: {note.trnsmitterNm || note.trnsmitterId}</p>
            </div>
          </div>
      ...
      {addresses.map((address: any) => (
        ...
        >
          ...
          <div className="space-y-1">
            ...
            <p className="text-[10px] opacity-40">{address.email || '?¥Î©î???ÜÏùå'}</p>
          </div>
        </Card>
      ))}
    </div>
          ...
          >
            <div className="flex items-start gap-6">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[8px] font-black tracking-tight opacity-40">?§ÌÅ¨???†Ïßú: {scrap.createdDate?.substring(0, 10) || '?????ÜÏùå'}</p>
              </div>
            </div>
          </div>
        ))
      ...
      <div className="grid grid-cols-12 gap-8 px-2 min-h-[700px]">
        ...
        <div className="col-span-12 lg:col-span-3 space-y-6">
          <Card className="rounded-[3rem] border-0 bg-white shadow-2xl p-4 ring-1 ring-slate-100 overflow-hidden">
            <NavButton icon={<Inbox size={20} />} label="Î©îÏãúÏßÄ" active={activeTab === 'MESSAGES'} onClick={() => { setActiveTab('MESSAGES'); setSelectedItemId(null); }} />
            <NavButton icon={<Users size={20} />} label="?ÑÏ≤¥ ?∞ÎùΩÏ≤? active={activeTab === 'ADDRESS_BOOK'} onClick={() => { setActiveTab('ADDRESS_BOOK'); setSelectedItemId(null); }} />
            <NavButton icon={<Bookmark size={20} />} label="?§ÌÅ¨??Í¥ÄÎ¶? active={activeTab === 'SCRAPS'} onClick={() => { setActiveTab('SCRAPS'); setSelectedItemId(null); }} />
          </Card>
          ...
          <Card className="rounded-[3rem] border-0 bg-primary text-white p-10 space-y-6 shadow-2xl relative overflow-hidden group">
            ...
            <div className="space-y-2 relative z-10">
              <h4 className="text-[10px] font-black tracking-tight opacity-60">?ëÎãµÎ•?/h4>
            ...
            <CardHeader className="bg-slate-50/50 border-b p-10 space-y-8">
              <div className="flex items-center justify-between">
                <CardTitle className="text-[10px] font-black text-slate-400 tracking-[0.4em] leading-tight">
                  {activeTab === 'MESSAGES' ? 'Î≥¥Ïïà Ï±ÑÎÑê' : activeTab === 'ADDRESS_BOOK' ? '?ÑÏ≤¥ Ï£ºÏÜåÎ°? : '?§ÌÅ¨???Ä?•ÏÜå'}
                </CardTitle>
            ...
            {selectedItemId ? (
              ...
              >
                <Card className="flex-1 rounded-[3.5rem] border-0 bg-white shadow-2xl flex flex-col ring-1 ring-slate-100 overflow-hidden">
                  ...
                  <CardContent className="flex-1 p-10 space-y-12">
                    ...
                    <div className="flex gap-4">
                      <Button className="flex-1 h-14 rounded-2xl bg-slate-900 text-white font-black tracking-tight text-[9px]">?µÏû• / ?∞Í∏∞</Button>
              ...
              <Card className="h-full rounded-[3.5rem] border-2 border-dashed border-slate-200 bg-white/50 flex flex-col items-center justify-center p-20 text-center grayscale opacity-30">
                ...
                <h3 className="text-2xl font-black text-slate-900 tracking-tighter leading-tight">
                  ??™©???†ÌÉù?òÏÑ∏??                </h3>
                <p className="text-[10px] mt-4 font-black tracking-tight">??™©???†ÌÉù?òÏó¨ ?¥Ïö©???ïÏù∏?òÏÑ∏??</p>
              </Card>
            )}
          </AnimatePresence>
        </div>

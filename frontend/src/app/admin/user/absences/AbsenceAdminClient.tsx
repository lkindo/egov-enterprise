...
export default function AbsenceAdminClient({
  ...
  const handleToggleAbsence = async (emplyrId: string, currentStatus: boolean) => {
    ...
    try {
      ...
      toast.success(`${emplyrId} ?¨Ïö©?êÏùò ?ÑÎ°ú?†ÏΩú??${newStatus === 'Y' ? 'Î∂Ä??Î™®Îìú' : '?úÏÑ± Î™®Îìú'}Î°??ÑÌôò?òÏóà?µÎãà??`);
    } catch {
      toast.error('?ÑÎ°ú?†ÏΩú ?ôÍ∏∞??Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.');
    }
  };
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="Î∂Ä??Í¥ÄÎ¶??§Ìçº?àÏù¥??
        breadcrumbs={[{ label: '?úÏä§?úÍ?Î¶? }, { label: '?¨Ïö©?êÍ?Î¶? }, { label: 'Î∂Ä??Í¥ÄÎ¶? }]}
      />

      <HubHeader
        title="Í∞Ä?©ÏÑ±"
        highlight="Matrix"
        subtitle="?ÑÏÇ¨ ?∏Ï†Å Î¶¨ÏÜå?§Ïùò ?§ÏãúÍ∞?Í∞Ä?©ÏÑ± Î∞?Î∂Ä???ÑÎ°ú?†ÏΩú ?µÌï© ?úÏñ¥ ?úÏä§??
        icon={UserX}
        actions={
          <div className="flex gap-4 p-2 items-center">
            ...
            >
              <Zap size={20} className="group-hover:animate-pulse" /> Í∞Ä?©ÏÑ± ?ÑÎ°ú?†ÏΩú ?ôÍ∏∞??              <ArrowUpRight size={16} className="opacity-0 group-hover:opacity-100 transition-opacity" />
            </Button>
          </div>
        }
      />
      ...
      <HubMetricGrid>
        ...
        <HubMetricCard title="STANDBY_UNITS" value={totalAbsents} icon={Ghost} color="rose" status={totalAbsents > 0 ? "ALERT" : "?àÏ†ï"} />
      ...
      <div className="grid grid-cols-12 gap-12">
          ...
          <div className="rounded-[3.5rem] p-12 bg-slate-900 text-white shadow-2xl relative overflow-hidden group h-full border-none">
            ...
            <div className="relative z-10 space-y-12">
              <div className="space-y-3">
                ...
                <h4 className="text-3xl font-black tracking-tighter leading-tight uppercase text-primary">Í∞Ä?©ÏÑ±<br />?∏ÌÖîÎ¶¨Ï†Ñ??/h4>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative group/search">
                    ...
                    <input
                      ...
                      placeholder="Î¶¨ÏÜå??Î™ÖÏπ≠ ?êÎäî UID ?ÑÌÑ∞Îß?
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-white/5 flex items-center justify-between">
                <p className="text-[10px] font-bold text-slate-500 leading-relaxed italic uppercase opacity-60 max-w-[200px]">
                  * Î™®Îì† Î∂Ä???ÑÎ°ú?†ÏΩú Î≥ÄÍ≤ΩÏÇ¨??? ?ëÏóÖ Îß§Ìä∏Î¶?ä§??Ï¶âÏãú ?ôÍ∏∞?îÎê©?àÎã§.
                </p>
                <Button
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-8">
          <HubSectionCard
            title="Î¶¨ÏÜå??Í∞Ä?©ÏÑ± ?ÅÌÉú Îß§Ìä∏Î¶?ä§"
            description="?∏Ï†Å Î¶¨ÏÜå?§Ïùò ?§ÏãúÍ∞??úÏÑ±/Î∂Ä???ÅÌÉúÎ•??§ÏãúÍ∞ÑÏúºÎ°?Î™®Îãà?∞ÎßÅ?òÍ≥† ?úÏñ¥?©Îãà??"
            icon={SearchCode}
          >
            <div className="overflow-hidden">
              <StandardDataTable
                ...
                loading={loading}
                emptyMessage="?ÑÏû¨ ?úÏÑ±?îÎêú Î¶¨ÏÜå???§Ìä∏Î¶ºÏù¥ Ï°¥Ïû¨?òÏ? ?äÏäµ?àÎã§."

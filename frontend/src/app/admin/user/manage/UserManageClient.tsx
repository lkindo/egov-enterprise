  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        ...
        breadcrumbs={[{ label: '사용자관리' }, { label: '사용자 정보' }]}
      />
      ...
      <StandardModal
        ...
        title={editingUser ? '사용자 아키텍트 명세 수정' : '신규 아이덴티티 프로비저닝'}
        maxWidth="2xl"
        footer={
      ...
      >
        <div className="space-y-10 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <FormField label="사용자 고유 식별 명칭" required description="시스템 접근을 위한 고유 엑세스 토큰">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <FormField label="아이덴티티 로드 프로토콜">
               <select 
                ...
                >
                    <option value="P">--- PENDING (비승인_대기) ---</option>
                    <option value="A">--- ACTIVE_LIVE (활성_정상) ---</option>
                    <option value="D">--- DISABLED_BLOCK (차단_비활성) ---</option>
                </select>
            </FormField>
          </div>
          ...
          <div className="p-8 rounded-[2rem] bg-indigo-50/30 border-2 border-indigo-100/50 flex items-start gap-4">
              ...
              <div className="space-y-1">
                  <h6 className="text-[10px] font-black text-indigo-900 tracking-widest uppercase">Encryption_Policy_Active</h6>
                  <p className="text-[10px] font-bold text-indigo-700/60 leading-relaxed italic uppercase">사용자 생성 및 수정 시 모든 개인정보는 AES-256 규격으로 암호화되어 저장됩니다.</p>
              </div>
          </div>
        </div>
      </StandardModal>

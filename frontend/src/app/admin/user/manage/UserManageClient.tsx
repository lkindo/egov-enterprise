...
export default function UserManageClient({ initialData, initialParams }: { initialData: PageResponse<UserManage>; initialParams: UserSearchParams }) {
  ...
  const handleDelete = async (userId: string) => {
    const ok = await confirm({
      title: '?¨Ïö©???∞Ïù¥???ÅÍµ¨ ??†ú',
      message: '?¥Îãπ ?¨Ïö©??Í≥ÑÏ†ï Î∞??∞Í≤∞???úÎèô Î°úÍ∑∏ ?∞Ïù¥?∞Í? Î™®Îëê ??†ú?©Îãà?? ?ïÎßêÎ°?ÏßÑÌñâ?òÏãúÍ≤†Ïäµ?àÍπå?',
      variant: 'destructive',
      confirmText: '??†ú'
    });
    if (!ok) return;
  ...
  const columns: Column<UserManage>[] = [
    ...
    {
      header: '?∏Ï¶ù ?ÅÌÉú',
      accessor: (item) => <HubStatusBadge status={item.userSttusCode === 'A' ? '?úÏÑ±' : item.userSttusCode === 'P' ? 'PENDING' : 'DISABLED'} />,
      className: 'w-32'
    },
    {
      header: 'MANAGEMENT',
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="?ÑÏÇ¨ ?¨Ïö©???∏Ï¶ù Í±∞Î≤Ñ?åÏä§"
        breadcrumbs={[{ label: '?¨Ïö©?êÍ?Î¶? }, { label: '?¨Ïö©???ïÎ≥¥' }]}
      />

      <HubHeader
        title="?ÑÏù¥?¥Ìã∞??
        highlight="Fabric"
        subtitle="?ÑÏÇ¨ ?¨Ïö©??Í≥ÑÏ†ï ?êÍ≤© Ï¶ùÎ™Ö Î∞??µÌï© ?îÎ†â?†Î¶¨ ?ôÍ∏∞???ÑÎ°ú?†ÏΩú ?úÏñ¥"
        icon={Users}
        actions={
      ...
      <div className="grid grid-cols-12 gap-12">
          ...
          <div className="rounded-[3.5rem] p-12 bg-slate-900 text-white shadow-2xl relative overflow-hidden group h-full border-none">
            ...
            <div className="relative z-10 space-y-12">
              <div className="space-y-3">
                ...
                <h4 className="text-3xl font-black tracking-tighter leading-tight uppercase">?∏Ï¶ù<br />ÏΩîÏñ¥ ?ÑÎ°ú?†ÏΩú</h4>
              </div>
              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  ...
                  <div className="relative group/search">
                    ...
                    <input
                      ...
                      placeholder="?¨Ïö©?êÎ™Ö ?êÎäî Í≥†Ïú† ID"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <option value="" className="bg-slate-900">--- ALL_ENTITIES (?ÑÏ≤¥) ---</option>
                    <option value="P" className="bg-slate-900 text-amber-500">--- PENDING (?πÏù∏?ÄÍ∏? ---</option>
                    <option value="A" className="bg-slate-900 text-emerald-500">--- ACTIVE_LIVE (?úÏÑ±) ---</option>
                    <option value="D" className="bg-slate-900 text-rose-500">--- BLOCKED (ÎπÑÌôú?? ---</option>
                  </select>
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-white/5 flex items-center justify-between">
                <p className="text-[10px] font-bold text-slate-500 leading-relaxed italic uppercase opacity-60 max-w-[180px]">
                  * ?§Ïöî???∏Ï¶ù(MFA)???ÅÏö©??Í≥ÑÏ†ï?§ÏûÖ?àÎã§.
                </p>
                <Button variant="ghost" className="h-10 px-4 rounded-xl bg-white/5 text-primary text-[9px] font-black tracking-widest uppercase hover:bg-primary hover:text-white transition-all">Ï°∞Ìöå</Button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-8">
          <HubSectionCard
            title="?¨Ïö©???ÑÏù¥?¥Ìã∞???§Ìä∏Î¶??∏Î≤§?†Î¶¨"
            description="?ÑÏÇ¨ ?µÌï© ?îÎ†â?†Î¶¨???±Î°ù??Î™®Îì† ?¨Ïö©??Í∞úÏ≤¥??Î≥¥Ïïà ?çÏÑ± Î∞??∏Ï¶ù ?ÅÌÉú Í¥ÄÎ¶?Î™ÖÏÑ∏?ÖÎãà??"
            icon={SearchCode}
          >
            <div className="overflow-hidden">
              <StandardDataTable<UserManage>
                columns={columns}
                data={users}
                emptyMessage="Ï°∞Ìöå???¨Ïö©???∞Ïù¥?∞Í? ?∞Ïù¥?∞Î≤†?¥Ïä§ ?§Ìä∏Î¶ºÏóê Ï°¥Ïû¨?òÏ? ?äÏäµ?àÎã§."
      ...
      <StandardModal
        ...
        title={editingUser ? '?¨Ïö©???ÑÌÇ§?çÌä∏ Î™ÖÏÑ∏ ?òÏ†ï' : '?†Í∑ú ?ÑÏù¥?¥Ìã∞???ÑÎ°úÎπÑÏ???}
        maxWidth="2xl"
        footer={
          <div className="flex w-full gap-4">
            <Button variant="outline" onClick={() => setIsModalOpen(false)} className="flex-1 h-14 rounded-2xl font-black text-[10px] tracking-widest border-2">Ï∑®ÏÜå</Button>
            <Button onClick={handleSubmit} disabled={loading} className="flex-[2] h-14 rounded-2xl bg-slate-900 border-none text-white font-black text-[10px] tracking-widest shadow-2xl shadow-primary/30 hover:bg-primary transition-all hover:-translate-y-2 group">
              <Zap size={18} className="group-hover:animate-pulse" /> ?Ä??            </Button>
          </div>
        }
      >
        <div className="space-y-10 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <FormField label="?¨Ïö©??Í≥†Ïú† ?ùÎ≥Ñ Î™ÖÏπ≠" required description="?úÏä§???ëÍ∑º???ÑÌïú Í≥†Ïú† ?ëÏÑ∏???†ÌÅ∞">
            ...
            <FormField label="?¨Ïö©???±Î™Ö (Canonical Name)" required>
              <div className="relative group/name">
                ...
                <Input
                  ...
                  placeholder="?úÏãú ?¥Î¶Ñ"
                />
              </div>
            </FormField>
          </div>

          <FormField label="?∏Ï¶ù ?¨Î¶¨?¥ÏÖú (Credential Phase)" required={!editingUser} description={editingUser ? "Î≥¥Ïïà Í∞ïÌôîÎ•??ÑÌï¥ ?ÑÏöî?úÏóêÎß???ñ¥?∞Ïã≠?úÏò§." : "Î≥¥Ïïà Í∞ïÎèÑÍ∞Ä ?íÏ? Î≥µÌï© ÎπÑÎ?Î≤àÌò∏Î•?Í∂åÏû•?©Îãà??"}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <FormField label="Ïª§Î??àÏ??¥ÏÖò ?îÎìú?¨Ïù∏?? required>
              <div className="relative group/email">
                ...
                <Input
                  ...
                  placeholder="?¥Î©î??Ï£ºÏÜå"
                />
              </div>
            </FormField>
            <FormField label="?ÑÏù¥?¥Ìã∞??Î°úÎìú ?ÑÎ°ú?†ÏΩú">
              <select
              ...
              >
                <option value="P">--- ?ÄÍ∏?Ï§?(ÎπÑÏäπ???ÄÍ∏? ---</option>
                <option value="A">--- ?úÏÑ± (?ïÏÉÅ) ---</option>
                <option value="D">--- ÎπÑÌôú??(Ï∞®Îã®) ---</option>
              </select>
            </FormField>
          </div>
          ...
          <div className="p-8 rounded-[2rem] bg-indigo-50/30 border-2 border-indigo-100/50 flex items-start gap-4">
            ...
            <div className="space-y-1">
              <h6 className="text-[10px] font-black text-indigo-900 tracking-widest">?îÌò∏???ïÏ±Ö ?úÏÑ±??/h6>
              <p className="text-[10px] font-bold text-indigo-700/60 leading-relaxed">?¨Ïö©???ùÏÑ± Î∞??òÏ†ï ??Î™®Îì† Í∞úÏù∏?ïÎ≥¥??AES-256 Í∑úÍ≤©?ºÎ°ú ?îÌò∏?îÎêò???Ä?•Îê©?àÎã§.</p>
            </div>
          </div>
        </div>
      </StandardModal>

...
export default function UserManageClient({ initialData, initialParams }: { initialData: any; initialParams: UserSearchParams }) {
 ...
 const currentPage: number = initialData?.page ?? initialParams.page번호 ?? 1;
 ...
 const query = `page번호=${page}&searchKeyword=${params.searchKeyword || ''}&searchCondition=${params.searchCondition || '0'}&sbscrbSttus=${params.sbscrbSttus || ''}`;
 ...
 { id: 'userId', header: t('admin.user.userId'), pinned: 'left', sortable: true, width: 200, accessor: (item) => <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-black text-[10px] shadow-inner">아이디</div><span className="font-black tracking-tight text-slate-900">{item.userId}</span></div> },
 ...
 onSearch={(values) => {
 setParams(values);
 const query = `page번호=1&searchKeyword=${values.searchKeyword || ''}&searchCondition=${values.searchCondition || '0'}&sbscrbSttus=${values.sbscrbSttus || ''}`;
 router.push(`/admin/user/manage?${query}`);
 }}
 ...
 <DialogHeader>
 <DialogTitle className="text-3xl font-black tracking-tighter italic flex items-center gap-4">
 ...
 {!editingUser && (
 <div className="space-y-3">
 <Label htmlFor="password" className="text-[10px] font-black text-slate-400 tracking-tight px-1">{t('login.pwLabel')}</Label> 
 ...
 <Button onClick={handleSubmit} className="rounded-2xl h-14 px-12 font-black shadow-2xl shadow-primary/20 tracking-tight italic">{t('common.confirm')}</Button>
 </DialogFooter>
 </DialogContent>
 </Dialog>
 </StandardAdminLayout>

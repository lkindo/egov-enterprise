...
vi.mock('@/app/components/ui/standard-modal', () => ({ 
  StandardModal: ({ children, isOpen, title, onClose, footer }: any) => isOpen ? (
    <div data-testid="dialog">
      <h2>{title}</h2>
      <button onClick={onClose}>ëª¨ë‹¬ ?«ê¸°</button>
      {children}
...
describe('UserManageClient Component', () => {
 ...
 const mockInitialParams = { searchKeyword: '', searchCondition: '0', sbscrbSttus: '', pageë²ˆí˜¸: 1 };
 ...
 render(<UserManageClient initialData={mockInitialData} initialParams={mockInitialParams} />);
 const createBtn = screen.getByText(/? ê·œ ?±ë¡/i);
 fireEvent.click(createBtn);

 const dialog = await screen.findByTestId('dialog');
 expect(dialog).toBeDefined();
 expect(within(dialog).getByText('? ê·œ ?„ì´?´í‹°???„ë¡œë¹„ì???)).toBeDefined();
 });
 ...
 const dialog = await screen.findByTestId('dialog');
 expect(dialog).toBeDefined();
 expect(within(dialog).getByText('?¬ìš©???„í‚¤?íŠ¸ ëª…ì„¸ ?˜ì •')).toBeDefined();
 
 const idInput = within(dialog).getByLabelText(/?¬ìš©??ê³ ìœ  ?ë³„ ëª…ì¹­/i) as HTMLInputElement;
 ...
 // Open dialog
 fireEvent.click(screen.getByText(/? ê·œ ?±ë¡/i));

 const dialog = await screen.findByTestId('dialog');

 // Fill form
 fireEvent.change(within(dialog).getByLabelText(/?¬ìš©??ê³ ìœ  ?ë³„ ëª…ì¹­/i), { target: { value: 'newuser' } });
 fireEvent.change(within(dialog).getByLabelText(/?¬ìš©???±ëª…/i), { target: { value: 'New User' } });
 fireEvent.change(within(dialog).getByLabelText(/?¸ì¦ ?¬ë¦¬?´ì…œ/i), { target: { value: 'password123' } });

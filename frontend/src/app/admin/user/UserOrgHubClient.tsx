...
export default function UserOrgHubClient({ defaultTab = 'USERS' }: { defaultTab?: UserOrgTab }) {
  ...
  const { data: usersData, isLoading: isUsersLoading, error: usersError, refetch: refetchUsers } = useQuery({
    ...
    queryFn: () => userAdminService.getUserList({ pageÎ≤àÌò∏: userPage, searchKeyword }),
  ...
  const { data: deptsData, isLoading: isDeptsLoading, error: deptsError, refetch: refetchDepts } = useQuery({
    ...
    queryFn: () => deptAdminService.getDeptList({ pageÎ≤àÌò∏: deptPage, searchKeyword }),
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        ...
        breadcrumbs={[{ label: '?¨Ïö©?êÍ?Î¶? }, { label: 'Ï°∞ÏßÅ ?µÌï© ?àÎ∏å' }]}
      />

      <HubHeader
        title="?ÑÏù¥?¥Ìã∞??
        highlight="Fabric"
        subtitle="?ÑÏÇ¨ ?∏Ï†Å ?êÏõê Îß§Ìä∏Î¶?ä§ Î∞?Ï°∞ÏßÅ??Í≥ÑÏ∏µ ?†Ìè¥Î°úÏ? ?µÌï© ?§Ï??§Ìä∏?àÏù¥???ºÌÑ∞"
        icon={UserCog}
        actions={
      ...
      <div className="grid grid-cols-12 gap-12 min-h-[900px]">
          ...
          <div className="mt-auto rounded-[3.5rem] bg-slate-900 text-white p-12 space-y-8 shadow-2xl relative overflow-hidden group border-none">
            ...
            <div className="relative z-10 space-y-6 text-center lg:text-left">
              ...
              <div className="space-y-4">
                <h4 className="text-2xl font-black tracking-tighter leading-tight uppercase">?ÑÏù¥?¥Ìã∞??br />?∏ÌÖîÎ¶¨Ï†Ñ??/h4>
                <p className="text-[10px] text-white/30 font-black tracking-[0.4em] uppercase leading-relaxed">Active Directory (AD)<br />?ôÍ∏∞???ÑÎ£å</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-4 h-full flex flex-col gap-8">
          <HubSectionCard
            ...
            description="?ÑÏÇ¨ ?µÌï© ?îÎ†â?†Î¶¨?êÏÑú ?§ÏãúÍ∞ÑÏúºÎ°??ôÍ∏∞?îÎêò??Í∞úÏ≤¥ ?ÑÎ°úÎ∏?Î∞?Î≥¥Ïïà ?ÅÌÉú Î™ÖÏÑ∏?ÖÎãà??"
          ...
          >
            <div className="space-y-8">
              <div className="flex items-center justify-between px-2 pt-2 border-b border-slate-100 pb-8">
                <div>
                  <span className="text-[10px] font-black text-muted-foreground/30 tracking-[0.4em] uppercase font-mono">?§ÏãúÍ∞??îÎ†â?†Î¶¨ ?ôÍ∏∞??/span>
                </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    <StandardDataTable<UserManage | Department>
                      ...
                      emptyMessage="Í≤Ä?âÎêú Í∞úÏ≤¥Í∞Ä Ï°¥Ïû¨?òÏ? ?äÏäµ?àÎã§."
                      isPremium={false}
          ...
          <AnimatePresence mode="wait">
            {selectedItemId ? (
                    ...
                    <div className="pt-12 border-t border-slate-100 space-y-10">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="text-[11px] font-black text-muted-foreground/40 tracking-[0.4em] uppercase font-mono leading-none mb-1">Í∂åÌïú Î∂Ä???ÑÎ°ú?†ÏΩú</h4>
                            <p className="text-sm font-black text-foreground tracking-tighter uppercase">?úÏÑ± Í∂åÌïú Îß§Ìä∏Î¶?ä§</p>
                          </div>
                        </div>
              ...
              <div className="h-full rounded-[4rem] border-4 border-dashed border-slate-100 bg-slate-50/50 flex flex-col items-center justify-center p-24 text-center select-none group">
                ...
                <p className="text-[12px] font-black text-slate-300 tracking-[0.6em] mt-6 uppercase leading-relaxed max-w-[280px]">?∏ÌÖîÎ¶¨Ï†Ñ???ôÍ∏∞?îÎ? ?úÏûë?òÎ†§Î©??†Ìè¥Î°úÏ? ?§Ìä∏Î¶ºÏóê???îÌÑ∞?∞Î? ?†ÌÉù?òÏÑ∏??/p>

...
<<<<<<< HEAD
      toast.error('ì¡°ì§ ì²´ê³„ ìŠ¤íŠ¸ë¦¼ ë¡œë“œì— ì‹¤íŒ¨í–ˆìŠµë‹ˆë‹¤.');
=======
      toast.error('ì¡°ì§ ì²´ê³„ ?¤íŠ¸ë¦?ë¡œë“œ???¤íŒ¨?ˆìŠµ?ˆë‹¤.');
>>>>>>> 99be2886750c05e99df098d47b5b4fd8f624093f
  ...
  const handleSubmit = async () => {
    if (!form.orgnztNm) {
      toast.error('? íš¨??ì¡°ì§ ëª…ì¹­???„ìš”?©ë‹ˆ??');
      return;
    }
    ...
    try {
      if (selectedDept?.orgnztId) {
        ...
        toast.success('ì¡°ì§ ?„í‚¤?ì²˜ê°€ ?±ê³µ?ìœ¼ë¡??…ë°?´íŠ¸?˜ì—ˆ?µë‹ˆ??');
      } else {
        await deptAdminService.createDept(form);
        toast.success('? ê·œ ì¡°ì§ ?¸ë“œê°€ ë°°í¬?˜ì—ˆ?µë‹ˆ??');
      }
      setIsAddOpen(false);
      handleRefresh();
    } catch {
<<<<<<< HEAD
      toast.error('ë°ì´í„° ì •í•©ì„± ì˜¤ë¥˜ë¡œ ì €ì¥ì´ ì·¨ì†Œë˜ì—ˆìŠµë‹ˆë‹¤.');
=======
      toast.error('?°ì´???•í•©???¤ë¥˜ë¡??€?¥ì´ ì·¨ì†Œ?˜ì—ˆ?µë‹ˆ??');
>>>>>>> 99be2886750c05e99df098d47b5b4fd8f624093f
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (deptId: string) => {
    if (!confirm('?´ë‹¹ ì¡°ì§ ?¸ë“œë¥??êµ¬?ìœ¼ë¡??œê±°?˜ì‹œê² ìŠµ?ˆê¹Œ? ê´€???°ì´?°ê? ?Œì‹¤?????ˆìŠµ?ˆë‹¤.')) return;

    setLoading(true);
    try {
      await deptAdminService.deleteDept(deptId);
      toast.success('ì¡°ì§ ?ì‚°???? œ?˜ì—ˆ?µë‹ˆ??');
      handleRefresh();
    } catch {
<<<<<<< HEAD
      toast.error('ê¶Œí•œ ë¶€ì¡± ë˜ëŠ” ì‹œìŠ¤í…œ ì˜¤ë¥˜ë¡œ ì‚­ì œë¥¼ ìˆ˜í–‰í•˜ì§€ ëª»í–ˆìŠµë‹ˆë‹¤.');
=======
      toast.error('ê¶Œí•œ ë¶€ì¡??ëŠ” ?œìŠ¤???¤ë¥˜ë¡??? œë¥??˜í–‰?˜ì? ëª»í–ˆ?µë‹ˆ??');
>>>>>>> 99be2886750c05e99df098d47b5b4fd8f624093f
    } finally {
      setLoading(false);
    }
  ...
  const columns: Column<DeptDto>[] = [
    ...
    {
      header: '?íƒœ',
      accessor: (item: DeptDto) => <HubStatusBadge status="?œì„±" />,
      className: 'w-32'
    },
    {
      header: 'MANAGEMENT',
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      ...
      <HubHeader
        title="Organization"
        highlight="Topology"
        subtitle="?„ì‚¬ ë¹„ì¦ˆ?ˆìŠ¤ ì¡°ì§ ì²´ê³„ ë°?ê³„ì¸µ??ë¶€??êµ¬ì¡°???¤ì‹œê°?ê´€ë¦??œìŠ¤??
        icon={Network}
        actions={
      ...
      <div className="grid grid-cols-12 gap-12">
          ...
          <div className="rounded-[3.5rem] p-12 bg-slate-900 text-white shadow-2xl relative overflow-hidden group h-full border-none">
            ...
            <div className="relative z-10 space-y-12">
              <div className="space-y-3">
                ...
                <h4 className="text-3xl font-black tracking-tighter leading-tight uppercase">ì¡°ì§<br />?¸í”„??/h4>
              </div>
              ...
              <div className="pt-8 border-t border-white/5 flex items-center justify-between">
                <p className="text-[10px] font-bold text-slate-500 leading-relaxed italic uppercase opacity-60 max-w-[200px]">
                  * ?„ë¡œë¹„ì??ëœ ëª¨ë“  ì¡°ì§ ?ì‚°?€ ?¤ì‹œê°„ìœ¼ë¡??¬ìš©??ë§¤íŠ¸ë¦?Š¤?€ ?™ê¸°?”ë©?ˆë‹¤.
                </p>
                <Button
                  onClick={handleRefresh}
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-8">
          <HubSectionCard
            title="ì¡°ì§ ?ì‚° ?”ë ‰? ë¦¬ ?¸ë²¤? ë¦¬"
            description="?œìŠ¤??? í´ë¡œì? ?´ì— êµ¬ì„±???„ì‚¬ ë¹„ì¦ˆ?ˆìŠ¤ ì¡°ì§ ?¨ìœ„???¤ì‹œê°??íƒœ ëª…ì„¸?…ë‹ˆ??"
            icon={SearchCode}
          >
            <div className="overflow-hidden">
              <StandardDataTable
                ...
                data={depts}
                loading={loading}
                emptyMessage="ì¡°íšŒ??ì¡°ì§ êµ¬ì¡° ?°ì´?°ê? ì¡´ì¬?˜ì? ?ŠìŠµ?ˆë‹¤."
      ...
      <StandardModal
        ...
        title={selectedDept ? 'ì¡°ì§ ?¸ë“œ ?¤í™ ?˜ì •' : '? ê·œ ë¶€???ì‚° ?„ë¡œë¹„ì???}
        maxWidth="xl"
        footer={
          <div className="flex w-full gap-4">
            <Button variant="outline" onClick={() => setIsAddOpen(false)} className="flex-1 h-14 rounded-2xl font-black text-[10px] tracking-widest border-2">ì·¨ì†Œ</Button>
            <Button onClick={handleSubmit} disabled={loading} className="flex-[2] h-14 rounded-2xl bg-slate-900 border-none text-white font-black text-[10px] tracking-widest shadow-2xl shadow-primary/30 hover:bg-primary transition-all hover:-translate-y-2 group">
              <Zap size={18} className="group-hover:animate-pulse" /> {selectedDept ? 'ì¡°ì§ ?˜ì •' : 'ë¶€??ë°°í¬'}
            </Button>
          </div>
        }
      >
        <div className="space-y-10 pt-4">
          <FormField label="ì¡°ì§ ë°?ë¶€??ëª…ì¹­" required description="?œìŠ¤???„ë°˜???¸ì¶œ??ì¡°ì§???œì? ëª…ì¹­">
          ...
          <FormField label="ì¡°ì§ ?„í‚¤?ì²˜ ëª…ì„¸" description="?´ë‹¹ ì¡°ì§??ì£¼ìš” ??•  ë°?ë©”í??°ì´???•ì˜">
          ...
          <div className="p-8 rounded-[2.5rem] bg-indigo-50/30 border-2 border-indigo-100/50 flex items-start gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] font-bold text-indigo-700/60 leading-relaxed italic uppercase">? ê·œ ì¡°ì§ ?¸ë“œ ?ì„± ??ê³„ì¸µ êµ¬ì¡° ?ë™ ê²€ì¦??„ë¡œ? ì½œ???˜í–‰?©ë‹ˆ??</p>
            </div>
          </div>
        </div>
      </StandardModal>

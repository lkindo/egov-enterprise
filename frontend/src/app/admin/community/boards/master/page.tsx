...
export default function BoardMasterListPage() {
      ...
      accessor: (board: BoardMaster) => (
        <div className="flex items-center gap-6">
           <div className="w-16 h-16 rounded-2xl bg-white border-2 border-slate-50 shadow-sm flex items-center justify-center text-slate-400 group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-all duration-500">
              {board.bbsTyCodeNm === 'ÏßÄ???àÎ∏å' ? <BookOpen size={28} /> : 
    ...
    },
    {
      ...
      accessor: (_board: BoardMaster) => (
        <div className="space-y-1 text-center">
           ...
           <p className="text-[10px] font-black text-slate-300 uppercase leading-none">Í≤åÏãúÍ∏Ä ??/p>
        </div>
      ),
      className: 'text-center'
    },
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000 max-w-[1600px] mx-auto px-4">
      <PageHeader title="Í≤åÏãú??ÎßàÏä§?? breadcrumbs={[{ label: 'Ïª§Î??àÌã∞' }, { label: 'Í≤åÏãú?êÍ?Î¶? }, { label: 'ÎßàÏä§??ÏΩòÏÜî' }]} />

      <HubHeader 
        title="ÎßàÏä§??ÏΩòÏÜî" 
        highlight="Í≤åÏãú???µÌï© Í¥ÄÎ¶? 
        subtitle="?ùÏÑ±??Î™®Îì† Í≤åÏãú?êÏùò ?ºÏù¥?ÑÏÇ¨?¥ÌÅ¥Í≥?Í∂åÌïú Îß§Ìä∏Î¶?ä§Î•??§ÏãúÍ∞ÑÏúºÎ°??úÏñ¥?òÍ≥† Î™®Îãà?∞ÎßÅ?©Îãà??" 
        icon={Settings2} 
        actions={
          <Button 
          ...
          >
            ...
            ?ùÏÑ± ÎßàÎ≤ï??            <Rocket className="w-4 h-4 text-primary opacity-40 group-hover:text-primary group-hover:opacity-100 transition-all" />
          </Button>
        }
      />
      ...
      >
        <InsightCard label="Ï¥ùÍ≥Ñ" value="32" desc="Active Board Masters" icon={Layers} color="text-indigo-500" />
        <InsightCard label="ÍµêÎ•ò" value="1.2k" desc="Engagement Traffic" icon={TrendingUp} color="text-rose-500" />
        <InsightCard label="?úÏä§?? value="Optimal" desc="Storage Status" icon={Zap} color="text-emerald-500" />
        <InsightCard label="Î≥¥Ïïà" value="L4" desc="Encrypted Access" icon={ShieldCheck} color="text-amber-500" />
      </motion.div>
      ...
      <div className="border border-slate-100 rounded-[3rem] overflow-hidden bg-white shadow-2xl shadow-slate-200/50">
        <StandardDataTable<BoardMaster>
          ...
          loading={isLoading}
          search={{
            placeholder: 'Í≤åÏãú??Î™ÖÏπ≠, ?úÏä§??ID Í≤Ä??..',
      ...
      <div className="p-12 rounded-[4rem] bg-slate-900 border-none text-white overflow-hidden relative group">
        ...
        <div className="max-w-3xl space-y-8 relative z-10">
           ...
           <p className="text-xl text-slate-400 font-bold leading-relaxed tracking-tight">?ùÌÉúÍ≥ÑÎ? ?ïÏû•??Ï§ÄÎπÑÍ? ?òÏÖ®?µÎãàÍπ? ÎßàÎ≤ï?¨Î? ?µÌï¥ Î≥µÏû°??Í≥ºÏ†ï ?ÜÏù¥ ??4?®Í≥ÑÎßåÏúºÎ°??¨ÎÇ¥ ÏßÄ???àÎ∏åÎ•?Íµ¨Ï∂ï?òÏã≠?úÏò§.</p>
           <Button 
              onClick={() => router.push('/admin/community/boards/maker')}
           ...
           >
              ?ºÏù¥Î∏?ÎßàÎ≤ï???§Ìñâ <Rocket className="w-8 h-8" />
           </Button>
        </div>
      </div>
    </div>

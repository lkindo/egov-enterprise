    ...
    <div className="flex flex-col gap-8 p-6 max-w-5xl mx-auto w-full pb-32 animate-in fade-in duration-700">
      ...
      <Card className="shadow-[0_40px_80px_-20px_rgba(0,0,0,0.12)] border-none overflow-hidden rounded-lg bg-card ring-1 ring-primary/5">
        <CardHeader className="border-b bg-slate-950 pb-20 pt-20 px-12 md:px-20 text-white relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-10 relative z-10">
            <div className="space-y-6 text-center md:text-left">
              ...
              <CardTitle className="text-2xl md:text-2xl font-bold tracking-tight leading-tight">
                {pathname?.includes('insertBoardArticle') ? '새로운 통찰을' : '내용을 수정하여'} <br />
                <span className="text-primary underline decoration-4 decoration-primary/20 underline-offset-8">Insight</span> 공유하세요
              </CardTitle>
            ...
            <div className="hidden lg:block relative">
              <div className="w-24 h-24 rounded-lg bg-gradient-to-br from-primary/20 to-transparent border-2 border-white/10 flex items-center justify-center rotate-12 hover:rotate-0 transition-all duration-700 shadow-2xl">
                <MessageSquare className="w-10 h-10 text-white/40" />
              </div>
              <div className="absolute -top-3 -right-3 w-10 h-10 rounded-lg bg-primary flex items-center justify-center shadow-xl animate-bounce duration-[2000ms]">
        ...
        <StandardForm action={formAction}>
          ...
          <CardContent className="pt-20 px-12 md:px-20 space-y-20">
            ...
            <div className="space-y-6 group">
              <div className="flex items-center justify-between">
                <Label htmlFor="nttSj" className={cn(
                  "text-xs font-bold tracking-widest text-muted-foreground group-focus-within:text-primary transition-colors flex items-center gap-3",
              ...
              <Input
                ...
                className={cn(
                  "h-11 text-xl font-bold border-2 border-primary/5 focus:border-primary focus-visible:ring-primary/10 transition-all rounded-lg px-8 bg-muted/30 shadow-inner group-focus-within:shadow-2xl group-focus-within:bg-background placeholder:text-muted-foreground/30",
            ...
            {tmplatId === 'TMPLT_QNA' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <Label className="text-xs font-bold tracking-widest text-amber-500 flex items-center gap-3">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {['TECHNICAL', 'HR_POLICY', 'PROJECT_MGMT', 'GENERAL'].map(cat => (
                    <Button 
                      ...
                      className={cn(
                        "h-12 font-bold text-xs tracking-widest border-2 transition-all rounded-lg",
            ...
            {tmplatId === 'TMPLT_CALENDAR' && (
              <div className="p-8 bg-cyan-900/10 border-2 border-cyan-500/20 rounded-lg flex flex-col md:flex-row items-center gap-8 animate-in slide-in-from-right-4 duration-500">
                 <div className="w-16 h-12 bg-cyan-500 rounded-lg flex items-center justify-center text-white shadow-xl shadow-cyan-500/20 shrink-0">
                    <Calendar size={32} />
                 </div>
                 <div className="flex-1 space-y-4">
                    <div className="space-y-1">
                        <p className="text-sm font-bold text-cyan-700 uppercase tracking-tight">Event Schedule</p>
            ...
            <div className="space-y-6 group">
              ...
              <div className="relative">
                <Textarea
                  ...
                  className={cn(
                    "min-h-[500px] p-10 text-lg font-medium leading-loose border-2 border-primary/5 focus:border-primary focus-visible:ring-primary/10 transition-all rounded-lg bg-muted/30 shadow-inner group-focus-within:shadow-2xl group-focus-within:bg-background resize-none",
                    tmplatId === 'TMPLT_QNA' && "focus:border-amber-500 focus-visible:ring-amber-500/10",
                    state?.field === 'nttCn' && "border-rose-500 bg-rose-50"
                  )}
                  required
                />
                <div className="absolute bottom-8 right-10 flex items-center gap-2.5 text-xs font-bold text-muted-foreground/40 tracking-widest pointer-events-none bg-muted/50 px-4 py-2 rounded-full border border-primary/5 backdrop-blur-sm">
            ...
            {/* 파일 첨부 영역 */}
            <div className="space-y-6 group">
              <Label className="text-xs font-bold tracking-widest text-muted-foreground flex items-center gap-3">
            ...
            <div className="p-10 bg-slate-900 rounded-lg flex flex-col md:flex-row items-center gap-10 shadow-2xl relative overflow-hidden group/notice">
              ...
              <div className="w-20 h-20 bg-slate-800 rounded-lg border border-slate-700 shadow-2xl flex items-center justify-center shrink-0 group-hover/notice:rotate-12 transition-transform">
                <CheckCircle2 className="w-8 h-8 text-primary" />
              </div>
              <div className="space-y-3 relative z-10 text-center md:text-left">
                <p className="font-bold text-xl text-white tracking-tight ">보안 준수 사항</p>
          ...
          <CardFooter className="flex flex-col md:flex-row justify-center gap-6 py-20 border-t border-primary/5 bg-muted/5 px-12 md:px-20 rounded-b-lg">
            <Button
              type="button"
              variant="ghost"
              onClick={() => router.back()}
              className="h-11 px-12 font-bold tracking-widest text-xs text-muted-foreground hover:bg-background hover:text-rose-500 hover:shadow-2xl transition-all rounded-lg active:scale-95 border-2 border-transparent"
            >
              <ArrowLeft className="w-5 h-5 mr-4" /> 이전으로
            </Button>
            <Button
              type="submit"
              className="h-11 px-16 gap-4 font-bold tracking-widest text-xs shadow-2xl shadow-primary/20 bg-primary hover:bg-primary/90 text-white transition-all active:scale-95 ring-[12px] ring-primary/5 rounded-lg"
              disabled={isPending}
            >
              {isPending ? (

...
export const BoardListClient = ({ initialData, params: initialParams }: { initialData: any; params: any }) => {
 ...
 const [pageë²ˆí˜¸, setPageë²ˆí˜¸] = useState(initialParams.pageë²ˆí˜¸ || 1);
 ...
 const [orderBy, setOrderBy] = useState(initialParams.orderBy || 'date');
  ...
  // ë©”ë‰´ë¥??µí•œ ì§„ì… ?¬ë? (ê´€ë¦¬ì ?€?œë³´?œìš©???„ë‹Œ ?¤ì œ ?œë¹„?¤ìš©?¸ì? ?•ì¸)
  // ê´€ë¦¬ì ê¶Œí•œ???ˆì–´???¼ë°˜ ê²Œì‹œ???œë¹„???„ì¹˜??ê²½ìš° ?µê³„ë¥??¨ê?
 ...
 <div className="flex flex-col gap-6 p-6 pb-20">
  {/* Breadcrumb - ?™ì  ë©”ë‰´ ?œìŠ¤???°ë™ */}
  <DynamicBreadcrumb 
    customItems={[
      { name: pathname.includes('/admin/system') ? '?œìŠ¤??ê´€ë¦? : 'ì»¤ë??ˆí‹° ë°?ì½˜í…ì¸? },
      { name: masterInfo?.bbsNm || (bbsId.includes('NOTICE') ? 'ê³µì??¬í•­' : 'ê²Œì‹œ??) }
    ]}
  />

  {/* ?œí”Œë¦¿ì— ?°ë¥¸ ì°¨ë³„?”ëœ ?ë‹¨ ?¤ë” */}
  <div className="flex flex-col gap-4 mb-4">
    <div className="flex items-center gap-3">
      <div className={cn("w-1.5 h-8 rounded-full", tmplatId === 'TMPLT_HUB' ? "bg-indigo-500" : "bg-primary")} />
      <h2 className="text-3xl font-black tracking-tight">
        {masterInfo?.bbsNm || (bbsId.includes('NOTICE') ? 'ê³µì??¬í•­' : 'ê²Œì‹œ??)}
      </h2>
      {tmplatId === 'TMPLT_HUB' && <Badge className="bg-indigo-500/10 text-indigo-500 border-indigo-500/20 font-bold ml-2">ì§€???ˆë¸Œ</Badge>}
    </div>
    <p className="text-muted-foreground font-medium ml-4">
      {masterInfo?.bbsIntrcn || '??ê²Œì‹œ?ì˜ ?œë™?‰ê³¼ ìµœì‹  ?Œì‹???•ì¸?˜ì„¸??'}
    </p>
  </div>

  {/* ê´€ë¦¬ì ë·°ì—?œë§Œ ?µê³„ ë¦¬í¬???¸ì¶œ */}
 ...
 <CardHeader className="flex flex-row items-center justify-between bg-slate-900 pb-12 pt-12 px-10 text-white relative overflow-hidden">
  ...
  <span>{masterInfo?.bbsNm || (bbsId.includes('NOTICE') ? 'ê³µì??¬í•­' : '?ìœ  ê²Œì‹œ??)}</span>
  </CardTitle>
  <p className="text-slate-400 font-bold text-sm">ì´?<span className="text-white">{totalCount}ê°?/span>???Œì¤‘???´ì•¼ê¸°ê? ?´ê²¨?ˆìŠµ?ˆë‹¤.</p>
  </div>
 <CardAction className="relative z-10 flex items-center gap-3">
  ...
  <Settings2 className="w-6 h-6" /> ë§ˆìŠ¤??ì½˜ì†”
  </Button>
  </Link>
  )}
 ...
 <SelectValue placeholder="ê²€??ì¡°ê±´" />
 </SelectTrigger>
 <SelectContent>
 <SelectItem value="0">?œëª©</SelectItem>
 <SelectItem value="1">?´ìš©</SelectItem>
 <SelectItem value="2">?‘ì„±??/SelectItem>
 </SelectContent>
 </Select>
 ...
 <span className="font-black text-slate-800">ê¸°ê°„ ?¤ì •</span>
 <Button
 variant="ghost"
 size="sm"
 ...
 >
 <X size={14} className="mr-1" /> ì´ˆê¸°?? </Button>
 </div>
 ...
 <SelectItem value="comments">?“ê???/SelectItem>
 </SelectContent>
 </Select>
 </div>
  ...
  <Search className="w-6 h-6" /> ì¡°íšŒ
  </Button>
 </div>
 </form>
 </div>
  ...
  <div className="rounded-[2.5rem] border-2 border-slate-50 overflow-hidden shadow-2xl bg-white mb-10">
    {loading ? (
          ...
          <Card key={item.nttId} className="group overflow-hidden rounded-[2.5rem] border-2 border-slate-50 hover:border-primary/20 hover:shadow-2xl transition-all cursor-pointer">
            <div className="h-56 bg-slate-100 relative overflow-hidden">
               ...
               <Badge className="absolute top-6 left-6 bg-white/90 text-slate-900 border-none font-black backdrop-blur-md">
                 ë²ˆí˜¸. {totalCount - ((pageë²ˆí˜¸ - 1) * 10) - idx}
               </Badge>
            </div>
            <CardContent className="p-8 space-y-4 bg-white relative z-10">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-50 mt-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="flex items-center gap-1.5 text-xs font-bold text-slate-400"><MessageSquare size={12} /> {0}ê°?/span>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <div className="flex flex-col items-center justify-center h-80 gap-4 text-slate-300">
        ...
        <p className="text-xl font-black">ê²Œì‹œê¸€???„ì§ ?†ìŠµ?ˆë‹¤.</p>
      </div>
    ) : (
      <Table>
        <TableHeader className="bg-slate-50/80">
          <TableRow className="hover:bg-transparent border-b-2">
            <TableHead className="w-[80px] text-center font-black text-slate-400 tracking-tight text-[11px] py-8">ë²ˆí˜¸</TableHead>
            <TableHead className="font-black text-slate-900 tracking-tight text-[11px] py-8 px-6">?œëª©</TableHead>
            <TableHead className="w-[150px] font-black text-slate-400 tracking-tight text-[11px] py-8 text-center">?‘ì„±??/TableHead>
            <TableHead className="w-[140px] font-black text-slate-400 tracking-tight text-[11px] py-8 text-center">?±ë¡??/TableHead>
            <TableHead className="w-[180px] font-black text-slate-400 tracking-tight text-[11px] py-8 text-center">ì°¸ì—¬??/TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {list.map((item: BoardPost, idx: number) => (
 ...
 <Button
 variant="outline"
 onClick={() => setPageë²ˆí˜¸((p: number) => Math.max(1, p - 1))}
 disabled={pageë²ˆí˜¸ === 1}
 ...
 <span className="text-lg font-black text-white">{pageë²ˆí˜¸}</span>
 <div className="h-4 w-px bg-white/20" />
 <span className="text-sm font-bold text-white/50">{totalPages}</span>
 </div>
 <Button
 variant="outline"
 onClick={() => setPageë²ˆí˜¸((p: number) => Math.min(totalPages, p + 1))}
 disabled={pageë²ˆí˜¸ === totalPages}
 className="h-12 px-8 font-black rounded-xl border-2 hover:bg-slate-50"
 >
 ?¤ìŒ
 </Button>

...
export async function getInitialBoardData(params: {
 ...
 try {
  // 諛깆뿏님BoardController 매핑님留욎떠 경로 蹂寃 /api/v1/boards/{bbsId}
  const { bbsId, page踰덊샇, ...restParams } = params;
  const queryParams = {
   page: page踰덊샇 - 1, // Spring Data Pageable 0遺님시작
   size: 10,
   ...restParams
  };
   ...
   // Spring Data Page 媛앹껜 援ъ“님留욎떠 데이터異붿텧 (content, totalElements, totalPages)
   return {
  ...
  // 401 ㅻ쪟님?몄쬆님?꾩슂님?곹깭?대濡님몄텧?먯뿉님redirect ?쒗궗 님?덈룄濡님ㅼ떆 ?섏쭚

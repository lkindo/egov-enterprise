...
export default function InsertBoardArticlePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        toast('ÏßÄ???êÏÇ∞???±Í≥µ?ÅÏúºÎ°??±Î°ù?òÏóà?µÎãà??', 'success');
        router.push(`/admin/help?bbsId=${bbsId}`);
      } else {
        toast(result.message || '?±Î°ù ?§Ìå®', 'error');
      }
    } catch {
      toast('?±Î°ù Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.', 'error');
    } finally {
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">ÏßÄ???∏Îìú ÏΩòÌÖêÏ∏?/h3>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">?§ÏãúÍ∞??ôÍ∏∞??Ï§ÄÎπÑÎê®</span>
            </div>
          </div>
          <RichTextEditor
            value={form.nttCn || ''}
            onChange={(content) => setForm({ ...form, nttCn: content })}
            placeholder="?ÅÏÑ∏ ?¥Ïö©??Í∏∞Ïà†?òÏã≠?úÏò§..."
          />
        </div>

...
import { templateAdminService, TmplatInfo } from '@/services/admin/system/TemplateAdminService';
import {
  Layout,
  Plus,
  Search,
  ...
  const columns = [
    {
      ...
      accessor: (item: TmplatInfo) => (
        <span className="font-mono font-black text-slate-400 italic text-[10px] tracking-widest">{item.tmplatId}</span>
      )
    },
    {
      header: '템플릿 명',
      accessor: (item: TmplatInfo) => (
        <div className="flex items-center gap-3">
          ...
          <span className="font-black italic uppercase tracking-tighter text-slate-900">{item.tmplatNm}</span>
        </div>
      )
    },
    {
      header: '구분',
      accessor: (item: TmplatInfo) => (
        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-100 px-2 py-1 rounded-md italic">
    ...
    },
    {
      ...
      accessor: (item: TmplatInfo) => (
        <div className={cn(
          ...
          <span className="text-[10px] font-black uppercase tracking-widest italic">{item.useAt === 'Y' ? 'Active' : 'Disabled'}</span>
        </div>
      )
    }
  ];
      ...
      <div className="responsive-card p-6 md:p-12 border-2 border-slate-100 bg-white/50 backdrop-blur-xl relative overflow-hidden group">
        <div className="flex items-center gap-4 mb-12">
            ...
            <div>
                ...
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Registered system templates</p>
            </div>
        </div>
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="sm:max-w-[500px] rounded-[3rem] p-10 border-none shadow-2xl bg-white">
          <DialogHeader className="space-y-4">
            ...
            <DialogTitle className="text-3xl font-black text-slate-900 uppercase tracking-tighter italic text-center">New Blueprint</DialogTitle>
          ...
          <div className="space-y-8 py-8">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic ml-2">Template Designation</label>
              <Input
            ...
            <div className="grid grid-cols-2 gap-6">
                <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic ml-2">Category</label>
                    <Select 
                        value={newTemplate.tmplatSeCode} 
                        onValueChange={(v) => setNewTemplate(prev => ({ ...prev, tmplatSeCode: v }))}
                    >
                        <SelectTrigger className="h-16 rounded-3xl border-2 border-slate-100 bg-slate-50/50 font-black text-[10px] uppercase tracking-widest italic focus:bg-white">
                            <SelectValue placeholder="SELECT CATEGORY" />
                        </SelectTrigger>
                        <SelectContent className="rounded-2xl border-none shadow-2xl">
                            <SelectItem value="TMPT01" className="font-black text-[10px] uppercase tracking-widest italic">게시판</SelectItem>
                            <SelectItem value="TMPT02" className="font-black text-[10px] uppercase tracking-widest italic">커뮤니티</SelectItem>
                            <SelectItem value="TMPT03" className="font-black text-[10px] uppercase tracking-widest italic">일반</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
                <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic ml-2">Status</label>
                    <Select 
                    ...
                    >
                        ...
                        <SelectContent className="rounded-2xl border-none shadow-2xl">
                            <SelectItem value="Y" className="font-black text-[10px] uppercase tracking-widest italic">Active</SelectItem>
                            <SelectItem value="N" className="font-black text-[10px] uppercase tracking-widest italic">Disabled</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
            </div>
            ...
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic ml-2">Source Path</label>

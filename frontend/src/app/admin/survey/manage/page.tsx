  ...
  return (
    <div className="p-8 space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <Button onClick={() => router.push('/admin/survey/manage/create')} className="h-14 px-8 rounded-2xl bg-slate-900 border-none text-white font-black text-sm tracking-tight shadow-xl hover:bg-slate-800 transition-all active:scale-95 gap-3">
            ...
            <form onSubmit={handleSearch} className="flex flex-col md:flex-row items-center gap-4">
                <div className="relative flex-1 w-full group">
                    ...
                    <Input
                        ...
                        className="h-14 pl-14 rounded-2xl border-2 border-transparent bg-white shadow-sm focus:border-primary focus:ring-0 transition-all font-bold"
                        value={params.searchKeyword || ''}
                        onChange={(e) => setParams(prev => ({ ...prev, searchKeyword: e.target.value }))}
                    />
                </div>
                <Button type="submit" className="h-14 px-10 rounded-2xl bg-white border-2 border-slate-200 text-slate-900 font-black text-sm hover:bg-slate-50 hover:border-slate-300 shadow-sm transition-all active:scale-95">조회하기</Button>
            </form>
        </CardHeader>
                ...
                polls.map((poll, index) => (
                  ...
                  >
                    ...
                    <TableCell className="text-center py-6 font-mono text-sm text-slate-400">
                        {poll.frstRegistPnttm?.slice(0, 10)}
                    </TableCell>
                  </TableRow>
                ))
              )}

    ...
    <div className="p-8 space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
            ...
            <h1 className="text-4xl font-bold tracking-tight text-foreground ">?�문 ?�계 <span className="text-amber-500">분석</span></h1>
            <p className="text-muted-foreground font-bold text-sm max-w-lg">?�시�??�답 ?�황???�각?�로 분석?�여 ?�이???�사?�트�??�출?�니??</p>
        </div>
      </div>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50 text-left">
              <TableRow className="hover:bg-transparent border-none">
                ...
                <TableHead className="font-bold text-slate-900 text-xs py-6 px-4 text-left">?�문 주제 (Survey Subject)</TableHead>
                <TableHead className="w-[120px] font-bold text-slate-400 text-xs py-6 text-center">?�답 ??/TableHead>
                <TableHead className="w-[250px] font-bold text-slate-400 text-xs py-6 text-center">조사 기간</TableHead>
                <TableHead className="w-[120px] font-bold text-slate-400 text-xs py-6 text-center">진행 ?�태</TableHead>
                <TableHead className="w-[150px] font-bold text-slate-400 text-xs py-6 text-right px-8">?�계 리포??/TableHead>
              </TableRow>
            </TableHeader>
              ...
              {isLoading ? (
                ...
                polls.map((poll, index) => (
                  <TableRow key={poll.pollId} className="hover:bg-amber-50/30 transition-all border-b last:border-0 group border-slate-50">
                    ...
                    <TableCell className="text-center py-6">
                        <Badge variant="outline" className="rounded-md font-bold px-3 border-amber-200 text-amber-600 bg-amber-50 group-hover:bg-amber-100 transition-colors">집계�?/Badge>
                    </TableCell>
                    <TableCell className="text-right px-8 py-6">
                      <Button variant="ghost" size="sm" onClick={() => router.push(`/admin/survey/stats/${poll.pollId}`)} className="rounded-lg font-bold text-xs gap-2 group-hover:bg-amber-500 group-hover:text-white transition-all">
                        <BarChart3 className="h-4 w-4" /> 결과 리포??                      </Button>
                    </TableCell>
                  </TableRow>
                ))
              )}

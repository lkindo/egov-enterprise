...
export default function SurveyStatsPage() {
  ...
  const [params, setParams] = useState<PollSearchParams>({
    pageNo: 1, // 변수명 정규화 (page踰덊샇 -> pageNo)
    searchKeyword: '',
  });
  ...
  return (
    <div className="p-8 space-y-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="rounded-[2rem] border-none shadow-sm bg-slate-900 text-white p-8 space-y-4">
                <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center">
                    <FileBarChart size={24} className="text-amber-400" />
                </div>
                <div>
                    <p className="text-slate-400 text-xs font-black tracking-widest uppercase">Targeted Polls</p>
                    <h3 className="text-4xl font-black">{totalCount || 0}</h3>
                </div>
            </Card>
            <Card className="rounded-[2rem] border-none shadow-sm bg-white p-8 space-y-4 ring-1 ring-slate-100">
                <div className="w-12 h-12 rounded-2xl bg-amber-100 flex items-center justify-center">
                    <TrendingUp size={24} className="text-amber-600" />
                </div>
                <div>
                    <p className="text-slate-400 text-xs font-black tracking-widest uppercase">Response Rate</p>
                    <h3 className="text-4xl font-black">78.4<span className="text-lg text-slate-300 font-bold ml-1">%</span></h3>
                </div>
            </Card>
            <Card className="rounded-[2rem] border-none shadow-sm bg-white p-8 space-y-4 ring-1 ring-slate-100">
                ...
                <div>
                    <p className="text-slate-400 text-xs font-black tracking-widest uppercase">Active Analytics</p>
            ...
            <form onSubmit={handleSearch} className="flex flex-col md:flex-row items-center gap-4">
                <div className="relative flex-1 w-full group">
                    ...
                    <Input
                        ...
                        className="h-14 pl-14 rounded-2xl border-2 border-transparent bg-white shadow-sm focus:border-amber-500 focus:ring-0 transition-all font-bold"
                        value={params.searchKeyword || ''}
                        onChange={(e) => setParams(prev => ({ ...prev, searchKeyword: e.target.value }))}
                    />
                </div>
                <Button type="submit" className="h-14 px-10 rounded-2xl bg-slate-900 border-none text-white font-black text-sm shadow-sm transition-all active:scale-95">분석 조회</Button>
            </form>
        </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="font-black text-slate-900 text-xs py-6 px-4">설문 주제 (Survey Subject)</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                polls.map((poll, index) => (
                  <TableRow key={poll.pollId} className="hover:bg-amber-50/30 transition-all border-b last:border-0 group">
                    <TableCell className="text-center font-mono text-sm text-slate-400 py-6">
                        {index + 1 + ((params.pageNo || 1) - 1) * 10}
                    </TableCell>
                    <TableCell className="px-4 py-6">
                        <span className="text-[17px] font-black text-slate-800 group-hover:text-amber-600 transition-colors uppercase">{poll.pollNm}</span>
                    </TableCell>
                    <TableCell className="text-center py-6">
                        <span className="font-mono font-black text-slate-900">0</span>
                    </TableCell>
                    <TableCell className="text-center py-6">
                        <div className="inline-flex items-center gap-2 text-slate-500 font-bold text-xs uppercase ">

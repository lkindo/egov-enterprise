  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      ...
      <HubHeader 
        title="설문 인텔리전스" 
        highlight="매트릭스" 
        subtitle="전사 피드백 및 사용자 분석 데이터 통합 매니지먼트 허브" 
        icon={ClipboardCheck} 
        actions={
      ...
      <div className="grid grid-cols-12 gap-10 px-2 min-h-[850px]">
        ...
        <div className="col-span-12 lg:col-span-3 space-y-8">
          <div className="hub-table-container p-6 bg-slate-50 shadow-inner">
            <NavButton icon={<Layout size={20} />} label="진행중인 설문" active={activeTab === 'SURVEYS'} onClick={() => setActiveTab('SURVEYS')} />
            <div className="h-4" />
            <NavButton icon={<BookOpen size={20} />} label="설문 템플릿" active={activeTab === 'TEMPLATES'} onClick={() => setActiveTab('TEMPLATES')} />
            <div className="h-4" />
            <NavButton icon={<BarChart3 size={20} />} label="고급 통계 분석" active={activeTab === 'STATS'} onClick={() => setActiveTab('STATS')} />
          </div>
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-8">
          <HubSectionCard
            title="설문 인벤토리"
            description="현재 시스템에서 운용 중인 데이터 수집 유닛들입니다."
            icon={Database}
          >
          ...
          <AnimatePresence mode="wait">
            {selectedSurveyId ? (
              ...
              >
                <HubSectionCard
                  title="실시간 지표 분석"
                  description="선택된 설문 프로토콜의 응답 데이터 시각화 및 트렌드 분석입니다."
                  icon={BarChart3}
                  statusBadges={
                    <div className="flex gap-2">
                       <HubStatusBadge label="STREAMING" icon={Activity} variant="default" className="text-[8px] font-black tracking-widest" />
                       <HubStatusBadge label="VERIFIED" icon={CheckCircle} variant="success" className="text-[8px] font-black tracking-widest" />
                    </div>
                  }
                >

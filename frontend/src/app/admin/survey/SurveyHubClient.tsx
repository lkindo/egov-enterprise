...
export default function SurveyHubClient() {
  ...
  const surveys: SurveyItem[] = [
    { id: 1, title: '2024???ÅÎ∞òÍ∏?ÏßÅÏõê ÎßåÏ°±??Ï°∞ÏÇ¨', participants: 450, status: '?úÏÑ±', startDate: '2024-03-01', endDate: '2024-03-31' },
    { id: 2, title: '?†Í∑ú Í∏∞ÏóÖ Î≥¥Ïïà ?ïÏ±Ö ?ºÎìúÎ∞?, participants: 120, status: '?úÏÑ±', startDate: '2024-03-10', endDate: '2024-03-24' },
    { id: 3, title: 'Î≥µÏ? ?¨Ïù∏???¨Ïö©Ï≤??†Ìò∏??Ï°∞ÏÇ¨', participants: 856, status: 'CLOSED', startDate: '2024-02-15', endDate: '2024-03-01' },
    { id: 4, title: '?¨ÎÇ¥ ?¥Ìïô ÍµêÏú° ?òÏöî Ï°∞ÏÇ¨', participants: 32, status: 'PENDING', startDate: '2024-04-01', endDate: '2024-04-15' },
  ];
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="Ï°∞ÏÇ¨ ÎßàÏä§???àÎ∏å"
        breadcrumbs={[{ label: 'Ïª§Î??àÌã∞' }, { label: '?§Î¨∏ ?∏ÌÖîÎ¶¨Ï†Ñ?? }]}
      />

      <HubHeader
        title="?§Î¨∏ ?∏ÌÖîÎ¶¨Ï†Ñ??
        highlight="Îß§Ìä∏Î¶?ä§"
        subtitle="?ÑÏÇ¨ ?ºÎìúÎ∞?Î∞??±Îä• Î∂ÑÏÑù ?∞Ïù¥???µÌï© Îß§ÎãàÏßÄÎ®ºÌä∏ ?àÎ∏å"
        icon={ClipboardCheck}
        actions={
          <div className="flex gap-4 p-2">
            <Button variant="outline" size="lg" className="h-10 rounded-lg border-2 font-bold text-xs tracking-widest uppercase gap-2">
              <Database size={16} /> Î∂ÑÏÑù ?ÑÏπ¥?¥Î∏å
            </Button>
            <Button size="lg" className="h-10 px-8 rounded-lg font-bold text-xs tracking-widest uppercase shadow-lg shadow-primary/20 hover:-translate-y-1 transition-all gap-2">
              <Plus size={18} /> ?†Í∑ú ?§Î¨∏ ?ùÏÑ±
            </Button>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-12 gap-10 px-2 min-h-[850px]">
        ...
        <div className="col-span-12 lg:col-span-3 space-y-8">
          <div className="hub-table-container p-6 bg-slate-50 shadow-inner">
            <NavButton icon={<Layout size={20} />} label="?ÑÏ≤¥ " active={activeTab === 'SURVEYS'} onClick={() => setActiveTab('SURVEYS')} />
            <div className="h-4" />
            <NavButton icon={<BookOpen size={20} />} label="?úÌîåÎ¶? active={activeTab === 'TEMPLATES'} onClick={() => setActiveTab('TEMPLATES')} />
            <div className="h-4" />
            <NavButton icon={<BarChart3 size={20} />} label="Í≥†Í∏â ?µÍ≥Ñ" active={activeTab === 'STATS'} onClick={() => setActiveTab('STATS')} />
          </div>
          ...
          <div className="hub-table-container p-10 bg-slate-900 border-none text-white relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-8">
              <div className="flex items-center justify-between">
                <div className="w-12 h-10 bg-white/10 rounded-lg flex items-center justify-center text-primary shadow-2xl border border-white/5">
                  <TrendingUp size={24} />
                </div>
                <HubStatusBadge label="?ôÍ∏∞?îÎê®" variant="success" className="bg-emerald-500/20 border-emerald-500/20 text-emerald-400 text-xs font-bold tracking-widest uppercase" />
              </div>
              <div className="space-y-1">
                <p className="text-xs font-bold text-white/30 tracking-[0.4em] uppercase leading-none">Ï¥?Ï∞∏Ïó¨</p>
                <h4 className="text-4xl font-bold tracking-tight text-white leading-none tabular-nums">1,458<span className="text-sm opacity-20 ml-2">Î™?/span></h4>
                <div className="flex items-center gap-2 text-xs font-bold text-emerald-400 mt-4 bg-emerald-400/10 w-fit px-3 py-1 rounded-full border border-emerald-400/20">
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-8">
          <HubSectionCard
            title="?§Î¨∏ ?∏Î≤§?†Î¶¨"
            description="?ÑÏû¨ ?úÏä§?úÏóê???¥Ïö© Ï§ëÏù∏ ?∞Ïù¥???òÏßë Î™©Î°ù?ÖÎãà??"
            icon={Database}
          >
                ...
                {surveys.map((survey) => (
                  ...
                  >
                    <div className="space-y-2 max-w-[75%] relative z-10">
                      <div className="flex items-center gap-2 mb-2">
                        {survey.status === '?úÏÑ±' ? (
                          <div className="flex items-center gap-1.5 px-2 py-0.5 bg-emerald-500/20 rounded-full border border-emerald-500/20">
                            ...
                            <span className="text-xs font-bold text-emerald-500 tracking-widest uppercase">?úÏÑ±</span>
                          </div>
                        ) : (
                      ...
                      <h4 className={cn("text-lg font-bold tracking-tight leading-none truncate uppercase", selectedSurveyId === survey.id ? "text-white" : "text-foreground")}>
                        {survey.title}
                      </h4>
                    ...
                    <div className="flex flex-col items-end gap-1 relative z-10">
                      <span className={cn("text-xl font-bold tracking-tight tabular-nums leading-none", selectedSurveyId === survey.id ? "text-white" : "text-primary")}>
                        {survey.participants.toLocaleString()}
                      </span>
                      <span className="text-xs font-bold tracking-[0.2em] opacity-40 uppercase">Î™?/span>
                    </div>
        ...
        <div className="col-span-12 lg:col-span-5 flex flex-col gap-10">
          <AnimatePresence mode="wait">
            {selectedSurveyId ? (
              ...
              >
                <HubSectionCard
                  title="?§ÏãúÍ∞?ÏßÄ??Î∂ÑÏÑù"
                  description="?†ÌÉù???§Î¨∏ ?ÑÎ°ú?åÏùº Î∞??ëÎãµ ?∞Ïù¥???úÍ∞Å??Î∞??∏Î†å??Î∂ÑÏÑù?ÖÎãà??"
                  icon={BarChart3}
                  statusBadges={
                    <div className="flex gap-2">
                      <HubStatusBadge label="?§Ìä∏Î¶? icon={Activity} variant="default" className="text-xs font-bold tracking-widest" />
                      <HubStatusBadge label="?ïÏù∏?? icon={CheckCircle} variant="success" className="text-xs font-bold tracking-widest" />
                    </div>
                  }
                >
                  <div className="space-y-12">
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs font-bold text-primary tracking-widest uppercase">?ºÏù¥Î∏?Í≤åÏù¥ÏßÄ</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-6 pb-2">
                      <DetailStat icon={<Users size={18} />} label="Ï¥?Ï∞∏Ïó¨?? value="2,480" color="primary" />
                      <DetailStat icon={<Clock size={18} />} label="?âÍ∑† ?åÏöî ?úÍ∞Ñ" value="4Î∂?12Ï¥? color="amber" />
                      <DetailStat icon={<Target size={18} />} label="?ÑÎ£å?? value="78.2%" color="emerald" />
                      <DetailStat icon={<AlertCircle size={18} />} label="?¥ÌÉàÎ•? value="12.4%" color="rose" />
                    </div>

                    <div className="flex gap-4 p-2">
                      <Button variant="outline" className="h-12 flex-1 rounded-lg border-2 border-border font-bold tracking-[0.2em] shadow-sm uppercase gap-3">
                        <Share2 size={18} /> Protocol Link
                      </Button>
                      <Button className="h-12 flex-[2] bg-slate-900 border-none text-white rounded-lg font-bold tracking-[0.3em] shadow-[0_20px_40px_-12px_rgba(0,0,0,0.3)] hover:-translate-y-1 transition-all uppercase gap-3">
              ...
              <div className="h-full flex flex-col items-center justify-center p-20 text-center opacity-30 select-none grayscale bg-slate-50 dark:bg-muted/5 rounded-lg border-4 border-dashed border-border/20">
                ...
                <div className="space-y-4">
                  <h3 className="text-3xl font-bold text-foreground tracking-tight uppercase opacity-50">?úÏä§???ÄÍ∏?/h3>
...
function NavButton({ icon, label, active, onClick }: { icon: React.ReactNode, label: string, active: boolean, onClick: () => void }) {
  return (
    ...
    >
      <div className={cn(
        "w-12 h-10 rounded-lg flex items-center justify-center transition-all shadow-md",
...
function DetailStat({ icon, label, value, color }: { icon: React.ReactNode, label: string, value: string, color: string }) {
  ...
  return (
    <div className={cn("p-8 rounded-lg border-2 space-y-4 shadow-sm group hover:scale-105 transition-transform", colorMap[color])}>
      ...
      <p className="text-2xl font-bold tracking-tight text-foreground leading-none tabular-nums">{value}</p>
    </div>
  );
}

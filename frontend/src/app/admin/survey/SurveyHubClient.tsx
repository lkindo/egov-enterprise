  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      ...
      <HubHeader
        title="?§Î¨∏ ?∏ÌÖîÎ¶¨Ï†Ñ??
        highlight="Îß§Ìä∏Î¶?ä§"
        subtitle="?ÑÏÇ¨ ?ºÎìúÎ∞?Î∞??¨Ïö©??Î∂ÑÏÑù ?∞Ïù¥???µÌï© Îß§ÎãàÏßÄÎ®ºÌä∏ ?àÎ∏å"
        icon={ClipboardCheck}
        actions={
      ...
      <div className="grid grid-cols-12 gap-10 px-2 min-h-[850px]">
        ...
        <div className="col-span-12 lg:col-span-3 space-y-8">
          <div className="hub-table-container p-6 bg-slate-50 shadow-inner">
            <NavButton icon={<Layout size={20} />} label="ÏßÑÌñâÏ§ëÏù∏ ?§Î¨∏" active={activeTab === 'SURVEYS'} onClick={() => setActiveTab('SURVEYS')} />
            <div className="h-4" />
            <NavButton icon={<BookOpen size={20} />} label="?§Î¨∏ ?úÌîåÎ¶? active={activeTab === 'TEMPLATES'} onClick={() => setActiveTab('TEMPLATES')} />
            <div className="h-4" />
            <NavButton icon={<BarChart3 size={20} />} label="Í≥†Í∏â ?µÍ≥Ñ Î∂ÑÏÑù" active={activeTab === 'STATS'} onClick={() => setActiveTab('STATS')} />
          </div>
          ...
          <div className="hub-table-container p-10 bg-slate-900 border-none text-white relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-8">
              <div className="flex items-center justify-between">
                ...
                <HubStatusBadge label="?ôÍ∏∞?îÎê®" variant="success" className="bg-emerald-500/20 border-emerald-500/20 text-emerald-400 text-[8px] font-black tracking-widest uppercase" />
              </div>
              <div className="space-y-1">
                <p className="text-[10px] font-black text-white/30 tracking-[0.4em] uppercase leading-none">Ï¥?Ï∞∏Ïó¨</p>
                <h4 className="text-4xl font-black tracking-tighter text-white leading-none tabular-nums">1,458<span className="text-sm opacity-20 ml-2">?®ÏúÑ</span></h4>
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-8">
          <HubSectionCard
            title="?§Î¨∏ ?∏Î≤§?†Î¶¨"
            description="?ÑÏû¨ ?úÏä§?úÏóê???¥Ïö© Ï§ëÏù∏ ?∞Ïù¥???òÏßë ?†Îãõ?§ÏûÖ?àÎã§."
            icon={Database}
          >
                ...
                {surveys.map((survey) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        {survey.status === '?úÏÑ±' ? (
                          <div className="flex items-center gap-1.5 px-2 py-0.5 bg-emerald-500/20 rounded-full border border-emerald-500/20">
                            ...
                            <span className="text-[8px] font-black text-emerald-500 tracking-widest uppercase">?úÏÑ±</span>
                          </div>
                        ) : (
                    ...
                    <div className="flex flex-col items-end gap-1 relative z-10">
                      ...
                      <span className="text-[9px] font-black tracking-[0.2em] opacity-40 uppercase">?®ÏúÑ</span>
                    </div>
            ...
            {selectedSurveyId ? (
              ...
              >
                <HubSectionCard
                  ...
                  description="?†ÌÉù???§Î¨∏ ?ÑÎ°ú?†ÏΩú???ëÎãµ ?∞Ïù¥???úÍ∞Å??Î∞??∏Î†å??Î∂ÑÏÑù?ÖÎãà??"
                  icon={BarChart3}
                  statusBadges={
                    <div className="flex gap-2">
                      <HubStatusBadge label="?§Ìä∏Î¶¨Î∞ç" icon={Activity} variant="default" className="text-[8px] font-black tracking-widest" />
                      <HubStatusBadge label="?ïÏù∏?? icon={CheckCircle} variant="success" className="text-[8px] font-black tracking-widest" />
                    </div>
                  }
                >
                  <div className="space-y-12">
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[9px] font-black text-primary tracking-widest uppercase">?ºÏù¥Î∏?Í≤åÏù¥ÏßÄ</span>
                      </div>
              ...
              <div className="h-full flex flex-col items-center justify-center p-20 text-center opacity-30 select-none grayscale bg-slate-50 dark:bg-muted/5 rounded-[4rem] border-4 border-dashed border-border/20">
                ...
                <div className="space-y-4">
                  <h3 className="text-3xl font-black text-foreground tracking-tighter uppercase opacity-50">?úÏä§???ÄÍ∏?/h3>

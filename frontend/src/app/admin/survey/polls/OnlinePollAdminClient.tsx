  ...
  const columns = [
    {
      ...
      accessor: (item: OnlinePollDto) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="font-black italic uppercase tracking-tighter text-slate-900 block">{item.pollNm}</span>
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest italic">{item.pollId}</span>
          </div>
        </div>
      )
    },
    ...
    },
    {
      ...
      accessor: (item: OnlinePollDto) => (
        <div className={cn(
          ...
          <span className="text-[10px] font-black uppercase tracking-widest italic">{item.useAt === 'Y' ? 'Active' : 'Ended'}</span>
        </div>
      )
    }
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 px-4 md:px-0 pb-24 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <StatCard title="Active Inquiries" value={polls.filter((p: OnlinePollDto) => p.useAt === 'Y').length} icon={<Activity size={24} />} color="slate" />
        <StatCard title="Total Interactions" value={totalCount} icon={<Layers size={24} />} color="primary" />
        <StatCard title="Analytic Nodes" value={polls.length} icon={<BarChart size={24} />} color="indigo" />
      </div>
      ...
      <div className="responsive-card p-6 md:p-12 border-2 border-slate-100 bg-white/50 backdrop-blur-xl relative overflow-hidden group">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 relative z-10">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h3 className="text-xl md:text-2xl font-black text-slate-900 uppercase tracking-tighter italic">Opinion Matrix</h3>
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Registered polling protocols</p>
                </div>
            </div>
            <div className="flex items-center gap-4">
      ...
      <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto rounded-[3rem] p-10 border-none shadow-2xl bg-white">
          <DialogHeader className="space-y-4">
            ...
            <DialogTitle className="text-3xl font-black text-slate-900 uppercase tracking-tighter italic text-center">New Inquiry</DialogTitle>
          ...
          <div className="space-y-8 py-8">
            <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic ml-2">Inquiry Designation</label>
              <Input
            ...
            <div className="grid grid-cols-2 gap-6">
                <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic ml-2">Temporal Start</label>
                    <Input
                        type="date"
                        value={newPoll.pollBeginDe}
                        onChange={(e) => setNewPoll(prev => ({ ...prev, pollBeginDe: e.target.value }))}
                        className="h-14 px-6 rounded-2xl border-2 border-slate-100 bg-slate-50/50 font-black text-xs italic focus:bg-white transition-all"
                    />
                </div>
                <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic ml-2">Temporal End</label>
                    <Input
                        type="date"
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Choice Parameters</label>
                    <Button 
                        type="button" 
                        variant="ghost" 
                        size="sm" 
                        onClick={handleAddItem}
                        className="h-8 px-3 rounded-xl text-[9px] font-black uppercase tracking-widest text-primary hover:bg-primary/5 italic gap-2"
                    >
                ...
                <div className="space-y-3">
                    {newPoll.pollItems?.map((item, index) => (
                        <div key={index} className="flex items-center gap-3 animate-in fade-in slide-in-from-left-4 duration-300">
                            <div className="w-10 h-14 rounded-xl bg-slate-100 flex items-center justify-center font-black italic text-slate-400 text-xs shadow-inner">

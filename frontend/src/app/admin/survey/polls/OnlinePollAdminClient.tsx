...
<<<<<<< HEAD
      toast.error('ÏÑ§Î¨∏ Î™©Î°ùÏùÑ Î∂àÎü¨Ïò§ÏßÄ Î™ªÌñàÏäµÎãàÎã§.');
=======
      toast.error('?§Î¨∏ Î™©Î°ù??Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??');
>>>>>>> 99be2886750c05e99df098d47b5b4fd8f624093f
  ...
  const handleAdd = async () => {
    if (!newPoll.pollNm || !newPoll.pollItems?.every(item => item.pollIemNm)) {
      toast.error('?§Î¨∏ Î™ÖÍ≥º Î™®Îì† ??™© ?¥Ïö©???ÖÎ†•?¥Ï£º?∏Ïöî.');
      return;
    }
    ...
    try {
      ...
      toast.success('???§Î¨∏???±Î°ù?àÏäµ?àÎã§.');
      setIsAddOpen(false);
      handleRefresh();
    } catch {
<<<<<<< HEAD
      toast.error('ÏÑ§Î¨∏ Îì±Î°ùÏóê Ïã§Ìå®ÌñàÏäµÎãàÎã§.');
=======
      toast.error('?§Î¨∏ ?±Î°ù???§Ìå®?àÏäµ?àÎã§.');
>>>>>>> 99be2886750c05e99df098d47b5b4fd8f624093f
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      ...
      <HubHeader 
        title="?®Îùº???§Î¨∏" 
        highlight="Îß§Ìä∏Î¶?ä§" 
        subtitle="?ÑÏÇ¨ ?¨Ïö©???ºÎìúÎ∞??òÏßë Î∞??∞Ïù¥???úÍ∞Å??Î∂ÑÏÑù ?úÏä§?? 
        icon={Vote} 
        actions={
          <div className="flex gap-4 p-2">
            ...
            >
              <RefreshCcw size={16} className={cn(loading && "animate-spin")} /> ?îÏßÑ ?ôÍ∏∞??            </Button>
            <Button
              size="lg"
      ...
      <HubSectionCard
        title="?§Î¨∏ Î≥¥Îìú ?§Ìä∏Î¶?
        description="?±Î°ù??Î™®Îì† ?ºÎìúÎ∞??òÏßë ?ÑÎ°ú?†ÏΩú???¥Ïö© ?ÅÌÉú Î∞?Ï∞∏Ïó¨ ?∞Ïù¥???îÏïΩ?ÖÎãà??"
        icon={TrendingUp}
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 pb-10 border-b border-border/30">
          <div>
            <h3 className="text-2xl font-black tracking-tighter uppercase leading-none">?∏ÌÖîÎ¶¨Ï†Ñ??Í≤åÏãú??/h3>
            <p className="text-[9px] font-bold text-muted-foreground tracking-[0.3em] uppercase mt-2 opacity-50">Í∏ÄÎ°úÎ≤å ?ºÎìúÎ∞?Î™®Îãà?∞ÎßÅ</p>
          </div>
          <div className="flex items-center gap-4">
        ...
        <div className="overflow-x-auto">
          <StandardDataTable
            ...
            data={polls}
            loading={loading}
            emptyMessage="Í∏∞Î°ù???∞Ïù¥???òÏßë ?ÑÎ°ú?†ÏΩú???ÜÏäµ?àÎã§."
        ...
        <DialogContent className="sm:max-w-[650px] max-h-[90vh] overflow-y-auto rounded-[3.5rem] p-12 border-none shadow-[0_40px_100px_-20px_rgba(0,0,0,0.5)] bg-white/95 backdrop-blur-3xl relative overflow-x-hidden">
          ...
          <DialogHeader className="space-y-6 relative z-10 text-center">
            ...
            <div className="space-y-2">
              <DialogTitle className="text-4xl font-black text-slate-900 tracking-tighter leading-none uppercase">?ÑÎ°ú?†ÏΩú Íµ¨ÏÑ±</DialogTitle>
          ...
          <div className="space-y-10 py-10 relative z-10">
            ...
            <section className="grid grid-cols-2 gap-8">
              <div className="space-y-4">
                <label className="text-[11px] font-black text-slate-400 tracking-[0.2em] uppercase ml-2">?úÏûë??/label>
              ...
              <div className="space-y-4">
                <label className="text-[11px] font-black text-slate-400 tracking-[0.2em] uppercase ml-2">Ï¢ÖÎ£å??/label>
                <div className="relative group">
                  <Calendar className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-primary transition-colors" size={18} />
                  <Input
                    type="date"
                ...
                {newPoll.pollItems?.map((item, index) => (
                  ...
                  >
                    <div className="w-14 h-16 rounded-2xl bg-slate-900/5 dark:bg-muted/30 flex flex-col items-center justify-center font-black text-slate-300 dark:text-muted-foreground text-[10px] shadow-inner border border-border/10 shrink-0">
                      <span className="opacity-40 uppercase mb-0.5">?∏Îìú</span>

...
export default function RewardManageClient({ initialData }: { initialData: any[] }) {
    ...
    const columns = [
            ...
            accessor: (item: any) => (
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-amber-600 font-medium uppercase tracking-wider">{item.rwardCode}</span>
                    </div>
                </div>
            )
        },
        { 
            header: '수상자 ID', 
            accessor: 'rwardwnrId',
            className: 'w-32 font-mono text-xs text-slate-500'
        },
        { 
            header: '포상일자', 
            accessor: (item: any) => (
        ...
        { 
            header: '승인일시', 
            accessor: 'sanctnDt',
            className: 'w-40 text-slate-400 text-xs'
        }
    ];

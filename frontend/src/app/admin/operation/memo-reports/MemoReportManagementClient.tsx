           ...
           <AnimatePresence mode="wait">
              {selectedReport ? (
                    ...
                    <Card className="h-full rounded-[4rem] border-0 bg-slate-900 text-white shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col relative group">
                       ...
                       <CardHeader className="p-12 border-b border-white/5 flex flex-row items-center justify-between relative z-10 bg-white/5 backdrop-blur-3xl">
                          <div className="space-y-1">
                             <span className="text-[10px] font-black text-primary uppercase tracking-[0.4em] leading-none">리포트 분석기 (Analytical View)</span>
                       ...
                       <CardContent className="flex-1 p-12 space-y-10 overflow-y-auto relative z-10 scrollbar-elegant">
                          <div className="flex items-center justify-between pb-8 border-b border-white/5">
                             <div className="flex items-center gap-8">
                                <div className="space-y-1">
                                   <p className="text-[10px] font-black text-white/30 uppercase tracking-widest">작성자 노드</p>
                          ...
                          <div className="space-y-4">
                             <div className="flex items-center gap-3 text-[10px] font-black text-primary tracking-[0.3em] uppercase">
                                <Layers size={14} /> 보고 내용 데이터 유닛 (Report Core)
                             </div>
                       ...
                       <div className="p-12 border-t border-white/5 bg-white/5 backdrop-blur-3xl flex gap-6 relative z-10">
                          <Button className="h-16 flex-1 rounded-[2rem] bg-white text-slate-900 font-black tracking-[0.2em] text-[10px] hover:scale-105 transition-all shadow-2xl">상세 수정</Button>
                          <Button className="h-16 flex-[2] rounded-[2rem] bg-primary text-white font-black tracking-[0.4em] text-[10px] hover:scale-105 transition-all shadow-2xl shadow-primary/40 uppercase italic">지시사항 등록</Button>
                       </div>
                    </Card>
                 </motion.div>
              ) : (
                 <div className="h-full flex flex-col items-center justify-center p-20 text-center opacity-30 select-none bg-white rounded-[4rem] border-2 border-dashed border-slate-200">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 tracking-[0.5em] mt-2 leading-relaxed">데이터 흐름을 확인하려면<br />좌측 스트림에서 리포트를 선택하세요</p>
                 </div>
              )}
           </AnimatePresence>
        </div>

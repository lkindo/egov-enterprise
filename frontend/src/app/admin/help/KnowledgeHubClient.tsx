  ...
  return (
    ...
    >
      ...
      <motion.div variants={hubItemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8 px-2">
        <div className="space-y-1.5 md:space-y-2">
          <div className="flex items-center gap-2 md:gap-3">
             ...
             <span className="text-[8px] md:text-[10px] font-black tracking-[0.3em] md:tracking-[0.5em] text-primary uppercase leading-none">?명뀛由ъ쟾???덈툕 肄섏넄</span>
          </div>
          <h2 className="text-2xl md:text-4xl font-black text-foreground tracking-tighter uppercase italic leading-none">?뷀꽣?꾨씪?댁쫰 吏??留ㅽ듃由?뒪</h2>
        </div>
        <div className="flex items-center gap-3 md:gap-4 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
           {isAdmin && (
             <Button 
               ...
               variant="outline"
               className="h-12 md:h-16 px-4 md:px-8 rounded-[0.1rem] md:rounded-[0.1rem] border-2 border-slate-200 bg-white text-slate-900 font-black tracking-widest text-[9px] md:text-[11px] uppercase hover:bg-slate-50 hover:scale-105 active:scale-95 transition shadow-xl gap-2 md:gap-3 group whitespace-nowrap"
             >
           ...
           <Button 
             ...
             className="h-12 md:h-16 px-4 md:px-8 rounded-[0.1rem] md:rounded-[0.1rem] bg-slate-900 text-white font-black tracking-widest text-[9px] md:text-[11px] uppercase hover:scale-105 active:scale-95 transition shadow-xl gap-2 md:gap-3 group whitespace-nowrap"
           >
             <Plus className="w-[14px] md:w-[18px] h-[14px] md:h-[18px] group-hover:rotate-90 transition-transform" /> ?좉퇋 ?깅줉
           </Button>
           <div className="h-10 md:h-16 w-[1px] md:w-[2px] bg-border/40 mx-1 md:mx-2 hidden sm:block" />
           <div className="hidden sm:flex flex-col items-end">
              <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none">?몃? ?묎렐 ?꾩슜</span>
      ...
      <motion.div variants={hubItemVariants} className="relative h-[320px] md:h-[420px] rounded-[0.1rem] md:rounded-[0.1rem] bg-slate-900 overflow-hidden flex flex-col items-center justify-center p-6 md:p-12 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] border-none mx-2">
        ...
        <div className="relative z-20 text-center w-full max-w-4xl space-y-8 md:space-y-12 px-2 font-sans">
          <div className="space-y-3 md:space-y-4">
            <h1 className="text-4xl md:text-6xl font-black text-white tracking-tighter leading-none uppercase italic">吏??踰좎씠??/h1>
          ...
          <div className="space-y-6 md:space-y-8">
            <div className="relative group max-w-3xl mx-auto w-full">
              <Search className={cn(
                "absolute left-6 md:left-8 top-1/2 -translate-y-1/2 transition scale-110 md:scale-150 duration-500",
      ...
      <motion.div variants={hubItemVariants} className="px-2 overflow-hidden">
        ...
        >
          <CategoryCard title="Global Wiki" desc="湲곗닠 ?ъ뼇" icon={<Library size={28} />} count={142} color="primary" active={activeCategory === 'WIKI'} onClick={() => setActiveCategory('WIKI')} />
          <CategoryCard title="怨좉컼吏?? desc="鍮좊Ⅸ ?듬?" icon={<BookOpen size={28} />} count={28} color="amber" active={activeCategory === 'FAQ'} onClick={() => setActiveCategory('FAQ')} />
          <CategoryCard title="湲곗닠 Q&A" desc="?щ읆 ?닿껐" icon={<MessageCircleQuestion size={28} />} count={567} color="rose" active={activeCategory === 'QNA'} onClick={() => setActiveCategory('QNA')} />
          <CategoryCard title="而ㅻ??덊떚" desc="?쒖꽦 寃뚯떆?? icon={<Users size={28} />} count={12} color="emerald" active={activeCategory === 'COMMUNITY'} onClick={() => setActiveCategory('COMMUNITY')} />
        </motion.div>
      </motion.div>
      ...
      <motion.div variants={hubItemVariants} className="grid grid-cols-12 gap-10 px-2 mt-4 relative z-0">
        <div className="col-span-12 lg:col-span-8 space-y-10">
          <HubSectionCard title="Knowledge Stream" description="?ㅼ떆媛꾩쑝濡??좎엯?섎뒗 吏???곗씠???ㅽ듃由쇱엯?덈떎." icon={Layers}>
                ...
                {isAccessRestricted ? (
                  ...
                  >
                    ...
                    <div className="text-center space-y-4 max-w-sm">
                      <h3 className="text-2xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">?≪꽭??留ㅽ듃由?뒪 嫄곕?</h3>
                      <p className="text-sm font-bold text-muted-foreground/60 leading-relaxed uppercase tracking-tighter">洹?섏쓽 ?꾩옱 ?몃뱶 沅뚰븳?쇰줈??Wiki 諛?FAQ ?곗씠?곗뀑???묎렐?????놁뒿?덈떎. ?쒖뒪??愿由ъ옄?먭쾶 ?곸쐞 蹂댁븞 ?곗뼱 ?뱀씤???붿껌?섏떗?쒖삤.</p>
                    </div>
                    <Button 
                       onClick={() => setActiveCategory('COMMUNITY')}
                       variant="outline"
                       className="h-14 px-8 rounded-[0.1rem] border-2 font-black tracking-widest text-[10px] uppercase gap-3 shadow-xl hover:bg-slate-900 hover:text-white transition"
                    >
                  ...
                  <>
                    {isLoading ? (
                      <div className="p-12 text-center text-muted-foreground animate-pulse">吏???ㅽ듃由쇱쓣 ?숆린??以묒엯?덈떎...</div>
                      ...
                      <div className="flex flex-col items-center justify-center p-20 space-y-4 border-2 border-dashed rounded-[0.1rem] border-border/50">
                        ...
                        <p className="text-muted-foreground font-black text-sm uppercase tracking-widest text-center">吏??湲곕줉??李얠쓣 ???놁쓬</p>
                      </div>
                      ...
                      <motion.div 
                        ...
                        role="button"
                        tabIndex={0}
                        onKeyDown={(e) => { if(e.key === 'Enter' || e.key === ' ') router.push(`/admin/community/boards/detail?bbsId=${item.bbsId || currentBbsId}&nttId=${item.id}`); }}
                        onClick={() => router.push(`/admin/community/boards/detail?bbsId=${item.bbsId || currentBbsId}&nttId=${item.id}`)}
                        className="flex flex-col sm:flex-row sm:items-center justify-between p-5 md:p-8 bg-white border border-border/40 rounded-[0.1rem] md:rounded-[0.1rem] hover:ring-[15px] md:hover:ring-[20px] hover:ring-primary/5 hover:border-primary/20 transition cursor-pointer group shadow-sm hover:shadow-2xl text-left w-full"
                      >
                        <div className="flex gap-4 md:gap-6 items-start">
                           <div className="w-12 h-12 md:w-16 md:h-16 rounded-[0.1rem] md:rounded-[0.1rem] bg-slate-50 flex flex-col items-center justify-center border border-border/50 group-hover:bg-primary/5 transition-colors shrink-0">
                              <span className="text-[8px] md:text-[10px] font-black text-muted-foreground/40 leading-none">?곹뼢??/span>
                        ...
                        <div className="mt-4 sm:mt-0 flex items-center justify-between sm:justify-end gap-4">
                           <div className="hidden sm:flex flex-col items-end">
                              <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none">?곹깭</span>
                              <StatusBadge status={item.statusCd} type={activeCategory as KnowledgeCategory} />
                           </div>
                           <ArrowRight className="text-muted-foreground/20 group-hover:text-primary group-hover:translate-x-2 transition w-5 h-5 md:w-6 md:h-6" />
                        </div>
                      </motion.div>
                    ))}
                  </>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-10">
          <HubSectionCard title="Trending Radar" description="?멸쾶?댁?癒쇳듃媛 ?믪? ?곗씠??吏?? icon={TrendingUp}>
            <div className="space-y-4">
              {hotItems.map((item: any, idx: number) => (
                <button type="button" key={item.id} onClick={() => router.push(`/admin/community/boards/detail?bbsId=${item.bbsId || currentBbsId}&nttId=${item.id}`)} className="w-full flex items-center gap-5 p-4 rounded-[0.1rem] hover:bg-slate-50 transition cursor-pointer group text-left">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                       ...
                       <span className="text-[9px] font-black text-rose-500 uppercase">?곹뼢 ?섏튂 ?믪쓬</span>
                    </div>
                  </div>
          ...
          <HubSectionCard title="?쒕룞 吏???붿쭊" description="?ㅼ떆媛?吏???ㅽ듃?뚰겕 ?쒕룞" icon={History} className="bg-slate-900 border-none text-white shadow-2xl overflow-hidden relative">
            ...
            <div className="space-y-6 relative z-10 font-sans">
              {(activityData || []).slice(0, 5).map((activity: any) => (
                <div key={activity.id} className="flex items-center gap-5 p-5 bg-white/5 border border-white/5 rounded-[0.1rem] hover:bg-white/10 transition group/activity shadow-lg backdrop-blur-3xl">
                  <div className="w-10 h-10 rounded-[0.1rem] bg-primary/10 flex items-center justify-center text-primary group-hover/activity:rotate-12 transition">
                    <Zap size={18} />
                  </div>
                  <div className="flex-1 min-w-0">
          ...
          <div className="hub-card-premium p-10 bg-gradient-to-br from-primary/10 to-transparent border-primary/20 flex flex-col items-center justify-center text-center space-y-6">
             ...
             <div>
                <h4 className="text-xl font-black text-slate-900 tracking-tighter italic uppercase leading-none">?명뀛由ъ쟾???붿쭊</h4>
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-2">?쒖꽦 ?곗씠??留ㅽ듃由?뒪 ?ㅼ틪 以?/p>
             </div>
...
function FilterButton({ active, onClick, label }: any) {
  return (
    <button
      onClick={onClick}
      className={cn(
        "px-6 py-2 rounded-[0.1rem] text-[10px] font-black tracking-widest uppercase transition",
        active 
...
function StatsCard({ label, value, desc, trend }: { label: string, value: string, desc: string, trend: string }) {
  return (
    <div className="hub-card-premium p-8 flex flex-col gap-4 group hover:ring-[20px] hover:ring-primary/5 transition">
...
function CategoryCard({ title, desc, icon, count, color, active, onClick }: any) {
  ...
  return (
    <button 
      ...
      onClick={onClick}
      className={cn(
        "text-left relative p-8 rounded-[0.1rem] border-2 transition duration-500 cursor-pointer group flex flex-col gap-6",
        active 
      ...
      <div className="flex items-center justify-between border-b border-border/40 pb-6 uppercase">
        ...
        <div className="flex items-center gap-4">
           <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black opacity-20 tracking-widest">?쇱씠釉??곗씠???쇰뱶</span>
           </div>
        </div>
      </div>
      {children}

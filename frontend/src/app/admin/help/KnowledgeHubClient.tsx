...
export default function KnowledgeHubClient({ defaultTab }: { defaultTab?: KnowledgeCategory }) {
  ...
  const { user } = useAuth(); // Auth context ?∞Îèô
  
  const isAdmin = user?.role === 'ADMIN'; // Í¥ÄÎ¶¨Ïûê Í∂åÌïú ?¨Î?
  const [searchQuery, setSearchQuery] = useState('');
  const [sortBy, setSortBy] = useState('latest'); // 'latest' | 'views'
  const [isScanning, setIsScanning] = useState(false);
  
  // URL??bbsId ?åÎùºÎØ∏ÌÑ∞???∞Îùº Ï¥àÍ∏∞ Ïπ¥ÌÖåÍ≥†Î¶¨ ?§Ï†ï
  ...
  // Í∂åÌïú ?ÑÎ∞ò ?¨Î? Ï≤¥ÌÅ¨ (WIKI??FAQ??Í¥ÄÎ¶¨ÏûêÎß?
  ...
  return (
    ...
    >
      ...
      <motion.div variants={hubItemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8 px-2">
        <div className="space-y-1.5 md:space-y-2">
          <div className="flex items-center gap-2 md:gap-3">
             ...
             <span className="text-[8px] md:text-[10px] font-black tracking-[0.3em] md:tracking-[0.5em] text-primary uppercase leading-none">?∏ÌÖîÎ¶¨Ï†Ñ???àÎ∏å ÏΩòÏÜî</span>
          </div>
          <h2 className="text-2xl md:text-4xl font-black text-foreground tracking-tighter uppercase italic leading-none">?îÌÑ∞?ÑÎùº?¥Ï¶à ÏßÄ??/h2>
        </div>
        <div className="flex items-center gap-3 md:gap-4 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
           ...
           >
             <Plus className="w-[14px] md:w-[18px] h-[14px] md:h-[18px] group-hover:rotate-90 transition-transform" /> ?†Í∑ú ?±Î°ù
           </Button>
           <div className="h-10 md:h-16 w-[1px] md:w-[2px] bg-border/40 mx-1 md:mx-2 hidden sm:block" />
           <div className="hidden sm:flex flex-col items-end">
              <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none">?¥Î? ?ëÍ∑º ?ÑÏö©</span>
              <span className="text-xs font-bold text-slate-800 tracking-tight mt-1 underline decoration-primary/30 decoration-2">Í¥ÄÎ¶¨Ïûê Î£®Ìä∏</span>
           </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={hubItemVariants} className="relative h-[320px] md:h-[420px] rounded-[2.5rem] md:rounded-[4rem] bg-slate-900 overflow-hidden flex flex-col items-center justify-center p-6 md:p-12 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] border-none mx-2">
        ...
        <div className="relative z-20 text-center w-full max-w-4xl space-y-8 md:space-y-12 px-2 font-sans">
          <div className="space-y-3 md:space-y-4">
            <h1 className="text-4xl md:text-6xl font-black text-white tracking-tighter leading-none uppercase italic">ÏßÄ???¨ÌÑ∏</h1>
      ...
      <motion.div variants={hubItemVariants} className="px-2 overflow-hidden">
        ...
        >
          <CategoryCard title="Global Wiki" desc="Í∏∞Ïà† ?¨Ïñë?? icon={<Library size={28} />} count={142} color="primary" active={activeCategory === 'WIKI'} onClick={() => setActiveCategory('WIKI')} />
          <CategoryCard title="Customer Help" desc="FAQ Í∞Ä?¥Îìú" icon={<BookOpen size={28} />} count={28} color="amber" active={activeCategory === 'FAQ'} onClick={() => setActiveCategory('FAQ')} />
          <CategoryCard title="Tech Forum" desc="Q&A Î∂ÑÏÑù" icon={<MessageCircleQuestion size={28} />} count={567} color="rose" active={activeCategory === 'QNA'} onClick={() => setActiveCategory('QNA')} />
          <CategoryCard title="Community" desc="?ÅÏã† ?¥Îü¨?§ÌÑ∞" icon={<Users size={28} />} count={12} color="emerald" active={activeCategory === 'COMMUNITY'} onClick={() => setActiveCategory('COMMUNITY')} />
        </motion.div>
      ...
      <motion.div variants={hubItemVariants} className="grid grid-cols-12 gap-10 px-2 mt-4 relative z-0">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-10">
          <HubSectionCard title="Knowledge Stream" description="?§ÏãúÍ∞ÑÏúºÎ°??†ÏûÖ?òÎäî ÏßÄ???∞Ïù¥???†Îãõ?ÖÎãà??" icon={Layers}>
                ...
                {isAccessRestricted ? (
                  ...
                  >
                    ...
                    <div className="text-center space-y-4 max-w-sm">
                      <h3 className="text-2xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">?°ÏÑ∏??Îß§Ìä∏Î¶?ä§ Í±∞Î???/h3>
                      <p className="text-sm font-bold text-muted-foreground/60 leading-relaxed uppercase tracking-tighter">Í∑Ä?òÏùò ?ÑÏû¨ ?∏Îìú Í∂åÌïú?ºÎ°ú??Wiki Î∞?FAQ ?∞Ïù¥?∞ÏÖã???ëÍ∑º?????ÜÏäµ?àÎã§. ?úÏä§??Í¥ÄÎ¶¨Ïûê?êÍ≤å ?ÅÏúÑ Î≥¥Ïïà ?¥Î¶¨?¥Îü∞?§Î? ?îÏ≤≠?òÏã≠?úÏò§.</p>
                    </div>
                    <Button 
                  ...
                  <>
                    {isLoading ? (
                      <div className="p-12 text-center text-muted-foreground animate-pulse">ÏßÄ???§Ìä∏Î¶ºÏùÑ ?ôÍ∏∞??Ï§ëÏûÖ?àÎã§...</div>
                      ...
                      <div className="flex flex-col items-center justify-center p-20 space-y-4 border-2 border-dashed rounded-[3rem] border-border/50">
                        ...
                        <p className="text-muted-foreground font-black text-sm uppercase tracking-widest text-center">ÏßÄ??Í∏∞Î°ù??Ï∞æÏùÑ ???ÜÏùå</p>
                      </div>
                      ...
                      >
                        <div className="flex gap-4 md:gap-6 items-start">
                           <div className="w-12 h-12 md:w-16 md:h-16 rounded-[1.2rem] md:rounded-[1.5rem] bg-slate-50 flex flex-col items-center justify-center border border-border/50 group-hover:bg-primary/5 transition-colors shrink-0">
                              <span className="text-[8px] md:text-[10px] font-black text-muted-foreground/40 leading-none">?êÏàò</span>
                              <span className="text-sm md:text-xl font-black text-slate-800 leading-none mt-1">{Math.floor(Math.random() * 20) + 80}</span>
                           </div>
                           <div className="space-y-1 md:space-y-2 min-w-0">
                              <div className="flex items-center gap-2 md:gap-3">
                                 <span className="text-[8px] md:text-[9px] font-black text-primary uppercase tracking-widest bg-primary/5 px-2 py-0.5 rounded leading-none whitespace-nowrap text-ellipsis overflow-hidden">?µÏã¨ ?®ÏúÑ</span>
                        ...
                        <div className="mt-4 sm:mt-0 flex items-center justify-between sm:justify-end gap-4">
                           <div className="sm:hidden flex items-center gap-2">
                              ...
                              <span className="text-[9px] font-black text-emerald-500 uppercase">?ôÍ∏∞?îÎê®</span>
                           </div>
                           <div className="hidden sm:flex flex-col items-end">
                              <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none">?àÏ†ï???ÅÌÉú</span>
                              <span className="text-xs font-black text-emerald-500 mt-1 uppercase">?ôÍ∏∞???ÑÎ£å</span>
                           </div>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-10">
          <HubSectionCard title="Trending Radar" description="?∏Í≤å?¥Ï?Î®ºÌä∏Í∞Ä ?íÏ? ?∞Ïù¥??ÏßÄ?? icon={TrendingUp}>
                ...
                <div key={item.id} onClick={() => router.push(`/admin/community/boards/detail?bbsId=${item.bbsId || currentBbsId}&nttId=${item.id}`)} className="flex items-center gap-5 p-4 rounded-2xl hover:bg-slate-50 transition-all cursor-pointer group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                       ...
                       <span className="text-[9px] font-black text-rose-500 uppercase">?ÅÌñ•???êÏàò ?íÏùå</span>
                    </div>
                  </div>
          ...
          <HubSectionCard title="?úÏÑ± " description="?§ÏãúÍ∞?ÏßÄ???§Ìä∏?åÌÅ¨ ?úÎèô" icon={History} className="bg-slate-900 border-none text-white shadow-2xl overflow-hidden relative">
          ...
          <div className="hub-card-premium p-10 bg-gradient-to-br from-primary/10 to-transparent border-primary/20 flex flex-col items-center justify-center text-center space-y-6">
             ...
             <div>
                <h4 className="text-xl font-black text-slate-900 tracking-tighter italic uppercase leading-none">?∏ÌÖîÎ¶¨Ï†Ñ???îÏßÑ</h4>
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-2">?úÏÑ± ?∞Ïù¥??Îß§Ìä∏Î¶?ä§ ?§Ï∫î Ï§?/p>
             </div>
      ...
      <div className="flex items-center justify-between border-b border-border/40 pb-6 uppercase">
        ...
        <div className="flex items-center gap-4">
           <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black opacity-20 tracking-widest">?ºÏù¥Î∏??∞Ïù¥???ºÎìú</span>
           </div>
        </div>
      </div>
      {children}

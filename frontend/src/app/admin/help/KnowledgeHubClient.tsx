...
export default function KnowledgeHubClient({ defaultTab }: { defaultTab?: KnowledgeCategory }) {
 ...
 <h2 className="text-2xl md:text-4xl font-bold text-foreground tracking-tight uppercase leading-none">엔터프라이즈 지식 매트릭스</h2>
 </div>
 ...
 variant="outline"
 className="h-10 md:h-10 px-4 md:px-8 rounded-lg md:rounded-lg border-2 border-slate-200 bg-white text-slate-900 font-bold tracking-widest text-xs md:text-xs uppercase hover:bg-slate-50 hover:scale-105 active:scale-95 transition-all shadow-xl gap-2 md:gap-3 group whitespace-nowrap"
 >
 ...
 className="h-10 md:h-10 px-4 md:px-8 rounded-lg md:rounded-lg bg-slate-900 text-white font-bold tracking-widest text-xs md:text-xs uppercase hover:scale-105 active:scale-95 transition-all shadow-xl gap-2 md:gap-3 group whitespace-nowrap"
 >
 <Plus className="w-[14px] md:w-[18px] h-[14px] md:h-[18px] group-hover:rotate-90 transition-transform" /> 신규 등록
 </Button>
 <div className="h-10 md:h-10 w-[1px] md:w-[2px] bg-border/40 mx-1 md:mx-2 hidden sm:block" />
 ...
 <h1 className="text-4xl md:text-6xl font-bold text-white tracking-tight leading-none uppercase ">지식 베이스</h1>
 ...
 className="h-10 md:h-20 bg-white/5 border-2 border-white/5 rounded-lg md:rounded-lg px-16 md:px-24 text-white text-lg md:text-3xl font-bold placeholder:text-white/10 focus:bg-white focus:text-slate-900 transition-all shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] focus:ring-[16px] md:focus:ring-[24px] focus:ring-primary/10 tracking-tight"
 ...
 >
 <div className="w-24 h-20 rounded-lg bg-white shadow-2xl flex items-center justify-center text-primary border-2 border-primary/10 animate-bounce">
 <ShieldAlert size={48} />
 </div>
 <div className="text-center space-y-4 max-w-sm">
 <h3 className="text-2xl font-bold text-slate-900 tracking-tight uppercase leading-none">액세스 매트릭스 거부</h3>
 <p className="text-sm font-bold text-muted-foreground/60 leading-relaxed uppercase tracking-tight">귀하의 현재 노드 권한으로는 Wiki 및 FAQ 데이터셋에 접근할 수 없습니다. 시스템 관리자에게 상위 보안 티어 승인을 요청하십시오.</p>
 </div>
 <Button 
 ...
 <div className="w-12 h-10 md:w-16 md:h-10 rounded-lg md:rounded-lg bg-slate-50 flex flex-col items-center justify-center border border-border/50 group-hover:bg-primary/5 transition-colors shrink-0">
 ...
 <h4 className="text-lg md:text-2xl font-bold text-slate-900 tracking-tight leading-tight group-hover:text-primary transition-colors line-clamp-1">{item.nttSj}</h4>
 ...
 <div className="w-20 h-20 rounded-lg bg-white flex items-center justify-center text-primary shadow-2xl border-2 border-primary/10 animate-spin-slow">
 <Sparkles size={32} />
 </div>
 <div>
 <h4 className="text-xl font-bold text-slate-900 tracking-tight uppercase leading-none">인텔리전스 엔진</h4>
...
function StatsCard({ label, value, desc, trend }: { label: string, value: string, desc: string, trend: string }) {
 ...
 <h4 className="text-4xl font-bold tracking-tight text-slate-900 tabular-nums group-hover:text-primary transition-colors">{value}</h4>
...
function CategoryCard({ title, desc, icon, count, color, active, onClick }: any) {
 ...
 >
 <div className={cn("w-16 h-10 rounded-lg flex items-center justify-center shadow-inner", colorMap[color])}>
 {icon}
 </div>
 <div className="space-y-1">
 <div className="flex items-center justify-between">
 <h3 className="font-bold text-2xl tracking-tight text-slate-900 uppercase leading-none">{title}</h3>

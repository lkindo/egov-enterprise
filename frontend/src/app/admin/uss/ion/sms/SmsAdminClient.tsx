...
export default function SmsAdminClient({ 
      ...
      accessor: (item: SmsDto) => (
        <div className="flex items-center gap-4 py-2">
          ...
          <div className="flex flex-col text-left">
            <span className="font-mono font-bold text-foreground tracking-tight leading-none">
    ...
    },
    {
      ...
      accessor: (item: SmsDto) => (
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-foreground tracking-tight">{item.trnsmitTelno}</span>
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      accessor: () => (
        <div className="flex items-center gap-2 px-4 py-1.5 bg-emerald-500/10 text-emerald-500 rounded-full border border-emerald-500/20 w-fit shadow-sm">
          <ShieldCheck size={14} />
          <span className="text-xs font-bold tracking-widest uppercase">?�송?�료</span>
        </div>
      )
    }
  ];
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
        ...
        actions={
          <div className="flex gap-4 p-2 items-center">
            <Button
              ...
              className="h-10 rounded-lg border-2 font-bold text-xs tracking-widest uppercase gap-2"
            >
              <RefreshCcw size={16} className={cn(loading && "animate-spin")} /> 로그 ?�기??            </Button>
            <Button
              size="lg"
              onClick={() => setIsSendOpen(true)}
              className="h-10 px-8 rounded-lg font-bold text-xs tracking-widest uppercase shadow-lg shadow-primary/20 hover:-translate-y-1 transition-all gap-2"
            >
              <Plus size={18} /> ??메시지 구성
            </Button>
          </div>
      ...
      <HubSectionCard
        title="메시지 ?�송 ?�트�?
        description="?�스?�에??처리??모든 ?�웃바운??메시지 ?�래?�의 ?�시�?기록?�니??"
        icon={MessageSquare}
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 pb-10 border-b border-border/30">
          <div className="text-left">
            <h3 className="text-2xl font-bold tracking-tight uppercase leading-none text-left">?�송 로그</h3>
            <p className="text-xs font-bold text-slate-600 tracking-[0.3em] uppercase mt-2 opacity-100 text-left">메시지 출력 모니?�링</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <StandardDataTable
            ...
            data={smsList}
            loading={loading}
            emptyMessage="?�스?�에 ?�록??메시지 ?�역??존재?��? ?�습?�다."
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSend)}>
              ...
              <DialogHeader className="p-12 pb-0 space-y-6 relative z-10">
                <div className="w-20 h-20 bg-slate-900 text-white rounded-lg flex items-center justify-center shadow-2xl shadow-primary/30 mx-auto transition-transform hover:rotate-12 duration-500 border-4 border-white/20">
                  <Send size={32} />
                </div>
                <div className="text-center space-y-2">
                  <DialogTitle className="text-4xl font-bold text-slate-900 tracking-tight leading-none uppercase">메시지 ?�성</DialogTitle>
                      ...
                      <FormControl>
                        <div className="relative group">
                          ...
                          <Input
                            ...
                            className="h-10 pl-16 pr-8 rounded-lg border-none bg-slate-50 text-xl font-bold tabular-nums focus:bg-white focus:ring-8 focus:ring-primary/5 transition-all shadow-inner uppercase tracking-wider"
                          />
                        </div>
                      </FormControl>
              ...
              <DialogFooter className="p-12 pt-0 relative z-10 gap-4 flex !justify-center">
                <Button
                  ...
                  className="h-10 px-10 rounded-lg border-2 border-slate-100 font-bold text-xs tracking-widest uppercase hover:bg-slate-50 transition-all hover:border-slate-200"
                >
                  Terminate
                </Button>
                <Button
                  type="submit"
                  disabled={loading}
                  className="h-10 px-16 bg-slate-900 border-none text-white rounded-lg font-bold text-xs tracking-[0.3em] uppercase shadow-2xl hover:bg-primary transition-all hover:-translate-y-1 active:scale-95 flex items-center gap-3 flex-1"
                >
...
function SummaryBlock({ title, value, icon, status, color, bg }: any) {
  return (
    <div className={cn("hub-table-container p-12 group hover:scale-[1.02] transition-all relative overflow-hidden bg-white text-left", bg)}>
      ...
      <div>
        <h3 className="text-4xl font-bold tracking-tight text-foreground leading-none tabular-nums">{value}</h3>

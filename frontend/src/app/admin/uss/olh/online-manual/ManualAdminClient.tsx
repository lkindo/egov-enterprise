...
export default function ManualAdminClient({ 
  ...
  return (
    ...
    >
      <PageHeader
        title="?�라??가?�드 ?�키?�처"
        breadcrumbs={[{ label: '부가?�비?? }, { label: '?�라??매뉴?? }]}
        actions={
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 relative z-10">
          <div className="flex items-center gap-4 text-left">
            ...
            <div className="text-left">
              <h3 className="text-xl md:text-2xl font-bold text-slate-900 tracking-tight text-left">지???�산</h3>
              <p className="text-xs font-bold text-slate-400 tracking-[0.3em] uppercase text-left">?�라??매뉴??관�?/p>
            </div>
          </div>
          <div className="flex items-center gap-4">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onFormSubmit)}>
              <DialogHeader className="p-8 pb-0 space-y-4">
                <div className="w-16 h-12 bg-primary text-white rounded-lg flex items-center justify-center shadow-2xl shadow-primary/30 mx-auto">
                  {mode === 'edit' ? <Edit2 size={28} /> : <Plus size={28} />}
                </div>
                <div className="text-center space-y-2">
                  <DialogTitle className="text-3xl font-bold text-slate-900 tracking-tight text-center uppercase">
                    {mode === 'edit' ? '가?�드 ?�정' : '가?�드 ?�록'}
                  </DialogTitle>
                  <DialogDescription className="text-center font-bold text-slate-400 text-xs tracking-widest uppercase">
                    ?�용??교육???�한 지???�산??{mode === 'edit' ? '?�정' : '?�의'}?�니??                  </DialogDescription>
                </div>
              </DialogHeader>
              ...
              <div className="p-8 space-y-8 text-left">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-3">
                      <FormLabel className="text-xs font-bold text-slate-400 tracking-[0.2em] uppercase ml-2">매뉴??명칭</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          placeholder="매뉴??명을 ?�력?�세??.."
                          className="h-12 px-8 rounded-lg border-2 border-slate-100 bg-slate-50/50 text-lg font-bold focus:bg-white focus:ring-4 focus:ring-primary/10 transition-all shadow-inner"
                        />
                      </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-3">
                      <FormLabel className="text-xs font-bold text-slate-400 tracking-[0.2em] uppercase ml-2">리소??경로</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          placeholder="/src/docs/manuals/..."
                          className="h-12 px-8 rounded-lg border-2 border-slate-100 bg-slate-50/50 font-mono text-sm font-bold focus:bg-white focus:ring-4 focus:ring-primary/10 transition-all shadow-inner"
                        />
                      </FormControl>
                      <FormMessage className="text-xs font-bold" />
                    </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-3">
                      <FormLabel className="text-xs font-bold text-slate-400 tracking-[0.2em] uppercase ml-2">?�세 ?�명</FormLabel>
                      <FormControl>
                        <Textarea
                          {...field}
              ...
              <DialogFooter className="p-8 pt-0 gap-3">
                <Button
                  ...
                  className="h-12 px-10 rounded-lg border-2 border-slate-100 font-bold text-xs tracking-[0.2em] uppercase hover:bg-slate-50 transition-all flex-1"
                >
                  취소
                </Button>
                <Button
                  type="submit"
                  disabled={loading}
                  className="h-12 px-16 bg-slate-900 border-none text-white rounded-lg font-bold text-xs tracking-[0.3em] uppercase shadow-2xl hover:bg-primary transition-all hover:-translate-y-1 active:scale-95 flex-1"
                >
                  {loading ? '처리 �?..' : mode === 'edit' ? '?�정 ?�료' : '?�록 ?�료'}
                </Button>
              </DialogFooter>
            </form>
          </Form>

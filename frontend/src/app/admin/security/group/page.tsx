...
export default function GroupManagePage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast('?†Í∑ú Î≥¥Ïïà Í∑∏Î£π ?ÑÌÇ§?çÏ≤òÍ∞Ä ?§Ïû•?òÏóà?µÎãà??', 'success');
    },
    onError: () => toast('Í∑∏Î£π ?ùÏÑ± Ï§??úÏä§???àÏô∏Í∞Ä Î∞úÏÉù?àÏäµ?àÎã§.', 'error')
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-groups'] });
      setIsDialogOpen(false);
      toast('Î≥¥Ïïà Í∑∏Î£π Î™ÖÏÑ∏Í∞Ä ?±Í≥µ?ÅÏúºÎ°??òÏ†ï?òÏóà?µÎãà??', 'success');
    },
    onError: () => toast('?ïÎ≥¥ ?òÏ†ï Ï§??úÏä§???àÏô∏Í∞Ä Î∞úÏÉù?àÏäµ?àÎã§.', 'error')
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-groups'] });
      toast('Î≥¥Ïïà Í∑∏Î£π ?ÑÎ°ú?ÑÏù¥ ?ÅÍµ¨?ÅÏúºÎ°??åÍ∏∞?òÏóà?µÎãà??', 'success');
    },
  ...
  const handleDelete = async (groupId: string) => {
    if (!confirm('?¥Îãπ Î≥¥Ïïà Í∑∏Î£π????†ú?òÏãúÍ≤†Ïäµ?àÍπå? Í¥Ä???ïÏ±Ö ?∞Ïù¥?∞Í? Î™®Îëê ?åÎ©∏?©Îãà??')) return;
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="Î≥¥Ïïà Í∑∏Î£π ?ÑÌÇ§?çÏ≤ò Í±∞Î≤Ñ?åÏä§"
        breadcrumbs={[{ label: 'Î≥¥ÏïàÍ¥ÄÎ¶? }, { label: 'Í∑∏Î£πÍ¥ÄÎ¶? }]}
      />

      <HubHeader
        title="Security"
        highlight="Group"
        subtitle="?úÏä§???ëÍ∑º ?òÏ????ïÏùò?òÎäî ?ºÎ¶¨??Î≥¥Ïïà Í∑∏Î£π ?îÌã∞??Î∞??ïÏ±Ö ?ÑÏπ¥?¥Î∏å ?µÌï© ?úÏñ¥"
        icon={Users}
        actions={
          <div className="flex gap-4 p-2 items-center">
            ...
            >
              <Plus size={20} className="group-hover:scale-110 transition-transform duration-500" /> ?†Í∑ú Î≥¥Ïïà Í∑∏Î£π ?§Ïû•
            </Button>
          </div>
        }
      />
      ...
      <HubMetricGrid>
        ...
        <HubMetricCard title="SYNC_STATUS" value="STEADY" icon={Activity} color="emerald" status="?ôÍ∏∞?îÎê®" />
        <HubMetricCard title="SECURITY_TIER" value="TIER_1" icon={Lock} color="amber" />
      </HubMetricGrid>

      <HubSectionCard
        title="Î≥¥Ïïà ?ÑÎ©î??Í∑∏Î£π Îß§Ìä∏Î¶?ä§"
        description="?úÏä§?úÏùò ?ºÎ¶¨??Î≥¥Ïïà Í≥ÑÏ∏µ??Íµ¨ÏÑ±?òÎäî Í∑∏Î£π ?∏Î≤§?†Î¶¨ Î∞??§ÏãúÍ∞??ÑÎ°úÎπÑÏ????ÅÌÉú?ÖÎãà??"
        icon={Network}
      >
        <div className="space-y-8">
          <div className="flex items-center justify-between px-2 pt-2 border-b border-slate-100 pb-10 mb-8">
            <div className="flex items-center gap-8">
              <form onSubmit={handleSearch} className="flex items-center gap-4 relative group/search">
                ...
                <Input
                  placeholder="Í∑∏Î£π UID ?êÎäî Î¶¨ÌÑ∞??Î™ÖÏπ≠?ºÎ°ú Î∂ÑÏÑù..."
            ...
            <div>
              <span className="text-[10px] font-black text-muted-foreground/30 tracking-[0.4em] uppercase font-mono italic">Í∏∞Îä•??Í∑∏Î£π ?åÏù¥Î∏??ÑÎ°úÎ∏?/span>
            </div>
          </div>
          ...
          <div className="min-h-[500px]">
            <StandardDataTable
              ...
              data={groups}
              loading={isLoading}
              emptyMessage="?ùÎ≥Ñ??Î≥¥Ïïà Í∑∏Î£π Î¶¨ÏÜå?§Í? Ï°¥Ïû¨?òÏ? ?äÏäµ?àÎã§."
      ...
      <StandardModal
        ...
        title={editingGroup ? 'Î≥¥Ïïà Í∑∏Î£π ?ÑÌÇ§?çÏ≤ò ?òÏ†ï' : '?†Í∑ú Î≥¥Ïïà ?ÑÎ©î??Í∑∏Î£π ?§Ïû•'}
        maxWidth="xl"
      >
        <div className="p-4 space-y-12">
          <div className="grid grid-cols-2 gap-10">
            <FormField label="?ÑÎ©î??Í∑∏Î£π ?ùÎ≥Ñ??(Group ID)" required description="Î≥¥Ïïà ?àÏù¥???¥Ïóê???†Ïùº???ºÎ¶¨???ùÎ≥Ñ??>
              <div className="relative group/id">
                ...
                <Input
                  ...
                  placeholder="Í∑∏Î£π ?ùÎ≥Ñ??
                />
              </div>
            </FormField>
            <FormField label="Í∑∏Î£π ?àÏù¥Î∏?Î™ÖÏπ≠" required description="UI ?ÅÏóê???∏Ï∂ú??Í∑∏Î£π??Î¶¨ÌÑ∞???¥Î¶Ñ">
              <div className="relative group/nm">
                ...
                <Input
                  ...
                  placeholder="Í∑∏Î£π Î™ÖÏπ≠ ?ÖÎ†•"
                />
              </div>
            </FormField>
          </div>

          <FormField label="Í∑∏Î£π ?ïÏ±Ö ?ÅÏÑ∏ Î™ÖÏÑ∏" description="?¥Îãπ Î≥¥Ïïà Í∑∏Î£π??ÎπÑÏ¶à?àÏä§ Î™©Ï†Å Î∞??∞Ïù¥???ëÍ∑º Î≤îÏúÑ Î™ÖÏÑ∏">
          ...
          <div className="flex gap-6 pt-4">
            ...
            <Button onClick={handleSubmit} disabled={createMutation.isPending || updateMutation.isPending} className="flex-[2] h-14 rounded-2xl bg-slate-900 border-none text-white font-black text-[10px] tracking-widest shadow-2xl hover:bg-primary transition-all hover:-translate-y-2 group">
              ...
              <span className="ml-2">{editingGroup ? 'Í∑∏Î£π ?òÏ†ï' : '?†Í∑ú Í∑∏Î£π Î∞∞Ìè¨'}</span>
            </Button>
          </div>
        </div>
      </StandardModal>

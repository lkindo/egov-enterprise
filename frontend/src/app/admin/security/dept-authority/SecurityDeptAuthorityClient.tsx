  ...
  return (
    <section aria-label="부서 구성원 그룹 배정" className="space-y-4 rounded-lg border border-border bg-card p-5">
      ...
      <div className="flex flex-wrap gap-4">
        <label className="space-y-1 text-sm">권한 그룹<select className="block rounded-md border border-input bg-background px-3 h-[var(--control-h)]" aria-label="권한 그룹" {...validation.fieldProps('groupCode')} value={groupCode} disabled={!writable} onChange={(event) => { setGroupCode(event.target.value); validation.clearError('groupCode'); }}><option value="">그룹 선택</option>{groups.filter((group) => action === 'REMOVE' || group.code !== 'ROLE_ANONYMOUS').map((group) => <option key={group.code} value={group.code}>{group.name}</option>)}</select><span className="block text-destructive" {...validation.messageProps('groupCode')} /></label>
        <label className="space-y-1 text-sm">변경 방식<select className="block rounded-md border border-input bg-background px-3 h-[var(--control-h)]" aria-label="변경 방식" {...validation.fieldProps('action')} value={action} disabled={!writable} onChange={(event) => { setAction(event.target.value as 'ADD' | 'REMOVE'); if (groupCode === 'ROLE_ANONYMOUS') setGroupCode(''); }}><option value="ADD">그룹 추가</option><option value="REMOVE">그룹 회수</option></select><span className="block text-destructive" {...validation.messageProps('action')} /></label>
      </div>

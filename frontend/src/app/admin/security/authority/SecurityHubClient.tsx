...
export default function SecurityHubClient() {
  ...
  return (
    <WorkListPage title="권한 그룹 관리" description="그룹에 기능권한을 연결하고 사용자에게 하나 이상의 그룹을 배정합니다." breadcrumbItems={[{ label: '보안' }, { label: '권한 그룹 관리' }]}
      ...
      filter={<nav aria-label="권한 관리 영역" className="flex flex-wrap gap-2">
        {canRead && <><Button type="button" variant={tab === 'groups' ? 'default' : 'outline'} aria-pressed={tab === 'groups'} onClick={() => setTab('groups')}>그룹 · 기능권한</Button><Button type="button" variant={tab === 'users' ? 'default' : 'outline'} aria-pressed={tab === 'users'} onClick={() => setTab('users')}>사용자 배정</Button></>}
        {canAudit && <Button type="button" variant={tab === 'history' ? 'default' : 'outline'} aria-pressed={tab === 'history'} onClick={() => setTab('history')}>변경 이력</Button>}
      </nav>}>
      ...
      {tab === 'groups' && canRead && <div className="grid gap-6 lg:grid-cols-[minmax(14rem,1fr)_minmax(0,3fr)]">
        <section aria-label="권한 그룹 목록" className="space-y-4">
          ...
          {canCreate && <Button type="button" disabled={pendingCreate} onClick={() => { setCreating(true); setSelectedGroup(''); }}>그룹 추가</Button>}
          ...
          <ul className="space-y-2">{shownGroups.map((entry) => <li key={entry.code}><Button type="button" variant={entry.code === selectedGroup ? 'secondary' : 'outline'} className="h-auto w-full justify-start whitespace-normal p-3 text-left" disabled={pendingCreate} aria-pressed={entry.code === selectedGroup}
            onClick={() => { setCreating(false); setSelectedGroup(entry.code); }}><span>{entry.name}<span className="block text-xs text-muted-foreground">{entry.code}</span></span></Button></li>)}</ul>

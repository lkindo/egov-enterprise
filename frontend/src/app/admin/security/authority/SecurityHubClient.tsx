...
export default function SecurityHubClient() {
  ...
  const saveUserMappingMutation = useMutation({
    ...
    onSuccess: () => {
      toast('?¨Ïö©??Í∂åÌïú ?†Îãπ Îß§Ìä∏Î¶?ä§Í∞Ä ?ÖÎç∞?¥Ìä∏?òÏóà?µÎãà??', 'success');
  ...
  const saveMenuMappingMutation = useMutation({
    ...
    onSuccess: () => {
      toast('Î©îÎâ¥ ?ëÍ∑º Í±∞Î≤Ñ?åÏä§ ?ïÏ±Ö???ôÍ∏∞?îÎêò?àÏäµ?àÎã§.', 'success');
  ...
  const handleAuthorDelete = async (code: string) => {
    if (!confirm('Í∂åÌïú ?ÑÌÇ§?çÏ≤òÎ•???†ú?òÏãúÍ≤†Ïäµ?àÍπå? Í¥Ä???†Îãπ ?ïÎ≥¥Í∞Ä Î™®Îëê ?ÅÍµ¨?ÅÏúºÎ°??åÎ©∏?©Îãà??')) return;
    try {
      await authorAdminService.deleteAuthor(code);
      toast('Í∂åÌïú ?ÑÎ°ú?ÑÏù¥ ?±Í≥µ?ÅÏúºÎ°???†ú?òÏóà?µÎãà??', 'success');
      queryClient.invalidateQueries({ queryKey: ['admin-authorities'] });
      if (selectedAuthorCode === code) setSelectedAuthorCode('');
    } catch (e) {
      toast('??†ú Ï§??úÏä§???àÏô∏Í∞Ä Î∞úÏÉù?àÏäµ?àÎã§.', 'error');
    }
  };
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Î≥¥ÏïàÍ¥ÄÎ¶? }, { label: 'Í∂åÌïú ?§Ï†ï' }, { label: '?µÌï© ÏΩòÌä∏Î°? }]}
      />

      <HubHeader
        title="Security"
        highlight="Fabric"
        subtitle="?úÏä§???ÑÎ∞ò??Î≥¥Ïïà ??ï†(Role), ?¨Ïö©???†Îãπ Îß§Ìä∏Î¶?ä§ Î∞??§Ï∞®???ëÍ∑º ?úÏñ¥ ?ïÏ±Ö ?µÌï© ?ÑÌÇ§?çÏ≤ò"
        icon={Lock}
        actions={
          <div className="flex gap-4 p-2 items-center">
            ...
            >
              <Plus size={20} className="group-hover:scale-110 transition-transform duration-500" /> ?†Í∑ú Î≥¥Ïïà ?ÑÌÇ§?çÏ≤ò ?§Ïû•
            </Button>
          </div>
        }
      />
        ...
        {viewMode === 'MATRIX' ? (
          ...
          >
            ...
            <div className="col-span-12 lg:col-span-3 space-y-8 h-full">
              <HubSectionCard title="??ï† ?∏Î≤§?†Î¶¨" description="?úÏä§???ëÍ∑º ?òÏ????ïÏùò?òÎäî Î≥¥Ïïà ?ÑÎ°ú??Î¶¨Ïä§?∏ÏûÖ?àÎã§." icon={Lock}>
                      ...
                      {!selectedAuthorCode ? (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center p-20 text-center space-y-6">
                          ...
                          <div className="space-y-2">
                            ...
                            <p className="text-[10px] font-black text-slate-200 tracking-[0.3em] uppercase leading-relaxed">Î≥¥Ïïà ??ï†???†ÌÉù?òÏó¨ ?ùÎ≥Ñ???ÑÎ°úÎ∏åÎ? ?úÏÑ±?îÌïò??ãú??</p>
                          </div>
                        </motion.div>
                      ) : (
                      ...
                      {!selectedAuthorCode ? (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center p-20 text-center space-y-6">
                          ...
                          <div className="space-y-2">
                            ...
                            <p className="text-[10px] font-black text-slate-200 tracking-[0.3em] uppercase leading-relaxed">Î≥¥Ïïà Í±∞Î≤Ñ?åÏä§ ??ï†???†ÌÉù?òÏó¨ Í≥ÑÏ∏µ ?∏ÎìúÎ•??ÑÎ°ú?úÌïò??ãú??</p>
                          </div>
                        </motion.div>
                      ) : isMenusLoading ? (
      ...
      <StandardModal
        ...
        title={authorMode === 'create' ? '?†Í∑ú Î≥¥Ïïà ??ï† ?ÑÎ°úÎπÑÏ??? : 'Î≥¥Ïïà ??ï† ?ÑÌÇ§?çÏ≤ò ?ÅÏÑ∏ ?òÏ†ï'}
        maxWidth="xl"
      >
        <div className="p-4 space-y-12">
          <div className="grid grid-cols-2 gap-10">
            <FormField label="Î≥¥Ïïà ??ï† ?ùÎ≥Ñ??(Role Code)" required description="?úÏä§???ÑÎ∞ò???ÅÏö©?òÎäî ?†Ïùº????ï† Í≥†Ïú† ÏΩîÎìú">
            ...
            <FormField label="??ï† ?àÏù¥Î∏?Î™ÖÏπ≠" required description="UI Î∞?ÎπÑÏ¶à?àÏä§ ?àÏù¥?¥Ïóê???ùÎ≥Ñ??Î™ÖÎ¨∏?îÎêú ?¥Î¶Ñ">
              <div className="relative group/nm">
                ...
                <Input
                  ...
                  placeholder="??ï† Î™ÖÏπ≠ ?ÖÎ†•"
                />
              </div>
            </FormField>
          </div>

          <FormField label="Î≥¥Ïïà ?ïÏ±Ö ?ïÎ? Î™ÖÏÑ∏" description="?¥Îãπ ??ï†???ÅÏÑ∏ Î™©Ï†Å Î∞??∞Ïù¥???ëÍ∑º Î≤îÏúÑ???Ä???ïÎ? Î™ÖÏÑ∏">
          ...
          <div className="flex gap-6 pt-4">
            <Button variant="outline" onClick={() => setIsAuthorModalOpen(false)} className="flex-1 h-14 rounded-2xl font-black text-[10px] tracking-widest border-2">Ï∑®ÏÜå</Button>
            <Button onClick={() => saveAuthorMutation.mutate(authorFormData)} disabled={saveAuthorMutation.isPending} className="flex-[2] h-14 rounded-2xl bg-slate-900 border-none text-white font-black text-[10px] tracking-widest shadow-2xl hover:bg-primary transition-all hover:-translate-y-2 group">
              <Zap size={18} className="group-hover:animate-pulse" /> {authorMode === 'create' ? 'Í∂åÌïú Î∞∞Ìè¨' : 'Í∂åÌïú ?òÏ†ï'}
            </Button>
          </div>
        </div>
      </StandardModal>

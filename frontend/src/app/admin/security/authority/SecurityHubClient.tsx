...
export default function SecurityHubClient({ 
  ...
  const onAuthorSubmit = async (values: AuthorFormValues) => {
    try {
      if (authorMode === 'create') {
        ...
        toast('Î≥¥Ïïà Í∂åÌïú ?ÑÌÇ§?çÏ≤òÍ∞Ä ?±Í≥µ?ÅÏúºÎ°?Î∞òÏòÅ?òÏóà?µÎãà??', 'success');
      } else {
        await authorAdminService.updateAuthor(values.authorCode, values as AuthorInfo);
        toast('Î≥¥Ïïà Í∂åÌïú ?ÑÌÇ§?çÏ≤òÍ∞Ä ?±Í≥µ?ÅÏúºÎ°??òÏ†ï?òÏóà?µÎãà??', 'success');
      }
      queryClient.invalidateQueries({ queryKey: ['admin-authorities'] });
      setIsAuthorModalOpen(false);
    } catch (error) {
      toast('?Ä??Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§. ?ÖÎ†•???ïÏù∏?¥Ï£º?∏Ïöî.', 'error');
    }
  };
  ...
  const saveUserMappingMutation = useMutation({
    ...
    onSuccess: () => {
      toast('?¨Ïö©??Í∂åÌïú ?†Îãπ??Î∞òÏòÅ?òÏóà?µÎãà??', 'success');
  ...
  const saveMenuMappingMutation = useMutation({
    ...
    onSuccess: () => {
      toast('Î©îÎâ¥ ?ëÍ∑º Í∂åÌïú???ÖÎç∞?¥Ìä∏?òÏóà?µÎãà??', 'success');
  ...
  const loadGlobalMappings = async () => {
    ...
    try {
      ...
      toast('Í∏ÄÎ°úÎ≤å Îß§Ìä∏Î¶?ä§ ?∞Ïù¥??Î°úÎìú Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.', 'error');
    } finally {
  ...
  const handleSaveGlobal = async () => {
    ...
    try {
      ...
      await Promise.all(promises);
      toast('Í∏ÄÎ°úÎ≤å Î≥¥Ïïà ?ïÏ±Ö???ÑÏÇ¨?ÅÏúºÎ°??ôÍ∏∞?îÎêò?àÏäµ?àÎã§.', 'success');
      queryClient.invalidateQueries({ queryKey: ['admin-author-menus'] });
    } catch (e) {
      toast('Í∏ÄÎ°úÎ≤å ?ïÏ±Ö ?Ä??Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.', 'error');
    } finally {
      setIsGlobalLoading(false);
    }
  };
  ...
  const handleAuthorDelete = async (code: string) => {
    if (!confirm('Í∂åÌïú????†ú?òÏãúÍ≤†Ïäµ?àÍπå? Í¥Ä???†Îãπ ?ïÎ≥¥Í∞Ä Î™®Îëê ?¨ÎùºÏßëÎãà??')) return;
    try {
      await authorAdminService.deleteAuthor(code);
      toast('Í∂åÌïú????†ú?òÏóà?µÎãà??', 'success');
      queryClient.invalidateQueries({ queryKey: ['admin-authorities'] });
      if (selectedAuthorCode === code) setSelectedAuthorCode('');
    } catch (e) {
      toast('??†ú Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.', 'error');
    }
  };
      ...
      accessor: (auth) => (
        <div className="flex items-center justify-between w-full group/role-item py-1">
          <div className="flex flex-col gap-1">
            <span className={cn("text-sm font-bold tracking-tight truncate leading-none", selectedAuthorCode === auth.authorCode ? "text-white" : "text-slate-900")}>
              {auth.authorNm}
            </span>
  ...
  return (
    ...
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="?µÌï© Î≥¥Ïïà Í±∞Î≤Ñ?åÏä§ ?àÎ∏å"
        breadcrumbs={[{ label: 'Î≥¥Ïïà Í¥ÄÎ¶? }, { label: 'Í∂åÌïú ?§Ï†ï' }, { label: '?µÌï© Ïª®Ìä∏Î°? }]}
      />

      <HubHeader
        title="Security"
        highlight="Fabric"
        subtitle="?úÏä§???ÑÎ∞ò Î≥¥Ïïà ??ï†(Role), ?¨Ïö©???†Îãπ Îß§Ìä∏Î¶?ä§ Î∞?Í≥ÑÏ∏µ???ëÍ∑º ?úÏñ¥ ?ïÏ±Ö ?µÌï© ?ÑÌÇ§?çÏ≤ò"
        icon={Lock}
        actions={
          <div className="flex gap-4 p-2 items-center">
            <div className="flex items-center gap-1 bg-slate-50 p-1 rounded-lg mr-4 border-2 border-slate-100">
              <Tooltip>
                ...
                <TooltipContent side="bottom" className="bg-slate-900 text-white border-none rounded-lg px-4 py-2 text-xs font-bold tracking-widest uppercase">
                  Î≥¥Ïïà Í∞ùÏ≤¥ Í∞ÑÏùò Í≥ÑÏ∏µ??Í¥ÄÍ≥??úÍ∞Å??                </TooltipContent>
              </Tooltip>
              ...
              <Tooltip>
                ...
                <TooltipContent side="bottom" className="bg-slate-900 text-white border-none rounded-lg px-4 py-2 text-xs font-bold tracking-widest uppercase">
                  ?ÑÏÇ¨??Í∂åÌïú ?†Îãπ ?ÑÌô© ?ºÍ¥Ñ Í≤Ä??Î∞??òÏ†ï
                </TooltipContent>
              </Tooltip>
            </div>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <Plus size={20} className="group-hover:scale-110 transition-transform duration-500" /> ?†Í∑ú Î≥¥Ïïà ?ÑÌÇ§?çÏ≤ò ?§Ï†ï
                </Button>
              </TooltipTrigger>
              <TooltipContent side="bottom" className="bg-slate-900 text-white border-none rounded-lg px-4 py-2 text-xs font-bold tracking-widest uppercase">
                ?àÎ°ú????ï† ?êÎäî Î≥¥Ïïà Í∑∏Î£π ?ïÏùò
              </TooltipContent>
            </Tooltip>
          </div>
        }
        ...
        {viewMode === 'MATRIX' ? (
          ...
          >
            ...
            <div className="col-span-12 lg:col-span-3 space-y-8 h-full">
              <HubSectionCard title="??ï† ?∏Î≤§?†Î¶¨" description="?úÏä§???ëÍ∑º Í∂åÌïú???ïÏùò?òÎäî Î≥¥Ïïà ?ÑÎ°ú?åÏùº Î¶¨Ïä§?∏ÏûÖ?àÎã§." icon={Lock}>
            ...
            <div className="col-span-12 lg:col-span-4 space-y-8 h-full">
              <HubSectionCard
                title="?¨Ïö©???†Îãπ"
                description="?†ÌÉù????ï†???†Îãπ??Í∞úÎ≥Ñ ?ùÎ≥Ñ?êÎì§???§ÏãúÍ∞??†Îãπ ?ÅÌÉú?ÖÎãà??"
                icon={Users}
                action={
                  <Tooltip>
                    <TooltipTrigger asChild>
                    ...
                    <TooltipContent side="top" className="bg-slate-900 text-white border-none rounded-lg px-4 py-2 text-xs font-bold tracking-widest uppercase">
                      ?òÏ†ï???¨Ïö©??Îß§Ìïë ?ïÎ≥¥Î•?DB??ÏµúÏ¢Ö Î∞òÏòÅ
                    </TooltipContent>
                  </Tooltip>
                }
              >
                    ...
                    <AnimatePresence mode="wait">
                      {!selectedAuthorCode ? (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center p-20 text-center space-y-6">
                          <div className="w-20 h-20 rounded-lg bg-slate-50 flex items-center justify-center text-slate-200">
                            <Users size={40} className="Í¥ÄÎ¶¨Ïûê Í∂åÌïú" />
                          </div>
                          <div className="space-y-2">
                            <h4 className="text-xl font-bold text-slate-300 uppercase tracking-tight">Identity_Idle</h4>
                            <p className="text-xs font-bold text-slate-200 tracking-[0.3em] uppercase leading-relaxed">Î≥¥Ïïà ??ï†???†ÌÉù?òÏó¨ ?ùÎ≥Ñ???ÑÎ°úÎ∏åÎ? ?úÏÑ±?îÌïò??ãú??/p>
                          </div>
                        </motion.div>
                      ) : (
            ...
            <div className="col-span-12 lg:col-span-5 h-full">
              <HubSectionCard
                title="?ëÍ∑º ?ïÏ±Ö ?†Ìè¥Î°úÏ?"
                description="??ï†Î≥??ôÏ†Å Î©îÎâ¥ ?∏Îìú Í≥ÑÏ∏µ Î∞??ÑÌÇ§?çÏ≤ò ?ëÍ∑º ?òÏ? ?§Ï†ï?ÖÎãà??"
                icon={Layers}
                action={
                  <Tooltip>
                    <TooltipTrigger asChild>
                    ...
                    <TooltipContent side="top" className="bg-slate-900 text-white border-none rounded-lg px-4 py-2 text-xs font-bold tracking-widest uppercase">
                      ?ÑÏû¨ ?∏Îìú Íµ¨Ï°∞Î•?Î≥¥Ïïà ?ïÏ±Ö???ôÍ∏∞??                    </TooltipContent>
                  </Tooltip>
                }
              >
                <div className="relative h-full flex flex-col pt-4">
                  <div className="flex items-center gap-4 bg-slate-900 rounded-lg p-8 mb-10 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 space-y-1">
                      ...
                      <div className="text-white text-lg font-bold tracking-tight leading-none">
                        {tempMenuMappings.size} Í∞úÏùò ?úÏÑ± ?∏ÎìúÍ∞Ä <span className="text-primary">{selectedAuthorCode || 'N/A'}</span> ??Îß§Ìïë??                      </div>
                    </div>
                  </div>
                    ...
                    <AnimatePresence mode="wait">
                      {!selectedAuthorCode ? (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center p-20 text-center space-y-6">
                          ...
                          <div className="space-y-2">
                            <h4 className="text-xl font-bold text-slate-300 uppercase tracking-tight">Topology_Idle</h4>
                            <p className="text-xs font-bold text-slate-200 tracking-[0.3em] uppercase leading-relaxed">Î≥¥Ïïà Í±∞Î≤Ñ?åÏä§ ??ï†???†ÌÉù?òÏó¨ Í≥ÑÏ∏µ ?∏ÎìúÎ•?Î°úÎìú?òÏã≠?úÏò§</p>
                          </div>
                        </motion.div>
                      ) : isMenusLoading ? (
      ...
      <StandardModal
        ...
        title={authorMode === 'create' ? '?†Í∑ú Í∂åÌïú ?±Î°ù' : 'Î≥¥Ïïà ??ï† ?ÑÌÇ§?çÏ≤ò ?ÅÏÑ∏ ?òÏ†ï'}
        maxWidth="xl"
      >
        <AuthorForm
          mode={authorMode}

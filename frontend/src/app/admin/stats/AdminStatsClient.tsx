...
export default function AdminStatsClient({
 ...
 <span className="font-black italic tracking-tighter text-slate-900">{item.menuNm}</span>
 </div>
 )
 },
 {
 ...
 <span className="text-[10px] font-black text-slate-400 w-10 text-right italic tracking-tight">{item.percentage}%</span>
 </div>
 )
 }
 ];
 ...
 <select className="h-10 md:h-14 px-3 md:px-6 rounded-xl md:rounded-2xl border-2 border-slate-100 bg-white font-black text-[9px] md:text-[10px] tracking-tight italic outline-none focus:ring-4 focus:ring-primary/10 transition-all cursor-pointer">
 <option>최근 14일</option>
 <option>최근 30일</option>
 <option>분기별 분석</option>
 </select>
 <Button
 ...
 <LuxuryStatCard title="누적 노드" value={initialSummary?.totalUsers} icon={<Users size={24} />} trend="+2.5%" color="slate" />
 <LuxuryStatCard title="활성 " value={initialSummary?.todayConnects} icon={<Zap size={24} />} trend="+12%" color="primary" />
 <LuxuryStatCard title="데이터 영속성" value={initialSummary?.totalPosts} icon={<FileText size={24} />} trend="+0.8%" color="indigo" />
 </div>
 ...
 <div>
 <h3 className="text-xl md:text-3xl font-black text-slate-900 tracking-tighter italic">심층 인텔리전스 보고서</h3>
 <p className="text-[9px] md:text-[11px] font-black text-slate-400 tracking-[0.4em]">하위 시스템 상호작용 매트릭스</p>
 </div>
 </div>
 ...
 <span className="text-[9px] md:text-[10px] font-black tracking-[0.2em] italic">실시간 스트림</span>
 </div>
 </div>
 ...
 <h2 className="text-3xl md:text-7xl font-black tracking-tighter leading-tight italic tabular-nums">
 Optimized <br />
 ...
 <button className="h-16 md:h-20 px-8 md:px-14 bg-white text-slate-900 rounded-2xl md:rounded-[2rem] font-black text-sm md:text-sm tracking-[0.2em] shadow-xl hover:bg-primary hover:text-white transition-all hover:-translate-y-2 active:scale-95 flex items-center gap-4 italic group/btn">
 보고서 생성
...
function LuxuryStatCard({ title, value, icon, trend, isAlert, color }: any) {
 ...
 <span className={cn(
 "text-[10px] font-black px-3 py-1 rounded-full italic tracking-tight",

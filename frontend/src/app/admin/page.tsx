...
export default function AdminDashboardPage() {
  ...
  const { data: usersData } = useQuery({
    ...
    queryFn: () => userAdminService.getUserList({ pageÎ≤àÌò∏: 1, size: 1 }),
    retry: 1,
    retryDelay: 5000,
  });
  ...
  const { data: authorsData } = useQuery({
    ...
    queryFn: () => authorAdminService.getAuthorList({ pageÎ≤àÌò∏: 1, size: 1 }),
    retry: 1,
    retryDelay: 5000,
  });
  ...
  const recentLogs: UIAuditLog[] = (auditData?.list || []).map(log => ({
    id: log.histId,
    action: log.histCn.includes('?ùÏÑ±') || log.histCn.includes('?±Î°ù') ? 'CREATE' :
      log.histCn.includes('??†ú') ? 'DELETE' :
        log.histCn.includes('Î≥µÏõê') ? 'RESTORE' : 'UPDATE',
    entityName: log.histCn,
    performedBy: log.frstRegisterId,
    timestamp: log.frstRegisterPnttm,
    ipAddress: log.sysNm || 'Unknown Subsystem',
    severity: log.histCn.includes('?§Î•ò') || log.histCn.includes('?§Ìå®') || log.histCn.includes('??†ú') ? 'high' :
      log.histCn.includes('Î≥¥Ïïà') || log.histCn.includes('Í∂åÌïú') ? 'medium' : 'low'
  }));
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 px-4 md:px-0 pb-20 animate-in fade-in duration-700">
      <HubHeader
        ...
        subtitle="?úÏä§???ÑÎ∞ò???§Ìçº?àÏù¥???ÅÌÉú, ÏßÄ?•Ìòï ?∞Ïù¥??Î∂ÑÏÑù Î∞?Î≥¥Ïïà Í±∞Î≤Ñ?åÏä§ ?µÌï© Í¥Ä???Ä?úÎ≥¥??
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        <DashboardStatCard
          ...
          description="?¨Ïö©??Î∞?Ï°∞ÏßÅ ?µÌï© Îß§Ìä∏Î¶?ä§"
        />
        <DashboardStatCard
          ...
          description="RBAC Í∏∞Î∞ò ?ëÍ∑º ?ïÏ±Ö Í±∞Î≤Ñ?åÏä§"
        />
        <DashboardStatCard
          ...
          description="Î™®Îìà Î∞?Î¶¨ÏÜå???§Ï??§Ìä∏?àÏù¥??
        />
        <DashboardStatCard
          title="Audit Streams"
          ...
          description="?§ÏãúÍ∞?Î≥¥Ïïà Í∞êÏÇ¨ ?§Ìä∏Î¶?Î∂ÑÏÑù"
        />
      </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-muted-foreground uppercase opacity-50 tracking-widest mt-1">?úÏä§???∏Îûò??Î∞??ÑÎ©î???úÎèô Î∂ÑÏÑù</p>
              </div>
            </div>
          </div>
          <ActivityAreaChart data={MOCK_ACTIVITY_DATA} title="ÏµúÍ∑º 7?ºÍ∞Ñ ?úÏä§???ëÏÜç ?ÑÎ°ú?? color="#6366F1" />
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-muted-foreground uppercase opacity-50 tracking-widest mt-1">?¨Ïö©??Í∂åÌïú Í∑∏Î£π Î∂ÑÌè¨</p>
              </div>
            </div>
          </div>
          <div className="flex-1 min-h-[300px]">
            <DistributionPieChart data={MOCK_DISTRIBUTION_DATA} title="RBAC ?òÏö©??Î∂ÑÏÑù" />
          </div>
        </div>
      </div>
          ...
          <div className="p-8 rounded-[3rem] bg-card border border-border shadow-sm overflow-hidden relative group">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-foreground">Ïª¥Ìì®???∏Îìú ?¨Ïä§Ï≤¥ÌÅ¨</h3>
                  <p className="text-[10px] font-medium text-muted-foreground mt-0.5">?§ÏãúÍ∞?Î¶¨ÏÜå???åÎπÑ Î™®Îãà?∞ÎßÅ</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase opacity-50">?âÍ∑† Î∂Ä??/p>
            ...
            <div className="p-8 rounded-[3rem] bg-card border border-border shadow-sm flex flex-col gap-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold text-foreground">?∞Ïù¥???åÏä§</span>
                </div>
            ...
            <div className="p-8 rounded-[3rem] bg-card border border-border shadow-sm flex flex-col gap-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold text-foreground">Í∏ÄÎ°úÎ≤å ÏßÄ?∞ÏãúÍ∞?/span>
                </div>

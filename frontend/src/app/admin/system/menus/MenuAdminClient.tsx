...
export default function MenuAdminClient({ 
  ...
  // ?∞Ïù¥??Ï¥àÍ∏∞??  useEffect(() => {
    // 1. ?úÎ≤Ñ?êÏÑú ???âÎ©¥ ?∞Ïù¥?∞Î? ?∏Î¶¨ Íµ¨Ï°∞Î°?Î≥Ä??    const tree = listToTree(initialMenus);
    // 2. ?∏Î¶¨ Íµ¨Ï°∞Î•??§Ïãú DnD???âÎ©¥ ?∞Ïù¥?∞Î°ú Î≥Ä??(depth Í≥ÑÏÇ∞ ?¨Ìï®)
  ...
  // Í∞Ä?úÏ†Å Î©îÎâ¥ ?ÑÌÑ∞Îß?  const visibleFlattenedMenus = useMemo(() => {
    ...
    flattenedMenus.forEach(item => {
        if (isParentExpanded(item.parentId)) {
            // ?úÎûòÍ∑?Ï§ëÏù∏ ?ÑÏù¥?úÏù∏ Í≤ΩÏö∞ projected depth ?ÅÏö©
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && projected) {
        setFlattenedMenus((items) => {
            ...
            // ÏµúÏ¢Ö depth?Ä parentId Î∞òÏòÅ
        ...
        setHasChanges(true);
        toast('Íµ¨Ï°∞Í∞Ä ?ÖÎç∞?¥Ìä∏?òÏóà?µÎãà??', 'info');
    }
  ...
  const handleSaveChanges = async () => {
    ...
    } catch (err: any) { console.error(err); toast('?Ä??Ï§??§Î•ò Î∞úÏÉù', 'error'); }
    finally { setIsSaving(false); }
  };

  const handleDelete = async (id: number) => {
    if (await confirm({ title: 'Î©îÎâ¥ ??†ú', message: '?òÏúÑ Î©îÎâ¥Í∞Ä ?¨Ìï®?????àÏäµ?àÎã§.', variant: 'destructive' })) {
  ...
  return (
    <div className="space-y-12 pb-24">
      <PageHeader title="?úÏä§??Î©îÎâ¥ ?ÑÌÇ§?çÏ≤ò" breadcrumbs={[{ label: '?úÏä§?úÍ?Î¶? }, { label: 'Î©îÎâ¥ Í¥ÄÎ¶? }]} />

      <HubHeader
        title="Î©îÎâ¥" highlight="?ÑÌÇ§?çÏ≤ò"
        subtitle="??™©??Ï¢åÏö∞Î°??ÄÏßÅÏó¨ Í≥ÑÏ∏µ(Depth)??Ï°∞Ï†ï?òÍ≥†, ?ÑÏïÑ?òÎ°ú ?åÏñ¥ ?úÏÑúÎ•??§ÏãúÍ∞ÑÏúºÎ°??§Í≥Ñ?òÏã≠?úÏò§."
        icon={FolderTree}
        actions={
      ...
      <HubMetricGrid>
        <HubMetricCard title="?ÑÏ≤¥ ?∏Îìú" value={flattenedMenus.length} icon={Database} color="primary" />
        <HubMetricCard title="Í≥ÑÏ∏µ ÍπäÏù¥" value={Math.max(...flattenedMenus.map(m => m.depth), 0) + 1} icon={Layers} color="indigo" />
        <HubMetricCard title="?úÏÑ± Í≤ΩÎ°ú" value={flattenedMenus.filter(m => !!m.modernRoute).length} icon={Network} color="emerald" />
        <HubMetricCard title="?∞Í≤∞ Î¨¥Í≤∞?? value="ÏµúÏ†Å" icon={ShieldCheck} color="amber" />
      </HubMetricGrid>

      <HubSectionCard
        title="?úÏä§???§ÎπÑÍ≤åÏù¥???∏Î¶¨"
        description="Í∑∏Î¶Ω ?∏Îì§???°Í≥† ??™©???êÏú†Î°?≤å Î∞∞Ïπò?òÏã≠?úÏò§. Í∞ÄÎ°??¥Îèô?ºÎ°ú Î∂ÄÎ™?Í¥ÄÍ≥ÑÍ? Î≥ÄÍ≤ΩÎê©?àÎã§."
        icon={SearchCode}
        action={
          <div className="flex gap-4 items-center">
            <div className="flex bg-slate-100 p-1 rounded-lg">
              <Button variant="ghost" className="h-8 px-3 text-xs font-bold" onClick={() => setExpandedIds(new Set(flattenedMenus.map(m => m.menuNo)))}><ChevronsUpDown size={14} className="mr-1" /> ?ºÏπòÍ∏?/Button>
              <Button variant="ghost" className="h-8 px-3 text-xs font-bold" onClick={() => setExpandedIds(new Set())}><ChevronsDownUp size={14} className="mr-1" /> ?ëÍ∏∞</Button>
            </div>
            {hasChanges && (
                <Button onClick={handleSaveChanges} disabled={isSaving} className="bg-emerald-500 text-white hover:bg-emerald-600 h-10 px-6 rounded-lg font-bold text-xs gap-2 shadow-lg scale-in-center">
                    {isSaving ? <Loader2 className="animate-spin w-4 h-4" /> : <Save size={16} />} ?àÏù¥?ÑÏõÉ ?ÅÏö©
                </Button>
            )}
          </div>
        }
      ...
      <StandardModal
        ...
        title={mode === 'create' ? '?†Í∑ú Î©îÎâ¥ ?ïÏùò' : 'Î©îÎâ¥ Íµ¨ÏÑ± ?òÏ†ï'}
        maxWidth="2xl"
        footer={
          <div className="flex w-full gap-4 pt-4">
            <Button variant="outline" onClick={() => setIsOpen(false)} className="flex-1 h-11 rounded-lg font-bold">Ï∑®ÏÜå</Button>
            <Button onClick={form.handleSubmit(onFormSubmit)} disabled={form.formState.isSubmitting} className="flex-[2] h-11 rounded-lg bg-primary text-white font-bold shadow-2xl hover:brightness-110 transition-all hover:-translate-y-1 gap-2">
              <Save size={18} /> {mode === 'create' ? '?±Î°ù ?ÑÎ£å' : '?òÏ†ï ?ÑÎ£å'}
            </Button>
          </div>
        }
      >
        <Form {...form}>
          <form className="space-y-6 pt-4">
            <ShadcnFormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-xs font-bold text-slate-700 ml-1">Î©îÎâ¥ Î™ÖÏπ≠ *</FormLabel>
                  <FormControl><Input {...field} className="h-11 rounded-lg font-bold px-5" placeholder="Î©îÎâ¥ ?¥Î¶Ñ ?ÖÎ†•" /></FormControl>
            ...
            <div className="grid grid-cols-2 gap-6">
                <FormItem>
                  <FormLabel className="text-xs font-bold text-slate-700 ml-1">?ÅÏúÑ ?∏Îìú</FormLabel>
                ...
                <ShadcnFormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-xs font-bold text-slate-700 ml-1">?ïÎ†¨ ?úÏÑú *</FormLabel>

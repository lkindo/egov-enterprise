...
export default function MenuAdminClient({ initialMenus, programs }: { initialMenus: MenuInfo[]; programs: any[] }) {
  ...
  // ?úÎ≤Ñ ?∞Ïù¥??initialMenus) ?ôÍ∏∞??- ?¨Ïö©?êÏùò ?ÑÏû¨ ?ëÏóÖ???ÜÍ±∞???úÎûòÍ∑?Ï§ëÏù¥ ?ÑÎãê ?åÎßå ?ÖÎç∞?¥Ìä∏
  useEffect(() => {
  ...
  const handleSaveChanges = async () => {
    ...
    const traverse = (items: MenuInfo[] | undefined, parentId: number) => {
      ...
      items.forEach((item, idx) => {
        // ?±Í≥µ ?¥Î†•???ïÏù∏??Í∑úÍ≤©?ºÎ°ú ?ÑÏÜ° (null Î∞©Ïãù)
        const parentNo = parentId === 0 ? null : parentId;

        // ?úÎ≤Ñ Î∂Ä?òÏ? Î°úÏßÅ Ï∂©Îèå Î∞©Ï?Î•??ÑÌï¥ ?µÏã¨ ?ÑÎìúÎß??ïÏ†ú (Payload Sanitization)
        flat.push({
          id: item.menuNo, // id ?ÑÎìú ?Ä??          menuNo: item.menuNo,
          menuOrdr: idx + 1,
    ...
    try {
      ...
      if (res.success) {
        toast(res.message, 'success');
        setHasChanges(false);
        // Î∂Ä?úÎü¨??Î∞∞Í≤Ω Í∞±Ïã†???ÑÌï¥ startTransition ?¨Ïö©
      ...
      toast('?úÏÑú Î≥Ä??Ï§??àÍ∏∞Ïπ??äÏ? ?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.', 'error');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    const isConfirmed = await confirm({
      title: 'Î©îÎâ¥ ??†ú ?ïÏù∏',
      message: '?¥Îãπ Î©îÎâ¥Î•???†ú?òÎ©¥ Î™®Îì† ?òÏúÑ Î©îÎâ¥Í∞Ä ?®Íªò ??†ú?òÎ©∞ Î≥µÍµ¨?????ÜÏäµ?àÎã§. Í≥ÑÏÜç?òÏãúÍ≤†Ïäµ?àÍπå?',
      variant: 'destructive',
      confirmText: '??†ú'
    });
    if (isConfirmed) {
  ...
  const handleDragStart = (e: React.DragEvent, id: number) => {
    ...
    // Î∏åÎùº?∞Ï?Í∞Ä ?úÎûòÍ∑??∏ÏÖò???ÑÏ†Ñ???ùÏÑ±??'ÏßÅÌõÑ'???ÅÌÉúÎ•??ÖÎç∞?¥Ìä∏ (0ms ÏßÄ??
    // ?úÎûòÍ∑??úÏûë Ï¶âÏãú DOM??Î≥Ä?òÎ©¥ Î∏åÎùº?∞Ï?Í∞Ä ?úÎûòÍ∑∏Î? Í∞ïÏ†ú Ï∑®ÏÜå?òÎäî Î≤ÑÍ∑∏ ?Ä??    setTimeout(() => {
      setDraggedMenuId(id);
    }, 0);
  };
  ...
  const handleDrop = (e: React.DragEvent, targetId: number, forcedPosition?: 'before' | 'inside' | 'after') => {
    ...
    // ?¥Îñ§ ?ÅÌô©?êÏÑú???îÏÉÅ???®Ï? ?äÎèÑÎ°??¥Î¶∞??Î≥¥Ïû•
    const cleanup = () => {
      setDraggedMenuId(null);
      setDropTargetId(null);
      setDropPosition(null);
    ...
    const findParentAndÎ≤àÌò∏ = (items: MenuInfo[], id: number): { parent: MenuInfo[], index: number, parentItem: MenuInfo | null } | null => {
      for (let i = 0; i < items.length; i++) {
        if (items[i].menuNo === id) return { parent: items, index: i, parentItem: null };
        if (items[i].children) {
          const found = findParentAndÎ≤àÌò∏(items[i].children!, id);
          if (found) {
    ...
    if (targetId === 0) {
      ...
      const targetInfo = findParentAndÎ≤àÌò∏(newTree, targetId);
      if (targetInfo) {
    ...
    setDropTargetId(null);
    setDropPosition(null);
    toast('Î©îÎâ¥ Íµ¨Ï°∞Í∞Ä Î≥ÄÍ≤ΩÎêò?àÏäµ?àÎã§. ?∏Î¶¨ ?πÏÖò ?ÅÎã®??[?àÏù¥?ÑÏõÉ ?ÅÏö©] Î≤ÑÌäº???¥Î¶≠?òÏó¨ ?Ä?•Ìï¥ Ï£ºÏÑ∏??', 'info');
  };
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="?§ÎπÑÍ≤åÏù¥???ïÎ≥¥ ?ÑÌÇ§?çÏ≤ò"
        breadcrumbs={[{ label: '?úÏä§?úÍ?Î¶? }, { label: 'Î©îÎâ¥ Í¥ÄÎ¶? }]}
      />

      <HubHeader
        title="Î©îÎâ¥"
        highlight="?ÑÌÇ§?çÏ≤ò"
        subtitle="?úÏä§???ÑÎ∞ò??Í≥ÑÏ∏µ??Î©îÎâ¥ Íµ¨Ï°∞ ?§Í≥Ñ?Ä Í∂åÌïú Í∏∞Î∞ò ?§ÎπÑÍ≤åÏù¥???∏Î¶¨ Í¥ÄÎ¶?
        icon={FolderTree}
        actions={
      ...
      <HubMetricGrid>
        <HubMetricCard title="?±Î°ù???∏Îìú_?? value={(initialMenus || []).length} icon={Database} color="primary" />
        <HubMetricCard title="Í≥ÑÏ∏µ_ÍπäÏù¥" value={3} icon={LayoutGrid} color="indigo" />
        <HubMetricCard title="?úÏÑ±_Í≤ΩÎ°ú_?? value={(initialMenus || []).filter(m => !!m?.modernRoute).length} icon={Network} color="emerald" />
        <HubMetricCard title="?ôÍ∏∞??Î¨¥Í≤∞?? value="ÏµúÏ†Å" icon={ShieldCheck} color="amber" />
      </HubMetricGrid>

      <HubSectionCard
        title="?úÏä§???§ÎπÑÍ≤åÏù¥???∏Î¶¨"
        description="ÏµúÎ? 3?®Í≥Ñ??Í≥ÑÏ∏µ Íµ¨Ï°∞Î•?ÏßÄ?êÌï©?àÎã§. ?úÎûòÍ∑????úÎ°≠?ºÎ°ú Î©îÎâ¥ Íµ¨Ï°∞Î•??§Í≥Ñ?òÏã≠?úÏò§."
        icon={SearchCode}
        action={
          <div className="flex gap-4 items-center">
            <div className="flex bg-white/10 backdrop-blur-md p-1 rounded-xl border border-white/10">
              ...
              >
                <ChevronsUpDown size={14} className="mr-2" /> ?ºÏπòÍ∏?              </Button>
              <Button
                variant="ghost"
                onClick={handleCollapseAll}
                className="h-10 px-3 text-[10px] font-black tracking-widest uppercase hover:bg-white/20 text-white/70 hover:text-white"
              >
                <ChevronsDownUp size={14} className="mr-2" /> ?ëÍ∏∞
              </Button>
            </div>
        ...
        <div className="space-y-6 px-4">
          ...
          >
            ...
            <div className="text-center space-y-2">
              <p className="text-[10px] font-black text-muted-foreground/40 tracking-[0.4em] uppercase">?ÑÌÇ§?çÏ≤ò ?¥ÏÖàÎ∏îÎ¶¨</p>
              <p className="text-sm font-black text-muted-foreground tracking-tight group-hover:text-foreground transition-colors uppercase">?úÎûòÍ∑∏Ìïú ?îÏÜåÎ•??¨Í∏∞???ìÏúºÎ©?'ÏµúÏÉÅ?? ?∏ÎìúÎ°??ÑÎ°úÎ™®ÏÖò?©Îãà??/p>
            </div>
          </div>
        </div>
      </HubSectionCard>
      ...
      <StandardModal
        ...
        title={mode === 'create' ? '?†Í∑ú ?§ÎπÑÍ≤åÏù¥???∏Îìú ?§Í≥Ñ' : 'Î©îÎâ¥ ?∏Îìú Íµ¨ÏÑ± ?çÏÑ± ?òÏ†ï'}
        maxWidth="2xl"
        footer={
      ...
      >
        <div className="space-y-8 pt-4">
          <FormField label="Î©îÎâ¥ Î™ÖÏπ≠" required description="?¨Ïö©???∏ÌÑ∞?òÏù¥?§Ïóê ?∏Ï∂ú???ºÎ≤®?ÖÎãà??">
            <Input
              ...
              placeholder="Î©îÎâ¥ ?¥Î¶Ñ ?ÖÎ†• (?? ?¨Ïö©??Í¥ÄÎ¶?"
            />
          </FormField>

          <div className="grid grid-cols-2 gap-8">
            <FormField label="?ÅÏúÑ ?∏Îìú ?ùÎ≥Ñ??>
              <div className="h-14 rounded-2xl border-2 border-slate-100 flex items-center px-6 text-[10px] font-black tracking-widest uppercase bg-slate-50/50 text-muted-foreground/60 italic overflow-hidden shadow-inner">
                {formData.upperMenuId === 0 ? 'SYSTEM_ROOT (ÏµúÏÉÅ??' : `PARENT_NODE_${formData.upperMenuId}`}
              </div>
            </FormField>

            <FormField label="?úÏó¥ ?úÏÑú (?∞ÏÑ†?úÏúÑ)" required>
              <Input
                type="number"
          ...
          <FormField label="?∞Îèô ?åÌîÑ?∏Ïõ®???êÏÇ∞ (?Ä??Î™®Îìà)">
            <Select
            ...
            >
              <SelectTrigger className="h-14 rounded-2xl border-2 border-slate-100 bg-slate-50 font-black text-[10px] tracking-widest uppercase focus:ring-4 focus:ring-primary/10 transition-all shadow-inner">
                <SelectValue placeholder="--- UNLINKED (?∞Îèô?òÏ? ?äÏùå) ---" />
              </SelectTrigger>
          ...
          <FormField label="?ºÏö∞???îÎìú?¨Ïù∏??(Í≤ΩÎ°ú)" description="?ÑÏûê?ïÎ? ?úÏ? ?ÑÎ†à?ÑÏõå??Í∏∞Î∞ò ?ÑÎ°†?∏Ïóî???ºÏö∞??Í≤ΩÎ°ú">
          ...
          <FormField label="?∏Îìú ?ÅÏÑ∏ Î©îÌ??∞Ïù¥??>
            <textarea
              ...
              placeholder="Î©îÎâ¥???Ä???ÅÏÑ∏ ?§Î™Ö Î∞?Ï£ºÏÑù"
            />
          </FormField>
        </div>
      </StandardModal>

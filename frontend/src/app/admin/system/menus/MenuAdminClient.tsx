...
export default function MenuAdminClient({ initialMenus, programs }: { initialMenus: MenuInfo[]; programs: any[] }) {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      toast('순서 변경 중 예기치 못한 오류가 발생했습니다.', 'error');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    const findParentAndIndex = (items: MenuInfo[], id: number): { parent: MenuInfo[], index: number, parentItem: MenuInfo | null } | null => {
      for (let i = 0; i < items.length; i++) {
        ...
        if (items[i].children) {
          ...
          if (found) {
            if (found.parentItem === null) return { ...found, parentItem: items[i] };
            return found;
          }
        }
      }
  ...
  const MenuNode = ({ item, level = 0 }: { item: MenuInfo; level: number }) => {
    ...
    return (
      <div
        draggable={!isSaving}
        onDragStart={(e) => { if (isSaving) return; handleDragStart(e, item.menuNo); }}
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-5 relative z-10">
            ...
            <div className="flex flex-col">
              <span className={cn("font-black tracking-tighter text-md", level === 0 ? "text-foreground" : "text-foreground/80")}>
                {item.menuNm}
              </span>
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      ...
      <HubHeader
        title="메뉴"
        highlight="아키텍처"
        subtitle="시스템 전반의 계층형 메뉴 구조 설계 및 권한 기반 네비게이션 트리 관리"
        icon={FolderTree}
        actions={
      ...
      <HubMetricGrid>
        <HubMetricCard title="등록된 노드 수" value={(initialMenus || []).length} icon={Database} color="primary" />
        <HubMetricCard title="계층의 깊이" value={3} icon={Layers} color="indigo" />
        <HubMetricCard title="활성 경로 수" value={(initialMenus || []).filter(m => !!m?.modernRoute).length} icon={Network} color="emerald" />
        <HubMetricCard title="동기화 무결성" value="최적" icon={ShieldCheck} color="amber" />
      </HubMetricGrid>

      <HubSectionCard
        title="시스템 네비게이션 트리"
        description="최대 3단계의 계층 구조를 지원합니다. 드래그앤드롭으로 메뉴 구조를 설계하십시오."
        icon={SearchCode}
        action={
        ...
        <div className="space-y-6 px-4">
          ...
          >
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-sm font-black text-muted-foreground tracking-tight group-hover:text-foreground transition-colors uppercase">드래그한 요소를 여기에 놓으면 '최상단' 노드로 이동합니다</p>
            </div>
          </div>
        </div>
      </HubSectionCard>

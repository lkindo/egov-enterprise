...
export default function MenuByAuthorityPage() {
  ...
  const { data: authorData } = useQuery({
    ...
    queryFn: () => authorAdminService.getAuthorList({ pageÎ≤àÌò∏: 1, searchCondition: '1', searchKeyword: '' } as any),
    staleTime: 5 * 60 * 1000,
  });
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="Í∂åÌïú Í∏∞Î∞ò ?¥ÎπÑÍ≤åÏù¥???†Ìè¥Î°úÏ?"
        breadcrumbs={[{ label: '?úÏä§?úÍ?Î¶? }, { label: 'Î©îÎâ¥Í¥ÄÎ¶? }, { label: 'Í∂åÌïúÎ≥?Î©îÎâ¥' }]}
      />

      <HubHeader 
        title="Í≥ÑÏ∏µ Íµ¨Ï°∞" 
        highlight="Í∞êÏÇ¨" 
        subtitle="?πÏ†ï Î≥¥Ïïà ??ï†(Role)???†Îãπ??Í∏∞Îä•???∏Îìú Í≥ÑÏ∏µ Íµ¨Ï°∞ Î∞??ëÍ∑º Í≤ΩÎ°ú Îß§Ìä∏Î¶?ä§ ?úÍ∞Å?? 
        icon={Workflow} 
        actions={
          <div className="flex gap-4 p-2 items-center">
            ...
            <Button className="h-14 px-10 rounded-2xl bg-slate-900 border-none text-white font-black text-[11px] tracking-widest uppercase shadow-2xl hover:bg-primary transition-all hover:-translate-y-1 gap-3 group">
                <ShieldCheck size={20} className="group-hover:scale-110 transition-transform duration-500" /> ??ï† ?∏Î≤§?†Î¶¨ Í¥ÄÎ¶?            </Button>
          </div>
        }
      />

      <HubMetricGrid>
        <HubMetricCard title="?úÏÑ±_?§ÌÇ§Îß??? value={authorities.length} icon={Database} color="primary" />
        <HubMetricCard title="Î≤îÏúÑ_???∏Îìú_?? value={rawMenus.length} icon={LayoutGrid} color="amber" />
        <HubMetricCard title="Í≥ÑÏ∏µ_ÍπäÏù¥" value={selectedAuthority ? "?πÌÑ∞_Ï§ÄÎπ? : "?ÄÍ∏?} icon={Compass} color="indigo" />
        <HubMetricCard title="Î≥¥Ïïà_?ÅÌÉú" value="ÏµúÏ†Å" icon={Lock} color="emerald" status="?ôÍ∏∞?îÎê®" />
      </HubMetricGrid>

      <div className="grid grid-cols-12 gap-12">
        {/* Authority Selection Sidebar */}
        <div className="col-span-12 lg:col-span-4 h-full space-y-8">
            <HubSectionCard title="??ï† ?ùÎ≥Ñ ?∞Ïù¥???†ÌÉù" description="Î∂ÑÏÑù??Î≥¥Ïïà Ïª®ÌÖç?§Ìä∏ ?êÎäî ?úÏä§??Í∑∏Î£π Í∂åÌïú???ùÎ≥Ñ?òÏÑ∏??" icon={Lock}>
                <div className="space-y-8">
                    <div className="space-y-4 pt-4">
                        <label className="text-[10px] font-black text-muted-foreground/40 tracking-[0.4em] uppercase ml-2">Î≥¥Ïïà ??ï† ?¨Ïñë (Access Role)</label>
                        <Select value={selectedAuthority} onValueChange={setSelectedAuthority}>
                            <SelectTrigger className="h-16 px-8 rounded-2xl bg-slate-50/50 border-none shadow-inner text-sm font-black tracking-tight focus:ring-4 focus:ring-primary/10 transition-all group active:scale-[0.98]">
                                <div className="flex items-center gap-4">
                                     ...
                                     <SelectValue placeholder="Î≥¥Ïïà ??ï†???†ÌÉù?òÏã≠?úÏò§..." />
                                </div>
                            </SelectTrigger>
                    ...
                    <div className="p-8 rounded-[2.5rem] bg-slate-900 text-white relative overflow-hidden group border-none shadow-2xl min-h-[300px] flex flex-col justify-end">
                        ...
                        <div className="relative z-10 space-y-6">
                            ...
                            <div className="space-y-3">
                                <h4 className="text-2xl font-black tracking-tighter leading-tight uppercase">?†Ìè¥Î°úÏ?<br />?∏ÌÖîÎ¶¨Ï†Ñ??/h4>
                                <p className="text-[9px] text-white/40 font-black tracking-[0.3em] uppercase leading-relaxed font-mono">?§ÏãúÍ∞?Í∂åÌïú ?§Ìä∏Î¶??úÏÑ±????/p>
                            </div>
                            {selectedAuthority && (
        ...
        <div className="col-span-12 lg:col-span-8 h-full">
            <HubSectionCard 
                title={currentAuth ? `[${currentAuth.authorNm}] ?¥ÎπÑÍ≤åÏù¥???ÑÌÇ§?çÏ≤ò` : "?ÑÌÇ§?çÏ≤ò ?†Ìè¥Î°úÏ? Î∂ÑÏÑù"} 
                description="?†Îãπ??Î™®Îì† ?úÏä§??Í∏∞Îä•Í≥??∞Ïù¥??ÏßÑÏûÖ?êÏóê ?Ä???ÑÍ≥Ñ??Ï≤?Ç¨ÏßÑÏûÖ?àÎã§." 
                icon={Network}
            >
                <div className="space-y-8">
                    <div className="flex items-center justify-between px-2 pt-2 border-b border-slate-100 pb-8">
                        <div>
                            <span className="text-[10px] font-black text-muted-foreground/30 tracking-[0.4em] uppercase font-mono italic">Í∏∞Îä• ?∏Îìú ?∏Î¶¨ ?§Ìä∏Î¶?(Functional Node Tree)</span>
                        </div>
                        <div className="flex items-center gap-4">
                             ...
                             <Button variant="ghost" size="sm" className="h-12 rounded-2xl px-6 text-[10px] font-black tracking-widest gap-2 hover:bg-slate-900 hover:text-white bg-slate-50 border border-slate-100 transition-all uppercase group shadow-sm">
                                 <SearchCode size={16} className="group-hover:rotate-12 transition-transform" /> ?∏Îìú Î∂ÑÏÑù
                            </Button>
                        </div>
                    </div>
                         ...
                         <AnimatePresence mode="wait">
                            {!selectedAuthority ? (
                                ...
                                >
                                    ...
                                    <h3 className="text-2xl font-black text-slate-300 tracking-tighter uppercase mb-4">??ï† ÎØ∏ÏÑ†??/h3>
                                    <p className="text-[10px] font-black text-slate-200 tracking-[0.5em] uppercase max-w-[240px] leading-relaxed">??ï†???ùÎ≥Ñ?òÏó¨ ?úÏä§???ÑÍ≥Ñ ?∞Ïù¥?∞Ïùò ?úÍ∞Å??Î∂ÑÏÑù???úÏûë?òÏã≠?úÏò§.</p>
                                </motion.div>
                            ) : isMenuLoading ? (
                                <motion.div 
                                    initial={{ opacity: 0 }} 
                                ...
                                >
                                    ...
                                    <span className="text-[11px] font-black text-muted-foreground/40 tracking-[0.4em] uppercase">?†Ìè¥Î°úÏ? Îß§Ìïë Ï§?..</span>
                                </motion.div>
                            ) : menuTree.length === 0 ? (
                                <motion.div 
                                    initial={{ opacity: 0 }} 
                                ...
                                >
                                    ...
                                    <div className="space-y-2 text-center">
                                        <h4 className="text-lg font-black tracking-tighter text-slate-400 uppercase">?úÏÑ± ?∏Îìú ?ÜÏùå</h4>
                                        <p className="text-[9px] font-black text-slate-300 tracking-[0.3em] uppercase">?¥Îãπ Í∂åÌïú???†Îãπ??Í∏∞Îä•???îÎìú?¨Ïù∏?∏Í? ?ùÎ≥Ñ?òÏ? ?äÏïò?µÎãà??</p>
                                    </div>
                                </motion.div>
                            ) : (
                                <motion.div 

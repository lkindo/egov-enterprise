...
const SystemLogAdminPage = () => {
    ...
    const { data, isLoading } = useQuery<PageResponse<SysLog>>({
        ...
        queryFn: () => systemLogAdminService.getSystemLogs({ 
            page: (Number(params.pageÎ≤àÌò∏) || 1) - 1, 
            size: params.size,
    ...
    return (
        <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
            <PageHeader title="?úÏä§??Î°úÍ∑∏" breadcrumbs={[{ label: '?úÏä§?úÍ?Î¶? }, { label: 'Î°úÍ∑∏Í¥ÄÎ¶? }, { label: '?úÏä§??Î°úÍ∑∏' }]} />

            <HubHeader title="?úÏä§???∏ÏÇ¨?¥Ìä∏" highlight="?úÏä§??Î°úÍ∑∏" subtitle="?úÎ≤Ñ???§ÏãúÍ∞??ôÏûë ?ÅÌÉú?Ä Î™®ÎìàÎ≥??§Ìñâ ?¥Î†•??Î™ÖÌôï?òÍ≤å Ï∂îÏ†Å?©Îãà??" icon={Activity} 
                actions={
                    <div className="flex gap-4 p-2 items-center">
                        <Button variant="outline" size="lg" className="h-12 rounded-xl border-2 font-black text-[10px] tracking-widest uppercase gap-2">?§ÏãúÍ∞?Î™®Îãà?∞ÎßÅ</Button>
                    </div>
                }
            />
            ...
            <StandardDataTable
                ...
                loading={isLoading}
                pagination={{
                    currentPage: (params.pageÎ≤àÌò∏ || 1) as number,
                    totalPages: data?.totalPage || pagination?.totalPageCount || 1,
                    onPageChange: (page: number) => setParams({ ...params, pageÎ≤àÌò∏: page }),
                }}
                search={{
                    placeholder: '?úÎπÑ?§Î™Ö, ?îÏ≤≠ID Í≤Ä??..',
                    onSearch: (keyword: string) => setParams({ ...params, searchKeyword: keyword, pageÎ≤àÌò∏: 1 }),
                }}
            />
        </div>
    );

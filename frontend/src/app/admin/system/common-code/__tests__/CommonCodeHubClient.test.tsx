...
describe('CommonCodeHubClient tabs', () => {
  ...
  it('uses roving tab focus and restores focus across STANDARD and A1 branch remounts', async () => {
    ...
    const institution = screen.getByRole('tab', { name: '기관 코드' });
    ...
    expect(within(standardPanel).queryByText('기관 코드 목록')).not.toBeInTheDocument();
    ...
    expect(within(screen.getByRole('tabpanel')).queryByText('기관 코드 목록')).not.toBeInTheDocument();
    ...
    view.rerenderHub();
    await waitFor(() => expect(screen.getByRole('tab', { name: '기관 코드' })).toHaveFocus());
    expect(screen.getByRole('tabpanel')).toHaveAttribute('aria-labelledby', 'code-hub-tab-institution');
    expect(within(screen.getByRole('tabpanel')).getByText('기관 코드 목록')).toBeVisible();
    ...
    fireEvent.keyDown(screen.getByRole('tab', { name: '기관 코드' }), { key: 'Home' });
    view.rerenderHub();

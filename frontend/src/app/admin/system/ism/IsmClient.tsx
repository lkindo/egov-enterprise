...
export default function IsmClient({ initialData }: { initialData: { content: InfrmlSanctn[] } }) {
 ...
 <span className="px-3 py-1.5 bg-slate-100 text-slate-900 rounded-xl text-[10px] font-black italic tracking-tight border border-slate-200 shadow-inner">
 ...
 <span className="font-black italic tracking-tighter text-slate-900 text-lg leading-tight group-hover/item:text-primary transition-colors">{item.sancltNm}</span>
 <div className="flex items-center gap-2">
 <span className="text-[9px] font-mono font-black text-slate-400 tracking-tight opacity-60 italic">결재 ID: {item.infrmlSanctnId}</span>
 </div>
 </div>
 )
 },
 ...
 <span className="text-sm font-black text-slate-700 italic tabular-nums">{item.applcntId}</span>
 <span className="text-[9px] font-bold text-slate-300 tracking-tight italic opacity-60">검색된 신청자</span>
 </div>
 </div>
 )
 },
 ...
 className="h-11 px-8 bg-slate-900 text-white rounded-[1.25rem] text-[10px] font-black italic tracking-tight hover:bg-primary transition-all active:scale-95 shadow-xl shadow-slate-900/10 flex items-center gap-3 group"
 >
 <ShieldCheck size={16} strokeWidth={3} className="group-hover:rotate-12 transition-transform" /> 결재 승인
 </button>
 )}
 <button
 ...
 <span className="text-[10px] font-black text-emerald-700 italic tracking-tight">로직 허브: 정상</span>
 </div>
 </div>
 }
 />
 ...
 <h4 className="text-4xl font-black italic tracking-tighter tabular-nums">약식 결재 보안 체계</h4>
 ...
 />
 <div className="flex justify-center items-center gap-12 mt-12 text-[10px] font-black italic text-slate-300 tracking-[0.4em] opacity-40">
 <div className="flex items-center gap-3">
 ...
 <div className="text-[10px] text-primary/60 font-black tracking-[0.4em] italic mb-3">결재 대상 시퀀스</div>
 <h4 className="text-2xl font-black text-white italic tracking-tighter">{selectedSanctn?.sancltNm}</h4>
 ...
 <label className="text-[10px] font-black tracking-[0.4em] italic text-slate-400 px-2 flex items-center gap-2">
 <Activity size={12} /> 승인/반려 의견 <span className="text-rose-500 font-black">*</span>
 </label>
 <textarea
 value={returnResn}
 ...
 <button onClick={() => setIsOpen(false)} className="flex-1 h-16 border-2 border-slate-100 rounded-2xl font-black text-[10px] tracking-[0.3em] hover:bg-slate-50 transition-all opacity-40 hover:opacity-100">취소</button>
 <button
 onClick={() => handleProcess('R')}
 className="flex-1 h-16 bg-rose-50 text-rose-500 rounded-2xl font-black text-[10px] tracking-[0.3em] hover:bg-rose-100 transition-all active:scale-95 border-2 border-rose-100/50 flex items-center justify-center gap-3"
 >
 <XCircle size={18} strokeWidth={3} /> 반려 처리
 </button>
 <button
 onClick={() => handleProcess('C')}
 className="flex-[2] h-16 bg-slate-900 text-white rounded-2xl font-black italic text-[10px] tracking-[0.3em] shadow-2xl shadow-slate-900/40 flex items-center justify-center gap-3 hover:-translate-y-1 transition-all active:scale-95 border border-white/10 group"
 >
 <CheckCircle2 size={18} strokeWidth={3} className="text-primary group-hover:rotate-12 transition-transform" /> 결재 승인
 </button>
 </div>
 </div>
 </StandardModal>
...
function StatusCard({ title, count, icon, color }: any) {
 ...
 <p className="text-[10px] font-black tracking-[0.4em] opacity-40 mb-2 italic font-mono">{title}</p>
 <h4 className="text-5xl font-black tracking-tighter tabular-nums italic leading-none">{count} 건</h4>
 </div>

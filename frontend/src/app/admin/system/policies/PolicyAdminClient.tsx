...
export default function PolicyAdminClient() {
  ...
  const fetchPolicies = async () => {
    ...
    try {
      ...
      toast.error('?•ì±… ëª©ë¡??ë¶ˆëŸ¬?¤ëŠ” ???¤íŒ¨?ˆìŠµ?ˆë‹¤.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('?•ì±…???±ê³µ?ìœ¼ë¡??˜ì •?˜ì—ˆ?µë‹ˆ??');
  ...
  const columns: Column<SystemPolicy>[] = [
    ...
    },
    {
      ...
      accessor: (item) => (
        ...
        >
          <Edit2 size={14} className="mr-2" /> ?˜ì •
        </Button>
      )
    }
  ];
      ...
      <div className="hub-table-container">
        <div className="flex items-center justify-between mb-8 px-4">
          <div className="space-y-1">
            <h3 className="text-xl font-black tracking-tight">?œë¹„???•ì±… ëª©ë¡</h3>
            <p className="text-sm text-muted-foreground">ë¡œê·¸?? ê°œì¸?•ë³´ì²˜ë°©ì¹? ?€?‘ê¶Œ ??ê²Œì‹œ???¸ì˜ ?œìŠ¤???•ì±…??ê´€ë¦¬í•©?ˆë‹¤.</p>
          </div>
        ...
        <DialogContent className="max-w-5xl rounded-[3rem] overflow-hidden border-none shadow-2xl p-0">
          <div className="bg-slate-900 p-8 text-white flex items-center justify-between">
            <DialogHeader>
              <DialogTitle className="text-2xl font-black flex items-center gap-3">
                <Edit2 className="text-primary" /> ?•ì±… ?˜ì • : <span className="opacity-50 tracking-widest uppercase">{selectedPolicy?.id || selectedPolicy?.type}</span>
              </DialogTitle>
            </DialogHeader>
            <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-full text-xs font-bold tracking-widest uppercase">
              <CheckCircle2 size={14} className="text-primary" /> ?¤ì‹œê°??€??ëª¨ë“œ
            </div>
          </div>

          <div className="p-10 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
            <div className="space-y-3">
              <Label className="text-sm font-black tracking-widest uppercase opacity-40 ml-2">?•ì±… ?œëª©</Label>
              <Input 
                value={editTitle} 
                onChange={(e) => setEditTitle(e.target.value)} 
                placeholder="?•ì±… ?œëª©???…ë ¥?˜ì„¸??
                className="h-14 rounded-2xl border-2 border-border/50 focus:border-primary/50 bg-slate-50/50"
              />
            </div>

            <div className="space-y-3">
              <Label className="text-sm font-black tracking-widest uppercase opacity-40 ml-2">?•ì±… ?´ìš©</Label>
              <RichTextEditor 
                value={editContent} 
          ...
          <DialogFooter className="p-8 bg-slate-50 border-t border-border/50 flex items-center justify-between">
             <div className="text-xs text-muted-foreground font-medium italic">
                * ?˜ì • ì¦‰ì‹œ ?„ë¡ ?¸ì—”???¸í„° ë°??•ì±… ?˜ì´ì§€??ë°˜ì˜?©ë‹ˆ??
             </div>
             <div className="flex gap-3">
                <Button variant="ghost" onClick={() => setIsEditModalOpen(false)} className="rounded-xl h-12 px-8">ì·¨ì†Œ</Button>
                <Button 
                  onClick={handleSave} 
                  disabled={isSaving}
                  className="rounded-xl h-12 px-8 bg-primary hover:bg-primary-dark transition-all shadow-lg shadow-primary/20"
                >
                  {isSaving ? '?€??ì¤?..' : 'ë³€ê²½ì‚¬???€?¥í•˜ê¸?}
                </Button>
             </div>
          </DialogFooter>
        </DialogContent>

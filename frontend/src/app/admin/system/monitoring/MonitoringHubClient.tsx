...
export default function MonitoringHubClient({ defaultTab = 'SECURITY' }: { defaultTab?: MonitoringTab }) {
  ...
  // URL ÏøºÎ¶¨ ?åÎùºÎØ∏ÌÑ∞?êÏÑú ???ïÎ≥¥Î•?Í∞Ä?∏Ïò§Í±∞ÎÇò Í∏∞Î≥∏Í∞íÏùÑ ?¨Ïö©?©Îãà?? (Îß§Ìïë Î≥¥Í∞ï)
      ...
      <div className="rounded-[3rem] p-12 bg-slate-900 text-white shadow-2xl relative overflow-hidden group border-none">
        ...
        <div className="relative z-10 space-y-12">
          <div className="flex items-center gap-6">
            ...
            <h3 className="text-3xl font-black tracking-tighter uppercase leading-none italic">ÏΩîÏñ¥ ?îÏßÑ: ÏµúÏ†Å ?ÅÌÉú</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <StatusIndicator label="API Microservices" status="?®Îùº?? icon={Network} />
            <StatusIndicator label="PostgreSQL Cluster" status="?ôÍ∏∞?îÎê®" icon={Database} />
        ...
        <div className="col-span-12 lg:col-span-3 space-y-8 h-fit lg:sticky lg:top-8">
          ...
          <div className="bg-slate-900 text-white rounded-[3rem] p-10 space-y-6 text-center shadow-2xl relative overflow-hidden flex flex-col items-center">
            ...
            <div className="space-y-2">
                <h4 className="text-xl font-black tracking-tighter uppercase">Í∞êÏÇ¨ ?ÑÎ°ú?†ÏΩú</h4>
                <p className="text-[9px] font-black text-white/30 tracking-[0.4em] uppercase">Î≥¥Ïïà ?òÏ?: ÏµúÏÉÅ</p>
            </div>
          ...
          <div className="rounded-[3.5rem] bg-white border-2 border-slate-100 shadow-2xl flex-1 flex flex-col p-12 space-y-10 relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-slate-100 pb-8 relative z-10">
              <div className="space-y-1">
                <h3 className="text-[10px] font-black text-muted-foreground/40 tracking-[0.4em] uppercase">?ºÌã∞???§Ìä∏Î¶?/h3>
                <p className="text-2xl font-black tracking-tighter text-foreground uppercase">?∞Ïù¥???∏Î≤†?§Ìã∞Í≤åÏù¥??/p>
              </div>
              <Button 
                variant="ghost" 
                size="icon" 
          ...
          <AnimatePresence mode="wait">
            {selectedItemId ? (
                ...
                <div className="rounded-[3.5rem] bg-white border-2 border-slate-900 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] h-full p-14 space-y-12 flex flex-col relative overflow-hidden">
                  <div className="border-b border-slate-100 pb-12 relative z-10">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="text-[10px] font-black text-muted-foreground/40 tracking-[0.5em] uppercase">?∏Ïä§?¥Ïä§ Î©îÌ??∞Ïù¥??/h3>
                    </div>
                    <h2 className="text-4xl font-black text-foreground tracking-tighter leading-none mb-4 uppercase">
                        Í∞ùÏ≤¥ ?ÅÏÑ∏ Î∂ÑÏÑù
                    </h2>
                    <p className="text-xs font-mono font-black text-primary/60 tracking-widest uppercase">Î°úÍ∑∏ Í≥†Ïú† ?ùÎ≥Ñ?? {selectedItemId}</p>
                  </div>
                  ...
                  <div className="pt-12 mt-auto border-t border-slate-100 space-y-8 relative z-10">
                    <div className="flex items-center justify-between px-6">
                       <span className="text-[10px] font-black text-muted-foreground/30 tracking-[0.4em] uppercase">Í≤∞Ï†ï Îß§Ìä∏Î¶?ä§</span>
              ...
              <div className="h-full flex flex-col items-center justify-center p-20 text-center opacity-40 select-none grayscale rounded-[3.5rem] border-4 border-dashed border-slate-100 bg-slate-50/50 group transition-all hover:bg-white hover:border-primary/20 duration-1000">
                ...
                <h3 className="text-4xl font-black text-foreground tracking-tighter uppercase mb-4">?∏ÌÖîÎ¶¨Ï†Ñ???ÄÍ∏?Ï§?/h3>
                <p className="text-[10px] font-black text-muted-foreground/30 tracking-[0.6em] uppercase leading-relaxed max-w-xs">Î∂ÑÏÑù??Î°úÍ∑∏ Í∞ùÏ≤¥Î•??§Ìä∏Î¶ºÏóê??Ï∫°Ï≤ò?òÏã≠?úÏò§</p>
              </div>
            )}
          </AnimatePresence>
        </div>
         ...
         <div className="p-10 space-y-10 font-sans">
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-1 gap-4">
                  <ReportOption icon={<FileText size={20} />} title="Executive Overview" description="?úÏä§???úÎèô Î∞?Î≥¥Ïïà ÏßÄ???µÌï© ?îÏïΩ (PDF)" />
                  <ReportOption icon={<Activity size={20} />} title="Infrastructure Metrics" description="Î¶¨ÏÜå???êÏú†??Î∞??±Îä• Ï∂îÏù¥ ?∞Ïù¥??(XLSX)" />
                  <ReportOption icon={<ShieldAlert size={20} />} title="Security Audit Log" description="?ÑÏ≤¥ Î≥¥Ïïà Í∞êÏÇ¨ Î∞??∏Ï¶ù ?úÎèÑ Í∏∞Î°ù (JSON)" />
               </div>
            </div>
            ...
            <div className="p-8 bg-slate-50 rounded-[2.5rem] border-2 border-slate-100 space-y-4">
               ...
               <p className="text-[10px] text-slate-400 font-medium">?∞Ïù¥???òÏßë Î∞??ïÌï©??Í≤ÄÏ¶ùÏù¥ Î∞±Í∑∏?ºÏö¥?úÏóê???§Ìñâ?©Îãà??</p>
            </div>

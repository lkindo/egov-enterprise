...
const TopologyMap = dynamic(() => import('@/app/components/ui/topology-map').then(mod => mod.TopologyMap), {
  ...
  loading: () => (
    <div className="w-full h-[700px] flex flex-col items-center justify-center bg-slate-950 rounded-lg space-y-6">
      <div className="w-16 h-12 border-4 border-primary/20 border-t-primary rounded-full animate-spin" />
...
export default function MonitoringHubClient({ defaultTab = 'SECURITY' }: { defaultTab?: MonitoringTab }) {
  ...
  const deleteCommentMutation = useMutation({
    ...
    onSuccess: () => {
      toast('?ìÍ????±Í≥µ?ÅÏúºÎ°???†ú?òÏóà?µÎãà??', 'success');
      ...
      accessor: (log) => (
        <div className="flex items-center gap-5 py-2">
          ...
          <div className="space-y-0.5">
            ...
            <h4 className={cn("text-sm font-bold tracking-tight truncate max-w-[280px]", selectedItemId === log.requstId ? "text-white" : "text-foreground")}>{log.methodNm}</h4>
          </div>
        </div>
      )
    }
      ...
      accessor: (log) => (
        <div className="flex items-center gap-5 py-2">
          ...
          <div className="space-y-0.5">
            ...
            <h4 className={cn("text-sm font-bold tracking-tight truncate max-w-[280px]", selectedItemId === log.logId ? "text-white" : "text-foreground")}>{log.loginMthd}</h4>
          </div>
        </div>
      )
    }
      ...
      accessor: (c) => (
        <div className="flex items-center gap-5 py-2 w-full pr-4">
          ...
          <div className="flex-1 space-y-0.5 min-w-0">
            <h4 className={cn("text-sm font-bold tracking-tight truncate", selectedItemId === c.commentNo ? "text-white" : "text-foreground")}>{c.commentCn}</h4>
  ...
  const renderObservability = () => (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <SystemStatusRadar 
         ...
         data={[
            { subject: 'Í∞Ä?©ÏÑ±', A: healthData?.status === 'UP' ? 100 : 0 },
            { subject: 'Î≥¥Ïïà??, A: 95 },
            { subject: '?ëÎãµ?çÎèÑ', A: 88 },
            { subject: 'Î¨¥Í≤∞??, A: 100 },
            { subject: '?ïÏû•??, A: 75 },
            { subject: '?àÏ†ï??, A: 92 },
         ]}
        ...
        <div className="relative z-10 space-y-12">
          <div className="flex items-center gap-6">
            ...
            <h3 className="text-3xl font-bold tracking-tight uppercase leading-none">
              ÏΩîÏñ¥ ?îÏßÑ: {healthData?.status === 'UP' ? 'ÏµúÏ†Å ?ÅÌÉú' : '?êÍ? ?ÑÏöî'}
            </h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            ...
            <StatusIndicator label="Redis Cache Fabric" status="?àÏ†ï" icon={CheckCircle2} />
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="?úÏä§???∏ÌÖîÎ¶¨Ï†Ñ??Í±∞Î≤Ñ?åÏä§"
        breadcrumbs={[{ label: '?úÏä§?úÍ?Î¶? }, { label: 'Î™®Îãà?∞ÎßÅ ?àÎ∏å' }]}
      />

      <HubHeader 
        title="Auditing" 
        highlight="Intelligence" 
        subtitle="?ÑÏÇ¨ ?∏ÌîÑ??Î°úÍπÖ ?ÑÎ°ú?†ÏΩú Î∞??§ÏãúÍ∞??∞Ïù¥??Î¨¥Í≤∞??Í¥ÄÏ∞??úÏä§?? 
        icon={Activity} 
        actions={
          <div className="flex gap-4 p-2">
            ...
            >
              <Download size={18} className="group-hover:translate-y-0.5 transition-transform" /> Î¶¨Ìè¨???§ÎÉÖ??            </Button>
      ...
      <div className="grid grid-cols-12 gap-12 px-2 min-h-[900px]">
        ...
        <div className="col-span-12 lg:col-span-3 space-y-8 h-fit lg:sticky lg:top-8">
          <div className="rounded-lg p-4 bg-white/40 backdrop-blur-xl border-2 border-slate-100 shadow-xl space-y-3">
            <NavButton icon={<ShieldAlert size={22} />} label="Î≥¥Ïïà Í∞êÏÇ¨ Îß§Ìä∏Î¶?ä§" active={activeTab === 'SECURITY'} onClick={() => { setActiveTab('SECURITY'); setSelectedItemId(null); }} />
            <NavButton icon={<Terminal size={22} />} label="?úÏä§??Î°úÍ∑∏ ?îÏßÑ" active={activeTab === 'SYSTEM'} onClick={() => { setActiveTab('SYSTEM'); setSelectedItemId(null); }} />
            <NavButton icon={<LogIn size={22} />} label="?∏Ï¶ù ?ëÏÜç ?àÏä§?†Î¶¨" active={activeTab === 'LOGIN'} onClick={() => { setActiveTab('LOGIN'); setSelectedItemId(null); }} />
            <NavButton icon={<MonitorCheck size={22} />} label="?∏ÌîÑ??Í∞Ä?ôÏÑ± ?ïÎ≥¥" active={activeTab === 'OBSERVABILITY'} onClick={() => { setActiveTab('OBSERVABILITY'); setSelectedItemId(null); }} />
            <NavButton icon={<Share2 size={22} />} label="?∏ÌîÑ???†Ìè¥Î°úÏ? Îß? active={activeTab === 'TOPOLOGY'} onClick={() => { setActiveTab('TOPOLOGY'); setSelectedItemId(null); }} />
            <NavButton icon={<MessageSquare size={22} />} label="?úÎπÑ???ºÎìúÎ∞?Í¥ÄÎ¶? active={activeTab === 'COMMENTS'} onClick={() => { setActiveTab('COMMENTS'); setSelectedItemId(null); }} />
          </div>
          ...
          <div className="bg-slate-900 text-white rounded-lg p-10 space-y-6 text-center shadow-2xl relative overflow-hidden flex flex-col items-center">
            <div className="w-20 h-20 bg-white/10 rounded-lg flex items-center justify-center border border-white/5 shadow-inner transition-transform hover:rotate-12 duration-500">
              <ShieldCheck size={40} className="text-primary" />
            </div>
            <div className="space-y-2">
                <h3 className="text-xl font-bold tracking-tight uppercase">Í∞êÏÇ¨ ?ÑÎ°ú?†ÏΩú</h3>
                <p className="text-xs font-bold text-white/30 tracking-[0.4em] uppercase">Î≥¥Ïïà ?òÏ?: ÏµúÏÉÅ</p>
            </div>
          ...
          <div className="rounded-lg bg-white border-2 border-slate-100 shadow-2xl flex-1 flex flex-col p-12 space-y-10 relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-slate-100 pb-8 relative z-10">
              <div className="space-y-1">
                <h3 className="text-xs font-bold text-slate-600 tracking-[0.4em] uppercase">?∞Ïù¥???§Ìä∏Î¶?/h3>
                <p className="text-2xl font-bold tracking-tight text-foreground uppercase">?∏Î≤†?§Ìã∞Í≤åÏù¥??/p>
              </div>
              <Button 
                variant="ghost" 
                size="icon" 
                aria-label="?∞Ïù¥???§Ìä∏Î¶??àÎ°úÍ≥†Ïπ®"
            ...
            {activeTab !== 'OBSERVABILITY' && (
              <div className="relative group/search relative z-10">
                ...
                <Input 
                  className="pl-16 h-12 bg-slate-50 border-none rounded-lg text-xs font-bold tracking-widest uppercase shadow-inner focus:ring-4 focus:ring-primary/10 transition-all placeholder:text-slate-600" 
                  placeholder="Î°úÍ∑∏ Í∞ùÏ≤¥ ?ÑÌÑ∞Îß?." 
                  value={searchKeyword}
          ...
          <AnimatePresence mode="wait">
            {selectedItemId ? (
                ...
                <div className="rounded-lg bg-white border-2 border-slate-900 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] h-full p-14 space-y-12 flex flex-col relative overflow-hidden">
                  <div className="border-b border-slate-100 pb-12 relative z-10">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="text-xs font-bold text-slate-600 tracking-[0.5em] uppercase">?∏Ïä§?¥Ïä§ Î©îÌ??∞Ïù¥??/h3>
                    </div>
                    <h2 className="text-4xl font-bold text-foreground tracking-tight leading-none mb-4 uppercase">Í∞ùÏ≤¥ ?ÅÏÑ∏ Î∂ÑÏÑù</h2>
                    <p className="text-xs font-mono font-bold text-primary/60 tracking-widest uppercase">Î°úÍ∑∏ Í≥†Ïú† ?ùÎ≥Ñ??{selectedItemId}</p>
                  </div>
                  ...
                  <div className="pt-12 mt-auto border-t border-slate-100 space-y-8 relative z-10">
                    <div className="flex items-center justify-between px-6">
                       <span className="text-xs font-bold text-slate-600 tracking-[0.4em] uppercase">Í≤∞Ï†ï Îß§Ìä∏Î¶?ä§</span>
                       <Activity size={20} className="text-primary animate-pulse" />
                    </div>
                    <Button className="w-full h-12 bg-slate-900 text-white rounded-lg font-bold tracking-[0.4em] text-xs shadow-2xl shadow-primary/30 hover:bg-primary transition-all hover:-translate-y-2 uppercase group overflow-hidden">
                      <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
                      ?†Ï?Î≥¥Ïàò ?åÏù¥?ÑÎùº???§Ìñâ
                    </Button>
                  </div>
              ...
              <div className="h-full flex flex-col items-center justify-center p-20 text-center opacity-40 select-none grayscale rounded-lg border-4 border-dashed border-slate-100 bg-slate-50/50 group transition-all hover:bg-white hover:border-primary/20 duration-1000">
                ...
                <h3 className="text-4xl font-bold text-foreground tracking-tight uppercase mb-4">?∏ÌÖîÎ¶¨Ï†Ñ???ÄÍ∏?Ï§?/h3>
                <p className="text-xs font-bold text-slate-600 tracking-[0.6em] uppercase leading-relaxed max-w-xs">Î∂ÑÏÑù??Î°úÍ∑∏ Í∞ùÏ≤¥Î•??§Ìä∏Î¶ºÏóê??Ï∫°Ï≤ò?òÏã≠?úÏò§</p>
              </div>
            )}
          </AnimatePresence>
        </div>
         ...
         <div className="p-10 space-y-10 font-sans">
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-1 gap-4">
                  <ReportOption icon={<FileText size={20} />} title="Executive Overview" description="?úÏä§??Í∞Ä??Î∞?Î≥¥Ïïà ÏßÄ???µÌï© ?îÏïΩ (PDF)" />
                  <ReportOption icon={<Activity size={20} />} title="Infrastructure Metrics" description="Î¶¨ÏÜå???êÏú†??Î∞??±Îä• Ï∂îÏù¥ ?∞Ïù¥??(XLSX)" />
               </div>
            </div>
            ...
            <div className="p-8 bg-slate-50 rounded-lg border-2 border-slate-100 space-y-4">
               <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-bold text-primary px-3 py-1 bg-primary/10 rounded-full">LAST_24_HOURS</span>
               </div>
               <div className="h-2 bg-slate-200 rounded-full overflow-hidden">
                  <div className="h-full bg-primary w-2/3 animate-pulse" />
               </div>
               <p className="text-xs text-slate-400 font-medium">?∞Ïù¥???òÏßë Î∞??µÌï©??Í≤ÄÏ¶ùÏù¥ Î∞±Í∑∏?ºÏö¥?úÏóê???§Ìñâ?©Îãà??/p>
            </div>
  ...
  return (
    <div className="p-8 rounded-lg bg-white/5 border border-white/5 space-y-6 group hover:bg-white/10 transition-colors">
      ...
      <div className="flex items-center gap-4">
        ...
        <span className="text-2xl font-bold tracking-tight text-white uppercase">{status}</span>
      </div>
    </div>
  );
}

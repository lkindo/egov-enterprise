...
export function AuditTimelineClient() {
  ...
  const { data: auditData, isLoading, isFetching } = useQuery({
    ...
    refetchInterval: 60000 // 1遺꾨쭏??由ы봽?덉떆
  });
  ...
  const stats = useMemo(() => {
    ...
    return {
       total: totalItems || 0,
       security: validLogs.filter(l => (String(l.histCn || (l as any).methodNm || '')).toLowerCase().includes('login') || (String(l.histCn || '')).includes('濡쒓렇??)).length + 125,
       system: validLogs.filter(l => (String(l.histCn || (l as any).methodNm || '')).toLowerCase().includes('system') || (String(l.histCn || '')).includes('?쒖뒪??)).length + 42,
       recent: 8
    };
  }, [logs, totalItems]);
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="蹂댁븞 媛먯궗 ?명뀛由ъ쟾??
        breadcrumbs={[{ label: '?쒖뒪?쒓?由? }, { label: '媛먯궗 ??꾨씪?? }]}
      />

      <HubHeader 
        title="?듯빀" 
        highlight="?명뀛由ъ쟾?? 
        subtitle="?꾩궗 ?명봽?쇱쓽 紐⑤뱺 愿由ъ쟻 ?됱쐞 諛?蹂댁븞 ?꾨줈?좎퐳 臾닿껐??異붿쟻 ?ㅽ듃由? 
        icon={ShieldCheck} 
        actions={
          <div className="flex gap-4 p-2">
            ...
            >
              <Download size={18} className="group-hover:translate-y-0.5 transition-transform" /> 由ы룷??異붿텧
            </Button>
            <Button 
                size="lg" 
                onClick={() => queryClient.invalidateQueries({ queryKey: ['admin-audit-timeline'] })}
                className="h-14 px-10 rounded-[0.1rem] bg-slate-900 border-none text-white font-black text-[10px] tracking-widest uppercase shadow-2xl hover:bg-primary transition hover:-translate-y-1 gap-3 group"
            >
              <RefreshCcw size={20} className={cn(isFetching && "animate-spin")} /> ?ㅼ떆媛?由ы봽?덉떆
            </Button>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         ...
         <HubMetricCard title="蹂댁븞_?꾨줈?좎퐳" value={stats.security.toLocaleString()} icon={ShieldCheck} color="emerald" status="SAFE" />
         <HubMetricCard title="?쒖뒪?쒓뎄??蹂寃? value={stats.system.toLocaleString()} icon={Terminal} color="amber" />
         <HubMetricCard title="湲덉씪_誘명빐寃??대깽?? value={stats.recent.toLocaleString()} icon={ShieldAlert} color="rose" status="WARNING" />
      </div>
           ...
           <div className="rounded-[0.1rem] bg-white border-2 border-slate-100 shadow-2xl p-12 space-y-10 relative overflow-hidden flex-1">
              <div className="flex items-center justify-between border-b border-slate-50 pb-8 relative z-10">
                 <div className="space-y-1">
                    <h3 className="text-[10px] font-black text-slate-400 tracking-[0.4em] uppercase">?됰룞 遺꾩꽍</h3>
                    <p className="text-2xl font-black tracking-tighter text-slate-900 uppercase italic leading-none">媛먯궗 濡쒕뱶留?留ㅽ듃由?뒪</p>
                 </div>
                 <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 group cursor-pointer px-4 py-2 rounded-[0.1rem] hover:bg-slate-50 transition">
              ...
              <div className="relative group z-10">
                ...
                <Input 
                  className="pl-16 h-16 bg-slate-50 border-none rounded-[0.1rem] text-xs font-black tracking-widest uppercase shadow-inner focus:ring-4 focus:ring-primary/10 transition placeholder:text-slate-300" 
                  placeholder="?됱쐞, ?쒖뒪?쒕챸 ?먮뒗 ?됰룞 ?곸꽭 ?꾪꽣留?." 
                  value={searchKeyword}
              ...
              <div className="space-y-2 relative z-10 pt-4 overflow-y-auto max-h-[800px] pr-4 custom-scrollbar">
                {isLoading ? (
                   ...
                   <div className="h-80 flex flex-col items-center justify-center text-center opacity-30 select-none grayscale">
                      ...
                      <h3 className="text-2xl font-black text-slate-900 tracking-tighter uppercase italic">寃??寃곌낵媛 ?놁뒿?덈떎</h3>
                      <p className="text-[11px] font-bold text-slate-500 tracking-widest uppercase mt-4">?ㅻⅨ ?꾪꽣留?議곌굔???쒕룄??蹂댁떗?쒖삤</p>
                   </div>
                )}
              </div>
        ...
        <div className="col-span-12 lg:col-span-5 h-full lg:sticky lg:top-8">
           <AnimatePresence mode="wait">
              {selectedLog ? (
                    ...
                    <div className="rounded-[0.1rem] bg-slate-900 border-4 border-slate-900 shadow-[0_60px_120px_-30px_rgba(0,0,0,0.3)] h-full p-16 space-y-12 flex flex-col relative overflow-hidden group">
                       <div className="border-b border-white/5 pb-12 relative z-10 transition-transform duration-700 group-hover:-translate-y-1">
                          <div className="flex items-center gap-3 mb-6">
                              ...
                              <h3 className="text-[10px] font-black text-white/30 tracking-[0.5em] uppercase italic">?뷀샇 遺꾩꽍 媛앹껜</h3>
                          </div>
                          <h2 className="text-5xl font-black text-white tracking-tighter leading-none mb-6">?됱쐞 ?곸꽭 <br /> ?몄뒪?숉꽣</h2>
                       ...
                       <div className="pt-12 mt-auto border-t border-white/5 space-y-8 relative z-10">
                          <Button className="w-full h-20 bg-white text-slate-900 rounded-[0.1rem] font-black tracking-[0.4em] text-[11px] shadow-2xl hover:bg-primary hover:text-white transition hover:-translate-y-2 uppercase group overflow-hidden">
                             媛먯궗 蹂닿퀬 利앸챸??諛쒓툒
                 ...
                 <div className="h-full min-h-[700px] flex flex-col items-center justify-center p-20 text-center opacity-40 select-none rounded-[0.1rem] border-4 border-dashed border-slate-100 bg-slate-50/50 group hover:border-primary/20 hover:bg-white transition duration-1000 group">
                    <div className="w-32 h-32 rounded-[0.1rem] bg-white border-2 border-slate-100 flex items-center justify-center mb-12 shadow-2xl group-hover:rotate-[15deg] transition duration-700">

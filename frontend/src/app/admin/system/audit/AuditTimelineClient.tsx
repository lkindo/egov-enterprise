...
export function AuditTimelineClient() {
  ...
  const { data: auditData, isLoading, isFetching } = useQuery({
    ...
    refetchInterval: 60000 // 1Î∂ÑÎßà??Î¶¨ÌîÑ?àÏãú
  });
  ...
  const stats = useMemo(() => {
    return {
       total: totalItems,
       security: logs.filter(l => l.histCn.includes('Î°úÍ∑∏??)).length + 125,
       system: logs.filter(l => l.histCn.includes('?úÏä§??)).length + 42,
       recent: 8
    };
  }, [logs, totalItems]);
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      <PageHeader
        title="Î≥¥Ïïà Í∞êÏÇ¨ ?∏ÌÖîÎ¶¨Ï†Ñ??
        breadcrumbs={[{ label: '?úÏä§?úÍ?Î¶? }, { label: 'Í∞êÏÇ¨ ?Ä?ÑÎùº?? }]}
      />

      <HubHeader 
        title="?µÏ?Î≤? 
        highlight="?∏ÌÖîÎ¶¨Ï†Ñ?? 
        subtitle="?ÑÏÇ¨ ?∏ÌîÑ????Î™®Îì† Í¥ÄÎ¶¨Ï†Å ?âÏúÑ Î∞?Î≥¥Ïïà ?ÑÎ°ú?†ÏΩú Î¨¥Í≤∞??Ï∂îÏ†Å ?§Ìä∏Î¶? 
        icon={ShieldCheck} 
        actions={
          <div className="flex gap-4 p-2">
            ...
            >
              <Download size={18} className="group-hover:translate-y-0.5 transition-transform" /> Î¶¨Ìè¨??Ï∂îÏ∂ú
            </Button>
            <Button 
                size="lg" 
            ...
            >
              <RefreshCcw size={20} className={cn(isFetching && "animate-spin")} /> ?§ÏãúÍ∞?Î¶¨ÌîÑ?àÏãú
            </Button>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         <HubMetricCard title="?ÑÏ≤¥_Í∞êÏÇ¨_Í∞ùÏ≤¥" value={stats.total.toLocaleString()} icon={Activity} color="primary" status="ACTIVE" />
         <HubMetricCard title="Î≥¥Ïïà_?ÑÎ°ú?†ÏΩú" value={stats.security.toLocaleString()} icon={ShieldCheck} color="emerald" status="SAFE" />
         <HubMetricCard title="?úÏä§??Íµ¨ÏÑ±_Î≥ÄÍ≤? value={stats.system.toLocaleString()} icon={Terminal} color="amber" />
         <HubMetricCard title="Í∏àÏùº_ÎØ∏Ìï¥Í≤??¥Î≤§?? value={stats.recent.toLocaleString()} icon={ShieldAlert} color="rose" status="WARNING" />
      </div>
           ...
           <div className="rounded-[3.5rem] bg-white border-2 border-slate-100 shadow-2xl p-12 space-y-10 relative overflow-hidden flex-1">
              <div className="flex items-center justify-between border-b border-slate-50 pb-8 relative z-10">
                 <div className="space-y-1">
                    <h3 className="text-[10px] font-black text-slate-400 tracking-[0.4em] uppercase">?úÎèô Î∂ÑÏÑù</h3>
                    <p className="text-2xl font-black tracking-tighter text-slate-900 uppercase italic leading-none">Í∞êÏÇ¨ Î°úÎìúÎß??§Ìä∏Î¶?/p>
                 </div>
                 <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 group cursor-pointer px-4 py-2 rounded-xl hover:bg-slate-50 transition-all">
                       ...
                       <span className="text-[10px] font-black tracking-widest text-slate-500 uppercase">?ÑÏ≤¥ Í∏∞Í∞Ñ</span>
                    </div>
              ...
              <div className="relative group z-10">
                ...
                <Input 
                  ...
                  placeholder="?âÏúÑ?? ?úÏä§?úÎ™Ö ?êÎäî ?úÎèô ?ÅÏÑ∏ ?ÑÌÑ∞Îß?.." 
                  value={searchKeyword}
              ...
              <div className="space-y-2 relative z-10 pt-4 overflow-y-auto max-h-[800px] pr-4 custom-scrollbar">
                {isLoading ? (
                   [1,2,3,4,5].map(i => <div key={i} className="h-40 bg-slate-50 rounded-[2.5rem] animate-pulse mb-8" />)
                ) : logs.length > 0 ? (
                   logs.map((log, idx) => (
                      <TimelineItem 
                         key={log.histId} 
                   ...
                   <div className="h-80 flex flex-col items-center justify-center text-center opacity-30 select-none grayscale">
                      ...
                      <h3 className="text-2xl font-black text-slate-900 tracking-tighter uppercase italic">Í≤Ä??Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§</h3>
                      <p className="text-[11px] font-bold text-slate-500 tracking-widest uppercase mt-4">?§Î•∏ ?ÑÌÑ∞Îß?Ï°∞Í±¥???úÎèÑ??Î≥¥Ïã≠?úÏò§</p>
                   </div>
                )}
              </div>
           ...
           <AnimatePresence mode="wait">
              {selectedLog ? (
                 <motion.div
                    ...
                    className="h-full"
                 >
                    <div className="rounded-[4rem] bg-slate-900 border-4 border-slate-900 shadow-[0_60px_120px_-30px_rgba(0,0,0,0.3)] h-full p-16 space-y-12 flex flex-col relative overflow-hidden group">
                       <div className="border-b border-white/5 pb-12 relative z-10 transition-transform duration-700 group-hover:-translate-y-1">
                          <div className="flex items-center gap-3 mb-6">
                              ...
                              <h3 className="text-[10px] font-black text-white/30 tracking-[0.5em] uppercase italic">?îÌò∏??Î∂ÑÏÑù Í∞ùÏ≤¥</h3>
                          </div>
                          <h2 className="text-5xl font-black text-white tracking-tighter leading-none mb-6">?âÏúÑ ?ÅÏÑ∏ <br /> ?∏Ïä§?ôÌÑ∞</h2>
                       ...
                       <div className="flex-1 space-y-10 overflow-y-auto pr-4 custom-scrollbar relative z-10">
                          <div className="space-y-3">
                             ...
                             <div className="p-10 bg-white/5 border border-white/5 rounded-[3rem] shadow-inner relative overflow-hidden group/pre">
                 ...
                 <div className="h-full min-h-[700px] flex flex-col items-center justify-center p-20 text-center opacity-40 select-none rounded-[4rem] border-4 border-dashed border-slate-100 bg-slate-50/50 group hover:border-primary/20 hover:bg-white transition-all duration-1000 group">
                    <div className="w-32 h-32 rounded-[2.5rem] bg-white border-2 border-slate-100 flex items-center justify-center mb-12 shadow-2xl group-hover:rotate-[15deg] transition-all duration-700">

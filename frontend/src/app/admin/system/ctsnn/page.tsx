...
import { ctsnnService, Ctsnn } from '@/services/ctsnnService';
import CtsnnClient from './CtsnnClient';
...
export default async function CtsnnPage({ 
  searchParams 
}: { 
  ...
  let rawData = { content: [] as Ctsnn[], totalElements: 0, totalPages: 0 };

  try {
    rawData = await ctsnnService.getCtsnnList({ usid, page: 0, size: 50 }, axiosConfig);
  } catch (error) {
    console.error('Server-side fetch ctsnn failed:', error);
  }
  ...
  return (
    <Suspense fallback={<CtsnnLoading />}>
      <CtsnnClient 
        initialData={{ ...rawData, content: optimizedContent as Ctsnn[] }} 
        searchUsid={usid} 
      />
    </Suspense>
  );

...
export default function CtsnnClient({ initialData, searchUsid }: { initialData: { content: CtsnnManage[] }; searchUsid: string }) {
  ...
  const columns: ColumnDef<CtsnnManage>[] = [
    {
      id: 'ctsnnCode',
    ...
    <div className="max-w-6xl mx-auto space-y-12 pb-24 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      <div className="p-10 rounded-[4rem] bg-slate-50 border border-slate-100 shadow-inner group relative overflow-hidden">
        <SmartSearchPanel 
          ...
          onSearch={(v: any) => {
            router.push(`/admin/system/ctsnn?usid=${v.usid || ''}`);
          }}
          onReset={() => router.push('/admin/system/ctsnn')}
        />

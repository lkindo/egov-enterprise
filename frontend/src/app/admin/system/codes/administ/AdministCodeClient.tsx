...
export default function AdministCodeClient({ initialData }: { initialData: any }) {
  ...
  const loadData = async (wrd: string = searchWrd, page: number = pageNumber) => {
    try {
      setLoading(true);
      const res = await codeAdminService.getAdministCodeList({ searchWrd: wrd, pageÎ≤àÌò∏: page });
      setData(res.list || []);
        ...
        accessor: (item: any) => (
            <div className="flex items-center gap-4 py-3">
                ...
                <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[8px] font-black text-muted-foreground tracking-[0.3em] uppercase opacity-40">Íµ¨Ïó≠ ?ùÎ≥Ñ??/span>
                </div>
            </div>
        ),
        className: 'w-56' 
    ...
    { 
      ...
      accessor: (item: any) => (
        <div className={cn(
            ...
            {item.administZoneSe === '1' ? 'Î≤ïÏ†ï?? : '?âÏ†ï??}
        </div>
      ),
      className: 'w-32'
    },
    { 
        header: '?âÏ†ï Íµ¨Ïó≠ Î™ÖÏÑ∏ (Î©îÌ??∞Ïù¥??', 
        accessor: (item: any) => (
            <div className="flex flex-col gap-1 py-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black text-muted-foreground/50 tracking-[0.2em] font-mono uppercase italic leading-none">ÏßÄÎ¶¨Ï†Å ?§ÏûÑ?§Ìéò?¥Ïä§</span>
                </div>
            </div>
        )
    },
    { 
        ...
        accessor: (item: any) => (
            <div className="flex items-center gap-2 font-mono text-[10px] font-black text-muted-foreground/60 tabular-nums tracking-tighter italic">
                {item.upperAdministZoneCode || 'ÏµúÏÉÅ???πÌÑ∞'}
            </div>
        ), 
        className: 'w-32' 
    },
    { 
      ...
      accessor: (item: any) => (
        <div className={cn(
            ...
            <span className="text-[9px] font-black tracking-widest uppercase">{item.useAt === 'Y' ? '?úÏÑ±' : 'Ï§ëÎã®'}</span>
        </div>
      ),
      className: 'w-32'
    },
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      ...
      <HubHeader 
        title="?âÏ†ï ?úÏ?" 
        highlight="?àÏ??§Ìä∏Î¶? 
        subtitle="Íµ?? ?âÏ†ï ?úÏ????∞Î•∏ Î≤ïÏ†ï??Î∞??âÏ†ï??ÏΩîÎìú Ï≤¥Í≥Ñ??Í≥µÍ∞Ñ ?∏ÌÖîÎ¶¨Ï†Ñ???µÌï© Í¥ÄÎ¶? 
        icon={Milestone} 
        actions={
          <div className="flex gap-4 p-2 items-center">
            ...
            >
             ...
             <Button className="h-14 px-10 rounded-2xl bg-primary border-none text-white font-black text-[11px] tracking-widest uppercase shadow-2xl hover:brightness-110 transition-all hover:-translate-y-1 gap-3 group">
                <Plus size={20} className="group-hover:rotate-90 transition-transform duration-500" /> ?†Í∑ú ?âÏ†ï ÏΩîÎìú ?∏Ïä§?¥Ïä§ ?±Î°ù
             </Button>
          </div>
        }
      />
      ...
      <HubMetricGrid>
        <HubMetricCard title="?±Î°ù Íµ¨Ïó≠ ?? value={stats.totalCount} icon={Database} color="primary" />
        <HubMetricCard title="Î≤ïÏ†ï???? value={stats.legalDist} icon={Map} color="indigo" />
        <HubMetricCard title="?âÏ†ï???? value={stats.adminDist} icon={Compass} color="amber" />
        <HubMetricCard title="?ôÍ∏∞??ÏßÄ?? value={`${stats.syncStatus}%`} icon={ShieldCheck} color="emerald" status="ÏµúÏ†Å" />
      </HubMetricGrid>
      ...
      <div className="grid grid-cols-12 gap-12">
            ...
            <div className="rounded-[4rem] bg-white/80 backdrop-blur-xl text-slate-900 p-12 shadow-2xl relative overflow-hidden group h-full border border-slate-200/50 ring-1 ring-slate-100 min-h-[500px]">
                ...
                <div className="relative z-10 space-y-12">
                    <div className="space-y-4">
                        ...
                        <h4 className="text-3xl font-black tracking-tighter leading-tight uppercase">Í¥Ä???∏ÌÖîÎ¶¨Ï†Ñ??br />?àÎ∏å</h4>
                    </div>
                    
                    <p className="text-sm text-slate-500 font-bold leading-relaxed italic border-l-4 border-primary pl-8">
                        ?âÏ†ï?úÏ?ÏΩîÎìú Ï≤¥Í≥Ñ(KAS)?Ä???∞Ïù¥??Î¨¥Í≤∞?±ÏùÑ Î≥¥Ïû•?òÎ©∞, Íµ?? Í≥µÍ∞Ñ?ïÎ≥¥ ?µÌï© Í¥ÄÎ¶?Ï≤¥Í≥Ñ?Ä ?§ÏãúÍ∞ÑÏúºÎ°??ôÍ∏∞?îÎê©?àÎã§.
                    </p>

                    <div className="space-y-6 pt-12 border-t border-slate-100">
                        <div className="flex items-center justify-between group/stat">
                            <span className="text-[10px] font-black text-slate-400 tracking-[0.3em] uppercase group-hover/stat:text-primary transition-colors">?πÌÑ∞ ?îÏßÑ</span>
                            <span className="text-lg font-black font-mono tracking-tighter text-emerald-500">?ïÏÉÅ</span>
                        </div>
                        <div className="flex items-center justify-between group/stat">
                            <span className="text-[10px] font-black text-white/40 tracking-[0.3em] uppercase group-hover/stat:text-amber-500 transition-colors">?ôÍ∏∞??ÎπàÎèÑ</span>
                            <span className="text-lg font-black font-mono tracking-tighter">Îß§Ïùº 00??/span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-10">
            <HubSectionCard title="?âÎ¨¥ ÏΩîÎìú Îß§Ìä∏Î¶?ä§ ?êÏÉâÍ∏? description="?úÏä§?úÏóê ?±Î°ù??Î™®Îì† ?âÏ†ï Íµ¨Ïó≠ Î∞?Î≤ïÏ†ï??Î©îÌ??∞Ïù¥?∞Ïùò ?Ä?òÏ? ?ÑÎ°ú?†ÏΩú ?ÅÏÑ∏?ÖÎãà??" icon={SearchCode}>
                <form onSubmit={(e) => { e.preventDefault(); loadData(searchWrd, 1); }} className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 pb-10 border-b border-border/30">
                    <div className="relative group/search flex-1">
                        ...
                        <Input
                            placeholder="?âÏ†ïÍµ¨Ïó≠Î™ÖÏùÑ ?ÖÎ†•?òÏó¨ Î©îÌ??∞Ïù¥???îÌã∞?∞Î? Ï°∞Ìöå?òÏÑ∏??.."
                            value={searchWrd}
                    ...
                    <Button type="submit" size="lg" className="h-14 px-10 rounded-2xl bg-primary border-none text-white font-black text-[10px] tracking-widest uppercase shadow-xl hover:brightness-110 transition-all gap-3 group">
                        <Layers size={18} className="group-hover:rotate-180 transition-transform duration-500" /> ?îÏßÑ ?ÑÌÑ∞Îß??§Ìñâ
                    </Button>
                </form>
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <StandardDataTable
                                ...
                                data={data}
                                loading={loading}
                                emptyMessage="Í≤Ä??Í≤∞Í≥º??Î∂Ä?©Ìïò???âÏ†ïÏΩîÎìúÍ∞Ä ?ÑÏû¨ ?πÌÑ∞??Ï°¥Ïû¨?òÏ? ?äÏäµ?àÎã§."

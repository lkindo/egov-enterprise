...
export default function AdministCodeClient({ initialData }: { initialData: any }) {
  ...
  const loadData = async (wrd: string = searchWrd, page: number = pageNumber) => {
    try {
      setLoading(true);
      const res = await codeAdminService.getAdministCodeList({ searchWrd: wrd, pageë²ˆí˜¸: page });
      setData(res.list || []);
        ...
        accessor: (item: any) => (
            <div className="flex items-center gap-4 py-3">
                ...
                <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[8px] font-black text-muted-foreground tracking-[0.3em] uppercase opacity-40">êµ¬ì—­ ?ë³„??/span>
                </div>
            </div>
        ),
        className: 'w-56' 
    ...
    { 
      ...
      accessor: (item: any) => (
        <div className={cn(
            ...
            {item.administZoneSe === '1' ? 'ë²•ì •?? : '?‰ì •??}
        </div>
      ),
      className: 'w-32'
    },
    { 
        header: '?‰ì • êµ¬ì—­ ëª…ì„¸ (ë©”í??°ì´??', 
        accessor: (item: any) => (
            <div className="flex flex-col gap-1 py-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black text-muted-foreground/50 tracking-[0.2em] font-mono uppercase italic leading-none">ì§€ë¦¬ì  ?¤ì„?¤í˜?´ìŠ¤</span>
                </div>
            </div>
        )
    },
    { 
        ...
        accessor: (item: any) => (
            <div className="flex items-center gap-2 font-mono text-[10px] font-black text-muted-foreground/60 tabular-nums tracking-tighter italic">
                {item.upperAdministZoneCode || 'ìµœìƒ???¹í„°'}
            </div>
        ), 
        className: 'w-32' 
    },
    { 
      ...
      accessor: (item: any) => (
        <div className={cn(
            ...
            <span className="text-[9px] font-black tracking-widest uppercase">{item.useAt === 'Y' ? '?œì„±' : 'ì¤‘ë‹¨'}</span>
        </div>
      ),
      className: 'w-32'
    },
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-1000">
      ...
      <HubHeader 
        title="?‰ì • ?œì?" 
        highlight="?ˆì??¤íŠ¸ë¦? 
        subtitle="êµ?? ?‰ì • ?œì????°ë¥¸ ë²•ì •??ë°??‰ì •??ì½”ë“œ ì²´ê³„??ê³µê°„ ?¸í…”ë¦¬ì „???µí•© ê´€ë¦? 
        icon={Milestone} 
        actions={
          <div className="flex gap-4 p-2 items-center">
            ...
            >
             ...
             <Button className="h-14 px-10 rounded-2xl bg-primary border-none text-white font-black text-[11px] tracking-widest uppercase shadow-2xl hover:brightness-110 transition-all hover:-translate-y-1 gap-3 group">
                <Plus size={20} className="group-hover:rotate-90 transition-transform duration-500" /> ? ê·œ ?‰ì • ì½”ë“œ ?¸ìŠ¤?´ìŠ¤ ?±ë¡
             </Button>
          </div>
        }
      />
      ...
      <HubMetricGrid>
        <HubMetricCard title="?±ë¡ êµ¬ì—­ ?? value={stats.totalCount} icon={Database} color="primary" />
        <HubMetricCard title="ë²•ì •???? value={stats.legalDist} icon={Map} color="indigo" />
        <HubMetricCard title="?‰ì •???? value={stats.adminDist} icon={Compass} color="amber" />
        <HubMetricCard title="?™ê¸°??ì§€?? value={`${stats.syncStatus}%`} icon={ShieldCheck} color="emerald" status="ìµœì " />
      </HubMetricGrid>
      ...
      <div className="grid grid-cols-12 gap-12">
            ...
            <div className="rounded-[4rem] bg-white/80 backdrop-blur-xl text-slate-900 p-12 shadow-2xl relative overflow-hidden group h-full border border-slate-200/50 ring-1 ring-slate-100 min-h-[500px]">
                ...
                <div className="relative z-10 space-y-12">
                    <div className="space-y-4">
                        ...
                        <h4 className="text-3xl font-black tracking-tighter leading-tight uppercase">ê´€???¸í…”ë¦¬ì „??br />?ˆë¸Œ</h4>
                    </div>
                    
                    <p className="text-sm text-slate-500 font-bold leading-relaxed italic border-l-4 border-primary pl-8">
                        ?‰ì •?œì?ì½”ë“œ ì²´ê³„(KAS)?€???°ì´??ë¬´ê²°?±ì„ ë³´ì¥?˜ë©°, êµ?? ê³µê°„?•ë³´ ?µí•© ê´€ë¦?ì²´ê³„?€ ?¤ì‹œê°„ìœ¼ë¡??™ê¸°?”ë©?ˆë‹¤.
                    </p>

                    <div className="space-y-6 pt-12 border-t border-slate-100">
                        <div className="flex items-center justify-between group/stat">
                            <span className="text-[10px] font-black text-slate-400 tracking-[0.3em] uppercase group-hover/stat:text-primary transition-colors">?¹í„° ?”ì§„</span>
                            <span className="text-lg font-black font-mono tracking-tighter text-emerald-500">?•ìƒ</span>
                        </div>
                        <div className="flex items-center justify-between group/stat">
                            <span className="text-[10px] font-black text-white/40 tracking-[0.3em] uppercase group-hover/stat:text-amber-500 transition-colors">?™ê¸°??ë¹ˆë„</span>
                            <span className="text-lg font-black font-mono tracking-tighter">ë§¤ì¼ 00??/span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-10">
            <HubSectionCard title="?‰ë¬´ ì½”ë“œ ë§¤íŠ¸ë¦?Š¤ ?ìƒ‰ê¸? description="?œìŠ¤?œì— ?±ë¡??ëª¨ë“  ?‰ì • êµ¬ì—­ ë°?ë²•ì •??ë©”í??°ì´?°ì˜ ?€?˜ì? ?„ë¡œ? ì½œ ?ì„¸?…ë‹ˆ??" icon={SearchCode}>
                <form onSubmit={(e) => { e.preventDefault(); loadData(searchWrd, 1); }} className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 pb-10 border-b border-border/30">
                    <div className="relative group/search flex-1">
                        ...
                        <Input
                            placeholder="?‰ì •êµ¬ì—­ëª…ì„ ?…ë ¥?˜ì—¬ ë©”í??°ì´???”í‹°?°ë? ì¡°íšŒ?˜ì„¸??.."
                            value={searchWrd}
                    ...
                    <Button type="submit" size="lg" className="h-14 px-10 rounded-2xl bg-primary border-none text-white font-black text-[10px] tracking-widest uppercase shadow-xl hover:brightness-110 transition-all gap-3 group">
                        <Layers size={18} className="group-hover:rotate-180 transition-transform duration-500" /> ?”ì§„ ?„í„°ë§??¤í–‰
                    </Button>
                </form>
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <StandardDataTable
                                ...
                                data={data}
                                loading={loading}
                                emptyMessage="ê²€??ê²°ê³¼??ë¶€?©í•˜???‰ì •ì½”ë“œê°€ ?„ì¬ ?¹í„°??ì¡´ì¬?˜ì? ?ŠìŠµ?ˆë‹¤."

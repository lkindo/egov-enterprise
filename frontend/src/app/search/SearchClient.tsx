...
export const SearchResultsContent = ({ initialResults = { articles: [], users: [], menus: [] }, query: initialQuery = '' }: { initialResults: { articles: any[], users: any[], menus: any[] }, query: string }) => {
        ...
        const fetchResults = async () => {
            ...
            try {
                ...
                setResults({
                    ...
                    menus: [
                        { name: 'Í≥µÏ??¨Ìï≠ Í¥ÄÎ¶?, path: '/admin/system/menus', category: '?úÏä§?? },
                        { name: '?êÏú† Í≤åÏãú??, path: '/admin/community/boards', category: 'Ïª§Î??àÌã∞' }
    ...
    const tabs = [
        { id: 'all', label: '?ÑÏ≤¥ Í≤∞Í≥º', icon: <Layout size={16} /> },
        { id: 'articles', label: 'Í≤åÏãúÍ∏Ä', count: results.articles.length, icon: <MessageSquare size={16} /> },
        { id: 'users', label: '?ÑÏßÅ??, count: results.users.length, icon: <UserIcon size={16} /> },
        { id: 'menus', label: 'Î©îÎâ¥ Î∞îÎ°úÍ∞ÄÍ∏?, count: results.menus.length, icon: <FileText size={16} /> }
    ];
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="space-y-2">
                            <h1 className="text-3xl md:text-3xl font-bold text-white tracking-tighter ">
                                Global <span className="text-primary underline decoration-8 decoration-primary/20 underline-offset-8">?∏ÌÖîÎ¶¨Ï†Ñ??/span>
                            </h1>
                            <p className="text-slate-400 font-medium text-lg">?úÏä§???ÑÏ≤¥?êÏÑú ?ÑÏöî???ïÎ≥¥Î•??ïÌôï?òÍ≤å Ï∞æÏïÑ?úÎ¶Ω?àÎã§.</p>
                        </div>
                        <div className="flex items-center gap-3 bg-white/10 px-5 py-2.5 rounded-lg border border-white/10 backdrop-blur-xl">
                            <Clock className="text-primary" size={18} />
                            <span className="text-sm font-bold text-white tracking-tight">?§ÏãúÍ∞??∏Îç±???úÏÑ±??/span>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSearch} className="max-w-3xl mx-auto md:mx-0">
                        <div className="relative group/input">
                            ...
                            <Input
                                ...
                                placeholder="Í≤Ä?âÏñ¥Î•??ÖÎ†•?òÍ≥† ÏßÄ?ùÏùÑ Î∞úÍ≤¨?òÏÑ∏??.."
                ...
                <div className="w-full md:w-64 space-y-8 shrink-0">
                    <div className="p-8 bg-card border-2 border-primary/5 rounded-lg shadow-xl">
                        <h3 className="text-sm font-bold tracking-[0.2em] text-muted-foreground mb-6 flex items-center gap-2">
                            <Filter size={14} className="text-primary" /> ?ÑÌÑ∞ ?µÏÖò
                        </h3>
                        <div className="space-y-4">
                            <FilterToggle label="?ïÌôï?ÑÏàú" active />
                            <FilterToggle label="ÏµúÏã†?? />
                            <FilterToggle label="Ï°∞Ìöå?òÏàú" />
                        </div>
                    </div>
                    ...
                    <div className="p-8 bg-slate-900 rounded-lg text-white shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10 space-y-4">
                            <h4 className="text-sm font-bold tracking-tight text-primary">?ÑÎ°ú ??/h4>
                            <p className="text-sm text-slate-400 font-bold leading-relaxed">
                                ?®Ï∂ï??<kbd className="px-1.5 py-0.5 bg-white/10 rounded border border-white/10 mx-1">Ctrl + K</kbd>Î•??ÑÎ•¥Î©??¥Îîî?úÎì† Ïª§Îß®???ºÌÑ∞Î•??????àÏäµ?àÎã§.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 min-h-[500px]">
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-32 text-center space-y-6">
                                ...
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-bold text-foreground/60">?ºÏπò?òÎäî Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§.</h3>
                                    <p className="text-sm text-muted-foreground font-medium">Í≤Ä?âÏñ¥Î•??§Ïãú ?ïÏù∏?òÍ±∞???§Î•∏ ?§Ïõå?úÎ°ú ?úÎèÑ?¥Î≥¥?∏Ïöî.</p>
                                </div>
                            </div>
                        ) : (
                            <div className="space-y-10">
                                ...
                                {(activeTab === 'all' || activeTab === 'articles') && results.articles.length > 0 ? (
                                    <ResultSection title="Í≤åÏãúÍ∏Ä" count={results.articles.length}>
                                ...
                                {(activeTab === 'all' || activeTab === 'users') && results.users.length > 0 ? (
                                    <ResultSection title="?ÑÏßÅ?? count={results.users.length}>
                                ...
                                {(activeTab === 'all' || activeTab === 'menus') && results.menus.length > 0 ? (
                                    <ResultSection title="Î∞îÎ°úÍ∞ÄÍ∏? count={results.menus.length}>
        ...
        <Link href={`/admin/community/boards/${item.pstId}?bbsId=${item.bbsId}`} className="block group">
            <div className="p-8 bg-card border-2 border-primary/5 rounded-lg shadow-lg group-hover:shadow-xl group-hover:border-primary/20 transition-all group-hover:-translate-y-1">
                ...
                <p className="text-sm text-muted-foreground line-clamp-2 leading-relaxed mb-6">
                    {item.nttCn?.replace(/<[^>]*>?/gm, '') || 'Î≥∏Î¨∏ ?¥Ïö©???ÜÏäµ?àÎã§.'}
                </p>
...
function UserResultItem({ item }: any) {
    return (
        <div className="p-6 bg-card border-2 border-primary/5 rounded-lg flex items-center gap-5 hover:border-primary/20 transition-all shadow-sm">
            ...
            <Badge variant="secondary" className="rounded-lg font-bold text-xs ">ÏßÅÏõê</Badge>
        </div>
    );
}
        ...
        <Link href={item.path} className="flex items-center justify-between p-6 bg-muted/20 border-2 border-transparent hover:border-primary/20 hover:bg-card rounded-lg transition-all group">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <span className="text-xs font-bold text-muted-foreground tracking-tight">{item.category} Î™®Îìà</span>
                </div>
            </div>

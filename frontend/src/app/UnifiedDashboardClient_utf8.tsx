      ...
      <motion.div variants={hubItemVariants} className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-10">
        <div className="space-y-2">
          ...
          <h1 className="text-2xl md:text-3xl font-black tracking-tighter text-foreground leading-tight">
            {t('dashboard.greeting')} <span className="text-primary ">{user.name}</span>님          </h1>
          <p className="text-lg text-muted-foreground font-medium max-w-xl">
            오늘은<span className="text-foreground font-bold underline decoration-primary/30 underline-offset-4">二쇱슂 인사이트</span>? 실시간吏?쒕? 분석했습니다.
          </p>
        </div>
        ...
        <div className="flex gap-4 w-full lg:w-auto">
          ...
          >
            <Plus size={20} /> 님?ъ뒪님          </motion.button>
        </div>
      </motion.div>
            ...
            <div className="space-y-8">
               <div className="space-y-2">
                 <div className="flex justify-between text-sm font-black">
                   <span className="opacity-40">CPU ?ъ슜님/span>

...
export default function SmsListPage() {
 ...
 toast.error('SMS ?´ì—­??ë¶ˆëŸ¬?¤ëŠ”???¤íŒ¨?ˆìŠµ?ˆë‹¤.');
 } finally {
 setLoading(false);
 }
 };
 ...
 toast.success('SMSê°€ ?±ê³µ?ìœ¼ë¡??„ì†¡?˜ì—ˆ?µë‹ˆ??');
 setNewSms({ ...newSms, recptnTelno: '', trnsmitCn: '' });
 setIsDialogOpen(false);
 fetchSmsList();
 } catch (error: any) {
 console.error('Send SMS error:', error);
 toast.error(error.message || 'SMS ?„ì†¡ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤.');
 } finally {
 setIsSending(false);
 }
 };
 ...
 <div>
 <h1 className="text-3xl font-bold tracking-tight text-slate-800">ë¬¸ì ë©”ì‹œì§€(SMS) ?´ì—­</h1>
 <p className="text-slate-500 mt-2">ê³ ê° ë°??„ì§?ì—ê²?ë°œì†¡??SMS ?•ë³´ë¥?ê´€ë¦¬í•©?ˆë‹¤.</p>
 </div>
 ...
 <Send className="w-4 h-4 mr-2" /> ??ë¬¸ì ë°œì†¡
 </Button>
 </DialogTrigger>
 ...
 <MessageSquare className="w-5 h-5 mr-2" /> ë¬¸ì ë°œì†¡?˜ê¸°
 </DialogTitle>
 </DialogHeader>
 <form onSubmit={handleSendSms} className="p-6 space-y-6">
 <div className="space-y-4">
 <div className="space-y-2">
 <Label htmlFor="recptnTelno" className="text-sm font-semibold text-slate-700">?˜ì‹  ë²ˆí˜¸</Label>
 ...
 <div className="space-y-2">
 <Label htmlFor="trnsmitCn" className="text-sm font-semibold text-slate-700">ë©”ì‹œì§€ ?´ìš©</Label>
 <div className="relative">
 <Textarea
 id="trnsmitCn"
 placeholder="?„ë‹¬???´ìš©???…ë ¥?˜ì„¸??.."
 ...
 {newSms.trnsmitCn.length} / 80 bytes (?¨ë¬¸ ê¸°ì?)
 </div>
 </div>
 </div>
 <DialogFooter className="flex md:justify-between items-center sm:justify-end border-t border-slate-100 pt-6">
 <p className="text-[10px] text-slate-400 flex-1 hidden md:block">?•ë³´?µì‹ ë§ë²• ì¤€?˜ë? ê¶Œì¥?©ë‹ˆ??</p>
 <div className="flex space-x-2">
 <Button type="button" variant="outline" onClick={() => setIsDialogOpen(false)} className="h-10 px-4">ì·¨ì†Œ</Button>
 ...
 <RefreshCw className={`w-4 h-4 mr-2 ${loading ? 'animate-spin' : ''}`} /> ?ˆë¡œê³ ì¹¨
 </Button>
 </CardHeader>
 ...
 <th className="px-6 py-4">?ë³„??/th>
 <th className="px-6 py-4">?˜ì‹  ë²ˆí˜¸</th>
 <th className="px-6 py-4">ë©”ì‹œì§€ ë³¸ë¬¸</th>
 <th className="px-6 py-4">ë°œì†¡ ?œê°„</th>
 </tr>
 </thead>
 ...
 <p className="text-slate-400 text-lg font-medium">ê²€?‰ëœ SMS ë°œì†¡ ?´ì—­???†ìŠµ?ˆë‹¤.</p>
 </div>
 </td>
 </tr>
 ) : (
 ...
 {sms.frstRegisterPnttm ? new Date(sms.frstRegisterPnttm).toLocaleString('ko-KR') : '?¥ë? ?†ìŒ'}
 </div>
 </td>
 </tr>
 ))

                    ...
                    <div className="flex flex-col md:flex-row items-center gap-8">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-center md:justify-start gap-3 px-4 py-1.5 bg-white/10 w-fit rounded-full border border-white/10 mx-auto md:mx-0">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.25em] text-white/80">Workflow System 2.0</span>
                            </div>
                            ...
                            <p className="text-slate-400 font-medium text-lg max-w-lg leading-relaxed mx-auto md:mx-0">
                                부서의 새로운 업무를 정의하고 할당합니다. <br />명확한 목표 설정을 통해 효율적인 협업을 시작하세요.
                            </p>
                        </div>
                    </div>
                </CardHeader>
                <form onSubmit={handleSubmit}>
                    <CardContent className="pt-24 pb-20 px-12 md:px-20 space-y-20">
                        ...
                        <div className="space-y-6 group">
                            <Label htmlFor="deptJobNm" className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 group-focus-within:text-slate-900 transition-all flex items-center gap-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {[
                                    ...
                                    >
                                        ...
                                        <div className="text-sm font-bold opacity-60 uppercase">{p.desc}</div>
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        {/* Task Description */}
                        <div className="space-y-6 group">
                            <Label htmlFor="deptJobCn" className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 group-focus-within:text-slate-900 transition-all flex items-center gap-3">
                        ...
                        <div className="p-10 bg-slate-50 border-2 border-white rounded-[3rem] shadow-xl flex items-center gap-8 relative overflow-hidden">
                            ...
                            <div className="space-y-1">
                                <p className="font-black text-2xl text-slate-900 tracking-tight italic uppercase">Validation Required</p>
                                <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-[450px]">
                                    등록된 업무는 부서 전체 대시보드에 즉시 노출됩니다. 기입한 내용이 부서 운영 가이드라인을 준수하는지 확인해 주세요.
                                </p>
                            </div>
                        </div>
                    </CardContent>
                    <CardFooter className="flex flex-col md:flex-row justify-center gap-8 py-20 border-t border-slate-50 bg-slate-50/30 px-12 rounded-b-[3.5rem]">
                        <Link href="/cop/smt/djm/selectDeptJobList">
                            <Button type="button" variant="ghost" className="h-20 px-16 font-black uppercase tracking-[0.3em] text-xs text-slate-400 hover:bg-white hover:text-rose-500 hover:shadow-2xl transition-all rounded-[1.75rem] border-2 border-transparent hover:border-rose-50">
                                <ArrowLeft className="w-6 h-6 mr-4" /> Discard & Return
                            </Button>
                        </Link>
                        <Button type="submit" className="h-20 px-24 gap-4 font-black uppercase tracking-[0.3em] text-xs shadow-[0_24px_48px_-8px_theme(colors.slate.900/40)] bg-slate-900 hover:bg-black transition-all active:scale-95 ring-[20px] ring-slate-100 rounded-[1.75rem]" disabled={loading}>
                            {loading ? (

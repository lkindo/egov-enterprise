...
export default function Error({
    ...
    return (
        <div className="min-h-[80vh] flex items-center justify-center p-6 relative overflow-hidden">
            {/* Background Decor */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-destructive/5 rounded-lg blur-[120px] -z-10" />
            ...
            <div className="max-w-lg w-full bg-background/60 backdrop-blur-2xl border-2 border-destructive/10 rounded-[0.1rem] p-12 shadow-2xl shadow-destructive/5 text-center space-y-8 animate-in slide-in-from-bottom-8 duration-700">
                ...
                <div className="space-y-4">
                    <h1 className="text-3xl font-bold tracking-tighter text-foreground">시스템 오류가 발생했습니다</h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                    <Button
                        onClick={() => reset()}
                        size="lg"
                        className="rounded-[0.1rem] h-11 font-bold bg-destructive hover:bg-destructive/90 text-white shadow-xl shadow-destructive/20 gap-2"
                    >
                        <RotateCcw size={20} /> 다시 시도하기
                    </Button>
                    <Link
                        href="/"
                        className={cn(buttonVariants({ variant: "outline", size: "lg" }), "rounded-[0.1rem] h-11 font-bold border-2 border-primary/10 gap-2")}
                    >

...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // AuthContext ??user ?ÅÌÉú ?ÖÎç∞?¥Ìä∏Í∞Ä ?ÑÎ£å???åÍπåÏßÄ ?†Ïãú ?ÄÍ∏?            await new Promise(resolve => setTimeout(resolve, 100));
            // router.push ?¨Ïö© (?òÏù¥ÏßÄ ?ÑÏ†Ñ Î¶¨Î°ú??Î∞©Ï?)
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 bg-[url('/images/login-bg.png')] bg-cover bg-center">
            ...
            <Card className="w-full max-w-md relative z-10 shadow-2xl border-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur-md">
                <CardHeader className="space-y-1 text-center">
                    <CardTitle className="text-2xl font-bold tracking-tight text-primary">?ÑÏûê?ïÎ? ?îÌÑ∞?ÑÎùº?¥Ï¶à</CardTitle>
                    <CardDescription>
                        {t('login.title')}
                    </CardDescription>
                </CardHeader>

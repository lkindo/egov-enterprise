...
export async function POST(request: NextRequest) {
  try {
    ...
    const { accessToken, groups, permissions, authorizationVersion } = tokenResponse;
    if (accessToken) {

      // Display grants may reach JavaScript; token material remains in HttpOnly cookies only.
      ...
      const nextResponse = reissueResponse({
        success: true,
        data: { groups, permissions, authorizationVersion },
      }, 200);

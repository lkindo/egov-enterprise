 ...
 *  2019.12.06   ???           checkFileExtension(), checkFileMaxSize() ??
 *  2020.08.05   ???           uploadFilesExt Parameter ??
...
public class EgovFileUploadUtil extends EgovFormBasedFileUtil {
	...
	public static List<EgovFormBasedFileVo> uploadFiles(HttpServletRequest request, String where, long maxFileSize) throws Exception {
		...
		if (mptRequest != null) {//2022.01 Possible null pointer dereference due to return value of called method ??
			...
			while (fileIter.hasNext()) {
				...
				if (StringUtils.isNotEmpty(tmp)) {
					...
					if (tmp.lastIndexOf(".") > 0) {
						...
						throw new SecurityException("Unacceptable file extension."); // ????? ?? ?????
					}

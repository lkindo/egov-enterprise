...
public class DefaultItemReader<T> implements ItemStreamReader<T> {
	...
	Range[] ranges; // fixedLength ?????
	String columns; // (fixedLength ?????
		...
		if (jobParameters.getString(stepName + READER_RESOURCE_TYPE_KEY) != null) {
			...
			if (DELIMITED_FILE_TYPE.equalsIgnoreCase(this.readerResourceType)
				...
				if (DELIMITED_FILE_TYPE.equalsIgnoreCase(this.readerResourceType)) {
					...
					if (resourceName == null || this.delimiter == null || names == null || type == null) {
						throw new RuntimeException(
							stepName + "??     ??Reader ??      ?   ?    resourceName, delimiter, names, type ?? ?         ??      ?? ??                ????      ??      ??\n"
					...
					if (resourceName == null || columns == null || names == null || type == null) {
						throw new RuntimeException(
							stepName + "??     ??Reader ??      ?   ?    resourceName, columns, names, type ?? ?         ??      ?? ??                ????      ??      ??\n"
								+ stepName + READER_RESOURCE_NAME_KEY + "=./inputs/csvData.csv "
				...
				if (this.sql == null || type == null) {
					throw new RuntimeException(stepName + "??     ??Writer ??      ?   ?    sql, type ???         ??      ?? ??                ????      ??      ??\n"
			...
			throw new RuntimeException(
				stepName + READER_RESOURCE_TYPE_KEY + "=delimitedFile'         ?? ??       ?         ??????       Job ???                     ???      ??      ??\n"
					+ "?         ???????         ? delimitedFile, fixedLengthFile, jdbcDb");
		}
	}
	...
	private void makeItemReader() {
		...
		if (DELIMITED_FILE_TYPE.equalsIgnoreCase(this.readerResourceType)
			...
			try {
				...
				throw new RuntimeException(
					this.readerResourceType + " ????       File??read ??       ?          FlatFileItemReader ??      ????       ?????     ??");
			}
		} else if (JDBC_DB_TYPE.equalsIgnoreCase(this.readerResourceType)) {
			...
			} catch (IllegalArgumentException e) {//KISA ?? ??(2018-10-29, ????
				throw new RuntimeException("Data source ?   ?    SQL properties         ??      ??? ??      ??     ??");
			} catch (Exception e) {
				throw new RuntimeException(
					this.readerResourceType + " ????       DB??read ??       ?          JdbcCursorItemReader ??      ????       ?????     ??");
			}
		}
	...
	private void printXmlConfig() {
		if (printXmlConf) {
			if (DELIMITED_FILE_TYPE.equalsIgnoreCase(this.readerResourceType)) {
				LOGGER.info("======= " + stepName + " READER ??      (XML          ?? =========\n"
					+ "<bean id=\"" + stepName

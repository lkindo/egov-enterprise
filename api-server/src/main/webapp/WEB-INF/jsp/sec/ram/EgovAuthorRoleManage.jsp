<% /** * @Class Name : EgovAuthorRoleManage.java * @Description : EgovAuthorRoleManage.jsp * @Modification Information *
	@ * @ 수정일 수정자 수정내용 * @ --------- -------- --------------------------- * @ 2009.02.01 lee.m.j 최초 생성 * 2016.06.13 장동한
	표준프레임워크 v3.6 개선 * * @author lee.m.j * @since 2009.03.21 * @version 1.0 * @see * */ %>
				...
				<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
					<%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
						<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
							<c:set var="pageTitle">
								<spring:message code="comCopSecRam.authorRoleList.title" />
							</c:set>
							<!DOCTYPE html>
							<html>
							...
							<body>
								<div class="board">
									<form:form name="listForm"
										action="${pageContext.request.contextPath}/sec/ram/EgovAuthorRoleList.do"
										method="post">
										<h1>${pageTitle}
											<spring:message code="title.list" />
										</h1><!-- 권한롤관리 목록 -->
										...
										<div class="search_box"
											...
											<ul>
												<li>
													...
													<div>
														<spring:message code="comCopSecRam.regist.authorCode" /> :
													</div>
												</li>
												<!-- 검색키워드 및 조회버튼 -->
												<li>
										...
										<table class="board_list"
											...
											<tbody class="ov">
												...
												<c:forEach var="authorRole" items="${authorRoleList}"
													varStatus="status">
													<tr>
														<td><input type="checkbox" name="delYn" class="check2"
																title="선택"><input type="hidden" name="checkId"
																value="<c:out value=" ${authorRole.roleCode}" />" />
														</td>
														<td>
															<c:out value="${authorRole.roleCode}" />
														</td>
														<td>
															<c:out value="${authorRole.roleNm}" />
														</td>
														<td>
															<c:out value="${authorRole.roleTyp}" />
														</td>
														<td>
															<c:out value="${authorRole.roleSort}" />
														</td>
														<td>
															<c:out value="${authorRole.roleDc}" />
														</td>
														<td>
															<c:out value="${fn:substring(authorRole.creatDt,0,10)}" />
														</td>
														<td>
															<select name="regYn"
																title='<spring:message code="comCopSecRam.authorRoleList.regYn" />'>
																<option value="Y" <c:if
																	test="${authorRole.regYn == 'Y'}">selected</c:if> >
																	<spring:message
																		code="comCopSecRam.authorRoleList.regY" />
																</option><!-- 등록 -->
																<option value="N" <c:if
																	test="${authorRole.regYn == 'N'}">selected</c:if> >
																	<spring:message
																		code="comCopSecRam.authorRoleList.regN" />
																</option><!-- 미등록 -->
															</select>
														</td>
													</tr>
	...
	<div class="btn">
	 ...
	 -->
										...
										<input type="hidden" name="pageIndex"
											value="<c:out value='${authorRoleManageVO.pageIndex}'/>" />
										<input type="hidden" name="authorCode" value="<c:out value="
											${authorRoleManageVO.searchKeyword}" />"/>

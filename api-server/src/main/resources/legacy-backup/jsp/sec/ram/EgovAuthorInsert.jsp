<% /** * @Class Name : EgovAuthorInsert.java * @Description : EgovAuthorInsert jsp * @Modification Information * @ * @
	수정일 수정자 수정내용 * @ ------- -------- --------------------------- * @ 2009.02.01 lee.m.j 최초 생성 * 2016.06.13 장동한 표준프레임워크
	v3.6 개선 * @ 2024.10.29 LeeBaekHaeng 검색조건 유지 * * @author lee.m.j * @since 2009.03.11 * @version 1.0 * @see * */ %>
					...
					<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
						<%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
							<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
								<c:set var="pageTitle">
									<spring:message code="comCopSecRam.title" />
								</c:set>
								<!DOCTYPE html>
								<html>
	...
	if(confirm("<spring:message code="common.delete.msg" />")){ //삭제하시겠습니까?
	    varFrom.submit();
								...
								<body>
									...
									<form:form modelAttribute="authorManage"
										action="${pageContext.request.contextPath}/sec/ram/EgovAuthorInsert.do"
										method="post" onSubmit="fncAuthorInsert(document.forms[0]); return false;">
										<input type="hidden" name="searchCondition" id="searchCondition"
											value="<c:out value=" ${authorManage.searchCondition}" />">
										<input type="hidden" name="searchKeyword" id="searchKeyword"
											value="<c:out value=" ${authorManage.searchKeyword}" />">
										<input type="hidden" name="pageIndex" id="pageIndex" value="<c:out value="
											${authorManage.pageIndex}" />">
										<div class="wTableFrm">
											<!-- 타이틀 -->
											<h2>${pageTitle}
												<spring:message code="title.create" />
											...
											<table class="wTable"
												...
												<tbody>
													...
													<tr>
														...
														<td class="left">
															<form:input path="authorCode" title="${title} ${inputTxt}"
													...
													<!-- 권한명 -->
													<c:set var="title">
														<spring:message code="comCopSecRam.regist.authorNm" />
													</c:set>
													<tr>
														<th>${title} <span class="pilsu">*</span></th>
														<td class="left">
															<form:input path="authorNm" title="${title} ${inputTxt}"
													...
													<!-- 설명 -->
													<c:set var="title">
														<spring:message code="comCopSecRam.regist.authorDc" />
													</c:set>
													<tr>
														<th>${title}</th>
														<td class="left">
															<form:textarea path="authorDc" title="${title} ${inputTxt}"
																cols="300" rows="10" />
															<div>

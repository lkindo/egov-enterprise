<% /** * @Class Name : EgovDiaryManageList.jsp * @Description : 일지관리 목록 화면 * @Modification Information * @ * @ 수정일 수정자
	수정내용 * @ ------- -------- --------------------------- * @ 2009.03.09 장동한 최초 생성 * 2016.08.05 장동한 표준프레임워크 v3.6 개선 *
	2024.10.29 권태성 element들의 올바른 id 값을 지정 * * @author 공통서비스팀 * @since 2009.03.09 * @version 1.0 * @see * */ %>
																...
																<ul>
																	<li>
																		<select name="searchCondition"
																			...
																			<option value='DIARY_NM' <c:if
																				test="${searchCondition == 'DIARY_NM'}">
																				selected</c:if>>
																				<spring:message
																					code="comCopSmtDsm.searchCondition.diaryNm" />
																			</option><!-- 일지명 -->
																			<option value='DRCT_MATTER' <c:if
																				test="${searchCondition == 'DRCT_MATTER'}">
																				selected</c:if>>
																				<spring:message
																					code="comCopSmtDsm.searchCondition.drctMatter" />
																			</option><!-- 지시사항  -->
																			<option value='PARTCLR_MATTER' <c:if
																				test="${searchCondition == 'PARTCLR_MATTER'}">
																				selected</c:if>>
																				<spring:message
																					code="comCopSmtDsm.searchCondition.partclrMatter" />
																			</option><!-- 특이사항 -->
																			<option value='SCHDUL_ID' <c:if
																				test="${searchVO.searchCondition == 'SCHDUL_ID'}">
																				selected</c:if>>
																				<spring:message
																					code="comCopSmtDsm.searchCondition.schdulId" />
																			</option><!-- 일정ID -->
																		</select>
																	</li>
																	<!-- 검색키워드 및 조회버튼 -->
																	<li>
																		<input class="s_input" name="searchKeyword"
																			type="text" size="35"
																		...
																		<input type="submit" class="s_btn"
																			...
																			title='<spring:message code="title.inquire" /> <spring:message code="input.button" />' /><!-- 조회 -->
																		<span class="btn_b"><a
																				href="<c:url value='/cop/smt/dsm/EgovDiaryManageRegist.do' />"
															...
															<thead>
																<tr>
																	...
																	<th class="board_th_link">
																		<spring:message
																			code="comCopSmtDsm.list.diaryNm" />
																	</th><!-- 일지명 -->
																	<th>
																		<spring:message
																			code="comCopSmtDsm.list.diaryProcsPte" />
																	</th><!-- 진척률 -->
																	<th>
																		<spring:message code="table.reger" />
																	</th><!-- 등록자 -->
																	<th>
															...
															<tbody class="ov">
																...
																<c:forEach items="${resultList}" var="resultInfo"
																	...
																	<tr>
																		...
																		<td>
																			<c:out
																				value='${resultInfo.diaryProcsPte}' />%
																		</td>
																		<td>
																			<c:out
																				value='${resultInfo.frstRegisterNm}' />
																		</td>
																		<td>
																			<c:out
																				value='${fn:substring(resultInfo.frstRegisterPnttm,0,10)}' />
																		</td>
																	</tr>
																</c:forEach>
															</tbody>

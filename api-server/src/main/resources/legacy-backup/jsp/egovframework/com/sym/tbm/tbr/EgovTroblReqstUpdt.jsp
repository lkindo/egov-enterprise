<%-- /** * @Class Name : EgovTroblReqstUpdt.java * @Description : EgovTroblReqstUpdt jsp * @Modification Information * @
	* @ 수정일 수정자 수정내용 * @ --------- -------- --------------------------- * @ 2010.07.0 lee.m.j 최초 생성 * 2024.10.29 권태성
	validateTroblReqst 함수 추가 * 2026.01.06 Antigravity 레이아웃 및 UI 표준화 적용 * * @author lee.m.j * @since 2010.07.01 *
	@version 1.0 * @see * * Copyright (C) 2009 by MOPAS All rights reserved. */ --%>
					...
					<%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
						<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
							...
							<head>
								...
								<title>
									<spring:message code="comSymTbmTbr.troblReqstUpdt.title" />
								</title><!-- 장애신청 수정 -->
...
-->
							...
							<c:if test="${troblOccrrncHtmp < 10}">
								<c:set var="troblOccrrncHtmp" value="${fn:substring(troblOccrrncTime,12,13)}" />
							</c:if>
							<c:if test="${troblOccrrncMtmp < 10}">
								<c:set var="troblOccrrncMtmp" value="${fn:substring(troblOccrrncTime,15,16)}" />
							</c:if>
							<c:if test="${troblOccrrncStmp < 10}">
								<c:set var="troblOccrrncStmp" value="${fn:substring(troblOccrrncTime,18,19)}" />
							</c:if>
													...
													<div class="content_wrap">
														<div id="contents" class="content">
															...
															<div class="wTableFrm">
																...
																<h2 class="tit_2">
																	<spring:message
																		code="comSymTbmTbr.troblReqstUpdt.pageTop.title" />
																</h2><!-- 장애신청 수정 -->

																<form name="troblReqst" id="troblReqst" method="post"
																	action="${pageContext.request.contextPath}/sym/tbm/tbr/updtTroblReqst.do">
																	...
																	<table class="wTable">
																		...
																		<tr>
																			<th>
																				<spring:message
																					code="comSymTbmTbr.troblReqstUpdt.troblId" />
																				<span class="pilsu">*</span>
																			</th><!-- 장애ID -->
																			<td class="left">
																				<input name="troblId" id="troblId"
																					type="text"
																					value="<c:out value='${troblReqst.troblId}'/>"
																					class="readOnlyClass"
																					readonly="readonly"
																					style="width:180px" />
																			</td>
																		...
																		<tr>
																			<th>
																				<spring:message
																					code="comSymTbmTbr.troblReqstUpdt.troblNm" />
																				<span class="pilsu">*</span>
																			</th><!-- 장애명 -->
																			<td class="left">
																				<input name="troblNm" id="troblNm"
																					type="text"
																					value="<c:out value='${troblReqst.troblNm}'/>"
																					maxLength="30"
																					style="width:180px" />&nbsp;
																		...
																		<tr>
																			<th>
																				<spring:message
																					code="comSymTbmTbr.troblReqstUpdt.troblKnd" />
																				<span class="pilsu">*</span>
																			</th><!-- 장애종류 -->
																			<td class="left">
																				<select name="troblKnd"
																					title="<spring:message code="
																					comSymTbmTbr.troblReqstUpdt.troblKnd" />">
																				<c:forEach var="cmmCodeDetail"
																					items="${cmmCodeDetailList}"
																					varStatus="status">
																					<option value="<c:out value="
																						${cmmCodeDetail.code}" />" <c:if
																						test="${cmmCodeDetail.code == troblReqst.troblKnd}">
																						selected</c:if>>
																					<c:out
																						value="${cmmCodeDetail.codeNm}" />
																					</option>
																				</c:forEach>
																				</select>
																			</td>
																		...
																		<tr>
																			<th>
																				<spring:message
																					code="comSymTbmTbr.troblReqstUpdt.troblDc" />
																				<span class="pilsu">*</span>
																			</th><!-- 장애설명 -->
																			<td class="left">
																				<textarea name="troblDc" rows="5"
																					cols="80"
																					title="<spring:message code="
																					comSymTbmTbr.troblReqstUpdt.troblDc" />">
																				<c:out value='${troblReqst.troblDc}' />
																				</textarea>
																		...
																		<tr>
																			<th>
																				<spring:message
																					code="comSymTbmTbr.troblReqstUpdt.troblOccrrncTime" />
																				<span class="pilsu">*</span>
																			</th><!-- 장애발생시간 -->
																			<td class="left">
																				...
																				<input type="text" name="troblOccrrncD"
																					id="troblOccrrncD"
																					value="<c:out value="
																					${troblOccrrncDtmp}" />"
																				maxlength="10" title="
																				...
																				" style="width:70px" />
																				<input type="hidden"
																					name="troblOccrrncDate" value="" />
																				<select name="troblOccrrncH"
																					id="troblOccrrncH">
																					<c:forEach var="i" begin="0"
																						end="23" step="1"
																						varStatus="status">
																						<option value="<c:if test=" ${i
																							< 10}">0</c:if>
																							<c:out value="${i}" />"
																							<c:if
																								test="${i == troblOccrrncHtmp}">
																								selected</c:if>><c:if
																								test="${i < 10}">0
																							</c:if>
																							<c:out value="${i}" />
																				...
																				<spring:message
																					code="comSymTbmTbr.troblReqstUpdt.hour" />
																				<!-- 시 -->&nbsp;
																				<select name="troblOccrrncM"
																					id="troblOccrrncM">
																					<c:forEach var="i" begin="0"
																						end="59" step="1"
																						varStatus="status">
																				...
																				<spring:message
																					code="comSymTbmTbr.troblReqstUpdt.minute" />
																				<!-- 분 -->&nbsp;
																				<select name="troblOccrrncS"
																					id="troblOccrrncS">
																					<c:forEach var="i" begin="0"
																						end="59" step="1"
																						varStatus="status">
																				...
																				<spring:message
																					code="comSymTbmTbr.troblReqstUpdt.second" />
																				<!-- 초 -->
																				<input type="hidden"
																					name="troblOccrrncTime" />&nbsp;
																		...
																		<tr>
																			<th>
																				<spring:message
																					code="comSymTbmTbr.troblReqstUpdt.troblRqesterNm" />
																				<span class="pilsu">*</span>
																			</th><!-- 장애등록자 -->
																			<td class="left">
																				<input name="troblRqesterNm"
																					id="troblRqesterNm" type="text"
																					value="<c:out value='${troblReqst.troblRqesterNm}'/>"
																					maxLength="30"
																					style="width:180px">&nbsp;
																	...
																	<div class="btn">
																		<input class="s_submit" type="submit"
																			...
																			onclick="fncTroblReqstUpdate(); return false;" /><!-- 저장 -->
																		<span class="btn_s"><a

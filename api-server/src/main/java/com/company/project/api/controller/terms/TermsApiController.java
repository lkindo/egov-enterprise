...
@Tag(name = "Terms", description = "?ΩÍ? Í¥ÄÎ¶?API")
@RestController
...
public class TermsApiController {
    ...
    @Operation(summary = "?ΩÍ? Î™©Î°ù Ï°∞Ìöå")
    @GetMapping
    ...
    @Operation(summary = "?ΩÍ? ?ÅÏÑ∏ Ï°∞Ìöå")
    ...
    @Operation(summary = "?ΩÍ? ?±Î°ù")
    @PostMapping
    public ResponseEntity<?> createTerms(
            @AuthenticationPrincipal CustomUserDetails userDetails,
            @Valid @RequestBody TermsDto vo) {
        String id = termsService.createTerms(userDetails.getUser().getEsntlId(), vo);
        return ResponseEntity.ok(ApiResponse.success(id));
    }

    @Operation(summary = "?ΩÍ? ?òÏ†ï")
    @PutMapping("/{useStplatId}")
    public ResponseEntity<?> updateTerms(
            @PathVariable String useStplatId,
            @AuthenticationPrincipal CustomUserDetails userDetails,
            @Valid @RequestBody TermsDto vo) {
        termsService.updateTerms(useStplatId, userDetails.getUser().getEsntlId(), vo);
        return ResponseEntity.ok(ApiResponse.success(null));
    }

    @Operation(summary = "?ΩÍ? ??†ú")
    @DeleteMapping("/{useStplatId}")
    public ResponseEntity<?> deleteTerms(
            @PathVariable String useStplatId,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        termsService.deleteTerms(useStplatId, userDetails.getUser().getEsntlId());
        return ResponseEntity.ok(ApiResponse.success(null));
    }
}

...
public class MainImageController {
    ...
    @Operation(summary = "메인 이미지 수정", description = "기존 메인 이미지 정보를 수정합니다.")
    @PutMapping("/{imageId}")
    ...
    @Operation(summary = "메인 이미지 삭제", description = "특정 메인 이미지를 삭제합니다.")
    ...
    @Operation(summary = "반영된 메인 이미지 목록 조회", description = "실제 화면에 반영(reflctAt='Y')된 이미지 목록을 조회합니다.")

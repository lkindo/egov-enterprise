...
/**
 * ?¨Ïö©?êÍ?Î¶¨Ïóê Í¥Ä??ÎπÑÏ??àÏä§ ?¥Îûò?§Î? ?ïÏùò?úÎã§.
 * 
 * @author Í≥µÌÜµ?úÎπÑ??Í∞úÎ∞ú?Ä Ï°∞Ïû¨??
 * @since 2009.04.10
 * @version 1.0
 * @see
 *
 ...
 *   ?òÏ†ï??     ?òÏ†ï??          ?òÏ†ï?¥Ïö©
 *  -------    --------    ---------------------------
 *   2009.04.10  Ï°∞Ïû¨??         ÏµúÏ¥à ?ùÏÑ±
 *   2014.12.08	 ?¥Í∏∞??		?îÌò∏?îÎ∞©??Î≥ÄÍ≤?EgovFileScrty.encryptPassword)
 *   2017.07.21  ?•Îèô??			Î°úÍ∑∏?∏Ïù∏Ï¶ùÏ†ú???ëÏóÖ
 *
 *      </pre>
 */
...
public class EgovUserManageServiceImpl extends EgovAbstractServiceImpl implements EgovUserManageService {
	...
	public void deleteUser(String checkedIdForDel) {
		...
		while (st.hasMoreTokens()) {
			...
			if ("USR03".equals(type)) { // ?ÖÎ¨¥?¨Ïö©??
				userIds.add(esntlId);
			} else if ("USR01".equals(type)) { // ?ºÎ∞ò?åÏõê
				generalUserIds.add(esntlId);
			} else if ("USR02".equals(type)) { // Í∏∞ÏóÖ?åÏõê

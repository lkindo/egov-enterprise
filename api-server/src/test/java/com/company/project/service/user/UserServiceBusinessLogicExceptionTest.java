...
class UserServiceBusinessLogicExceptionTest {
        ...
        @Test
        @DisplayName("????ì¡°íšŒ - ì¡´ì¬?? ???????ID???????ë°œìƒ")
        ...
        @Test
        @DisplayName("????ëª©ë¡ ???ì¡°íšŒ - ?ëª????´ì? ë²ˆí˜¸???????ë°œìƒ")
        ...
        @Test
        @DisplayName("????ëª©ë¡ ???ì¡°íšŒ - ?ëª????´ì? ?ê¸???????ë°œìƒ")
        ...
        void registerUser_fail_withDatabaseSaveError() {
                ...
                // When & Then
                assertThatThrownBy(() -> userService.registerUser("newUser", "password123!", "?ê·?????", "hint", "answer",
                                Role.USER))
                                .isInstanceOf(RuntimeException.class)
        ...
        @Test
        @DisplayName("ë¹?ë²ˆí˜¸ ê²€?- ?ì½???ë¹?ë²ˆí˜¸ê°€ null??ê²½ìš° ???ë°œìƒ")
        ...
        @Test
        @DisplayName("?????ë¡?- ????IDê°€ null??ê²½ìš° ???ë°œìƒ")
        void registerUser_fail_withNullUserId() {
                // When & Then
                assertThatThrownBy(() -> userService.registerUser(null, "password123!", "?ê·?????", "hint", "answer",
                                Role.USER))
                                .isInstanceOf(IllegalArgumentException.class);
        }

        @Test
        @DisplayName("?????ë¡?- ?????ë¦??null??ê²½ìš° ???ë°œìƒ")
        ...
        @Test
        @DisplayName("????ëª©ë¡ ì¡°íšŒ - ???ë²????ê²??ë¥?ë°œìƒ")

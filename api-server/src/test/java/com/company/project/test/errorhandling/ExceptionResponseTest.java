...
class ExceptionResponseTest {
    ...
    void requestTimeout_occurs_returns408Error() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/users/signup")
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestBody)
                .requestAttr("org.springframework.web.util.WebUtils.ERROR_REQUEST_URI_ATTRIBUTE", "/api/v1/users/signup"))
    ...
    void serviceUnavailable_occurs_returns503Error() throws Exception {
        // Given
        when(userService.getUserList())
                .thenThrow(new com.company.project.core.exception.BusinessException(ErrorCode.SERVER_OVERLOAD));
    ...
    void errorResponse_withTraceId() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/users/errorWithTrace")
                ...
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.error").exists())
                .andExpect(jsonPath("$.error.traceId").exists()) // Should include trace ID for debugging

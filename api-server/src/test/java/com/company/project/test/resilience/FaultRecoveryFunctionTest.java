...
class FaultRecoveryFunctionTest {
    ...
    void temporaryDbFailure_recovery_success() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenReturn(Arrays.asList(
                        UserDto.builder().userId("user1").userNm("복구 후 사용자1").esntlId("USR001").build(),
                        UserDto.builder().userId("user2").userNm("복구 후 사용자2").esntlId("USR002").build()
                )); // 다음 요청은 정상 동작
    ...
    void circuitBreaker_operationAndRecovery() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenThrow(new RuntimeException("Circuit breaker open")) // 3rd call - circuit breaker open
                .thenReturn(Arrays.asList( // 4th call - recovery after timeout simulation
                        UserDto.builder().userId("circuitBreakerUser").userNm("서킷 브레이커 사용자").esntlId("USR001").build()
                ));
    ...
    void timeoutRecovery_afterTimeoutOccurs() throws Exception {
        ...
        when(userService.getUserList())
                .thenAnswer(invocation -> {
                    ...
                    return Arrays.asList(UserDto.builder().userId("timeoutUser").userNm("타임아웃 사용자").esntlId("USR001").build());
                })
                .thenReturn(Arrays.asList( // 다음 요청은 정상 처리
                        UserDto.builder().userId("recoveredUser").userNm("복구된 사용자").esntlId("USR002").build()
                ));
    ...
    void dbConnectionPoolExhaustion_recovery() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenReturn(Arrays.asList( // 복구 후 정상 동작
                        UserDto.builder().userId("poolUser").userNm("풀 사용자").esntlId("USR001").build()
                ));
    ...
    void outOfMemoryRecovery_afterGC() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenReturn(Arrays.asList( // GC 후 정상 동작
                        UserDto.builder().userId("memoryUser").userNm("메모리 사용자").esntlId("USR001").build()
                ));
    ...
    void fallbackMechanism_whenFailureOccurs() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenReturn(Arrays.asList( // 폴백 후 정상 동작
                        UserDto.builder().userId("fallbackUser").userNm("폴백 사용자").esntlId("USR001").build()
                ));
    ...
    void recovery_withoutPerformanceDegradation() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenReturn(Arrays.asList( // 복구 후 정상 동작
                        UserDto.builder().userId("performanceUser1").userNm("성능 테스트 사용자1").esntlId("USR001").build(),
                        UserDto.builder().userId("performanceUser2").userNm("성능 테스트 사용자2").esntlId("USR002").build()
                ));
    ...
    void multipleFailures_recovery() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenReturn(Arrays.asList( // 복구 후 정상 동작
                        UserDto.builder().userId("multiFailureUser").userNm("다중 장애 사용자").esntlId("USR001").build()
                ));
    ...
    void cacheInvalidationAndRegeneration_afterFailure() throws Exception {
        ...
        when(userService.getUserById("cachedUser"))
                .thenReturn(UserDto.builder().userId("cachedUser").userNm("캐시 사용자").esntlId("USR001").build()) // 캐시에 저장
                .thenThrow(new RuntimeException("Cache invalidated due to error")) // 캐시 무효화
                .thenReturn(UserDto.builder().userId("cachedUser").userNm("갱신된 캐시 사용자").esntlId("USR001").build()); // 캐시 재생성
    ...
    void sessionPreservation_afterFailure() throws Exception {
        ...
        when(userService.getUserById("sessionUser"))
                .thenThrow(new RuntimeException("Temporary failure"))
                .thenReturn(UserDto.builder().userId("sessionUser").userNm("세션 사용자").esntlId("USR001").build());
    ...
    void resourceCleanup_afterFailure() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenReturn(Arrays.asList( // 리소스 정리 후 정상 동작
                        UserDto.builder().userId("resourceUser").userNm("리소스 사용자").esntlId("USR001").build()
                ));
    ...
    void memoryLeakPrevention_afterFailure() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenReturn(Arrays.asList( // 정상 동작
                        UserDto.builder().userId("memoryLeakUser").userNm("메모리 누수 테스트 사용자").esntlId("USR001").build()
                ));
    ...
    void normalRequestProcessing_afterFailure() throws Exception {
        ...
        when(userService.getUserById("normalUser"))
                .thenReturn(UserDto.builder().userId("normalUser").userNm("정상 사용자").esntlId("USR001").build())
                .thenThrow(new RuntimeException("Temporary failure"))
                .thenReturn(UserDto.builder().userId("recoveredUser").userNm("복구된 사용자").esntlId("USR002").build());
    ...
    void asyncTaskProcessing_afterFailure() throws Exception {
        ...
        when(userService.getUserList())
                ...
                .thenReturn(Arrays.asList( // 복구 후 정상 동작
                        UserDto.builder().userId("asyncUser").userNm("비동기 사용자").esntlId("USR001").build()
                ));

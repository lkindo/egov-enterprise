...
class PopupIntegrationTest {
        ...
        @Test
        @DisplayName("GET /api/v1/popups - ?ùÏóÖ Î™©Î°ù Ï°∞Ìöå API ?åÏä§??)
        void getPopups_ReturnsPage() throws Exception {
                // Given
                PopupDto dto = PopupDto.builder()
                                .popupId("POP_001")
                                .popupTitleNm("?åÏä§???ùÏóÖ")
                                .ntceAt("Y")
                                .build();
                ...
                mockMvc.perform(get("/api/v1/popups")
                                ...
                                .andExpect(jsonPath("$.data.content[0].popupTitleNm").value("?åÏä§???ùÏóÖ"));

                verify(popupService, times(1)).getPopupList(any(), any(Pageable.class));
        }

        @Test
        @DisplayName("GET /api/v1/popups/active - ?úÏÑ± ?ùÏóÖ Ï°∞Ìöå API ?åÏä§??)
        void getActivePopups_ReturnsList() throws Exception {
                // Given
                PopupDto dto = PopupDto.builder()
                                .popupId("POP_001")
                                .popupTitleNm("?úÏÑ± ?ùÏóÖ")
                                .build();
                ...
                mockMvc.perform(get("/api/v1/popups/active")
                                ...
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.success").value(true))
                                .andExpect(jsonPath("$.data[0].popupTitleNm").value("?úÏÑ± ?ùÏóÖ"));
        ...
        @DisplayName("POST /api/v1/popups - ?ùÏóÖ ?±Î°ù API ?åÏä§??)
        ...
        @Test
        @DisplayName("DELETE /api/v1/popups/{id} - ?ùÏóÖ ??†ú API ?åÏä§??)

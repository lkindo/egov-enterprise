...
class DatabaseServiceIntegrationTest {
    ...
    void getUserList_retrievesFromDatabase() {
        ...
        assertThat(userList).extracting(UserDto::getUserNm).containsExactlyInAnyOrder("Î¶¨Ïä§??????", "Î¶¨Ïä§??????");
        ...
        assertThat(dbUsers).extracting(User::getUserNm).containsExactlyInAnyOrder("Î¶¨Ïä§??????", "Î¶¨Ïä§??????");
    }

    @Test
    @DisplayName("?????Îπ??- ??¥Ïßï??????Î™©Î°ù Ï°∞Ìöå ?????Î≤????????????Í∞Ä???)
    ...
    @Test
    @DisplayName("?????Îπ??- Ï§ëÎ≥µ ????ID??Î°?????????Î≤?????Ï§ëÎ≥µ ???)
    ...
    void multipleUsers_signup_consistentPersistence() {
        ...
        for (int i = 0; i < numberOfUsers; i++) {
            ...
            assertThat(user.getUserNm()).isEqualTo("?Ï§????? + i");
        }
    }

    @Test
    @DisplayName("?????Îπ??- ???Î≤?????ÏßÅÏ†ë ?????????Î≥¥Í? ?Îπ????????Ï°∞Ìöå??")
    void databaseDirectUpdate_reflectedInService() {
        ...
        user.update("ÏßÅÏ†ë ????????", "newHint", "newAnswer", null, null, null, null, null, null, null, null, null, null,
        ...
        assertThat(retrievedUser.getUserNm()).isEqualTo("ÏßÅÏ†ë ????????");
        // Note: Role might not be directly mapped to the role field in UserDto
    }

    @Test
    @DisplayName("?????Îπ??- ????????????Ï°∞Ìöå ???????")

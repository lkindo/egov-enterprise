...
class UserServiceDatabaseIntegrationTest {
    ...
    void signup_persistsToDatabase() {
        ...
        assertThat(savedUser.get().getUserNm()).isEqualTo("DB ?µí•© ?ŒìŠ¤???¬ìš©??);
        assertThat(savedUser.get().getRole()).isEqualTo(Role.USER);

        // ê¶Œí•œ ?Œì´ë¸”ì— ?€?¥ë˜?ˆëŠ”ì§€ ?•ì¸
    ...
    void getUserById_retrievesFromDatabase() {
        ...
        assertThat(dbUser.get().getUserNm()).isEqualTo("DB ?µí•© ?ŒìŠ¤???¬ìš©??);
    }
    ...
    void getUserList_retrievesFromDatabase() {
        ...
        assertThat(userList).extracting(UserDto::getUserId).containsExactlyInAnyOrder("dbIntegrationUser",
                "dbIntegrationUser2");
        assertThat(userList).extracting(UserDto::getUserNm).containsExactlyInAnyOrder("DB ?µí•© ?ŒìŠ¤???¬ìš©??,
                "DB ?µí•© ?ŒìŠ¤???¬ìš©??");

        // ?°ì´?°ë² ?´ìŠ¤?ì„œ ì§ì ‘ ?•ì¸
        List<User> dbUsers = userRepository.findAll();
        assertThat(dbUsers).hasSize(2);
        assertThat(dbUsers).extracting(User::getUserNm).containsExactlyInAnyOrder("DB ?µí•© ?ŒìŠ¤???¬ìš©??, "DB ?µí•© ?ŒìŠ¤???¬ìš©??");
    }
    ...
    @Test
    @DisplayName("?¬ìš©???œë¹„??- ?¬ìš©???±ë¡ ???ì„±?¼ì ?ë™ ?¤ì • ?•ì¸")
    void signup_autoSetCreatedAt() {
        ...
        Optional<User> savedUser = userRepository.findById("dbIntegrationUser");
        assertThat(savedUser).isPresent();
        assertThat(savedUser.get().getSbscrbDe()).isNotNull(); // ê°€?…ì¼???•ì¸
    }

    @Test
    @DisplayName("?¬ìš©???œë¹„??- ?¬ëŸ¬ ?¬ìš©???±ë¡ ???°ì´?°ë² ?´ìŠ¤??ëª¨ë‘ ?€?¥ë¨")
    void signup_multipleUsers_allPersisted() {
        ...
        assertThat(allUsers).extracting(User::getUserId).containsExactlyInAnyOrder("multiUser1", "multiUser2",
                "multiUser3");
        assertThat(allUsers).extracting(User::getUserNm).containsExactlyInAnyOrder("?¤ì¤‘ ?¬ìš©??", "?¤ì¤‘ ?¬ìš©??", "?¤ì¤‘ ?¬ìš©??");
    ...
    @Test
    @DisplayName("?¬ìš©???œë¹„??- ?°ì´?°ë² ?´ìŠ¤ ?¸ëœ??…˜ ?ŒìŠ¤??)
    void signup_transactionRollbackOnFailure() {
        ...
        try {
            ...
            // ì¤‘ë³µ??IDë¡??¤ì‹œ ?œë„?˜ì—¬ ?¤íŒ¨ ? ë„
            userService.signup(signupRequest);
        } catch (Exception e) {
            // ?ˆì™¸ ë°œìƒ?€ ?ˆìƒ???™ì‘
        }

        // Then - ?¸ëœ??…˜??ë¡¤ë°±?˜ì–´ ì´ˆê¸° ?íƒœ?€ ê°™ì•„????
        long finalCount = userRepository.count();
        assertThat(finalCount).isEqualTo(initialCount + 1); // ì²?ë²ˆì§¸???±ê³µ?ˆìœ¼ë¯€ë¡?1 ì¦ê?
    }
}

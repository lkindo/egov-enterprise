...
class BottleneckIdentificationTest {
  ...
  void bottleneck_identification_databaseConnectionPoolUsage() throws Exception {
    ...
    latch.await(60, TimeUnit.SECONDS);
    long avgResponseTime = (long) responseTimes.stream().filter(java.util.Objects::nonNull).mapToLong(l -> l).average().orElse(0.0);
    long maxResponseTime = responseTimes.stream().filter(java.util.Objects::nonNull).mapToLong(l -> l).max().orElse(0L);

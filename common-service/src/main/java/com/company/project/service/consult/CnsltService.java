...
public class CnsltService implements EgovCnsltService {
    ...
    @Override
    public Page<CnsltManageDto> getCnsltList(String keyword, Pageable pageable) {
        if (keyword == null || keyword.isEmpty()) {
            return cnsltManageRepository.findAll(pageable).map(CnsltManageDto::from);
        }
        return cnsltManageRepository.findByCnsltSjContaining(keyword, pageable).map(CnsltManageDto::from);
    }

    @Override
    @Transactional
    public CnsltManageDto getCnslt(String cnsltId) {
        CnsltManage entity = cnsltManageRepository.findById(cnsltId)
    ...
    public void insertCnslt(CnsltManageDto dto) {
        ...
        cnsltManageRepository.save(CnsltManage.builder()
                .cnsltId(id)
    ...
    public void updateCnslt(CnsltManageDto dto) {
        CnsltManage entity = cnsltManageRepository.findById(dto.getCnsltId())
                .orElseThrow(() -> new BusinessException(ErrorCode.RESOURCE_NOT_FOUND));
    ...
    public void deleteCnslt(String cnsltId) {
        cnsltManageRepository.deleteById(cnsltId);
    }

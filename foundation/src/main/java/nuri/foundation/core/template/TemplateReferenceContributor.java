 ...
 *
 * <p>{@code tb_bbs_master.tmplt_id}·{@code tb_blog_info.tmplt_id} 는 물리 FK 없이 문자열로 템플릿을 가리킨다(V2_0).
 * 템플릿 삭제 전에 참조 여부를 물어야 하는데, 템플릿 도메인이 게시판·블로그 도메인을 직접 import 하면
...
public interface TemplateReferenceContributor {
    ...
    /** 사람이 읽는 참조원 이름(예: {@code 게시판}, {@code 블로그}). 삭제 거부 메시지에 실린다. */
    String sourceLabel();
